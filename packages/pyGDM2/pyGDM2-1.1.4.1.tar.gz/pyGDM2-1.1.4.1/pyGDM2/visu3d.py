...
def structure(struct, scale=0.75, abs_scale=False, 
              tit='', color='auto', colorlist=None, mode='cube', 
    ...
    ----------
      ...
      - colorlist: List of colors for different materials (optional, by default (=`None`), use matplotlib color cycle)
...
def vectorfield(NF, struct=None, scale=1.5, abs_scale=False, 
    ...
    if len(NF.T) == 6:
        X,Y,Z, UXcplx,UYcplx,UZcplx = np.transpose(NF)
