...
async def get_recently_accessed(request: web.Request) -> web.Response:
    ...
    count = 0
    recently_accessed_object_ids = set()
    ...
    async with _lock:
        if volume_ids:
            ...
            volume_id_query = {}
        async for a in mongoservicelib.get_all_gen(request,
                                                MONGODB_DESKTOP_OBJECT_ACTION_COLLECTION,
                                                mongoattributes={'$and': [{'new_object_type_name': {'$eq': type_}}, volume_id_query]},
                                                sort={'status_updated': -1}):
            desktop_object_action = DesktopObjectAction()
            desktop_object_action.from_dict(a)
            if desktop_object_action.new_object_id in recently_accessed_object_ids or desktop_object_action.new_object_id is None:
                continue
            ...
            recently_accessed_object_ids.add(recently_accessed_view.id)
