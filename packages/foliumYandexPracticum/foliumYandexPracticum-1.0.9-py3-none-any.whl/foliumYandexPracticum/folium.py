...
from foliumYandexPracticum.elements import JSCSSMixin
from foliumYandexPracticum.map import FitBounds
from foliumYandexPracticum.raster_layers import TileLayer
from foliumYandexPracticum.utilities import (
    _parse_size,
    temp_html_filepath,
    validate_location,
    parse_options,
...
ENV = Environment(loader=PackageLoader('foliumYandexPracticum', 'templates'))
...
def set_mbtiles():
    ...
    TileLayer._template = Template(u"""
...
class Map(JSCSSMixin, MacroElement):
    ...
    **kwargs
    ...
    def __init__(
        ...
        if tiles:
            use_custom_raster = False
            if tiles == 'OpenStreetMap':
                use_custom_raster = True
                offline_file_path = 'https://code.s3.yandex.net/data_faculty/folium/offline/'
                tile_path = 'https://code.s3.yandex.net/data_faculty/folium/moscow.mbtiles'
            ...
            if use_custom_raster:
                set_mbtiles()
            self.add_child(tile_layer, name=tile_layer.tile_name)
            if use_custom_raster:
                set_offline(self, offline_file_path)
    ...
    def choropleth(self, *args, **kwargs):
        ...
        from foliumYandexPracticum.features import Choropleth
