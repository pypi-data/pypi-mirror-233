...
class DataFrameWrapper(ABC, Generic[T]):
    ...
    def capture_columns(self, minimum_rows_for_statistics, capture_schema_only: bool) -> list[Column]:
        list_schema = self.capture_column_schema()
        if self.rows >= minimum_rows_for_statistics and capture_schema_only is False:
            list_col_with_stats = self.capture_column_statistics(list_schema)
