...
class SparkDFWrapper(DataFrameWrapper):
    ...
    def __compute_boolean_column_statistics__(self, dataframe: SparkDF) -> Dict[str, BooleanStat]:
        ...
        agg_exprs = [
            ...
            *[f.expr(f"SUM(CASE WHEN {col} = True THEN 1 ELSE 0 END)").alias(f"{col}_true") for col in columns],
            *[f.expr(f"SUM(CASE WHEN {col} IS NULL THEN 1 ELSE 0 END)").alias(f"{col}_missing") for col in columns],
            *[f.expr(f"SUM(CASE WHEN {col} = False THEN 1 ELSE 0 END)").alias(f"{col}_false") for col in columns],
        ]
    ...
    def __compute_numeric_column_statistics__(self, dataframe: SparkDF) -> Dict[str, NumericalStat]:
        ...
        agg_exprs = [
            f.expr("COUNT(*)").alias("count"),
            *[f.expr(f"SUM(CASE WHEN {col} IS NULL THEN 1 ELSE 0 END)").alias(f"{col}_missing") for col in columns],
            *[f.expr(f"AVG({col})").alias(f"{col}_mean") for col in columns],
            *[f.expr(f"STDDEV({col})").alias(f"{col}_stddev") for col in columns],
            *[f.expr(f"MIN({col})").alias(f"{col}_min") for col in columns],
            *[f.expr(f"APPROX_PERCENTILE({col}, 0.25)").alias(f"{col}_q25") for col in columns],
            *[f.expr(f"APPROX_PERCENTILE({col}, 0.5)").alias(f"{col}_q50") for col in columns],
            *[f.expr(f"APPROX_PERCENTILE({col}, 0.75)").alias(f"{col}_q75") for col in columns],
            *[f.expr(f"MAX({col})").alias(f"{col}_max") for col in columns],
        ]
    ...
    def __compute_string_column_statistics__(self, dataframe: SparkDF) -> Dict[str, TextStat]:
        ...
        agg_exprs = [
            f.expr("COUNT(*)").alias("count"),
            *[f.expr(f"SUM(CASE WHEN {col} IS NULL THEN 1 ELSE 0 END)").alias(f"{col}_missing") for col in columns],
            *[f.expr(f"APPROX_COUNT_DISTINCT({col})").alias(f"{col}_unique") for col in columns],
        ]
    ...
    def __compute_date_column_statistics__(self, dataframe: SparkDF) -> Dict[str, DateStat]:
        ...
        agg_exprs = [
            ...
            *[f.expr(f"MIN({col})").alias(f"{col}_min") for col in columns],
            *[f.expr(f"MAX({col})").alias(f"{col}_max") for col in columns],
            *[f.avg(f.expr(f"UNIX_TIMESTAMP({col}, 'yyyy-MM-dd')")).alias(f"{col}_mean") for col in columns],
            *[
                f.expr(f"APPROX_PERCENTILE(UNIX_TIMESTAMP({col}, 'yyyy-MM-dd'), 0.25, 1000)").alias(f"{col}_median")
                for col in columns
            ],
        ]
        ...
        for col in columns:
            ...
            mean_date = datetime.fromtimestamp(int(agg_results[f"{col}_mean"]))
            median_date = datetime.fromtimestamp(int(agg_results[f"{col}_median"]))
