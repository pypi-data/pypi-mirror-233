...
class Model:
    ...
    def _format_metrics(self, metrics: dict[str, int | float] | list[Metric] | Metric | None) -> list[Metric]:
        if metrics is None:
            return []
    ...
    def _remove_incorrect_extra_info(extra_info: list[ExtraInfo]) -> list[ExtraInfo]:
        for info in extra_info:
            if not isinstance(info, ExtraInfo):
    ...
    def mlflow(
        ...
        """Extract automatically information from an Mlflow run into a Vectice model to be assigned to a step.
        ...
        Parameters:
            ...
            url (Optional): The Mlflow UI URL to access the run info.
            derived_from (Optional): List of dataset (or version ids) to link as lineage.
        """
