...
from fandango.log import tracer, shortstr, pformat
from fandango import clmatch
# import panic
import panic
import panic.view

from panic.properties import ACTIVE_STATES, DISABLED_STATES, AlarmStates
from panic.gui.actions import QAlarmManager
from panic.gui.utils import getThemeIcon, trace, \
    SNAP_ALLOWED, Catched, CleanMainWindow, AlarmPreview, AlarmsSelector
...
from panic.widgets import getIconForAlarm
from panic.gui.phonebook import PhoneBook

from taurus.external.qt import Qt, QtGui
from taurus.external.qt.QtCore import pyqtSignal
...
class QAlarmList(QAlarmManager, PARENT_CLASS):
    ...
    alarm_selected_signal = pyqtSignal(str)
    devices_selected_signal = pyqtSignal(str)
    ...
    def init_timers(self):
        ...
        # first fast loading
        self.reloadTimer.start(int(min((5000, self.REFRESH_TIME / 2.))))
    ...
    def onItemSelected(self):
        try:
            ...
            if len(items) == 1:
                ...
                self.alarm_selected_signal.emit(a.tag)
                ...
                self.devices_selected_signal.emit('|'.join(devices + tags))
        except:
            traceback.print_exc()
...
class QFilterGUI(QAlarmList):
    ...
    def getFilters(self):
        ...
        trace("%s -> AlarmGUI.getFilters(%s)" % (time.ctime(), ['%s=%s' % (
            s, getattr(self, s, None)) for s in
                                                                ('regEx', 'severities', 'timeSortingEnabled',
                                                                 'changed',)]), level=2)

        filters = []
        device = receiver = state = priority = condition = userfilter = ''
    ...
    def setSecondCombo(self):
        ...
        if source == 'Devices':
            ...
            r, sort, values = 3, False, self.severities.keys()
...
class AlarmGUI(QFilterGUI):
    ...
    def connectAll(self):
        ...
        #     self.valueChanged.triggered.connect(self.hurry)
        ...
        fn.qt.getApplication().aboutToQuit.connect(self.exitThreads)
    ...
    def main(args=[]):
        ...
        uniqueapp = TaurusApplication(sys.argv)  # opts)
        ...
        print('=' * 80)
        AlarmGUI(filters='|'.join(args), options=opts, mainwindow=True)
    ...
    def exitThreads():
        ...
        [s.unsubscribeEvents() for s
         in fn.callbacks.EventSource.get_thread().sources]
        fn.threads.ThreadedObject.kill_all()
    ...
    def loadFromFile(self, default='*.csv', ask=True):
        try:
            ...
            if filename:
                ...
                if not self.validate('LoadFromFile(%s)' % filename): 
                    return
