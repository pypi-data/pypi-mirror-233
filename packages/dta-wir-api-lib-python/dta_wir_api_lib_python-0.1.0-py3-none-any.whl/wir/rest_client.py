...
from wir.request_utils import get_default_logger

log = get_default_logger(__name__)


class WIRRestAPI(object):
    default_headers = {
        "Content-Type": "application/json",
        "Accept": "application/json",
        "x-request-dtx-src-service-name": "dta-wir-api-lib-python",
        "x-request-dtx-dst-protocol": "http",
        "x-request-dtx-dst-service-name": "dta-wir-api",
        "x-request-dtx-dst-service-version": "v1",
    }
    ...
    def __init__(
        ...
        advanced_mode=None,
        token=None,
        dtx_src_service_name="dta-wir-api-lib-python",
        verify_ssl=True,
        proxies=None,
        cert=None,
    ):
        ...
        self.dtx_src_service_name= dtx_src_service_name
        if token is not None:
            self._create_token_session(token)
        if dtx_src_service_name is not None:
            self._update_header("x-request-dtx-src-service-name", dtx_src_service_name)
    ...
    def base_url(self, api_root=None, api_version=None):
        if api_root is None:
            api_root = self.api_root
        if api_version is None:
            api_version = self.api_version
        return "/".join(str(s).strip("/") for s in [api_root, api_version] if s is not None)
