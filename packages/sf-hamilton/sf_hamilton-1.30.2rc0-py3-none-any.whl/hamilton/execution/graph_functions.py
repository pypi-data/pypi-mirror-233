...
def execute_subdag(
    ...
    def dfs_traverse(
        ...
        # E.G. if something is not needed by this current execution due to the selection of nodes to run,
        # it might not prune everything

        for dep in node_.dependencies:
            if dep.name in computed:
                for downstream_node in dep.depended_on_by:
                    # if it isn't computed, and it isn't in the queue, we can prune
                    if (
