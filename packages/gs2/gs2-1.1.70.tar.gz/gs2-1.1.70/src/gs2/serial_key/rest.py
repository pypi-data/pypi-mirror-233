...
class Gs2SerialKeyRestClient(rest.AbstractGs2RestClient):
    ...
    def _describe_issue_jobs(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/campaign/{campaignModelName}/issue".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
    ...
    def _get_issue_job(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/campaign/{campaignModelName}/issue/{issueJobName}".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            campaignModelName=request.campaign_model_name if request.campaign_model_name is not None and request.campaign_model_name != '' else 'null',
    ...
    def _issue(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/campaign/{campaignModelName}".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            campaignModelName=request.campaign_model_name if request.campaign_model_name is not None and request.campaign_model_name != '' else 'null',
        )
    ...
    def _use(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/user/me/serialKey".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
        )
    ...
    def _use_by_user_id(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/user/{userId}/serialKey".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
    ...
    def _revert_use_by_user_id(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/user/{userId}/serialKey/revert".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            userId=request.user_id if request.user_id is not None and request.user_id != '' else 'null',
        )
    ...
    def _describe_campaign_model_masters(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/master/campaign".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
        )
    ...
    def _get_campaign_model_master(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='serial-key',
            region=self.session.region,
        ) + "/{namespaceName}/master/campaign/{campaignModelName}".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            campaignModelName=request.campaign_model_name if request.campaign_model_name is not None and request.campaign_model_name != '' else 'null',
        )
