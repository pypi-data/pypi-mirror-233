...
class Gs2JobQueueRestClient(rest.AbstractGs2RestClient):
    ...
    def _describe_dead_letter_jobs_by_user_id(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='job-queue',
            region=self.session.region,
        ) + "/{namespaceName}/user/{userId}/dead".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            userId=request.user_id if request.user_id is not None and request.user_id != '' else 'null',
        )
    ...
    def _get_dead_letter_job_by_user_id(
        ...
        url = Gs2Constant.ENDPOINT_HOST.format(
            service='job-queue',
            region=self.session.region,
        ) + "/{namespaceName}/user/{userId}/dead/{deadLetterJobName}".format(
            namespaceName=request.namespace_name if request.namespace_name is not None and request.namespace_name != '' else 'null',
            userId=request.user_id if request.user_id is not None and request.user_id != '' else 'null',
            deadLetterJobName=request.dead_letter_job_name if request.dead_letter_job_name is not None and request.dead_letter_job_name != '' else 'null',
        )
