...
class _Package:
    ...
    def __init__(self, name: str, regex: str, is_conda: bool, required_version: Optional[str] = None):
        ...
        if required_version is not None:
            try:
                specifier = Requirement.parse(name + required_version).specifier
                if len(specifier) < 1:
...
def modify_run_configuration(settings: AzureAutoMLSettings,
    ...
    automl_core_pkg = "azureml-automl-core"
    automl_runtime_pkg = "azureml-automl-runtime"
    dataset_pkg = "azureml-dataset-runtime[pandas,fuse]"
    train_automl_client_pkg = "azureml-train-automl-client"
    train_automl_runtime_pkg = "azureml-train-automl-runtime"
    ...
    sklearn_regex = r"scikit-learn<=1.1.3,>=1.0.0"
    ...
    xgboost_regex = r"py-xgboost<=1\.3\.3"
    ...
    if is_sdk_candidate or is_gated_build:
    ...
    # For an automl runtime release (such as 1.53.2), azureml-default is not part of the release and
    # should be pinned to the main release version.  For example 1.53.0
    default_pin = automl_pin_generic_version
    version_elements = azureml.train.automl.VERSION.split(".")
    if len(version_elements) > 2 and version_elements[0] == "1" and version_elements[2] != "0":
        default_pin = "~={}.{}.0".format(version_elements[0], version_elements[1])
    ...
    if azureml.train.automl.VERSION.startswith("1.53."):
        numpy_version_pin = "==1.22.3"
    else:
        numpy_version_pin = "==1.23.5"
    ...
    required_package_list = [
        ...
        # issues with a patch.
        _Package(automl_core_pkg, automl_regex, False, automl_pin_generic_version),
        _Package(automl_runtime_pkg, automl_regex, False, automl_pin_generic_version),
        _Package(dataset_pkg, automl_regex, False, default_pin),
        _Package(train_automl_client_pkg, automl_regex, False, automl_pin_generic_version),
        _Package(train_automl_runtime_pkg, automl_regex, False, automl_pin_generic_version),
        _Package(defaults_pkg, defaults_regex, False, default_pin),
        _Package(pandas_pkg, pandas_regex, True, "==1.3.5"),
        _Package(psutil_pkg, psutil_regex, True, ">5.0.0,<6.0.0"),
        _Package(sklearn_pkg, sklearn_regex, True, "==1.1.3"),
        _Package(numpy_pkg, numpy_regex, True, numpy_version_pin),
        _Package(xgboost_pkg, xgboost_regex, True, "==1.3.3"),
        _Package(inference_schema_pkg, inference_schema_regex, False),
        _Package(prophet_pkg, prophet_regex, True, "==1.1.4"),
    ...
    dependencies.add_conda_package('pip==20.2.4')
