...
class Grid:
    ...
    def from_faces_loc(
        x0_faces: NDArray,
        x1_faces: NDArray,
        x2_faces: Optional[NDArray] = None,
        spherical: bool = False,
    ) -> Grid:
        if x2_faces is None:
    ...
    @staticmethod
    def from_header(header: Mapping[str, Any]) -> Grid:
        spherical = header["geometry"] == 1
        return Grid.from_faces_loc(
            header["face_loc_1"],
            header["face_loc_2"],
            header.get("face_loc_3"),
            spherical=spherical,
        )

    @property
    def geometry_flag(self) -> int:
        return 1 if self.spherical else 0
