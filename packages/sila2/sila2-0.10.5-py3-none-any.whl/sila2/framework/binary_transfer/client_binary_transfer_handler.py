...
class ClientBinaryTransferHandler(BinaryTransferHandler):
    ...
    def __init__(self, client: SilaClient):
        self.upload_stub = binary_transfer_grpc_module.BinaryUploadStub(client._channel)
        self.download_stub = binary_transfer_grpc_module.BinaryDownloadStub(client._channel)
        self.known_binaries = {}
    ...
    def to_message(
        ...
        *,
        ...
        try:
            create_binary_response = self.upload_stub.CreateBinary(
            ...
            if isinstance(ex, grpc.RpcError) and ex.code() == grpc.StatusCode.ABORTED:
                ...
                if details[0] == 0x08 or details[0] == 0x12 and details[-2:] == b"\x08\x01":
                    raise BinaryTransferError.from_rpc_error(ex)
                elif details[0] in (0x1A, 0x22, 0x12):
                    raise SilaError.from_rpc_error(ex, self.parent_client)
    ...
    def __compute_chunk_length(self, binary_size, chunk_index) -> int:
        return min(binary_size - chunk_index * self.chunk_size, self.chunk_size)
