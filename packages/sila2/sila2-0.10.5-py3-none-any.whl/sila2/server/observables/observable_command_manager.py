...
class ObservableCommandManager:
    ...
    def __init__(
        ...
        if wrapped_command.intermediate_responses is not None:
            ...
            command_instance = ObservableCommandInstanceWithIntermediateResponses(
                self.command_execution_uuid, self.execution_info_queue, self.intermediate_response_queue
            )
        else:
            command_instance = ObservableCommandInstance(self.command_execution_uuid, self.execution_info_queue)

        self.result_future = parent_server.child_task_executor.submit(impl_func, command_instance)
    ...
    def subscribe_to_execution_infos(self, context: ServicerContext) -> Stream[SilaExecutionInfo]:
        ...
        queue.put(
            ExecutionInfo(self.wrapped_command.parent_feature._pb2_module.SiLAFramework__pb2).to_message(
                CommandExecutionInfo(status, 1, estimated_remaining_time=datetime.timedelta(0))
            )
        )
    ...
    def __after_execution(self, result_future: Future):
        ...
        if ex is not None:
            self.logger.error(f"Command finished with error: {ex!r}")

        # ensure that `running` is observable at least once (prevent transition `waiting` -> `finished`)
        self.execution_info_queue.put(
            CommandExecutionInfo(CommandExecutionStatus.running, 1, estimated_remaining_time=datetime.timedelta(0))
        )
        status = CommandExecutionStatus.finishedSuccessfully if ex is None else CommandExecutionStatus.finishedWithError
        self.execution_info_queue.put(CommandExecutionInfo(status, 1, estimated_remaining_time=datetime.timedelta(0)))
