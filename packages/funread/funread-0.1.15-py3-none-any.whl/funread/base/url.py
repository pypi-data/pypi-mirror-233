...
from sqlalchemy import String, UniqueConstraint, DateTime, func, Integer, select, PrimaryKeyConstraint
...
class ReadODSUrlData(Base):
    ...
    gmt_create = mapped_column(DateTime(timezone=False), server_default=func.now())
    gmt_updated = mapped_column(DateTime(timezone=False), server_default=func.now())
    gmt_modified = mapped_column(DateTime(timezone=False), server_default=func.now(), onupdate=func.now())
