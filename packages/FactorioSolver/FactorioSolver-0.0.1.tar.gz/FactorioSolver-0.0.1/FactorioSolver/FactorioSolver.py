...
"""
Model of the factorio data, with utility functions to gather information

:param data_path:          path to the dataset file
:param choice_map_path:    path to the choice map json file
:param categories_filter   filter
:type data_path: path to the data
"""
class Model:
    ...
    :type data_path: path to the data
    """
    def __init__(self, data_path:str, choice_map_path:str, categories_filter:list=sSpaceAssemblyCategories):
        self.mRawData = None
        self.mRecipes = None
        self.mProducts = None
        self._load_data(data_path, choice_map_path, categories_filter)
    ...
    def _load_data(self, data_path:str, choice_map_path:str, categories_filter:list=sAsm3Categories):

        self.mDataPath = data_path

        # Load the data file
        self.mRawData = pd.read_csv(self.mDataPath, sep='|')
        self.mChoiceMap = pd.read_json(choice_map_path)
        
        # Setup recipe data set
        recipes = self.mRawData
        ...
        products = products.set_index("name", drop=False)
    ...
    def get_product_usage_graph(self, product_name:str):
        ...
        sub = base_graph.subgraph(base_graph.subcomponent(base_graph.vs.select(name_eq=product_name)[0], mode="in"))
        return sub
    ...
    def get_choice_list(self, product_name = None, choice_map = None):
        ...
        if type(choice_map) is not type(None):
            graph = graph.copy()
            self.apply_choice_map(graph, self.mChoiceMap)
    ...
    def print_production_single(self, product_name:str, product_num:int, figsize=(30, 17), show_fig:bool=not _in_notebook(), file_name:str = ""):
        self.print_production({product_name:product_num}, figsize, show_fig, file_name)

    def print_production(self, produced_items:dict, figsize=(30, 17), show_fig:bool=not _in_notebook(), file_name:str = ""):
        fig, ax = plt.subplots(figsize=figsize)
        plt.margins(0.04, 0.04)
        
        assert len(self.get_choice_list(produced_items, self.mChoiceMap)) == 0, "Multiple possible recipes to choose from, please check the choices left via print_choice_list and choose which recipe to use via the choice map"

        draw = self.compute_prod_graph(produced_items)
        ...
        draw.vs["desc"]     = [f'{v["name"]}'
                            + f'\n ---------------' 
                            + f'\n Tot: {v["total"]:.1f} (y/2={v["belt_count.y"]:.1f})'
        ...
        labels = [f'in:{e["in.amount"]} | out:{e["out.amount"]}\n(y/2:{e["belt_count.y"]:.1f})' for e in draw.es]
        ...
        ig.plot(draw, layout="sugiyama", edge_align_label=False, vertex_label=draw.vs["desc"], vertex_shape="square", vertex_label_color="black", vertex_size=0.7, edge_background="white", edge_label_size=10, edge_label=labels, target=ax, margin=100, bbox=[0,0,1000, 500])
    ...
    def apply_choice_map(self, graph, choice_map):
    ...
    def compute_prod_graph(self, item_list:dict):
        graph = self.mDataGraph.copy()
        graph.vs["output_amount"] = 0
