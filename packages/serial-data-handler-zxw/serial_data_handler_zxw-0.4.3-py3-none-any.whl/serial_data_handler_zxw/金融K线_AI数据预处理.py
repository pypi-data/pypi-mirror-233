...
class 金融K线_AI数据预处理:
    ...
    原始数据pd: pd.DataFrame = None  # 原始数据
    标准化数据pd: pd.DataFrame = None  # 标准化后的数据 , 只有执行 标准化() 方法后才会有值
    ...
    def 计算Z_Score参数(self):
        self.mean = self.原始数据pd.mean()
        self.std = self.原始数据pd.std()
