...
1. Generate training 原始数据pd based on the incoming 原始数据pd, and ensure the continuity of the training 原始数据pd

# Attention:
传入的数据必须指定时间列(或其他顺序列)
You must specify the time column (or other sequential column) of the incoming 原始数据pd
传入的数据必须已按顺序排列好
The incoming 原始数据pd must be sorted in order
"""
import numpy as np
import pandas as pd
...
def 时间列_三角函数化(timeSerial: pd.Series, 起始日期: datetime = None, 周期=timedelta(days=1)):
    ...
    if timeSerial.diff().min() < timedelta(seconds=1):
        ...
        print("If your 原始数据pd interval < 1s, please do the corresponding multiplication conversion,"
              "\n such as: 1ms 原始数据pd, you should multiply by 1000, convert to second-level 原始数据pd")
        raise ValueError("timeSerial内数据的最小间隔必须大于1秒(原始数据pd interval must be greater than 1 second)")
