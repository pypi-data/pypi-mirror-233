...
def add_observations(
    ...
    :param bool addAddresses: Whether to add addresses \(default true\)
    :param bool addDomains: Whether to add domains \(default true\)
...
def fetch_observations_for_domain(
    ...
    :param int countMaxWait: Max wait time for result count calculation. 0 equates to unlimited wait and \-1 to skip counting altogether.
    :param json: return the response's body as a ``dict`` parsed from json. ``True`` by
      default. If set to false, the raw ``requests.Response`` object will be returned.
    :param verify: path to a certificate bundle or boolean indicating whether SSL
      verification should be performed.
...
def find_address_observations(
    ...
    :param int countMaxWait: How many seconds to wait for count calculation to complete. \-1\: Skip counting altogether to return search results immediately. 0\: Unlimited count time for when the count matters and response time does not.
...
def find_domain_observations(
    ...
    :param list domainNames: DEPRECATED\: Search for observations by domain name. Deprecated\, use domains instead.
...
#: **DEPRECATED** : ``fetch_observations_for_i_p`` is an alias for ``fetch_observations_for_ip``. Exists
#: only for backward compatibility - **do not use** - use ``fetch_observations_for_ip`` instead.
fetch_observations_for_i_p = register_command(
    extending=("reputation", "v1", "observation"),
    module=argus_cli_module,
    alias="fetch_observations_for_i_p"
)(deprecated_alias("fetch_observations_for_i_p")(fetch_observations_for_ip))