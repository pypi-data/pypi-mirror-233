...
from sop_ingestion.sop_deutils.sql.y4a_monitor_user_external \
    import MonitorUserExternal
from sop_ingestion.sop_deutils.y4a_retry import retry_on_error
...
class PostgreSQLUtils:
    ...
    :param db_user: username or account to connect to PostgreSQL
    :param db_password: password to connect to PostgreSQL
    :param db_host: host url to connect to PostgreSQL
    :param db: PostgreSQL database to connect to
        defaults to 'serving'
    """
    def __init__(
        self,
    ...
    def generate_log_conn(
        ...
        try:
            self.external_monitor = MonitorUserExternal(
                ...
                conn_host=self.db_host,
                conn_db=self.db,
                conn_number=conn_number,
            )
            self.external_monitor.create_pool_conn()
    ...
    def update_log_conn(self) -> None:
        try:
            ...
            self.external_monitor.close_pool_conn()
        except Exception as e:
    ...
    def update_table(
        ...
        """
        Update new data for specific columns in the table
    ...
    def select_distinct(
        ...
        """
        Get the distinct data of a specified column in the table
    ...
    def show_columns(
        ...
        """
        Get all the columns name of a specific table
        ...
        columns = self.get_data(sql)['column_name'].to_list()
    ...
    def table_exists(
        ...
        exists = self.get_data(sql)['exists'].values[0]
