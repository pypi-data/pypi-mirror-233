...
class Runner(ABC):
    ...
    def __call__(self, filePath:str) -> List[RepoResultObject]:
        return self.scan(mystring.string)
...
class Logger(ABC):
    ...
    @abstractmethod
    def message(msg:mystring.string)->bool:
        pass

    @abstractmethod
    def parameter(parameter:RepoObject)->bool:
        pass

    @abstractmethod
    def result(result:RepoResultObject)->bool:
        pass

    def __enter__(self, stage:mystring.string):
        self.stage = stage
        self.message("Entering the stage: {0}".format(self.stage))
        return self

    def __call__(self, msg:Union[mystring.string, RepoObject, RepoResultObject])->bool:
        if isinstance(msg, mystring.string):
            return self.message(msg)
        elif isinstance(msg, RepoObject):
            return self.parameter(msg)
        else:# isinstance(msg, mystring.string)
            return self.result(msg)

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.message("Exiting the stage: {0}".format(self.stage))
        return self
...
class LoggerSet(ABC):
    ...
    def __iadd__(self, logger:Logger):
        self.loggers += [logger]
        return self

    def __enter__(self, stage:mystring.string):
        for logger in self.loggers:
            logger.__enter__(stage)
        return self
