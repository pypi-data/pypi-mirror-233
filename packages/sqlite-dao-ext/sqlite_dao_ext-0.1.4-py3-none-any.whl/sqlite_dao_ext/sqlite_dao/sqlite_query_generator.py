...
from sqlite_dao_ext.utils.dataclass_utils import remove_none_from_dict
...
class SqliteQueryGenerator:
    ...
    def insert_sql(
        ...
        items = [obj.dumps() for obj in objs]
        return SqliteQuery(
    ...
    def query_by_values_sql(self, value_map: Dict[str, Any], limit: int, offset: int):
        value_map = remove_none_from_dict(value_map)
        keys = [key for key in value_map.keys() if key in self._field_names]
        return SqliteQuery(
    ...
    def delete_by_values_sql(self, value_map: Dict[str, Any]):
        value_map = remove_none_from_dict(value_map)
        key = [key for key in value_map.keys() if key in self._field_names]
        return SqliteQuery(
            f"DELETE FROM {self._table_name}",
            f"WHERE {' AND '.join([f'{key} = ?' for key in key])}" if key else "",
            args=[value_map[key] for key in key],
        )

    def count_by_values_sql(self, value_map: Dict[str, Any]):
        value_map = remove_none_from_dict(value_map)
        keys = [key for key in value_map.keys() if key in self._field_names]
        return SqliteQuery(
