...
class OSOWaterHeater:
    ...
    def get_heater_state(self, device: dict):
    ...
    def turn_on(self, device: dict, full_utilization: bool):
        """Turn device on.
        ...
        try:
            resp = self.session.api.turn_on(device["device_id"], full_utilization)
            if resp["original"] == 200:
                final = True
                self.session.get_devices()
    ...
    def turn_off(self, device: dict, full_utilization: bool):
        """Turn device off.
        ...
        try:
            resp = self.session.api.turn_off(device["device_id"], full_utilization)
            if resp["original"] == 200:
                final = True
                self.session.get_devices()
    ...
    def set_v40_min(self, device: dict, v40min: float):
        ...
        try:
            resp = self.session.api.set_v40_min(device["device_id"], v40min)
            if resp["original"] == 200:
                final = True
                self.session.get_devices()
    ...
    def set_optimization_mode(self, device: dict, option: Number, sub_option: Number):
        ...
        try:
            resp = self.session.api.set_optimization_mode(
                device["device_id"],
    ...
    def set_profile(self, device: dict, profile: array):
        """Set heater profile.
        ...
        try:
            resp = self.session.api.set_profile(device["device_id"], hours=profile)
            if resp["original"] == 200:
                final = True
                self.session.get_devices()
...
class WaterHeater(OSOWaterHeater):
    ...
    def get_water_heater(self, device: dict):
        ...
        if(device["online"]):
            ...
            dev_data = {
                ...
                "status": {"current_operation": self.get_heater_state(device)},
