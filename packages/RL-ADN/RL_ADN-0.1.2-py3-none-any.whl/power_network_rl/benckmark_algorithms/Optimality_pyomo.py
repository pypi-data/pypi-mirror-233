...
that is to say the voltage regulation only considers that constrainting the limitation connected to the nodes connected to batteries,
 This script is used to test one day global optimality, Vnom important '''
...
config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'environments', 'env_config.py'))
...
if __name__=='__main__':
    ...
    env=PowerNetEnv(env_config_path=config_path)
    env.reset()
    net=env.net
    ppc = net._ppc
    branch = ppc['branch']
    ...
    for i, node in enumerate(NODES):
        if i == 0:
            ...
            Tb[i] = 0
    # only the last node i connected to battery [11,15,26,29,33]
    SORTED_BATTERY_NODES =env.battery_list

    year=2020
    month=8
    day=20
    data_manager=env.data_manager
    day_data=data_manager.select_day_data(year,month,day)
    # day_data shape is (96,36), we now create our active power (PD)
    active_power=day_data[:,0:34]
    renewable_active_power=day_data[:,34:68]
    price=day_data[:,-1]
    netload=active_power
    PD_array=netload/1000
    PRICE = price/100
    Data_Network = {'TIMES': TIMES, 'NODES': NODES, 'LINES': LINES, 'Tb': Tb, 'PD': PD_array, 'QD': None, 'R': R,
                    'X': X, 'BATTERY_NODES': SORTED_BATTERY_NODES, 'PRICE': PRICE}
    Vnom = 1.01
    Vmax = 1.05
    Vmin = 0.95
