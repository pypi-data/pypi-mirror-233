...
import numpy as np
from power_network_rl.DRL_algorithms.utility import Config, ReplayBuffer, clip_grad_norm_, build_mlp, get_optim_param
...
class AgentBase:
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        Args:
            env: RL training environment. Should support `env.reset()` and `env.step()` methods.
            horizon_len (int): Number of steps to collect while exploring to update networks.
            if_random (bool): Use random action for warm-up exploration. Default is False.

        Returns:
            tuple: A tuple containing states, actions, rewards, and undones for off-policy training.
        """
...
class AgentDDPG(AgentBase):
    ...
    def update_net(self, buffer: ReplayBuffer) -> Tuple[float, ...]:
        """
        Update the agent's networks based on the experiences stored in the buffer.

        Args:
            buffer (ReplayBuffer): Replay buffer containing past experiences.

        Returns:
            tuple: A tuple containing the critic's and actor's objectives.
        """
        obj_critics = 0.0
        obj_actors = 0.0
