...
from power_network_rl.utility.utils import create_pandapower_net
from power_network_rl.data_manager.data_manager import GeneralPowerDataManager
from power_network_rl.utility.grid import GridTensor
from power_network_rl.environments.Component_Battery import Battery,battery_parameters
...
class PowerNetEnv(gym.Env):
    """
        ...
        """
    def __init__(self, env_config_path: str = 'env_config.py') -> None:
        """
        ...
        self.voltage_limits = tuple(config['voltage_limits'])
        ...
        if self.network_info == 'None':
            ...
            self.network_info={'vm_pu':1.0,'s_base':1000,'bus_info_file': os.path.join('../data_sources/network_data/node_34', 'Nodes_34.csv'),'branch_info_file': os.path.join('../data_sources/network_data/node_34', 'Lines_34.csv')}
            self.s_base=1000
            self.node_num=34
        else:
            self.s_base=self.network_info['s_base']
            network_bus_info=pd.read_csv(self.network_info['bus_info_file'])
            self.node_num=(network_bus_info.NODES)
