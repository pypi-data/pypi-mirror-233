...
def create_list(data_path,NAC_FLAG=False):
    """
    ...
    """
    pat_folders = []
    for dr in glob.glob(data_path+"/*",recursive=True):
        pat_folders.append(Path(dr))

    list_all=[]
    for pat_folder in pat_folders:
        if NAC_FLAG==True:
            for data in (pat_folder.glob("*PSFTOF_NAC_PET*.nii")):
                label_path = str(data).replace("PET","CT")
                data_path = str(data)
                list_all.extend([{'data': data_path, 'label': label_path}])
        if NAC_FLAG==False:
            for data in (pat_folder.glob("*PSFTOF_PET*.nii")):
                label_path = str(data).replace("PET","CT")
                data_path = str(data)
                list_all.extend([{'data': data_path, 'label': label_path}])
    return list_all
...
def resample_sitk_image_exact_dimension(sitk_image,new_spacing=None, new_size=None, interpolator=None, fill_value=0):
...
class CropBackground(object):
    ...
    def __call__(self, sample):
        ...
        x_centroid = np.int32(centroid[0])
        y_centroid = np.int32(centroid[1])
...
class Augmentation(object):
    ...
    def __call__(self, sample):
        ...
        choice = np.random.choice([0, 1, 3, 5, 12])
