...
from simba.utils.read_write import read_df, write_df, get_fn_ext, find_files_of_filetypes_in_directory
...
class OutlierCorrecterLocationAdvanced(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        if len(self.data_files) == 0:
            raise NoFilesFoundError(msg=f'No data of filetype {input_dir} for in directory {self.file_type}', source=self.__class__.__name__)
        if type not in ['animal', 'body-parts']:
            raise InvalidInputError(msg=f"Type {type} not supported. Valid options: {['animal', 'body-parts']}", source=self.__class__.__name__)
        if agg_method not in ['mean', 'median']:
            raise InvalidInputError(msg=f"Aggregation method {agg_method} not supported. Valid options: {['mean', 'median']}", source=self.__class__.__name__)
        self.settings, self.type, self.agg_method, self.criterion_body_parts = settings, type, agg_method, criterion_body_parts
    ...
    def _save_data_log(self, data_log: dict):
        out = pd.DataFrame(columns=['VIDEO', 'ANIMAL', 'BODY-PART', 'OUTLIER COUNT', 'OUTLIER RATIO (%)'])
        save_path = os.path.join(self.logs_path, f'location_outliers_{self.datetime}.csv')
        for video_name, video_data in data_log.items():
            for animal_name, animal_data in video_data.items():
                for body_part_name, body_part_data in animal_data.items():
                    out.loc[len(out)] = [video_name, animal_name, body_part_name, body_part_data['outlier_cnt'], body_part_data['outlier_ratio']]
        write_df(df=out, file_type='csv', save_path=save_path)
        stdout_success(msg=f'Location outlier log saved at {save_path}', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.data_files):
            ...
            for animal_name, animal_bps in self.animal_bp_dict.items():
                ...
                bp_1_headers = [animal_criterion_bps[0] + '_x', animal_criterion_bps[0] + '_y']
                bp_2_headers = [animal_criterion_bps[1] + '_x', animal_criterion_bps[1] + '_y']
                distances = self.framewise_euclidean_distance(location_1=self.data_df[bp_1_headers].values, location_2=self.data_df[bp_2_headers].values, px_per_mm=1)
            ...
            for animal_name, animal_bps in self.settings.items():
                ...
                animal_bp_headers = np.array([item for pair in zip(self.animal_bp_dict[animal_name]['X_bps'], self.animal_bp_dict[animal_name]['Y_bps']) for item in pair])
                animal_body_part_names = [x[:-2] for x in self.animal_bp_dict[animal_name]['X_bps']]
                animal_body_part_dict = {k:v for (v,k) in zip(animal_body_part_names, range(0, len(animal_body_part_names)))}
                animal_arr = self.data_df[animal_bp_headers].values.reshape(len(self.data_df), -1, 2)
                data = self.three_dim_cdist(data=animal_arr)
                df = pd.concat([pd.DataFrame(arr) for arr in data], keys=np.arange(len(data))).reset_index().rename(columns={'level_0': 'frame', 'level_1': 'body_part'})
                df['body_part'] = df['body_part'].map(animal_body_part_dict)
                above_criterion_dict = {}
                for bp, criterion_multiplier in animal_bps.items():
                    bp_criterion = animal_movement_agg[animal_name] * criterion_multiplier
                    bp_df = df[df['body_part'] == bp]
                    bp_df['counts_above_criterion'] = self.count_values_in_range(data=bp_df.drop(['frame', 'body_part'], axis=1).values, ranges=np.array([[bp_criterion, np.inf]]))
                    above_criterion_dict[bp] = list(bp_df['frame'][bp_df['counts_above_criterion'] > 1])

                for bp, body_part_frames_above_criterion in above_criterion_dict.items():
                    self.results.loc[body_part_frames_above_criterion, [f'{bp}_x', f'{bp}_y']] = np.nan
                    self.results[[f'{bp}_x', f'{bp}_y']] = self.results[[f'{bp}_x', f'{bp}_y']].ffill().bfill().astype(int)
                    video_log[animal_name][bp] = {'outlier_cnt': len(body_part_frames_above_criterion), 'outlier_ratio': len(body_part_frames_above_criterion) / len(self.results)}

            df_save_path = os.path.join(self.outlier_corrected_dir, self.video_name + f'{self.file_type}.')
            write_df(df=self.results, file_type=self.file_type, save_path=df_save_path)
            video_timer.stop_timer()
            stdout_success(msg=f'Location outliers complete for video {self.video_name}.', elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)
            data_log[self.video_name] = video_log

        self._save_data_log(data_log=data_log)
        stdout_success(msg=f'{len(self.data_files)} video(s) corrected for location outliers. Saved in {self.outlier_corrected_dir}', source=self.__class__.__name__)
