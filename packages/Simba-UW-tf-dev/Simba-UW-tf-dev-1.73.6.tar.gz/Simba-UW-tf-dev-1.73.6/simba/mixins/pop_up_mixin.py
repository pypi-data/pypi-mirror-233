...
class PopUpMixin(object):
    ...
    def add_values_to_several_listboxes(self,
        ...
        if len(list_boxes) != len(values):
            raise CountError(msg='Value count and listboxes count are not equal', source=self.__class__.__name__)
    ...
    def create_import_pose_menu(self,
        ...
        def run_call(data_type: str,
            ...
            if self.animal_name_entry_boxes is None:
                raise CountError(msg='Select animal number and animal names BEFORE importing data.', source=self.__class__.__name__)
