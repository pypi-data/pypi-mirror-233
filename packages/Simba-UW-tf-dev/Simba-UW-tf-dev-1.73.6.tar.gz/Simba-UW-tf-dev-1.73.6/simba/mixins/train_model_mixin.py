...
class TrainModelMixin(object):
    ...
    def create_clf_report(self,
        ...
        try:
            ...
            NotEnoughDataWarning(msg=f'Not enough data to create classification report: {class_names[1]}', source=self.__class__.__name__)
    ...
    def dviz_classification_visualization(self,
        ...
        try:
            ...
            NoModuleWarning(
                msg='Skipping dtreeviz example decision tree visualization. Make sure "graphviz" is installed.', source=self.__class__.__name__)
        ...
        for n in range(model_cnt):
            try:
                ...
                if config.get('SML settings', 'model_path_' + str(n + 1)) == '':
                    MissingUserInputWarning(
                        msg=f'Skipping {str(config.get("SML settings", "target_name_" + str(n + 1)))} classifier analysis: no path set to model file', source=self.__class__.__name__)
                    continue
                if config.get('SML settings', 'model_path_' + str(n + 1)) == 'No file selected':
                    MissingUserInputWarning(
                        msg=f'Skipping {str(config.get("SML settings", "target_name_" + str(n + 1)))} classifier analysis: The classifier path is set to "No file selected', source=self.__class__.__name__)
                    continue
                ...
                MissingUserInputWarning(
                    msg=f'Skipping {str(config.get("SML settings", "target_name_" + str(n + 1)))} classifier analysis: missing information (e.g., no discrimination threshold and/or minimum bout set in the project_config.ini', source=self.__class__.__name__)

        if len(model_dict.keys()) == 0:
            raise NoDataError(
                msg=f'There are no models with accurate data specified in the RUN MODELS menu. Specify the model information to SimBA RUN MODELS menu to use them to analyze videos', source=self.get_model_info.__name__)
        else:
            return model_dict
        ...
        if len(new_headers) != len(data_df.columns):
            ...
            if difference < 0:
                raise DataHeaderError(msg=
                    f'SimBA expects {len(new_headers)} columns of data inside the files within project_folder/csv/input_csv directory. However, within file {filepath} file, SimBA found {len(data_df.columns)} columns. Thus, there is {abs(difference)} missing data columns in the imported data, which may represent {int(bp_missing)} bodyparts if each body-part has an x, y and p value. Either revise the SimBA project pose-configuration with {str(bp_missing)} less body-part, or include {bp_missing} more body-part in the imported data', source=self.__class__.__name__)
            else:
                raise DataHeaderError(msg=
                    f'SimBA expects {len(new_headers)} columns of data inside the files within project_folder/csv/input_csv directory. However, within file {filepath} file, SimBA found {len(data_df.columns)} columns. Thus, there is {abs(difference)} more data columns in the imported data than anticipated, which may represent {int(bp_missing)} bodyparts if each body-part has an x, y and p value. Either revise the SimBA project pose-configuration with {str(bp_missing)} more body-part, or include {bp_missing} less body-part in the imported data', source=self.__class__.__name__)
        else:
            ...
            if len(x_nan_cnt) < 10:
                raise FaultyTrainingSetError(
                    msg=f'{str(len(x_nan_cnt))} feature column(s) exist in some files within the project_folder/csv/targets_inserted directory, but missing in others. ' \
                        ...
                        f'column names with mismatches are: {list(x_nan_cnt.index)}', source=self.__class__.__name__)
            else:
                raise FaultyTrainingSetError(
                    msg=f'{str(len(x_nan_cnt))} feature columns exist in some files, but missing in others. The feature files are found in the project_folder/csv/targets_inserted directory. ' \
                        ...
                        f'column names with mismatches are: {list(x_nan_cnt.index)[0:9]}', source=self.__class__.__name__)

        if len(y_df.unique()) == 1:
            if y_df.unique()[0] == 0:
                raise FaultyTrainingSetError(
                    msg=f'All training annotations for classifier {str(y_df.name)} is labelled as ABSENT. A classifier has be be trained with both behavior PRESENT and ABSENT ANNOTATIONS.', source=self.__class__.__name__)
            if y_df.unique()[0] == 1:
                raise FaultyTrainingSetError(
                    msg=f'All training annotations for classifier {str(y_df.name)} is labelled as PRESENT. A classifier has be be trained with both behavior PRESENT and ABSENT ANNOTATIONS.', source=self.__class__.__name__)
    ...
    def clf_predict_proba(self,
        ...
        if len(x_df.columns) != clf.n_features_:
            if model_name and data_path:
                raise FeatureNumberMismatchError(f'Mismatch in the number of features in input file {data_path}, and what is expected by the model {model_name}. The model expects {str(clf.n_features_)} features. The data contains {len(x_df.columns)} features.', source=self.__class__.__name__)
            else:
                raise FeatureNumberMismatchError(f'The model expects {str(clf.n_features_)} features. The data contains {len(x_df.columns)} features.', source=self.__class__.__name__)
    ...
    def clf_fit(self,
        ...
        if len(nan_target) > 0:
            raise FaultyTrainingSetError(
                msg=f'{len(nan_target)} frame(s) in your project_folder/csv/targets_inserted directory contains ANNOTATIONS with non-numerical values', source=self.__class__.__name__)
    ...
    def _read_data_file_helper(self,
        ...
        if clf_names != None:
            for clf_name in clf_names:
                if not clf_name in df.columns:
                    raise ColumnNotFoundError(column_name=clf_name, file_name=file_path, source=self.__class__.__name__)
                elif len(set(df[clf_name].unique()) - {0, 1}) > 0:
                    raise InvalidInputError(msg=f'The annotation column for a classifier should contain only 0 or 1 values. However, in file {file_path} the {clf_name} field contains additional value(s): {list(set(df[clf_name].unique()) - {0, 1})}.', source=self.__class__.__name__)
        timer.stop_timer()
    ...
    def read_all_files_in_folder_mp(self,
        ...
        try:
            ...
            if len(df_concat) == 0:
                raise NoDataError(msg='SimBA found 0 observations (frames) in the project_folder/csv/targets_inserted directory', source=self.read_all_files_in_folder_mp.__name__)
            ...
            MultiProcessingFailedWarning(msg='Multi-processing file read failed, reverting to single core (increased run-time).', source=self.__class__.__name__)
    ...
    def read_all_files_in_folder_mp_futures(self,
        ...
        try:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_cnt) as executor:
                results = [executor.submit(self._read_data_file_helper_futures, data, file_type, classifier_names) for data in file_paths]
            ...
            MultiProcessingFailedWarning(msg='Multi-processing file read failed, reverting to single core (increased run-time on large datasets).', source=self.__class__.__name__)
            return self.read_all_files_in_folder(file_paths=file_paths,
                                            file_type=file_type,
                                            classifier_names=classifier_names)
        ...
        if len(nan_cols) == 0:
            ...
            raise FaultyTrainingSetError(
                msg=f'{len(nan_cols)} feature columns exist in some files, but missing in others. The feature files are found in the project_folder/csv/targets_inserted directory. ' \
                    f'SimBA expects all files within the project_folder/csv/targets_inserted directory to have the same number of features: the first 10 ' \
                    f'column names with mismatches are: {nan_cols[0:9]}. For a log of the files that contain, and not contain, the mis-matched columns, see {save_log_path}', source=self.__class__.__name__)
