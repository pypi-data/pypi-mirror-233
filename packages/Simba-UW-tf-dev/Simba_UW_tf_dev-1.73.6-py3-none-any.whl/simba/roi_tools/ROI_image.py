...
class ROI_image_class():
    ...
    def check_if_ROIs_exist(self):
        roi_measurement_path = os.path.join(self.project_path, 'logs', Paths.ROI_DEFINITIONS.value)

        if os.path.isfile(roi_measurement_path):
            rectangles_found = pd.read_hdf(roi_measurement_path, key=Keys.ROI_RECTANGLES.value)
            circles_found = pd.read_hdf(roi_measurement_path, key=Keys.ROI_CIRCLES.value)
            polygons_found = pd.read_hdf(roi_measurement_path, key=Keys.ROI_POLYGONS.value)
    ...
    def insert_all_ROIs_into_image(self, ROI_ear_tags = False, change_frame_no = False, show_zoomed_img = False, show_size_info=False):
        ...
        for e in self.out_rectangles:
            ...
            if int(e['Thickness']) == 1:
                ...
                cv2.rectangle(self.working_frame, (int(e['topLeftX']), int(e['topLeftY'])), (int(e['topLeftX']) + int(e['width']), int(e['topLeftY']) + int(e['height'])), self.colors[e['Color name']], int(e['Thickness']), lineType=self.line_type)
            if ROI_ear_tags is True:
                for t in e['Tags']:
                    cv2.circle(self.working_frame, e['Tags'][t], int(e['Ear_tag_size']), self.colors[e['Color name']], -1)
