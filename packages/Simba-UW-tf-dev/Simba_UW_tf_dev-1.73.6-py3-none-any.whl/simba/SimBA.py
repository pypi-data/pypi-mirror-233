...
class SimbaProjectPopUp(ConfigReader, PopUpMixin):
    def __init__(self,
        ...
        self.standardize_roi_size_popup_btn = Button(self.new_ROI_frm, text='STANDARDIZE ROI SIZES', fg='blue', compound='left', image=self.btn_icons['calipher']['img'], command=lambda: ROISizeStandardizerPopUp(config_path=self.config_path))
        ...
        btn_agg_boolean_conditional_statistics = Button(processmovementdupLabel,text='AGGREGATE BOOLEAN CONDITIONAL STATISTICS', fg='purple', command=lambda: BooleanConditionalSlicerPopUp(config_path=self.config_path))
        ...
        btn_agg_boolean_conditional_statistics.grid(row=6, sticky=NW)
    ...
    def open_plotly_interface(self, url):
        ...
        self.p = subprocess.Popen([sys.executable, os.path.join(os.path.dirname(__file__), 'dash_app', 'SimBA_dash_app.py'), filePath, groupPath])
        ...
        self.p2 = subprocess.Popen([sys.executable, os.path.join(os.path.dirname(__file__), 'dash_app', 'run_dash_tkinter.py'), url])
...
class App(object):
    def __init__(self):
        ...
        if OS.PYTHON_VER.value != '3.6':
            PythonVersionWarning(msg=f'SimBA is not extensively tested beyond python 3.6. You are using python {OS.PYTHON_VER.value}. If you encounter errors in python>3.6, please report them on GitHub or Gitter and we will fix! (links in the help toolbar)', source=self.__class__.__name__)
    ...
    def paste_to_txt(self):
        try:
            ...
            raise InvalidInputError(msg='Can only paste utf-8 compatible text', source=self.__class__.__name__)
