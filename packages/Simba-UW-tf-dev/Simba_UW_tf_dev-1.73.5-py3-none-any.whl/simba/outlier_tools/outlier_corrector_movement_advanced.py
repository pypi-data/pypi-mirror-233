...
class OutlierCorrecterMovementAdvanced(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        if type not in ['animal', 'body-parts']:
            raise InvalidInputError(msg=f"Type {type} not supported. Valid options: {['animal', 'body-parts']}")
        if agg_method not in ['mean', 'median']:
            raise InvalidInputError(msg=f"Aggregation method {agg_method} not supported. Valid options: {['mean', 'median']}")
        ...
        self.save_path, self.criterion_body_parts = self.outlier_corrected_movement_dir, criterion_body_parts
    ...
    def _save_data_log(self, data_log: dict):
        ...
        save_path = os.path.join(self.logs_path, f'movement_outliers_{self.datetime}.csv')
        ...
        stdout_success(msg=f'Movement outlier log saved at {save_path}', source=self.__class__.__name__)
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.data_files):
            ...
            for animal_name, animal_bps in self.animal_bp_dict.items():
                animal_bp_headers = np.array([item for pair in zip(animal_bps['X_bps'], animal_bps['Y_bps']) for item in pair]).reshape(len(animal_bps['X_bps']), 2)
                ...
                for bps in animal_bp_headers:
                    ...
                    distances = self.framewise_euclidean_distance(location_1=self.data_df_combined[bps].values, location_2=self.data_df_combined[shifted_bps].values, px_per_mm=1)
            ...
            for animal_name, animal_bps in self.settings.items():
                ...
                for bp, criterion_multiplier in animal_bps.items():
                    ...
                    over_criterion_idx = list(self.movements.index[self.movements[bp] > bp_criterion])
                    video_log[animal_name][bp] = {'outlier_cnt': len(over_criterion_idx), 'outlier_ratio': len(over_criterion_idx) / len(self.movements)}
                    self.results.loc[over_criterion_idx, [bp + '_x', bp + '_y']] = np.nan
                    self.results[[bp + '_x', bp + '_y']] = self.results[[bp + '_x', bp + '_y']].ffill().bfill().astype(int)

            df_save_path = os.path.join(self.outlier_corrected_movement_dir, self.video_name + f'.{self.file_type}')
            write_df(df=self.results, file_type=self.file_type, save_path=df_save_path)
            video_timer.stop_timer()
            stdout_success(msg=f'Movement outliers complete for video {self.video_name}.', elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)
            data_log[self.video_name] = video_log
        self._save_data_log(data_log=data_log)
        stdout_success(msg=f'{len(self.data_files)} video(s) corrected for movement outliers. Saved in {self.outlier_corrected_movement_dir}', source=self.__class__.__name__)
#
