...
class ROI_definitions(ConfigReader):
    ...
    def call_delete_all_rois(self):
        ...
        if len(self.image_data.out_rectangles) + len(self.image_data.out_circles) + len(self.image_data.out_polygon) == 0:
            NoDataFoundWarning(msg='SimBA finds no ROIs to delete.', source=self.__class__.__name__)
        else:
    ...
    def create_draw(self):
        ...
        if self.stored_shape is None:
            raise NoROIDataError(msg='No shape type selected.', source=self.__class__.__name__)
        if not self.name_box.get():
            raise NoROIDataError(msg='No shape name selected.', source=self.__class__.__name__)
        if not self.name_box.get().strip():
            raise NoROIDataError(msg='Shape name contains only spaces.', source=self.__class__.__name__)
    ...
    def save_data(self):
        ...
        store.close()
        stdout_success(msg=f'ROI definitions saved for video: {self.file_name}', source=self.__class__.__name__)
