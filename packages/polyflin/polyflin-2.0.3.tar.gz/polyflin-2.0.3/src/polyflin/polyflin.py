        ...
        for chain in chains:
            ...
            for _ in range(chain_nums):
                ...
                if cyclic is not None:
                    temp_chain = make_cyclic(temp_chain, sequence, cyclic, monomers)        
...
def writing_output(format, mol, output, forcefield, hydrogen, ob_func):
    ...
    if format == "TOP":
        if not os.path.exists(f'{output}.mol'):
            process = subprocess.Popen('obabel %s.pdb -O %s.mol --gen3D' % (output, output), shell=True)
            process.wait()  
...
def make_cyclic(temp_chain, sequence, cyclic, monomers):
