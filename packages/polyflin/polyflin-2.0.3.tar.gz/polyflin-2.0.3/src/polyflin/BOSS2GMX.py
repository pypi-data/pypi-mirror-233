from polyflin.BOSSReader import ucomb, pairing_func, bossPdbAtom2Element, bossElement2Mass
from polyflin.BOSSReader import refine_pdb_lines, get_coos_from_pdb
import pickle
import pandas as pd
import numpy as np
...
def boss2opmTorsion(bnd_df, num2opls, st_no, molecule_data, itpf):
    ...
    if len(final_df.index) > 0:
        ...
        tor_bos = final_df.drop(columns=['I', 'J', 'K', 'L', 'TI', 'TJ', 'TK', 'TL'])
...
def pdb2gro(atoms, coos, resid):
    ...
    grof.write('PolyFlin generated GRO file (written by Abd. Kakhar Umar)\n')
    grof.write('%5d\n' % len(atoms))
    num = 0
...
def boss2gmx(resid, molecule_data, pdb_file):
    ...
    itpf.write("""
; Writted by Abd. Kakhar Umar <abdulkaharumar@gmail.com>
; GENERATED BY PolyFlin
; ETFLIN & Jorgensen Lab @ Yale University
;
""")
    ...
    for pair in ppairs:
        itpf.write('%s    1\n' % pair)
    itpf.close()
    pdblines = refine_pdb_lines(pdb_file)
