...
def fepZmatFromFile(filenme, QBCC=None):
    ...
    qdat[ol1] = qdat[ol1].replace(
        "AM1 CM1Ax1.14", "CM1Ax1.14TO1.14CM1A-BCC", 1)
...
def CM5_file2zmat(zmat, QBCC, oname):
    qfile = open(zmat, 'r+')
    qdat, nl1, cl1, ol1 = read_files(qfile)
    cdat, tb, ta = read_coords(qdat[1:cl1])
    ndat = rel_nbd(qdat[ol1 + 1:], ta, QBCC)
    qdat[ol1] = qdat[ol1].replace("AM1 CM1Ax1.14", "CM5 charges scaled by 1.20(for Neutral)/1.00(for Ions)", 1)
    qfile.close()
    target = open(oname, 'w+')
    for i in range(0, ol1 + 1):
        target.write(qdat[i])
