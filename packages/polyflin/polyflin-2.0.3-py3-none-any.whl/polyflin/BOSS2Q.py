from polyflin.BOSSReader import bossPdbAtom2Element, bossElement2Mass, ucomb
import pickle
import pandas as pd
import numpy as np
...
def Boss2CharmmRTF(num2typ2symb, Qs, resid, bnd_df, angs,props,imps):
    ...
    rtf.write('#Remarks generated for Q (by Abd. Kakhar Umar <abdulkaharumar@gmail.com>)\n')
    rtf.write('\n{%s}\n'%resid)
    rtf.write('[atoms]\n')
...
def Boss2CharmmPRM(resid, num2typ2symb, Qs, bnd_df, ang_df, tor_df):
    ...
    prm.write('# generated Q-PARAM file for Aqvist group (by Abd. Kakhar Umar <abdulkaharumar@gmail.com>)\n')
    prm.write('\n[options]\n')
    prm.write('\n[atom_types]\n')
    for i in range(len(Qs)): 
        eps = float(Qs[i][3])
