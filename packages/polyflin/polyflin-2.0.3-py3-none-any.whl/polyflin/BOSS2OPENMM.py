...
from polyflin.BOSSReader import ucomb, bossPdbAtom2Element, bossElement2Mass, tor_cent
import pickle
import pandas as pd
import numpy as np
import os
...
def pdb_prep(atoms, coos, resid, connects):
    ...
    opdb.write('REMARK PolyFlin GENERATED PDB FILE\n')
    num = 0
...
def pqr_prep(atoms, coos, resid, connects, num2pqrtype):
    ...
    opdb.write('REMARK PolyFlin GENERATED PQR FILE\n')
    num = 0
    for (i, j) in zip(atoms, coos):
        num += 1
...
def mainBOSS2OPM(resid, clu):
    ...
    if clu:
        pdb_file = current_path + '/clu.pdb'
    else:
        pdb_file = current_path + '/plt.pdb'
