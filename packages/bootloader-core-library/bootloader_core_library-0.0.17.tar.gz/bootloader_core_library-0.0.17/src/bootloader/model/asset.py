...
class Asset:
    ...
    def data_checksum(self, file_checksum: str):
        """
        Set the SHA256 message digest of the asset's binary data when its
        content has changed.


        :param file_checksum: The SHA256 message digest of the asset's binary
            data.
        """
        logging.debug(
            f"The checksum of the asset file {self._asset_name} has changed "
            f"from the value {self._data_checksum} to the value {file_checksum}"
        )
        self._data_checksum = file_checksum
    ...
    @data_size.setter
    def data_size(self, file_size: int):
        """
        Set the size of the asset's binary data when its content has changed.


        :param file_size: The size in bytes of the asset's binary data.
        """
        logging.debug(
            f"The size of the asset file {self._asset_name} has changed "
            f"from the value {self._data_size} to the value {file_size}"
        )
