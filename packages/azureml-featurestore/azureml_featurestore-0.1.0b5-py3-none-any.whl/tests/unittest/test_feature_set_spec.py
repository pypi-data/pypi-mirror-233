...
{
  "source": "MlTableFeatureSource(path: abfss://container@storage.dfs.core.windows.net/data_sources/customer_transactions/mltable, timestamp_column: TimestampColumn(Name=timestamp,Format=%Y-%m-%d %H:%M:%S), source_delay: DateTimeOffset(Days=1,Hours=0,Minutes=0))",
  "features": [
  ...
  "temporal_join_lookback": "DateTimeOffset(Days=2,Hours=0,Minutes=0)"
    ...
    def test_feature_set_spec_dump(self, mock_copy_rename_and_zip):
        ...
        self.assertEqual(dumped_spec.feature_transformation_code, fset_spec.feature_transformation_code)
        ...
        self.assertEqual(dumped_spec.feature_transformation_code.path, "./code")
