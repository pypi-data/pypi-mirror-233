...
class FeatureSetSpec:
    ...
    def dump(self, dest: Union[str, PathLike], overwrite: bool = False, **kwargs) -> None:
        ...
        If there is a transformation code path specified, it will be copied to destination folder
        If there is a source transformation code path specified, it will be copied to destination folder, and an exception is raised if code folder exists.
        ...
        if self.feature_transformation_code:
            relative_path = self.feature_transformation_code.path
            ...
            self.feature_transformation_code.path = os.path.join("./", os.path.basename(relative_path))
    ...
    def from_config(
        ...
        try:
            ...
            if spec.feature_transformation_code:
                spec.feature_transformation_code._patch_zip(spec_folder_path, datastore_operations)
    ...
    def to_spark_dataframe(
        ...
        **kwargs,
        ...
        try:
            ...
            # apply source lookback, if source lookback is not provided, assume data does not use aggregation outside the time window
            ...
            df = self.source.load(
                start_time=source_start_date_time,
                end_time=feature_window_end_date_time,
            )

            # transform
            if self.feature_transformation_code:
                df = self.feature_transformation_code._apply_spark_transform(transform_function="transform", df=df)
    ...
    if _is_private_preview_enabled():
        ...
        @monitor_with_activity(
            _get_logger(), f"{PACKAGE_NAME}->FeatureSetSpec.ToPandasDataframe", ActivityType.INTERNALCALL
        )
        def _to_pandas_dataframe(
            ...
            **kwargs,
            ...
            try:
                ...
                if self.feature_transformation_code:
                    df = feature_transform(df, self.feature_transformation_code, **kwargs)
                ...
                if isinstance(ex, MlException):
                    _get_logger().error(
                        f"{PACKAGE_NAME}->FeatureSetSpec.ToPandasDataframe, {type(ex).__name__}: {ex.no_personal_data_message}"
                    )
                else:
                    _get_logger().error(f"{PACKAGE_NAME}->FeatureSetSpec.ToPandasDataframe, {type(ex).__name__}: {ex}")
