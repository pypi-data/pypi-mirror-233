...
def materialize_online(feature_set, dataframe_to_store, materialization_version=None):
    ...
    time_to_live_formatted_lua = "'" + str(feature_set.temporal_join_lookback).lower() + "'"
    ...
    def saveIntoRedisViaRedisScript(rdd):
        ...
        for record in rdd:
            # Arglist is the list of values to be send to redis script.
            argList = []
            # Order of operations matters here
            # Redis matches these values to the correct key via the index
            # Redis script expects the value in the format of
            # [keys1, keys2, ...keysn, value_not_serialize1, 2 ... n, value_to_serialize1, 2, ...n, timestamp, TTL]
    ...
    dataframe_to_store.rdd.foreachPartition(lambda x: saveIntoRedisViaRedisScript(x))
