...
class OnTheFlyFeatureGetter:
    ...
    @monitor_with_activity(_get_logger(), f"{PACKAGE_NAME}->GetOnTheFlyFeatures", ActivityType.INTERNALCALL)
    def get_on_the_fly_features(
        self, feature_uris: List[str], observation_df: pyarrow.Table, **kwargs
    ) -> pyarrow.Table:
