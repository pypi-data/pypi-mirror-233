...
from azureml.featurestore._utils._constants import AZUREML_FEATURESTORE_DEBUG_ENVVAR, NETWORK_LATENCY_COLUMN_NAME
...
def _msgpack_unpacker(s):
    ...
    #     packed_byte = msgpack.packb(current_record_value)
    ...
    return msgpack.unpackb(base64.b64decode(s), raw=False)


_type_converters = {
    ColumnType.STRING: lambda s: s.decode("utf-8"),
    ColumnType.INTEGER: lambda s: int(s),
    ColumnType.LONG: lambda s: int(s),
...
def _parse_feature_uris(feature_uris, feature_set_uris_to_recompute, credential):
    ...
    for feature_uri in feature_uris:
        ...
        if feature_set_key not in feature_set_cache:
            feature_set_cache[feature_set_key] = fs_client.feature_sets.get(feature_set_name, feature_set_version)
...
def _init_redis_clients(features, credential):
    ...
    for feature in features:
        ...
        if feature.feature_set_reference.online_store.type != OnlineStoreType.REDIS:
            raise Exception(
                f"Feature '{feature.uri}' belongs to a feature store that specifies an online store connection of type '{feature.feature_set_reference.online_store.type}'. Only Redis stores are currently supported."
            )
...
def _init_hashkey_formats(features):
    ...
    for feature in features:
        ...
        hash_key_format = f"featurestore:{feature.feature_set_reference.feature_store_guid}:featureset:{feature.feature_set_reference.name}:version:{feature.feature_set_reference.version}"
        
        if feature.feature_set_reference.online_materialization_version:
            hash_key_format = f"{hash_key_format}:{feature.feature_set_reference.online_materialization_version}"
...
class OnlineFeatureGetterV2(object):
    def __init__(self, credential, initial_feature_uris=None, feature_sets_to_recompute: Optional[List[str]] = None):
        ...
        # maps feature_set_uris to a format string that can be used with str.format and the observation row dict to produce the redis hashkey for this featureset and any observation row.
        ...
        # maps feature URIs (without query params) to a tuple of (redis_resource_arm_id, feature_set_uri, feature_name, feature_data_type, feature_output_name)
        ...
        if _is_private_preview_enabled():
            ...
            if self.featuresets_to_recompute:
                recomputed_feature_dataframe = self.on_the_fly_feature_getter.get_on_the_fly_features(
                    feature_uris, observation_df, **kwargs
                )
    ...
    def _fetch_feature_data(self, feature_uris: "List[str]", observation_df: "pyarrow.Table"):
        ...
        #         Fetch the keys builder for the featureset, and use it to build the redis hash keys. There should be one for each row in `observation_df`.
        ...
        def fetch_feature_data_from_redis_resource(redis_resource_id: "str"):
            ...
            for observation_dict in observation_df_pylist:
                ...
                for feature_set_uri, feature_tuples in feature_group.items():
                    ...
                    # issue an HMGET for all the features we care about, for this featureset, for this row of observations
        ...
        # For 2 or fewer databases, the overhead from using ThreadPoolExecutor.map() is greater than the speedup obtained from querying in parallel.
    ...
    def _group_features_by_redis_resource_and_featureset(
        ...
        # the top-level dictionary keys are redis resource IDs. All the features under a single key in the top-level dict can be found in that redis resource.
        # the nested dictionary keys are feature set uris. All the features under a single key in the nested dict can be found under a single hash key in redis.
        # The values in the nested dictionary are a list of tuples, with this structure: (feature_name, data_type, output_name)
        redis_resources_to_features = dict()

        for feature_uri in feature_uris:
            if "?" in feature_uri:
                feature_uri, feature_uri_params = feature_uri.split("?")
                # Yes, I know urllib.parse.parse_qs will do this for me. But I profiled parse_qs, and this implementation is 3.5x faster than urllib.parse.parse_qs
