...
class PointAtTimeRetrievalJob(OfflineRetrievalJob):
    ...
    def to_spark_dataframe(self):
        from pyspark.sql.functions import array, col, concat, lit, max, row_number
        ...
        # make a copy of the timestamp column in the ovservation data
    ...
    def _process_feature_set(self, observation_df, feature_set, features, observation_row_id_col_name):
        from pyspark.sql.functions import array, col, concat, expr, lit, max, row_number
        ...
        # if a join lookback limit is provided, we can limit the dataframe to be the entity df start timestamp - temporal join lookback limit
        # also if source_delay is set, shift the feature window accordingly
        # note that this is just define the rough window. the source_delay and temp join look back will be considered accurately in the join itslef
        feature_window_start = None
        # add 1 milli second to the end time
        ...
        observation_dataframe_deup = observation_df.select(col_list).distinct()

        # 4. inner join the feature_set df with observation_dataframe_deup
        ...
        df = (
            df.alias("subquery")
            .join(observation_dataframe_deup.alias("observation_dataframe"), on=idx_cols, how="inner")
            .where(
