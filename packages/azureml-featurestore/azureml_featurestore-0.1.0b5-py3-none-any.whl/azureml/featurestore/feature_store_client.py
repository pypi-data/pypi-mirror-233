...
class FeatureStoreClient(AbstractFeatureStore):
    ...
    :type credential: TokenCredential, Required
    ...
    def __init__(
        ...
        **kwargs: Dict,
        ...
        """
        try:
            with log_activity(_get_logger(), f"{PACKAGE_NAME}->FeaturesSore.Init", ActivityType.PUBLICAPI):
            ...
            if isinstance(ex, MlException):
                ...
                _get_logger().error(f"{PACKAGE_NAME}->FeatureStoreClient.Init, {type(ex).__name__}: {ex}")

            log_and_raise_error(error=ex, debug=True)

    @monitor_with_activity(
        _get_logger(), f"{PACKAGE_NAME}->FeatureStoreClient.Generate_Feature_Retrieval_Spec", ActivityType.PUBLICAPI
    )
    ...
    def resolve_feature_uri(self, feature_uris: List[str], **kwargs: Dict) -> List[Feature]:
        try:
            ...
            if isinstance(ex, MlException):
                _get_logger().error(
                    f"{PACKAGE_NAME}->FeatureStoreClient.Resolve_FeatureUri, {type(ex).__name__}: {ex.no_personal_data_message}"
                )
            else:
                _get_logger().error(f"{PACKAGE_NAME}->FeatureStoreClient.Resolve_FeatureUri, {type(ex).__name__}: {ex}")
    ...
    class FeatureStoreDataplaneOperations:
        ...
        @monitor_with_activity(
            _get_logger(), f"{PACKAGE_NAME}->FeatureStoreDataplaneOperations.Get", ActivityType.PUBLICAPI
        )
        def get(self, **kwargs) -> FeatureStore:
            ...
            try:
                ...
                if isinstance(ex, MlException):
                    _get_logger().error(
                        f"{PACKAGE_NAME}->FeatureStoreDataplaneOperations.Get, {type(ex).__name__}: {ex.no_personal_data_message}"
                    )
                else:
                    _get_logger().error(
    ...
    class FeatureStoreEntityDataplaneOperations:
        ...
        @monitor_with_activity(
            _get_logger(), f"{PACKAGE_NAME}->FeatureStoreEntityDataplaneOperations.Get", ActivityType.PUBLICAPI
        )
        def get(self, name: str, version: str, **kwargs: Dict) -> FeatureStoreEntity:
            ...
            try:
                ...
                if isinstance(ex, MlException):
                    _get_logger().error(
                        f"{PACKAGE_NAME}->FeatureStoreEntityDataplaneOperations.Get, {type(ex).__name__}: {ex.no_personal_data_message}"
                    )
                else:
                    _get_logger().error(
        ...
        @monitor_with_activity(
            _get_logger(), f"{PACKAGE_NAME}->FeatureStoreEntityDataplaneOperations.List", ActivityType.PUBLICAPI
        )
        def list(
            self,
            ...
            **kwargs: Dict,
            ...
            try:
                ...
                if isinstance(ex, MlException):
                    _get_logger().error(
                        f"{PACKAGE_NAME}->FeatureStoreEntityDataplaneOperations.List, {type(ex).__name__}: {ex.no_personal_data_message}"
                    )
                else:
                    _get_logger().error(
    ...
    class FeatureSetDataplaneOperations:
        ...
        @monitor_with_activity(
            _get_logger(), f"{PACKAGE_NAME}->FeatureSetDataplaneOperations.Get", ActivityType.PUBLICAPI
        )
        def get(self, name: str, version: str, **kwargs: Dict) -> FeatureSet:
            ...
            try:
                ...
                feature_set = FeatureSet._from_rest_object(
                    feature_set_version_resource, self._ml_client, self._ml_client.datastores
                )
                entities_dto = []
                ...
                if isinstance(ex, MlException):
                    _get_logger().error(
                        f"{PACKAGE_NAME}->FeatureSetDataplaneOperations.Get, {type(ex).__name__}: {ex.no_personal_data_message}"
                    )
                else:
                    _get_logger().error(f"{PACKAGE_NAME}->FeatureSetDataplaneOperations.Get, {type(ex).__name__}: {ex}")

                log_and_raise_error(error=ex, debug=True)

        @monitor_with_activity(
            _get_logger(), f"{PACKAGE_NAME}->FeatureSetDataplaneOperations.List", ActivityType.PUBLICAPI
        )
        def list(
            ...
            try:
                ...
                if isinstance(ex, MlException):
                    _get_logger().error(
                        f"{PACKAGE_NAME}->FeatureSetDataplaneOperations.List, {type(ex).__name__}: {ex.no_personal_data_message}"
                    )
                else:
                    _get_logger().error(
...
@monitor_with_activity(_get_logger(), f"{PACKAGE_NAME}->Get_OfflineFeatures", ActivityType.PUBLICAPI)
def get_offline_features(
    *, features: List[Feature], observation_data, timestamp_column: str, use_materialized_data: bool = True, **kwargs
    ...
    Time travel is based on the configured temporal join lookback for each feature set. A shorter lookback will limit the
    ...
    Args:
        ...
        features: The list of features that should be retrieved from the offline store. Feature is obtained by feature setspec['feature_name']
...
def get_online_features(features: List[Feature], observation_data: "pyarrow.Table"):
    ...
    Args:
        observation_data: (pyarrow.Table): An entity dataframe is a collection of rows containing all entity
            columns (e.g., customer_id, driver_id) on which features need to be joined. The entity dataframe is a pyarrow Table.
