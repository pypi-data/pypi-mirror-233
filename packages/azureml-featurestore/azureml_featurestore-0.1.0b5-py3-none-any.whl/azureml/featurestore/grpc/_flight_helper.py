...
from azureml.featurestore._utils._constants import IS_IN_CI_PIPELINE, ON_THE_FLY_FEATURE_SETS
...
def initialize(features: List[Feature], credential, force, **kwargs):
    global __FEATURESTORE_SERVICE_PROCESS
    
    for feature in features:
        if feature.feature_set_reference.online_store.connection_name != feature.feature_set_reference.online_store_connection_name:
    ...
    with __INITIALIZER_MUTEX:
        ...
        on_the_fly_feature_sets = []
        if ON_THE_FLY_FEATURE_SETS in kwargs:
            on_the_fly_feature_sets = kwargs[ON_THE_FLY_FEATURE_SETS]
