...
class CustomFeatureSource(FeatureSourceBase):
    ...
    def load(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> "DataFrame":
        source_df = self.source_process_code._apply_spark_transform(
    ...
    if _is_private_preview_enabled():
        ...
        def _load_pandas(self, start_time: datetime, end_time: datetime) -> pd.DataFrame:
