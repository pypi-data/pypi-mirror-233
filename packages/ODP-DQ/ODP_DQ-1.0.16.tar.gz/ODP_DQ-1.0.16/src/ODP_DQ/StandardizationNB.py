...
class Standardization:
    def __init__(self,columns,df,api_key,api_base,api_version,deployment_model,model_name):
    ...
    def format_issue_detection(self):
        ...
        for column in columns:
            ...
            agent = create_pandas_dataframe_agent(llm, self.df[[column]], verbose=True, max_iterations=15, early_stopping_method="generate")
        ...
        final= pd.DataFrame(final,columns=columns_order)
        #final_df = ps.DataFrame(final, columns=columns_order)
        final_df=ps.from_pandas(final)
