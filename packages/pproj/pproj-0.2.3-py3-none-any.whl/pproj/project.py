...
class PipMetaPathFinder(importlib.abc.MetaPathFinder):
    ...
    def find_spec(
            fullname: str, path: Sequence[str | bytes] | None, target: ModuleType | None = None
    ) -> importlib._bootstrap.ModuleSpec | None:  # noqa: SLF001
...
class Project:
    ...
    def _openai(self):
        ...
        try:
            ...
            diff_string = stdout(f"{self.git} diff --cached .")
            prompt = (
                f"What follows '-------' is a git diff for a potential commit. "
    ...
    def coverage(self) -> int:
        ...
        if (
                ...
                and subprocess.check_call(f"{self.executable()} "
                                          f"-m coverage run -m pytest {self.root}", shell=True) == 0
    ...
    def extras(self, default: bool = True, as_list: bool = False) -> dict[str, list[str]] | list[str]:
        ...
        if self.pyproject_toml.config:
            ...
            extras |= {
                item.split("; extra == ")[1].replace('"', ""): item.split("; extra == ")[0]
        ...
        if (
            ...
            with tempfile.NamedTemporaryFile() as tmp:
                subprocess.check_call(
                    f"grep -v GITHUB_ {secrets} > {tmp.name} && gh secret set -f {tmp.name}", shell=True
                )
    ...
    def twine(
        ...
        if (
            ...
            return subprocess.run(
                f"{self.executable()} -m twine upload -u __token__  {self.build()}", shell=True
            ).returncode
        return 0
    ...
    def venv(self, version: str = PYTHON_DEFAULT_VERSION, force: bool = False, upgrade: bool = False):
        ...
        if all([not in_tox(), ci]):
            ...
            subprocess.check_call(
                [self.executable(), "-m", "pip", "install", "--upgrade", "-q", "pip", "wheel", "setuptools", "build"]
            )
