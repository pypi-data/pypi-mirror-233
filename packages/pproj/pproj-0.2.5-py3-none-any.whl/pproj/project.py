...
class Project:
    ...
    def _openai(self):
        ...
        try:
            ...
            output = openai.Completion.create(  # noqa: F821
    ...
    def coverage(self) -> int:
        ...
        if (
            ...
            and subprocess.check_call(
                f"{self.executable()} -m coverage report --data-file={self.root}/reports/.coverage", shell=True
            )
            == 0
        ):
    ...
    def extras(self, default: bool = True, as_list: bool = False) -> dict[str, list[str]] | list[str]:
        ...
        if self.pyproject_toml.config:
            ...
            for item in d.requires:
                if "; extra" in item:
                    key = item.split("; extra == ")[0].replace("'", "")
                    if key not in extras:
                        extras[key] = []
                    extras[key].append(item.split("; extra == ")[1].replace('"', ""))
        else:
