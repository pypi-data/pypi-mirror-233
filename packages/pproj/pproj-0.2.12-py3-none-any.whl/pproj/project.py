...
class Project:
    ...
    def extras(
        self, default: bool = True, as_list: bool = False
        ...
        extras = {}
        if default:
            extras = Project._extras(
                importlib.metadata.Distribution.from_name(PPROJ_PROJECT_NAME)
            )
            if self.name != PPROJ_PROJECT_NAME:
                if "dev" not in extras:
                    extras["dev"] = []
                extras["dev"].append(f"{PPROJ_PROJECT_NAME}")
        if self.pyproject_toml.config:
            extras |= self.pyproject_toml.config.get("project", {}).get(
                "optional-dependencies", {}
            )
        elif d := self.distribution():
            extras |= self._extras(d)
        else:
...
venv.CORE_VENV_DEPS = Project._extras(
    importlib.metadata.Distribution.from_name(PPROJ_PROJECT_NAME), as_list=True
)
