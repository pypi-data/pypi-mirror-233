...
class Project:
    ...
    def extras(self, as_list: bool = False) -> dict[str, list[str]] | list[str]:
        ...
        if "dev" in extras and PPROJ_PROJECT_NAME in extras["dev"]:
            pproj_extras = Project.pproj().extras()
    ...
    def github(self) -> dict[str, str | list | dict[str, str | list | dict[str, str | list]]]:
        """GitHub repo api.
    ...
    def pypi(self) -> dict[str, str | list | dict[str, str | list | dict[str, str | list]]]:
        ...
        if self.pyproject_toml.file:
            ...
            if self.name != PPROJ_PROJECT_NAME:
                ...
                for key, value in extras.items():
                    if "optional-dependencies" not in self.pyproject_toml.config["project"]:
                        self.pyproject_toml.config["project"]["optional-dependencies"] = {}
                    if key not in self.pyproject_toml.config["project"]["optional-dependencies"]:
                        self.pyproject_toml.config["project"]["optional-dependencies"][key] = value
                    else:
                        for item in value:
                            if item not in self.pyproject_toml.config["project"]["optional-dependencies"][key]:
                                self.pyproject_toml.config["project"]["optional-dependencies"][key].append(item)
                if extras != self.pyproject_toml.config["project"]["optional-dependencies"]:
                    changed = True

            if original_project != self.pyproject_toml.config["project"] or changed:
