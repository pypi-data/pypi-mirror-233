...
from .serialisation import Deserializer, File, ItemType, Serializer, Trigger, all_actors, OverviewTableItem
        ...
        while True:
            ...
            if break_on_newline:
                if token.type == TokenType.NEWLINE:
                    ...
                    break_on_newline = False
            match token:
                case Token(type=TokenType.PARENTHESIS, data='('):
                    content.append((True, self.expect(type=TokenType.TEXT).data))
                    self.expect(type=TokenType.PARENTHESIS, data=')')
                case Token(type=TokenType.TEXT):
                    content.append((False, token.data))
                case Token(type=TokenType.ESCAPE_CHR):
                    content.append((False, next(self.token_stream).data))
...
class Database:
    ...
    @classmethod
    def open(cls, database_location: str, play_name: str) -> Any | None:
...
def detect_changes(new_file: File, database: Database):
    old_file = database.serialized_file
    if old_file is None:
...
def convert_with_error(in_file_path: str, database_location: str):
    ...
    if (database := Database.open(database_location, play_name)) is not None:
        detect_changes(serialized_file, database)
