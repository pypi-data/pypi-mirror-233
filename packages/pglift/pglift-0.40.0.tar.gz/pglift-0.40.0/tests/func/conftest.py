...
def pgbackrest_repo_host(
    ...
    pgbackrest_execpath: Optional[Path],
    ca_cert: Path,
    cert_factory: CertFactory,
    tmp_path_factory: pytest.TempPathFactory,
    tmp_port_factory: Iterator[int],
    logger: logging.Logger,
) -> Iterator[Optional[PgbackrestRepoHost]]:
...
@pytest.fixture(scope="package")
def passfile(
    postgresql_auth: AuthType, postgresql_settings: PostgreSQLSettings
) -> pathlib.Path:
...
def settings(
    ...
    etcd_host: Etcd,
    pgbackrest_execpath: Optional[pathlib.Path],
...
def require_pgbackrest_localrepo(
    settings: Settings, pgbackrest_repo_host: Optional[PgbackrestRepoHost]
) -> None:
...
def tablespace_factory(
    instance: system.Instance, tmp_path_factory: pytest.TempPathFactory
