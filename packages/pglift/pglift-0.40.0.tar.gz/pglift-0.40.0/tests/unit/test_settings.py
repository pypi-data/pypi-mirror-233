...
def test_postgresqlsettings_bindir() -> None:
    with pytest.raises(
        ...
        PostgreSQLSettings.parse_obj({"bindir": "xxx"})
...
def test_postgresqlsettings_versions(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="either a value is required"):
        PostgreSQLSettings.parse_obj({"versions": [], "bindir": None})
    ...
    s = PostgreSQLSettings.parse_obj({"versions": [], "bindir": bindir})
    assert [v.dict() for v in s.versions] == [
        {"bindir": bindir_15, "version": PostgreSQLVersion.v15}
    ]
...
def test_postgresqlsettings_default_version(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError, match="empty 'versions' field"):
        PostgreSQLSettings.parse_obj(
            {
                "default_version": "14",
                "bindir": None,
    ...
    # Same with a bindir, but unusable to build a 'versions' field.
    with pytest.raises(ValidationError, match="empty 'versions' field"):
        PostgreSQLSettings.parse_obj(
            {
                "default_version": "14",
...
def test_role_pgpass(bindir_template: str) -> None:
    ...
    assert not PostgreSQLSettings.parse_obj(
...
@pytest.mark.parametrize(
    "obj, expecting",
    [
        ({"postgresql": {"datadir": "/var/lib/{name}"}}, "name, version"),
        (
            {
                "pgbackrest": {
                    "repository": {
                        "mode": "path",
                        "path": "/mnt/backups/{version}-{name}",
                    }
                }
            },
            "none",
...
def test_validate_templated_path(
    bindir_template: str, obj: dict[str, Any], expecting: str
) -> None:
    obj.setdefault("postgresql", {})["bindir"] = bindir_template
    with pytest.raises(
        ValidationError,
        match=(
            r"value contains unknown or missing template variable\(s\); expecting: "
            + expecting
        ),
    ):
...
def test_settings_validate_prefix(postgresql_settings: PostgreSQLSettings) -> None:
...
def test_systemd_sudo_user() -> None:
    with pytest.raises(ValidationError, match="cannot be used with 'user' mode"):
        SystemdSettings(user=True, sudo=True)


def test_systemd_disabled(postgresql_settings: PostgreSQLSettings) -> None:
...
def test_pgbackrest_repository(
    ...
    f = tmp_path / "f"
    f.touch()
    s = PgBackRestSettings.parse_obj(
        {
            "execpath": str(pgbackrest_execpath),
            "repository": {
                "mode": "host-tls",
    ...
    assert isinstance(s.repository, PgBackRestSettings.HostRepository)
    ...
    assert isinstance(s.repository, PgBackRestSettings.PathRepository)

    with pytest.raises(ValidationError, match="repository -> PathRepository -> foo"):
        PgBackRestSettings.parse_obj(
            {
                "execpath": str(pgbackrest_execpath),
...
def test_patroni_etcd_cert_and_protocol(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError, match="'https' protocol is required"):
        Etcd(cacert=cacert)
    Etcd(cacert=cacert, protocol="https")
    Etcd(protocol="https")
...
def test_patroni_restapi_verify_client(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="requires 'certfile' to enable TLS"):
        RESTAPI(verify_client="required")

    certfile = tmp_path / "cert.pem"
    certfile.touch()
    RESTAPI(certfile=certfile, verify_client="required")
...
def test_patroni_settings_moved_to_postgresql() -> None:
    with pytest.warns(
        FutureWarning, match="passfile is deprecated; use postgresql.passfile instead"
    ):
        s = PatroniSettings.parse_obj({"passfile": "/tmp/{name}.pass"})
    assert str(s.postgresql.passfile) == "/tmp/{name}.pass"
    assert s.passfile is None

    with pytest.raises(
        ValidationError,
        match="use_pg_rewind and postgresql.use_pg_rewind are mutually exclusive",
    ):
        PatroniSettings.parse_obj(
            {"use_pg_rewind": True, "postgresql": {"use_pg_rewind": False}}
        )
