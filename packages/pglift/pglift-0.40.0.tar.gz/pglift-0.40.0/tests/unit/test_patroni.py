...
def test_servicemanifest_postgresql_connect_host(tmp_path: Path) -> None:
    cert = tmp_path / "cert"
    cert.touch()
    key = tmp_path / "key"
    ...
    with pytest.warns(
        FutureWarning, match="postgresql_connect_host field is deprecated"
    ):
        s = models.ServiceManifest.parse_obj(
            {
                "cluster": "test",
                "postgresql_connect_host": "0.0.0.0:8080",
                "postgresql": {"replication": {"ssl": {"cert": cert, "key": key}}},
            }
        )
    assert s.postgresql and s.postgresql.connect_host == "0.0.0.0:8080"
    ...
    with pytest.raises(
        ...
        match="postgresql_connect_host and postgresql.connect_host are mutually exclusive",
    ):
        models.ServiceManifest.parse_obj(
            {
                "cluster": "test",
                "postgresql_connect_host": "0.0.0.0:8080",
                "postgresql": {"connect_host": "1.2:3"},
            }
        )
