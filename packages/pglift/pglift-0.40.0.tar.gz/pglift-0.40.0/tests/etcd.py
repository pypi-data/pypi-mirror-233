...
class Etcd:
    ...
    proc: Optional[subprocess.Popen[bytes]] = field(init=False, default=None)
    ...
    def try_connect(self) -> None:
        ...
        with socket.create_connection(
            (self.host, self.client_port), timeout=5
        ) as sock, context.wrap_socket(sock, server_hostname=self.host) as ssock:
            logger.info("successfully connected to etcd (%s)", ssock.version())

    def setup_auth(
        self, *, credentials: tuple[str, str], role: str, prefix: str
    ) -> None:
    ...
    def running(self) -> Iterator[Self]:
        ...
        with self.server_certificate.cert_chain_pems[0].tempfile(
            str(ssldir)
        ) as certfile, self.server_certificate.private_key_pem.tempfile(
            str(ssldir)
        ) as keyfile:
            cmd = [
