...
from .. import conf, exceptions, plugin_manager, types
...
class BaseInstance:
    ...
    @classmethod
    def get(cls, name: str, version: Optional[str], settings: Settings) -> Self:
...
class PostgreSQLInstance(BaseInstance):
    ...
    @classmethod
    def system_lookup(
        cls, value: Union[BaseInstance, tuple[str, Optional[str], Settings]]
    ) -> Self:
        ...
        if not isinstance(value, BaseInstance):
            ...
            name, version, settings = value.name, value.version, value._settings
        try:
            self = cls.get(name, version, settings)
            self.exists()
    ...
    @property
    def socket_directory(self) -> Optional[str]:
...
class Instance(PostgreSQLInstance):
    ...
    def system_lookup(
        ...
        values["services"] = [
            s for s in pm.hook.system_lookup(instance=pg_instance) if s is not None
        ]
        return cls(**values)
...
class DatabaseDump:
    id: str
    dbname: str
    date: datetime = field(converter=datetime.fromisoformat)  # type: ignore[misc]

    @classmethod
    def build(cls, dbname: str, date: str) -> Self:
        """Build a DatabaseDump from dbname and date.

        >>> DatabaseDump.build("postgres", str(datetime(2023, 3, 14)))  # doctest: +ELLIPSIS
        DatabaseDump(id='postgres_...', dbname='postgres', date=datetime.datetime(2023, 3, 14, 0, 0))
        """
        id = "_".join(
            [
                dbname,
        ...
        return cls(id=id, dbname=dbname, date=date)
...
class DefaultPrivilege:
    ...
    query: ClassVar[str] = "database_default_acl"

    database: str
    schema: str
    object_type: str
    role: str
    privileges: list[str] = field(converter=_sort)
...
class Privilege(DefaultPrivilege):
    ...
    query: ClassVar[str] = "database_privileges"

    object_name: str
    column_privileges: dict[str, list[str]] = field(converter=_sort_values)
