...
from .models import Backup, Service, ServiceManifest
...
def create_include_directory(settings: "PgBackRestSettings") -> None:
    logger.info("creating pgbackrest include directory")
    config_directory(settings).mkdir(mode=0o750, exist_ok=True, parents=True)


def delete_include_directory(settings: "PgBackRestSettings") -> None:
    logger.info("deleting pgbackrest include directory")
    util.rmdir(config_directory(settings))
...
def service(
    ...
    """
    if upgrading_from:
        if s := system_lookup(upgrading_from.datadir, settings):
            return s
    if s := system_lookup(instance.datadir, settings):
        if manifest.stanza != s.stanza:
            raise exceptions.InstanceStateError(
                f"instance {instance} is already bound to pgbackrest stanza {s.stanza!r} (path={s.path})"
            )
        return s
...
def site_configure_install(
    ...
    global_configpath = base_configpath(s)
...
def site_configure_uninstall(settings: "Settings") -> None:
    ...
    base_configpath(s).unlink(missing_ok=True)
...
def instance_configured(
    ...
    service_ = service(instance, service_manifest, settings, upgrading_from)
    setup(ctx, service_, settings, config, instance.datadir)


@contextlib.contextmanager
def instance_dropped(
    ctx: "Context", instance: system.Instance
) -> Iterator[Optional[Service]]:
    try:
        service = instance.service(Service)
    except ValueError:
        yield None
    ...
    settings = get_settings(instance._settings)
    yield service
    revert_setup(ctx, service, settings, instance.config(), instance.datadir)
