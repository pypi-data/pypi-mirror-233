...
class IcodeAPI():
    ...
    '''
    cookie : str = ''
    userAgent : str = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36 Edg/115.0.1901.203'
    info : dict = {}
    client : httpx.Client = None
    loginStatus = False

    def __init__(self, cookie : str = '', userAgent : str = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36 Edg/115.0.1901.203', httpxClient : httpx.Client = httpx.Client(), timeout : Union[int, float] = 10):
    ...
    def login(self) -> dict:
        '''
        Login to https://icodeshequ.youdao.com use self.cookie.
        ...
        ```
        '''
        headers = {'User-Agent' : self.userAgent, 'Cookie' : self.cookie}
    ...
    def readAllMessages(self, tab : int = 1) -> dict:
        ...
        if not self.loginStatus:
            raise LoginError('User is not logged in')
        headers = {'Cookie': self.cookie, 'User-Agent': self.userAgent}
        headers['Content-Type'] = 'application/json'
        body = {
            "tab" : tab
        }
...
class AsyncIcodeAPI(IcodeAPI):
    ...
    def __init__(self, cookie : str = '', userAgent : str = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36 Edg/115.0.1901.203', httpxClient : httpx.AsyncClient = httpx.AsyncClient(), timeout : Union[float, int] = 10):
        self.cookie = cookie.encode('utf-8')
        self.userAgent = userAgent
        self.client = httpxClient
        self.client.timeout = timeout
    ...
    async def deleteComment(self, commentId : int = None, replyId : int = None) -> dict:
        ...
        headers = {'Cookie' : self.cookie, 'User-Agent' : self.userAgent}
        if commentId:
            response = await self.client.post(f'https://icodeshequ.youdao.com/api/works/comment/delete?commentId={commentId}', body = 'IcodeAPI delete comment'.encode('utf-8'), headers = headers)
        elif replyId:
            response = await self.client.post(f'https://icodeshequ.youdao.com/api/works/reply/delete?replyId={replyId}', body = 'IcodeAPI delete reply'.encode('utf-8'), headers = headers)
        else:
            raise ValueError(f'Both commentId and replyId is None')
        result = response.json()
        return result
...
2019,
2020,
在v1.0版本, icodeapi的__init__.py总共拥有2023条代码!
庆祝icodeapi正式版v1.0在2023/10/2发布!
'''