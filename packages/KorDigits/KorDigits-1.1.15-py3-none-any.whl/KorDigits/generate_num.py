...
class Label2Num:
            ...
            for i in range(len(self.sentence_list)):
                ...
                for w, p in self.mecab.pos(self.sentence_list[i]):
                    if p == 'SN' and re.search('\d+', w):
                        is_SN = True
                    elif is_SN and (p == 'NNBC' or p == 'NR'):
                        ''' 한국어로 읽히는 리스트에 있는지 확인 '''
                        if w in kor_unit_list:
    ...
    def _generate_pred(self, label, tagging):
        if label == 0:
            ...
            try:
                if len(tagging) < 13 or tagging[-3:] == '000':
    ...
    def _label_0(self, tagging):
        # 몇시 몇분 -> '시' 12이하는 한자어, 13이상은 고유어, '분' 한자어
        # 몇시간 몇분 -> '시간' 고유어, '분' 한자어
        # 9:00 처리
        output = tagging[:]
        ...
        if re.search(r"\d+", output):
            ...
            for n in nums:
                ...
                output = output.replace(n, num_words)
...
if __name__ == '__main__':
    ...
    print(label2num.generate("'267명 324명 352명 303명 450명 2019년 기준으로 선주 6만 3천여 명 중 50세 이상이 79  70세 이상이 18  차지 외국인 선원   1181511305122941358313901 이에 따라 정부도 어선 안전공간 확대를 위한 검사 지침 등을 마련하여 시행하였으나 안전 복지 공간 확보라는 취지와 달리 어획량을 늘리기 위한 불법 증개축의 수단으로 활용되어 폐지한 바 있다.' "))
