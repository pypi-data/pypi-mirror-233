...
class DynamicRateLimiterManager:
    ...
    def log_request(self, service_name, success):
        ...
        print(f"[{service_name}] Request {'succeeded' if success else 'denied'}. Current tokens: {self.services[service_name].tokens}")
class DynamicRateLimiter:
    def __init__(self, low_limit, high_limit, limit_epoch, starting_tokens=None):
        self.low_limit = low_limit
        self.high_limit = high_limit
        self.limit_epoch = limit_epoch  # in seconds
        self.request_status_json = {"succesful":[],"unsuccesful":[]}
        self.tokens = starting_tokens or low_limit  # Default to high_limit if starting_tokens isn't provided
    ...
    def _refill_tokens(self):
        ...
        self.tokens = min(self.current_limit, self.tokens + new_tokens)
    ...
    def request(self):
        ...
        if self.tokens > 1:
            self.tokens -= 1
            self.last_request_time = get_time_stamp()
            self.request_count_in_current_epoch += 1
    ...
    def _adjust_limit(self):
        ...
        if current_time - self.last_adjusted_time >= self.limit_epoch:
            if len(self.clear_epoch()["succesful"]) >= self.tokens:
                ...
                self.tokens = max(self.low_limit, self.tokens - 1)
            else:
                ...
                if self.successful_epochs_since_last_adjustment >= 5:
                    ...
                    self.tokens = min(self.high_limit, self.tokens + 1)
                    self.successful_epochs_since_last_adjustment = 0
    ...
    def adjust_limit(self):
        # Set the tokens to requests_made - 1
        self.tokens = len(self.clear_epoch()["succesful"])

        # Adjust the high_limit
        self.current_limit = self.tokens

        # Log the adjustment
        print(f"Adjusted tokens to: {self.tokens} and high_limit to: {self.high_limit}")
...
def get_limited_request(request_url=str,service_name="default"):
    ...
    if not manager.request(service_name):
        ...
        count = manager.services[service_name].clear_epoch()["succesful"]
        current_limit = manager.services[service_name].current_limit
       
        percent_epoch = manager.services[service_name].limit_epoch-(manager.services[service_name].clear_epoch()["succesful"][0]-manager.services[service_name].clear_epoch()["succesful"][-1])
        sleep_count_down(manager.services[service_name].limit_epoch/current_limit)  # Wait for the limit_epoch duration
        return None
    ...
    if response.status_code == 429:
        ...
        manager.services[service_name].clear_epoch()
        if manager.services[service_name].clear_epoch()["succesful"]<2:
            sleep_count_down(manager.services["coin_gecko"].limit_epoch)  # Wait for the limit_epoch duration
        else:
            manager.services[service_name].current_limit-=1
            sleep_count_down(manager.services[service_name].limit_epoch/len(manager.services[service_name].clear_epoch()["succesful"]))  # Wait for the limit_epoch duration
        return 
