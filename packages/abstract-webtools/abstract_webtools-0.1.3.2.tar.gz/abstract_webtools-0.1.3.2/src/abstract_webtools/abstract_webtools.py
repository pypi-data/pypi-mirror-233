...
from abstract_utilities.time_utils import get_time,get_sleep
...
class SafeRequest:
    def __init__(self, max_retries=3, request_wait_limit=1.5):
    ...
    def wait_between_requests(self):
        ...
        if self.last_request_time:
            sleep_time = self.request_wait_limit - (get_time() - self.last_request_time)
            if sleep_time > 0:
            ...
            for cleaned_url in cleaned_urls:
                try:
                    ...
                    if response.status_code == 200:
                        self.last_request_time = get_time()
                        return response
                    elif response.status_code == 429:
                        logging.warning(f"Rate limited by {cleaned_url}. Retrying...")
                        sleep(5)  # adjust this based on the server's rate limit reset time
