...
class SafeRequest:
        ...
        def __init__(self, url_manager,soup_manager=None, headers=None, max_retries=3, request_wait_limit=1.5):
            ...
            self.response = self.try_request()
        ...
        def try_request(self,timeout=10) -> (requests.Response or None):
            ...
            """
            if self.url_manager.url:  # Using the URL from URLManager
                input(self.url_manager.url)
                return self.session.get(url=self.url_manager.url, timeout=10)
        ...
        def get_source_code(self):
            """Return the source code if available."""
            return self.source_code if self.source_code else None
        ...
        def make_request(self):
            ...
            for _ in range(self.max_retries):
                ...
                if self.response:
                    ...
                    logging.error("Failed to make request after max retries")
                if self.response and self.response.status_code == 200:
                    self.last_request_time = get_time_stamp()
                    self.process_response()
                    return self.response

                elif self.response and self.response.status_code == 429:
...
class URLManager:
    ...
    def clean_url(self) -> list:
        ...
        if self.url:
            ...
            cleaned = self.url.replace("http://", "").replace("https://", "")
...
class SoupManager:
    def __init__(self, url_manager,request_manager, parse_type="html.parser"):
        ...
        self.web_soup= BeautifulSoup(self.request_manager.source_code, parse_type)
        self.meta_tags =  {}
        self.get_meta_tags()
        self.all_desired = []
        self.get_all_desired()
    ...
    def create_soup(self):
        self.web_soup = BeautifulSoup(self.request_manager.source_code, self.parse_type)
        self.all_tags = self.find_all(element=True)
    def get_all_desired(self, tag=None, class_name=None, class_value=None):
        if not tag:
            tags = self.web_soup.find_all(True)  # get all tags
        else:
            tags = self.web_soup.find_all(tag)  # get specific tags
        extracted_tags = []
        for tag in tags:
            if class_name:
    ...
    def get_all_website_links(self,url=None,tag="a",class_name="href") -> list:
        ...
        for tag in self.all_desired:
            href = tag.attrs.get(class_name)
            if href == "" or href is None:
                # href empty tag
                continue
            href=self.get_relative_href(self.url_manager.url,href)
            ...
            url_manager.all_urls.append(href)
    ...
    def get_react_source_code(self) -> list:
            ...
            """
            if  self.url_manager.url == None:
                return []
            soup_manager = SoupManager(url=self.url_manager.corr,source_code=self.request_manager)
            script_tags =  self.web_soup.find_all('script', type=lambda t: t and ('javascript' in t or 'jsx' in t))
            for script_tag in script_tags:
                self.react_source_code.append(script_tag.string)
            request_manager.react_source_code= self.react_source_code.append(script_tag.string)
    ...
    def find_all(self,element,soup=None):
        soup = self.web_soup if soup == None else soup
    ...
    def get_find_all_with_attributes(self, *attrs):
        return self.web_soup.find_all(lambda t: self.has_attributes(t, *attrs))
    ...
    def get_images(self, tag_name, class_name, class_value):
        images = []
        for tag in self.web_soup.find_all(tag_name):
    ...
    def discover_classes_and_meta_images(self, tag_name, class_name_1, class_name_2, class_value, attrs):
        ...
        def get_all_website_links(self,url=None,tag="a",class_name="href") -> list:
            ...
            if url is None:
                return []
            self.all_desired=self.get_all_desired(tag=tag,class_name=class_name)
            for tag in self.all_desired:
                href = tag.attrs.get(class_name)
                if href == "" or href is None:
                    # href empty tag
...
def CrawlManager():
    ...
    def crawl(url, max_depth=3, depth=1):
        ...
        try:
            ...
            links = [a['href'] for a in self.web_soup.find_all('a', href=True)]
            valid_links = []
