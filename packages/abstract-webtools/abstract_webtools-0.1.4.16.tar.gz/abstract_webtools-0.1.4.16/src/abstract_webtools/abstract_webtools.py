...
class SoupManager:
    def __init__(self,url=None,source_code=None,parse_type="html.parser"):
        ...
        if self.source_code ==None:
            if self.url != None:
                ...
                return 
        self.comments = []
        self.pure_source = self.un_comment_source_code()
        self.attributes = self.get_all_attributes()
        self.class_names=[]
        self.class_values=[]
    ...
    def get_meta_tags(self):
        new_soup = self.find_all("meta")
        meta_tags={}
        for meta_component in new_soup:
            meta_property = self.get_class(soup = meta_component,class_name="property")
            if meta_property != None:
                if meta_property not in meta_tags:
                    meta_tags[meta_property] = []
                content = self.get_class(soup = meta_component,class_name="content")
                if content != None and content not in  meta_tags[meta_property]:
                    meta_tags[meta_property].append(content)
        return meta_tags
    def get_rid_of_sections(self,source_code,comment_delimeters=['!--','-->']):
        begin_comment=source_code.split(comment_delimeters[0])
        source = begin_comment[0]
        for section in begin_comment[1:]:
            if comment_delimeters[1] in section:
                end_section = section.split(comment_delimeters[1])[0]+comment_delimeters[1]
                self.comments.append(comment_delimeters[0]+end_section)
                source+=section[len(end_section):]
        return source
    def un_comment_source_code(self):
        comment_delimeters = ['!--','-->']
        comments = []
        pure_source_code=self.get_rid_of_sections(self.source_code,comment_delimeters=comment_delimeters)
        comment_delimeters = ['/*','*/']
        pure_source_code=self.get_rid_of_sections(pure_source_code,comment_delimeters=comment_delimeters)
        return pure_source_code
    def get_all_attributes(self):
        attributes=[]
        sources = self.pure_source.replace('<','\n<').split('\n<')
        for source in sources:
            attribute = source.split(' ')[0].split('>')[0]
            attribute=eatAll(attribute,['',' ','\n','\t'])
            if len(attribute)>0:
                if '/' == attribute[0]:
        ...
        return attributes
    def determine_if_quotes_open(self,quotes,char):
        quote_tracker = quotes[char]
        if quote_tracker[0] !=  quote_tracker[1]:
           if (quote_tracker[0]-quote_tracker[1])<0:
               quote_tracker[0]+=1
           else:
                quote_tracker[1]+=1
        elif quote_tracker[0] ==  quote_tracker[1]:
            quote_tracker[0]+=1
        for each in list(quotes.keys()):
            if each != char:
                other_quote_tracker = quotes[each]
        if other_quote_tracker[0]  !=   other_quote_tracker[1] or quote_tracker[0] !=  quote_tracker[1]:
            return quotes,True
        if other_quote_tracker[0]  ==   other_quote_tracker[1] and quote_tracker[0] ==  quote_tracker[1]:
            return quotes,False
            ...
            for attrib_source in attrib_sources[1:]:
                ...
                while class_items != None:
                    ...
                    if class_name and class_value:
                        ...
                        if class_value not in self.class_values:
                           self.class_values.append(class_value)
                        if class_value not in self.attribute_tracker_js[class_name]["class_values"]:
                            self.attribute_tracker_js[class_name]["class_values"].append(class_value)
                        if attribute not in self.attribute_tracker_js[class_name]["attributions"]:
                            self.attribute_tracker_js[class_name]["attributions"].append(attribute)
class SoupManagerSingleton():
    _instance = None
    @staticmethod
    def get_instance(url,parse_type="html.parser"):
        if SoupManagerSingleton._instance is None:
            SoupManagerSingleton._instance = SoupManager(url=url,parse_type=parse_type)
        elif parse_type != SoupManagerSingleton._instance.parse_type or url != SoupManagerSingleton._instance.url :
            SoupManagerSingleton._instance = SoupManager(url=url,parse_type=parse_type)
