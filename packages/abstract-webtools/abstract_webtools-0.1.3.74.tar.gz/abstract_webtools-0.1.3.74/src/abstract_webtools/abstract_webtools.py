...
class DynamicRateLimiterManager:
    ...
    def add_service(self, service_name, low_limit, high_limit, limit_epoch, starting_tokens=None, epoch_cycle_adjustment=None):
...
class DynamicRateLimiter:
    def __init__(self, low_limit, high_limit, limit_epoch, starting_tokens=None, epoch_cycle_adjustment=None):
    ...
    def initialize_request_status(self):
        self.request_status_json = {
            "successful": [],
            "unsuccessful": [],
            "last_requested": get_time_stamp(),
            "epoch_initial_time": None,
            "first_requested": get_time_stamp(),
            "epoch_left": self.limit_epoch,
            "last_fail": get_time_stamp(),
            "count_since_fail": 0,
            "current_sleep": 0,
            "token_sleep": 0,
            "last_5_successful":[self.current_limit,self.current_limit,self.current_limit,self.current_limit,self.current_limit]
        }
    ...
    def request_tracker(self,success):
        if self.request_status_json["epoch_initial_time"]==None:
            self.request_status_json["epoch_initial_time"] = get_time_stamp()
        if success == True:
            self.request_status_json["successful"].append(get_time_stamp())
            self.request_status_json["count_since_fail"]=max(self.request_status_json["count_since_fail"]+1,1)
        else:
            self.request_status_json["unsuccessful"].append(get_time_stamp())
            self.request_status_json["last_fail"]=get_time_stamp()
            self.request_status_json["count_since_fail"]=min(self.request_status_json["count_since_fail"]-1,0)
            print(f"count since fail {self.request_status_json['count_since_fail']}")
    ...
    def get_elapsed_time_for_current_epoch(self):
        ...
        if self.request_status_json["epoch_initial_time"] != None:
            elapsed_time_in_current_epoch = (current_time - self.request_status_json["epoch_initial_time"]) % self.limit_epoch
        self.request_status_json["current_epoch_start"]=current_time-elapsed_time_in_current_epoch
        self.request_status_json["epoch_left"]=self.limit_epoch-elapsed_time_in_current_epoch
        #print(f" epoch initial time = {self.request_status_json['epoch_initial_time']}\ncurrent epoch time elapsed = {elapsed_time_in_current_epoch}")
    ...
    def requests_in_current_epoch(self,type_request:str="successful"):
        ...
        successful = []
        for each in self.request_status_json[type_request]:
            if (each - self.request_status_json["current_epoch_start"])<self.limit_epoch:
                successful.append(each)
        return len(successful)
    def get_token_sleep(self):
        self.request_status_json["token_sleep"]=self.request_status_json["epoch_left"]/max(self.tokens,1)
        return self.request_status_json["token_sleep"]
    def calculate_tokens(self):
        successful = []
        start_time = self.last_adjusted_time
        current_time = get_time_stamp()
        elapsed_time_in_current_epoch = self.get_elapsed_time_for_current_epoch()
        self.tokens = max(self.current_limit - (self.requests_in_current_epoch(type_request="successful")+ self.requests_in_current_epoch(type_request="unsuccessful")),0)
        self.request_status_json["current_sleep"]=(self.request_status_json["epoch_left"] if self.tokens == 0 else 0)
        self.request_status_json["token_sleep"] = self.get_token_sleep()
        return self.request_status_json
    def is_past_first_epoch(self):
        if self.request_status_json["epoch_initial_time"] == None:
            return False
        return ((get_time_stamp()-self.request_status_json["epoch_initial_time"])>self.limit_epoch)
    def get_highest_in_list(self,list_obj):
        list_obj.sort()
        return list_obj[0]
    def request(self):
        
        if self.is_past_first_epoch() and self.request_status_json["count_since_fail"] <-2:
            if abs(self.request_status_json["count_since_fail"])%2 == float(0):
                self.limit_epoch +=1
                print(f"limit epoch changed to {self.limit_epoch} sleeping for the epoch")
        ...
        if self.tokens > 0:
            return True  # The request can be made
                ...
                if self.epoch_cycle_adjustment != None:
                    if (get_time_stamp() - self.last_adjusted_time)>self.limit_epoch:
                        prev_limit = self.current_limit
                        if self.requests_in_current_epoch(type_request="unsuccessful")>1:
                            if self.is_past_first_epoch():
                                if self.request_status_json["count_since_fail"] >=1:
                                    self.current_limit = max(self.current_limit-1,1)
                                elif self.request_status_json["count_since_fail"] <0:
                                    self.request_status_json["last_5_successful"] = self.request_status_json["last_5_successful"][-5:]
                                    self.current_limit = max(get_highest_in_list(self.request_status_json["last_5_successful"]),1)
                                self.request_status_json["count_since_fail"] = min(self.request_status_json["count_since_fail"]-1,0)
                            else:
                                sleep_count_down(self.limit_epoch)
                        elif self.requests_in_current_epoch(type_request="unsuccessful") == 0:
                            self.last_adjusted_time = get_time_stamp()
                            self.request_status_json["count_since_fail"]=max(self.request_status_json["count_since_fail"] +1,1)
                        ...
                        limit_change = self.current_limit-prev_limit
                        if limit_change != 0:
                            self.last_adjusted_time = get_time_stamp()
                            if limit_change > 0:
                                change = f'increased by {limit_change} new limit = {self.current_limit}'
                            elif limit_change < 0:
                                change = f'decreased by {limit_change} new limit = {self.current_limit}'
                            logging.info(f"Limit {change}")
                        logging.info(f"Count since fail: {self.request_status_json['count_since_fail']}")
...
def get_token_sleep(manager, service_name, success, response):
    ...
    key = "epoch_left"
    if manager.services[service_name].request_status_json["count_since_fail"]>=-1:
        key = "token_sleep"
    sleep_count_down(manager.services[service_name].calculate_tokens()[key])
    print(response)
    return True
...
def get_limited_request(request_url, service_name="default"):
    ...
    while True:
        if not manager.request(service_name):
            ...
            sleep_count_down(manager.services[service_name].calculate_tokens()["current_sleep"])
            manager.services[service_name].request_status_json["last_5_successful"].append(manager.services[service_name].requests_in_current_epoch(type_request="successful"))
            manager.services[service_name].request_status_json["successful"] = []
            manager.services[service_name].request_status_json["unsuccessful"] = []
            
        else:
            response = requests.get(request_url)
            if manager.services[service_name].request_status_json["epoch_initial_time"] is None:
                manager.services[service_name].request_status_json["epoch_initial_time"] = get_time_stamp()
            ...
            if response.status_code == 200:
                ...
                if 'response' in response_data:
                    input((any(limit_message in response_data['response'].lower() for limit_message in ['max rate limit reached', "you've exceeded the rate limit"])))
...
def get_request(request_url:str=None, service_name: str = None, low_limit: int = 20, high_limit: int = 30,limit_epoch: int = 60, starting_tokens: int = None,epoch_cycle_adjustment:int=None):
    """
    ...
    if service_name not in manager.services:
        request_manager.add_service(service_name=service_name, low_limit=low_limit, high_limit=high_limit, limit_epoch=limit_epoch, starting_tokens=starting_tokens,epoch_cycle_adjustment=epoch_cycle_adjustment)
