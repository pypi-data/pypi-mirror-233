...
class Transformer(nn.Module):
    def __init__(self, embed_size, hidden_size, num_layers, vocab_size=None, nhead=8):
        ...
        self.transformer = nn.Transformer(embed_size, nhead, num_layers, num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        x = self.embedding(x)
        zeros_tensor = torch.zeros((x.size(0), 1), dtype=torch.long, device=x.device)
        tgt_indices = torch.cat([zeros_tensor, x.argmax(dim=2)[:, :-1]], dim=1)
        tgt = self.embedding(tgt_indices)
