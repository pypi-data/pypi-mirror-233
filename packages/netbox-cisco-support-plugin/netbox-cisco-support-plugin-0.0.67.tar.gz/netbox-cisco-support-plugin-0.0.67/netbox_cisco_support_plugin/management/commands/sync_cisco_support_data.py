...
class Command(BaseCommand):
    ...
    def update_device_ss_data(self, serial_number, pid, data):
        ...
        if isinstance(data, str):
            ...
            self.stdout.write(f"{serial_number} - {recommended_release_stdout}")
            # Save model object for device
            ds.save()
            return True
    ...
    def update_device_sni_summary_data(self, device):
        ...
        if "ErrorResponse" in device:
            self.stdout.write(self.task_error(text=text, changed=False))
            error = device["ErrorResponse"]
            self.stdout.write(f"{device['sr_no']} - ErrorCode: {error['NO_RECORDS_FOUND']}")
            self.stdout.write(f"{device['sr_no']} - ErrorDescription: {error['ErrorDescription']}")
            self.stdout.write(f"{device['sr_no']} - SuggestedAction: {error['SuggestedAction']}")
            return False
    ...
    def handle(self, *args, **kwargs):
        ...
        for serial_number, pid in serial_pid.items():
            ...
            if not pid:
                ...
                self.stdout.write(self.task_error(text=text, changed=False))
                self.stdout.write(f"{serial_number} - No PID found for this serial number")
                continue
        ...
        #### Step 5: Print the sync statuc summary ##########################################################
        task = "Sync Cisco Support API Data Result"
        self.stdout.write(self.task_name(text=task))

        if SYNC_FAILED:
            self.stdout.write(self.task_error(text=task, changed=False))
            self.stdout.write(f"\U0001f4a5 {task.upper()} FAILED \U0001f4a5")
            self.stdout.write(f"{Style.BRIGHT}{Fore.RED}-> Analyse the output for failed results")
            self.stdout.write("\n")
            sys.exit(1)
        else:
            self.stdout.write(self.task_info(text=task, changed=False))
            self.stdout.write(f"\u2728 {task.upper()} SUCCESSFUL \u2728")
            self.stdout.write("\n")
