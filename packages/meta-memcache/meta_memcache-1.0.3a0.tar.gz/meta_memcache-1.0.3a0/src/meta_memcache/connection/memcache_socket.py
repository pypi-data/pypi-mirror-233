...
class MemcacheSocket:
    ...
    def _recv_info_buffer(self) -> int:
        data = self._conn.recv(self._buffer_size - self._read)
        read = len(data)
        if read > 0:
            self._buf_view[self._read : self._read + read] = data
            self._read += read
