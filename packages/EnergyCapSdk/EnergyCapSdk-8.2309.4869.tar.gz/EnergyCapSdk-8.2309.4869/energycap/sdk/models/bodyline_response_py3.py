# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class BodylineResponse(Model):
    """BodylineResponse.

    :param bodyline_id: The bodyline identifier
    :type bodyline_id: int
    :param bill_id: The bill identifier
    :type bill_id: int
    :param value: The bodyline's value
    :type value: float
    :param cost: The cost attributed to the bodyline
    :type cost: float
    :param display_order: The display order
    :type display_order: int
    :param caption: The caption
    :type caption: str
    :param help_tag: The help tag
    :type help_tag: str
    :param commodity:
    :type commodity: ~energycap.sdk.models.CommodityChild
    :param meter:
    :type meter: ~energycap.sdk.models.MeterChild
    :param account:
    :type account: ~energycap.sdk.models.AccountChild
    :param general_ledger:
    :type general_ledger: ~energycap.sdk.models.GeneralLedgerChild
    :param observation_type:
    :type observation_type: ~energycap.sdk.models.ObservationTypeChild
    :param unit:
    :type unit: ~energycap.sdk.models.UnitChild
    :param cost_unit:
    :type cost_unit: ~energycap.sdk.models.UnitChild
    """

    _attribute_map = {
        'bodyline_id': {'key': 'bodylineId', 'type': 'int'},
        'bill_id': {'key': 'billId', 'type': 'int'},
        'value': {'key': 'value', 'type': 'float'},
        'cost': {'key': 'cost', 'type': 'float'},
        'display_order': {'key': 'displayOrder', 'type': 'int'},
        'caption': {'key': 'caption', 'type': 'str'},
        'help_tag': {'key': 'helpTag', 'type': 'str'},
        'commodity': {'key': 'commodity', 'type': 'CommodityChild'},
        'meter': {'key': 'meter', 'type': 'MeterChild'},
        'account': {'key': 'account', 'type': 'AccountChild'},
        'general_ledger': {'key': 'generalLedger', 'type': 'GeneralLedgerChild'},
        'observation_type': {'key': 'observationType', 'type': 'ObservationTypeChild'},
        'unit': {'key': 'unit', 'type': 'UnitChild'},
        'cost_unit': {'key': 'costUnit', 'type': 'UnitChild'},
    }

    def __init__(self, *, bodyline_id: int=None, bill_id: int=None, value: float=None, cost: float=None, display_order: int=None, caption: str=None, help_tag: str=None, commodity=None, meter=None, account=None, general_ledger=None, observation_type=None, unit=None, cost_unit=None, **kwargs) -> None:
        super(BodylineResponse, self).__init__(**kwargs)
        self.bodyline_id = bodyline_id
        self.bill_id = bill_id
        self.value = value
        self.cost = cost
        self.display_order = display_order
        self.caption = caption
        self.help_tag = help_tag
        self.commodity = commodity
        self.meter = meter
        self.account = account
        self.general_ledger = general_ledger
        self.observation_type = observation_type
        self.unit = unit
        self.cost_unit = cost_unit
