# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class CostCenterDigestActualMonthlyResponseCommodityData(Model):
    """CostCenterDigestActualMonthlyResponseCommodityData.

    :param commodity_code: The commodity code
    :type commodity_code: str
    :param commodity_info: The commodity info
    :type commodity_info: str
    :param commodity_id: The commodity identifier
    :type commodity_id: int
    :param common_use_unit:
    :type common_use_unit: ~energycap.sdk.models.UnitChild
    :param cost_unit:
    :type cost_unit: ~energycap.sdk.models.UnitChild
    :param results: An array of commodity monthly data
    :type results:
     list[~energycap.sdk.models.CostCenterDigestActualMonthlyResponseCommodityResults]
    """

    _attribute_map = {
        'commodity_code': {'key': 'commodityCode', 'type': 'str'},
        'commodity_info': {'key': 'commodityInfo', 'type': 'str'},
        'commodity_id': {'key': 'commodityId', 'type': 'int'},
        'common_use_unit': {'key': 'commonUseUnit', 'type': 'UnitChild'},
        'cost_unit': {'key': 'costUnit', 'type': 'UnitChild'},
        'results': {'key': 'results', 'type': '[CostCenterDigestActualMonthlyResponseCommodityResults]'},
    }

    def __init__(self, *, commodity_code: str=None, commodity_info: str=None, commodity_id: int=None, common_use_unit=None, cost_unit=None, results=None, **kwargs) -> None:
        super(CostCenterDigestActualMonthlyResponseCommodityData, self).__init__(**kwargs)
        self.commodity_code = commodity_code
        self.commodity_info = commodity_info
        self.commodity_id = commodity_id
        self.common_use_unit = common_use_unit
        self.cost_unit = cost_unit
        self.results = results
