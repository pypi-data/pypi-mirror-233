...
.. code-block:: none
        ...
        Options:
        -f, --files FILES
            Names of the files to download. Available files are: morph, ner, salma_model, salma_tokenizer, glosses_dic, lemma_dic, five_grams, four_grams, three_grams, two_grams. 
            If no file is specified, all files will be downloaded.

      -d, --directory DIRECTORY
            Directory to download the files to. Defaults to the application data directory which varies based on the operating system.
...
.. code-block:: none
    ...
    sina_download_files -d /path/to/directory
    This command will download all files to `/path/to/directory`.

.. code-block:: none

    sina_download_files -f morph ner -d /path/to/directory
    This command will download the `morph` and `ner` files to `/path/to/directory`.
...
.. code-block:: none
    ...
    - The URLs for the files are predefined in the `urls` dictionary within the script.
