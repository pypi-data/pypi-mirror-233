import os
from ...core import Utils, Logging, EntityBundleWrapper
            ...
            async for child in Synapsis.Chain.getChildren(parent, includeTypes=['folder', 'file']):
                ...
                if child_type in self.include_types:
                    ...
                    if self.count_only:
                        ...
                        file_info = ''
                        if child_type.is_file and self.with_filesize:
                            file_size = synapse_item.content_size
                            file_info = 'Size: {0}'.format(file_size)
                            filename = synapse_item.filename
                            file_info = file_info + ', Filename: {0}'.format(filename)
                            file_info = ' ({0})'.format(file_info)
                        Logging.info(
