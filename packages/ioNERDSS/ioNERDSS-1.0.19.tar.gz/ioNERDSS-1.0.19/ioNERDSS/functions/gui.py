...
def gui():
    ...
    def create_labeled_entry_with_hint(
        parent, label_text, hint_text, row, column, width=50
    ):
        ...
        entry.bind(
            "<FocusIn>",
            lambda event, entry=entry, hint_text=hint_text: on_focus_in(
                event, entry, hint_text
            ),
        )
        entry.bind(
            "<FocusOut>",
            lambda event, entry=entry, hint_text=hint_text: on_focus_out(
                event, entry, hint_text
            ),
        )
    ...
    def create_labeled_combobox_entry(
        parent, label_text, options_list, row, column, width=30
    ):
        label = tk.Label(parent, text=label_text)
    ...
    def create_labeled_combobox_entry_with_callback(
        parent, label_text, options, row, column, callback, width=30
    ):
        label = tk.Label(parent, text=label_text)
    ...
    def generate_inp_file():
        ...
        messagebox.showinfo(
            "File Generated", "The .inp file has been successfully generated."
        )
    ...
    def create_mol_file_from_entries():
        ...
        if is_implicit_lipid and not added_molecules:
            messagebox.showerror(
                "Error", "Please add the implicit lipid before other mols."
            )
            return
    ...
    def add_reaction():
        ...
        if react_type == "Zeroth Creation: null -> A":
            ...
            if react_state == "":
                ...
                react_str += (
                    react_name + "(" + react_site + "~" + react_state + ") -> NULL\n"
                )
            react_str += "    rate = " + r + "\n"
            if l != "":
                react_str += "    rxnLagbel = " + l + "\n"
            ...
            if not np.isclose(np.linalg.norm(np.cross(v1, sigma1)), 0):
                ...
                n_proj = n1 - v_unit * np.dot(v_unit, n1)
                sigma_proj = sigma1 - v_unit * np.dot(v_unit, sigma1)
                phi_dir = np.cross(sigma_proj, n_proj)
                if np.dot(v_unit, phi_dir) > 0:
                    phi1 = -phi1
            else:
                phi1 = "nan"
            ...
            if not np.isclose(np.linalg.norm(np.cross(v2, sigma2)), 0):
                ...
                n_proj = n2 - v_unit * np.dot(v_unit, n2)
                sigma_proj = sigma2 - v_unit * np.dot(v_unit, sigma2)
                phi_dir = np.cross(sigma_proj, n_proj)
                if np.dot(v_unit, phi_dir) > 0:
                    phi2 = -phi2
            else:
            ...
            # get omega
            if not np.isclose(np.linalg.norm(v1), 0) and not np.isclose(
                np.linalg.norm(v2), 0
            ):
                if not np.isclose(
                    np.linalg.norm(np.cross(v1, sigma1)), 0
                ) and not np.isclose(np.linalg.norm(np.cross(v2, sigma2)), 0):
                ...
                omega = np.arccos(
                    np.dot(t1, t2) / (np.linalg.norm(t1) * np.linalg.norm(t2))
                )
                # the sign of omega is determined by the direction of t2 relative to the right-hand rule of cross product of sigma1 and t1
                sigma1_unit = sigma1 / np.linalg.norm(sigma1)
                omega_t1 = t1 / np.linalg.norm(t1)
                omega_t2 = t2 / np.linalg.norm(t2)
                omega_dir = np.cross(
                    np.cross(sigma1, omega_t1), np.cross(sigma1, omega_t2)
                )
                omega_dir = omega_dir / np.linalg.norm(omega_dir)
                if np.dot(sigma1_unit, omega_dir) > 0:
                    omega = -omega
            else:
                omega = "nan"
            ...
            if react1_state == "":
                ...
                if react1_req == "":
                    react1_str = (
                        react1_name + "(" + react1_site + "~" + react1_state + ")"
                    )
                    prod1_str = (
                        react1_name + "(" + react1_site + "~" + react1_state + "!1)"
                    )
                else:
                    react1_str = (
                        f"{react1_name}({react1_site}~{react1_state}, {react1_req}!*)"
                    )
                    prod1_str = (
                        f"{react1_name}({react1_site}~{react1_state}!1, {react1_req}!*)"
                    )
            if react2_state == "":
                if react2_req == "":
                ...
                if react2_req == "":
                    react2_str = (
                        react2_name + "(" + react2_site + "~" + react2_state + ")"
                    )
                    prod2_str = (
                        react2_name + "(" + react2_site + "~" + react2_state + "!1)"
                    )
                else:
                    react2_str = (
                        f"{react2_name}({react2_site}~{react2_state}, {react2_req}!*)"
                    )
                    prod2_str = (
                        f"{react2_name}({react2_site}~{react2_state}!1, {react2_req}!*)"
                    )
            ...
            if micro_on_rate_str != "":
                ...
                messagebox.showerror(
                    "Error", "Please enter reaction rates (either micro or macro)."
                )
                return
            ...
            if kcat_str != "":
                ...
                messagebox.showerror(
                    "Error", "Please specify kcat for Michaelis-Menten."
                )
                return
            ...
            if coupledLabel_str != "":
                ...
                messagebox.showerror(
                    "Error", "Please specify coupled reaction for Michaelis-Menten."
                )
                return
            ...
            if prod1_state == "":
                ...
                if prod1_req == "":
                    ...
                    prod1_str = (
                        f"{prod1_name}({prod1_site}~{prod1_state}, {prod1_req}!*)"
                    )
            if prod2_state == "":
                if prod2_req == "":
                ...
                if prod2_req == "":
                    ...
                    prod2_str = (
                        f"{prod2_name}({prod2_site}~{prod2_state}, {prod2_req}!*)"
                    )
    ...
    def add_molecule():
        ...
        if not copy_num and not concentration:
            messagebox.showerror(
                "Error", "Please enter either copy number or concentration."
            )
            return
        ...
        if copy_num and concentration:
            messagebox.showerror(
                "Error",
                f"Please enter either copy number {copy_num} OR concentration {concentration}, not both.",
            )
            return
        ...
        if concentration:
            ...
            copy_num = int(
                float(concentration) / 1000000 * volume / 1000000000000000 * 6.02e23
            )

        percentage = states_entry.get().strip()
        if not percentage:
            mol_data = {"name": mol_name, "copy_number": copy_num}
            ...
            mol_data = {"name": mol_name, "copy_number": new_copy_number_str}
            added_molecules.append(mol_data)
        ...
        messagebox.showinfo(
            "Molecule Added",
            f"The {mol_name}.mol file has been successfully generated.",
        )
    ...
    nItr_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter total iteration (steps):", "1000", 0, 0
    )
    timeStep_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter timeStep (us):", "0.1", 1, 0
    )
    timeWrite_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter how often to write output (steps):", "10", 2, 0
    )
    trajWrite_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter how often to write traj (steps):", "100", 3, 0
    )
    pdbWrite_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter how often to write pdb (steps):", "100", 4, 0
    )
    transitionWrite_entry = create_labeled_entry_with_hint(
        parameters_tab,
        "Enter how often to write transition matrix (steps):",
        "100",
        5,
        0,
    )
    restartWrite_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter how often to write restart file (steps):", "100", 6, 0
    )
    checkPoint_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter how often to write check points (steps):", "100", 7, 0
    )
    scaleMaxDisplace_entry = create_labeled_entry_with_hint(
        parameters_tab, "Enter threshold to reject association (<RMSD>):", "100", 8, 0
    )
    overlapSepLimit_entry = create_labeled_entry_with_hint(
        parameters_tab,
        "Enter threshold for overlap check between COMs (nm):",
        "0.1",
        9,
        0,
    )
    clusterOverlapCheck_entry = create_labeled_entry_with_hint(
        parameters_tab,
        "Enter whether overlap is checked based on cluster (bool):",
        "False",
        10,
        0,
    )
    ...
    isSphere_entry = create_labeled_entry_with_hint(
        boundaries_tab, "Enter if the system is spherical (bool):", "False", 0, 0
    )
    waterBox_entry = create_labeled_entry_with_hint(
        boundaries_tab,
    ...
    sphereR_entry = create_labeled_entry_with_hint(
        boundaries_tab,
    ...
    mol_name_entry = create_labeled_entry_with_hint(
        molecules_tab, "Enter molecule name:", "", 0, 0
    )
    mol_copy_num_entry = create_labeled_entry_with_hint(
        molecules_tab, "Enter copy number:", "", 1, 0
    )
    mol_concentration_entry = create_labeled_entry_with_hint(
        molecules_tab,
    ...
    checkOverlap_entry = create_labeled_entry_with_hint(
        molecules_tab,
    ...
    countTransition_entry = create_labeled_entry_with_hint(
        molecules_tab,
    ...
    transitionMatrixSize_entry = create_labeled_entry_with_hint(
        molecules_tab,
        "Enter transition matrix size for this molecule type, which should be larger than the size of the formed largest complex:",
        "500",
        5,
        0,
    )
    isImplicitLipid_entry = create_labeled_entry_with_hint(
        molecules_tab, "Enter if it is isImplicitLipid (bool):", "False", 6, 0
    )
    isLipid_entry = create_labeled_entry_with_hint(
        molecules_tab, "Enter if it is lipid (bool):", "False", 7, 0
    )
    D_entry = create_labeled_entry_with_hint(
        molecules_tab,
        "Enter translational diffusion constants ([Dx,Dy,Dz] (um2/s)):",
        "[10,10,10]",
        8,
        0,
    )
    Dr_entry = create_labeled_entry_with_hint(
        molecules_tab,
        "Enter rotational diffusion constants ([Drx,Dry,Drz] (rad2/us)):",
        "[0.1,0,1,0.1]",
        9,
        0,
    )
    coords_entry = create_labeled_entry_with_hint(
        molecules_tab,
    ...
    bonds_entry = create_labeled_entry_with_hint(
        molecules_tab, "Enter bonds between sites (com c1;com c2;com c3):", "", 11, 0
    )
    states_entry = create_labeled_entry_with_hint(
        molecules_tab,
    ...
    reactant1_entry = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select reactant 1 if it is not creation reaction:",
        mol_names,
        0,
        0,
        on_reactant1_selected,
        width=10,
    )
    reactant1_site = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select reactant 1 site:",
        [],
        0,
        2,
        on_reactant1_site_selected,
        width=10,
    )
    reactant1_state = create_labeled_combobox_entry(
        reactions_tab, "Select reactant 1 state (optional):", [], 1, 0, width=10
    )
    reactant1_bound_site_requirment = create_labeled_combobox_entry(
        reactions_tab,
        "Select the site of reactant 1 that needs to be already \nbound for the reaction to occur (optional):",
        [],
        1,
        2,
        width=10,
    )
    reactant2_entry = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select reactant 2 if it is not creation reaction:",
        mol_names,
        2,
        0,
        on_reactant2_selected,
        width=10,
    )
    reactant2_site = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select reactant 2 site:",
        [],
        2,
        2,
        on_reactant2_site_selected,
        width=10,
    )
    reactant2_state = create_labeled_combobox_entry(
        reactions_tab, "Select reactant 2 state (optional):", [], 3, 0, width=10
    )
    reactant2_bound_site_requirment = create_labeled_combobox_entry(
        reactions_tab,
        "Select the site of reactant2 that needs to be already \nbound for the reaction to occur (optional):",
        [],
        3,
        2,
        width=10,
    )
    product1_entry = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select product 1 if it is not bimolecular reaction or destruction reaction:",
        mol_names,
        4,
        0,
        on_product1_selected,
        width=10,
    )
    product1_site = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select product 1 site:",
        [],
        4,
        2,
        on_product1_site_selected,
        width=10,
    )
    product1_bound_site_requirment = create_labeled_combobox_entry(
        reactions_tab,
        "Select the site of product 1 that needs to be already \nbound for the reaction to occur (optional):",
        [],
        5,
        0,
        width=10,
    )
    product1_state = create_labeled_combobox_entry(
        reactions_tab, "Select product 1 state (optional):", [], 5, 2, width=10
    )
    product2_entry = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select product 2 if it is not bimolecular reaction or destruction reaction:",
        mol_names,
        6,
        0,
        on_product2_selected,
        width=10,
    )
    product2_site = create_labeled_combobox_entry_with_callback(
        reactions_tab,
        "Select product 2 site:",
        [],
        6,
        2,
        on_product2_site_selected,
        width=10,
    )
    product2_state = create_labeled_combobox_entry(
        reactions_tab, "Select product 2 state (optional):", [], 7, 0, width=10
    )
    product2_bound_site_requirment = create_labeled_combobox_entry(
        reactions_tab,
        "Select the site of product 2 that needs to be already \nbound for the reaction to occur (optional):",
        [],
        7,
        2,
        width=10,
    )
    coord_com_reactant1_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter reactant 1's com coords \nfor bimolecular reaction ([x,y,z](nm)):",
        "[0,0,0]",
        8,
        0,
        width=10,
    )
    coord_site_reactant1_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter reactant 1's reaction site coords \nfor bimolecular reaction ([x,y,z](nm)):",
        "",
        8,
        2,
        width=10,
    )
    coord_com_reactant2_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter reactant 2's com coords \n(reactant 1 and 2 are in the same coordinate system) ([x,y,z](nm)):",
        "",
        9,
        0,
        width=10,
    )
    coord_site_reactant2_entry = create_labeled_entry_with_hint(
        reactions_tab,
    ...
    onRate3Dka_entry = create_labeled_entry_with_hint(
        reactions_tab, "Enter microscopic on rate (nm^3 us^-1):", "", 10, 0, width=10
    )
    offRatekb_entry = create_labeled_entry_with_hint(
        reactions_tab, "Enter microscopic off rate (s^-1):", "", 10, 2, width=10
    )
    onRate3DMacro_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter macroscopic on rate if micro not provided (uM^-1 s^-1):",
        "",
        11,
        0,
        width=10,
    )
    offRateMacro_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter macroscopic off rate if micro not provided (s^-1):",
        "",
        11,
        2,
        width=10,
    )
    rate_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter rate for creation / destruction / uni statechange \n/ uni creation (M s^-1 / s^-1 / s^-1 / s^-1):",
        "",
        12,
        0,
        width=10,
    )
    length3dto2d_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter length scale to convert 3D rate to 2D rate\n for bimolecular association (nm):",
        "10",
        13,
        0,
        width=10,
    )
    bindRadSameCom_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter distance betweeen two reactants to force reaction \nwithinin the same complex for bimolecular association (sigma):",
        "1.1",
        13,
        2,
        width=10,
    )
    loopCoopFactor_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter scale factor of rate when closing loops \nfor bimolecular association (ka):",
        "1",
        14,
        0,
        width=10,
    )
    label_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter reaction label, used if you want to couple a \ndifferent reaction to this one:",
        "",
        14,
        2,
        width=10,
    )
    coupledLabel_entry = create_labeled_entry_with_hint(
        reactions_tab,
        "Enter coupled reaction label, used if you allow \nthe completion of this reaction to cause another reaction:",
        "",
        15,
        0,
        width=10,
    )
    kcat_entry = create_labeled_entry_with_hint(
        reactions_tab, "Enter rate of the coupled reaction (s^-1):", "", 15, 2, width=10
    )
    exclude_entry = create_labeled_entry_with_hint(
        reactions_tab,
    ...
    reaction_type = [
        "Bimolecular Association: (A + B <-> A.B)",
        "Zeroth Creation: null -> A",
        "Destruction: A -> null",
        "Michaelis-Menten: A + B <-> A.B -> A + C",
        "Unimolecular Creation: A -> A + B",
        "Bimolecular Statechange: A + B(b~U) -> A + B(b~P)",
        "Statechange: B(b~U) -> B(b~P)",
    ]
    reaction_type_entry = create_labeled_combobox_entry(
        reactions_tab, "Select reaction type:", reaction_type, 17, 0
    )
    add_reaction_button = tk.Button(
        reactions_tab, text="Add Reaction", command=add_reaction
    )
    ...
    generate_button = tk.Button(
        root, text="Generate .inp File", command=generate_inp_file
    )
