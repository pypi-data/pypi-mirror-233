    ...
    if noise_type == None or noise_type.upper() == 'UNIFORM':
        for col in range(data_columns):
            ...
            for row in range(data_length):
                value = X[row, col]
                new_X[row, col] = value + value * np.random.uniform(-1, 1) * noise
        ...
        for col in range(data_columns):
            ...
            for row in range(data_length):
                value = X[row, col]
                new_X[row, col] = value + mean * np.random.uniform(-1, 1) * noise
...
def augment_data(data, num_samples, label = 'RA', selected_label = 0, epochs = 20,
                  augment_type = 'nbinom', polars = False, normalize = False):
    '''
