...
class Quantizer:
    ...
    def build(self) -> None:
        ...
        if self.model is None:
            if not self.config.low_cpu_mem_usage:
                dist_logger.warning("low_cpu_mem_usage at config is False. Recommended to set to True for quantization")
    ...
    def save(self) -> None:
        ...
        tokenizer = build_tokenizer(config=self.config)

        tokenizer.save_pretrained(save_directory=self.config.quantized_model_path)
