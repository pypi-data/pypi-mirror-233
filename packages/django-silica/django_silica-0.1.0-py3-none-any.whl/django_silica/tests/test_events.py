...
class EventsCaller(SilicaComponent):
    def mount(self):
...
class ComponenentWithListener(SilicaComponent):
...
class LazyComponentEventReceiver(SilicaComponent):
    received_event_payload = {}
...
class EventsTest(SilicaTestCase):
    ...
    def test_an_event_listener_is_called_and_payload_received(self):
        (
            SilicaTest(component=ComponenentWithListener)
