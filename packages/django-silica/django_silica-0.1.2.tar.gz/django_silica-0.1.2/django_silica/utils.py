...
from django_silica.errors import SilicaCacheError
...
def pascal_to_snake(s):
    ...
    s1 = re.sub("([A-Z][a-z]+)", r"_\1", s)
    # Handle acronyms, e.g., convert "HTTPServer" to "HTTP_Server"
    s2 = re.sub("([A-Z]+)([A-Z][a-z]|$)", r"_\1\2", s1)
...
def cache_full_tree(component: "silica.SilicaComponent"):
    root = component
...
def restore_from_cache(
...
) -> "silica.SilicaComponent":
    """
    Gets a cached silica component by key, restoring and getting cached parents and children
    ...
    if cached_component:
        roots = {}
        root: "silica.SilicaComponent" = cached_component
        ...
        to_traverse: List["silica.SilicaComponent"] = []
...
def cast_value(type_hint, value):
    ...
    `silica.call_method_parser.CASTERS`.
...
class CacheableComponent:
    ...
    def __init__(self, component: "silica.SilicaComponent"):
        self._state = {}
    ...
    def components(self) -> List["silica.SilicaComponent"]:
