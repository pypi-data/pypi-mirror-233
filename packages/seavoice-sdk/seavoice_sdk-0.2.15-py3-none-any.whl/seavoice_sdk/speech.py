...
class Recognizer:
    ...
    class RecognizerWebSocket(WebSocketClient):
        ...
        def received_message(self, message: ws4py.messaging.TextMessage) -> None:
            ...
            try:
                ...
                svc_logger.warn(f"Could not parse message: {str(message)}, e: {e} {e.__class__.__name__}")
                return
            ...
            try:
                        ...
                        if evt["result"]["final"]:
                            if self.recognizer._Recognizer__recognized_event.is_set:
                                ...
                                try:
                                    ...
                                    svc_logger.error(f"Could not access word alignments {e}")
                ...
                svc_logger.error(f"Exception in receive_message: {str(message)}, e: {e} {e.__class__.__name__}")
        ...
        def offset_segment(self, evt_res):
            ...
            try:
                ...
                svc_logger.error(f"Could not offset results: {e}")
    ...
    def change_lang(self, lang):
        ...
        if not self.__speech_config.apikey:
            ...
            if not self.get_server_url():
                svc_logger.error("Can't get an available stt server, please confirm language setting, then try again.")
                return False
        svc_logger.info(f"Changing recognizer language to {lang}")
    ...
    def send_thread(self):
        ...
        if self.monitor_session:
            ...
            self.ws.send(data)
            svc_logger.info("Send the starting package")
...
class SpeechSynthesizer(SlothWebsocketClient):
    ...
    def change_speaker(self, lang, speaker):
        svc_logger.info(f"Changing synthesizer voice to {lang}: {speaker}")
