...
import aiofiles
import filetype
from dotserve.client.base import ElementDict, ElementDisplay, ElementSize, ElementType
from dotserve.client.cloud import DotserveCloudClient, dotserve_client
...
class Element:
    ...
    async def persist(self, client: DotserveCloudClient) -> Optional[ElementDict]:
    ...
    async def send(self, for_id: Optional[str] = None):
        ...
        if dotserve_client:
            element_dict = await self.persist(dotserve_client)
            if element_dict:
