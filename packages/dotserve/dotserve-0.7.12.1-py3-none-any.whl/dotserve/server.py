...
from dotserve.oauth_providers import get_oauth_provider
...
from dotserve.auth import create_jwt, get_configuration, get_current_user
from dotserve.client.acl import is_conversation_author
from dotserve.client.cloud import AppUser, PersistedAppUser, dotserve_client
...
from dotserve.markdown import get_markdown_str
from dotserve.playground.config import get_llm_providers
from dotserve.telemetry import trace_event
from dotserve.types import (
    CompletionRequest,
...
app.mount(
    "/assets",
    StaticFiles(
        packages=[("dotserve", os.path.join(build_dir, "assets"))],
...
def get_html_template():
    ...
    default_url = "https://github.com/dot-agent/dotagent"
    ...
    <meta property="og:image" content="https://res.cloudinary.com/dzznkbdrb/image/upload/v1696158901/hero-img_kt8rrq.png">
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if dotserve_client:
        await dotserve_client.create_app_user(app_user=app_user)
    return {
...
async def update_feedback(
    ...
    await dotserve_client.set_human_feedback(
...
async def get_user_conversations(
    ...
    res = await dotserve_client.get_conversations(payload.pagination, payload.filter)
...
async def get_conversation(
    ...
    res = await dotserve_client.get_conversation(conversation_id)
...
async def get_conversation_element(
    ...
    res = await dotserve_client.get_element(conversation_id, element_id)
    return JSONResponse(content=res)
...
async def delete_conversation(
    ...
    await dotserve_client.delete_conversation(conversation_id)
    return JSONResponse(content={"success": True})
