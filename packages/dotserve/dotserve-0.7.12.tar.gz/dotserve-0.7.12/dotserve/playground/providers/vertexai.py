...
from dotserve.input_widget import Select, Slider, Tags
from dotserve.playground.provider import BaseProvider
...
class ChatVertexAIProvider(BaseProvider):
    async def create_completion(self, request):
        ...
        async def create_event_stream():
            for response in await gui.make_async(chat.send_message_streaming)(
...
class GenerationVertexAIProvider(BaseProvider):
    async def create_completion(self, request):
        ...
        async def create_event_stream():
            for response in await gui.make_async(model.predict_streaming)(
                prompt, **llm_settings
            ):
                yield response.text
