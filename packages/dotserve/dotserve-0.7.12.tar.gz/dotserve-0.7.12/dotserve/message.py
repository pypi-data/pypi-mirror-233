...
class MessageBase(ABC):
    ...
    async def _create(self):
        ...
        if dotserve_client and not self.persisted:
            try:
                persisted_id = await dotserve_client.create_message(msg_dict)
                if persisted_id:
    ...
    async def update(
        ...
        if dotserve_client and self.id:
            await dotserve_client.update_message(self.id, msg_dict)
    ...
    async def remove(self):
        ...
        if dotserve_client and self.id:
            await dotserve_client.delete_message(self.id)
