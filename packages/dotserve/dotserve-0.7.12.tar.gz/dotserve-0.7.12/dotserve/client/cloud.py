...
class DotserveCloudClient(DotserveGraphQLClient):
    dotserve_server: str

    def __init__(self, api_key: str, dotserve_server="https://cloud.dotagent.dev/serve"):
        # Remove trailing slash
        dotserve_server = dotserve_server.rstrip("/")
        super().__init__(api_key=api_key, dotserve_server=dotserve_server)
        self.dotserve_server = dotserve_server
...
dotserve_client = None  # type: Optional[DotserveCloudClient]

if config.data_persistence:
    dotserve_client = DotserveCloudClient(
        api_key=os.environ.get("DOTSERVE_API_KEY", ""),
        dotserve_server=config.dotserve_server,
    )
