...
from dotserve.context import context
from haystack.agents import Agent, Tool
from haystack.agents.agent_step import AgentStep

import dotserve as gui
...
class HaystackAgentCallbackHandler:
    stack: Stack[gui.Message]
    latest_agent_message: Optional[gui.Message]
    ...
    def get_root_message(self):
        if not context.session.root_message:
            root_message = gui.Message(author=config.ui.name, content="")
            gui.run_sync(root_message.send())
    ...
    def on_agent_start(self, **kwargs: Any) -> None:
        ...
        self.stack = Stack[gui.Message]()
        self.stack.push(self.get_root_message())

        agent_message = gui.Message(
    ...
    def on_agent_step(self, agent_step: AgentStep, **kwargs: Any) -> None:
        ...
        gui.run_sync(self.latest_agent_message.send())
    ...
    def on_new_token(self, token, **kwargs: Any) -> None:
        ...
        gui.run_sync(self.stack.peek().stream_token(token))
    ...
    def on_tool_start(self, tool_input: str, tool: Tool, **kwargs: Any) -> None:
        ...
        tool_message = gui.Message(author=tool.name, parent_id=parent_id, content="")
    ...
    def on_tool_finish(
        ...
        **kwargs: Any
        ...
        gui.run_sync(tool_message.send())

    def on_tool_error(self, exception: Exception, tool: Tool, **kwargs: Any) -> None:
        # Tool error, send error message
        gui.run_sync(self.stack.pop().remove())
        gui.run_sync(gui.ErrorMessage(str(exception), author=tool.name).send())
