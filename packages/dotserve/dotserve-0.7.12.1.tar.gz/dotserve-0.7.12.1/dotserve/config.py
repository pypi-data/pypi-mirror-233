...
if TYPE_CHECKING:
    ...
    from dotserve.client.base import AppUser
...
config_dir = os.path.join(APP_ROOT, ".dotserve")
...
dotserve_prod_url = os.environ.get("DOTSERVE_PROD_URL")
...
@dataclass()
class DotserveConfig:
    # Directory where the Dotserve project is located
    root = APP_ROOT
    # Dotserve server URL. Used only for cloud features
    dotserve_server: str
    # Whether or not a dotserve api key has been provided
    data_persistence: bool
    # The url of the deployed app. Only set if the app is deployed.
    dotserve_prod_url = dotserve_prod_url
...
def load_config():
    ...
    dotserve_server = os.environ.get("DOTSERVE_SERVER", "https://cloud.dotagent.dev/serve")
    data_persistence = "DOTSERVE_API_KEY" in os.environ

    config = DotserveConfig(
        dotserve_server=dotserve_server,
