...
import jwt
from dotserve.client.cloud import AppUser, dotserve_client
...
def get_jwt_secret():
    return os.environ.get("DOTSERVE_AUTH_SECRET")


def ensure_jwt_secret():
    if require_login() and get_jwt_secret() is None:
        raise ValueError(
            "You must provide a JWT secret in the environment to use password authentication. Run `dotserve create-secret` to generate one."
        )
...
async def authenticate_user(token: str = Depends(reuseable_oauth)):
    ...
    if dotserve_client:
        try:
            persisted_app_user = await dotserve_client.get_app_user(app_user.username)
        except Exception as e:
