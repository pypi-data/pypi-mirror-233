...
from typing import Dict, Union, List, Optional, Callable, AsyncGenerator

import fipper
from fipper import __version__, __license__
from fipper import enums
from fipper import raw
from fipper import utils
from fipper.crypto import aes
from fipper.errors import CDNFileHashMismatch
from fipper.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest, ListenerCanceled
)
from fipper.handlers.handler import Handler
from fipper.methods import Methods
from fipper.session import Auth, Session
from fipper.storage import FileStorage, MemoryStorage, MongoDB
from fipper.types import User, TermsOfService
from fipper.utils import ainput
...
class Client(Methods):
    """Fipper Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~fipper.Client.export_session_string` before stopping the client to get a session string you can
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where Fipper will store the session files.
            Defaults to the parent directory of the main script.

        plugins (``dict``, *optional*):
            Smart Plugins settings as dict, e.g.: *dict(root="plugins")*.

        parse_mode (:obj:`~fipper.enums.ParseMode`, *optional*):
    ...
    APP_VERSION = f"Fipper {__version__}"
    DEVICE_MODEL = f"Fipper {__version__}" #p2-1150l{platform.python_implementation()} {platform.python_version()}
    ...
    def __init__(
        ...
        if self.mongo_url:
            self.mongo_ = MongoDB(self.mongo_url)
            if self.mongo_name:
                self.mongo_async = self.mongo_._mongo_async_[self.mongo_name]
                self.mongo_sync = self.mongo_._mongo_sync_[self.mongo_name]
            else:
                self.mongo_async = self.mongo_._mongo_async_.Fipper
                self.mongo_sync = self.mongo_._mongo_sync_.Fipper
        ...
        self.button_regex = re.compile(r"(\[([^\[]+?)\]\(buttonurl:(?:/{0,2})(.+?)(:same)?\))")
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to Fipper (version {__version__})")
        print(f"Fipper is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~fipper.enums.ParseMode`):
        ...
        try:
            ...
            if isinstance(r, raw.types.upload.File):
                ...
                try:
                    ...
                    raise e
        except fipper.StopTransmission:
            raise
        except Exception as e:
            log.error(e, exc_info=True)
