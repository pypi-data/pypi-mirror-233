...
from geodesic.utils import datetime_to_utc, deprecated
...
def get_objects(
    *,
    ...
    page = 1
    params = {"page": page, "page_size": page_size, "project": project}
...
def add_objects(objects: list, overwrite=False, project: Union[str, Project] = None):
    ...
    raise_on_error(
        entanglement_client.post(
            "objects", objects=_objects, project=project, error_on_exists=not overwrite
        )
    )
...
def add_connections(connections: list, overwrite=False, project: Union[str, Project] = None):
    ...
    params = {"connections": connections, "project": project, "overwrite": overwrite}
...
class Object(_APIObject, metaclass=ObjectResolver):
    ...
    alias = _StringDescr(doc="a short name/description for this object - doesn't need to be unique")
    xid = _StringDescr(
        ...
        " A typical example is an RDF IRI including the namespace prefix (e.g."
        " https://schema.org/Organization/SeerAI)"
    )
    ...
    description = _StringDescr(doc="a text description of this object")
    ...
    def connect_with(
        self, predicate: Union["Predicate", str], object: "Object", overwrite=False
    ) -> "Connection":
        """
        Adds a connection from this Object to another using the specified Predicate.

        Args:
            predicate: name of a Predicate or a Predicate object defining the connection and any edge attributes.
