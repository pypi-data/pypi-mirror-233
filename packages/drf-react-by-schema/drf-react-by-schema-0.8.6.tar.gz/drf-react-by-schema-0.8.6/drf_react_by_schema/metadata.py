...
class Metadata(SimpleMetadata):
    ...
    def determine_metadata(self, request, view):
        ...
        for field in model._meta.get_fields():
            ...
            serializer_field = getattr(serializer_fields, field_name, None)
            if field_name in meta['actions']['POST']:
                # Print default value in OPTIONS:
                if getattr(field, 'get_default', None) and field.get_default():
                    meta['actions']['POST'][field_name]['model_default'] = field.get_default()
                elif getattr(field, 'auto_now_add', None):
                    meta['actions']['POST'][field_name]['model_default'] = date.today(
                    ) if field.__class__.__name__ == 'DateField' else datetime.now()
                    ...
                    if class_name in ['int', 'string']:
                        meta['actions']['POST'][field_name]['model_default'] = serializer_field.default
                    elif class_name == 'CurrentUserDefault':
                        meta['actions']['POST'][field_name]['model_default'] = 'currentUser'
                ...
                for validator in validators:
                    ...
                    if regex:
                        validators_regex.append(
                            {'regex': validator.regex.pattern, 'message': validator.message})
                if len(validators_regex) > 0:
                    meta['actions']['POST'][field_name]['validators_regex'] = validators_regex

                # Print if required in OPTIONS:
                is_required = not getattr(field, 'blank', False) and getattr(
                ...
                if related_editable is not None:
                    meta['actions']['POST'][field_name]['related_editable'] = related_editable

                # Add DecimalField decimal_places in OPTIONS:
                decimal_places = getattr(field, 'decimal_places', None)
                if decimal_places is not None:
                    meta['actions']['POST'][field_name]['decimal_places'] = decimal_places
                ...
                if field_name in serializer_fields and serializer_fields[field_name].__class__.__name__ in ['ManyRelatedField', 'ListSerializer']:
