...
class DRFReactBySchemaBaseModelViewSet(viewsets.ModelViewSet):
        ...
        if self.app and self.serializer_class is not None:
            ...
            try:
                app_serializers = importlib.import_module(
                    f"{self.app}.serializers" if not self.is_autocomplete else f"{self.app}.autocomplete_serializers")
            except:
                return

            for (app_serializer_name, app_serializer) in inspect.getmembers(app_serializers, predicate=inspect.isclass):
                if app_serializer_name == self.serializer_class.__name__:
                    self.serializer_class = app_serializer
                    found += 1

                if self.serializer_list_class and app_serializer_name == self.serializer_list_class.__name__:
    ...
    def get_queryset(self):
        ...
        if self.parent_related_name and self.parent_pk_field:
            ...
            if self.many:
                value = [value]
                key = f"{self.parent_related_name}__contains" if self.reverse_many else f"{self.parent_related_name}__in"
            kwargs = {
                key: value
            }
        ...
        if self.filter_by_callback:
            filter_kwargs = self.filter_by_callback(queryset)
            if filter_kwargs:
        ...
        if 'columnField' in self.request.query_params and self.serializer_class:
            column_field = self.request.query_params['columnField']
            operator_value = self.request.query_params.get(
                'operatorValue', None)
            value = self.request.query_params.get('value', None)
            queryset = utils.apply_data_grid_filter(
                queryset, self.serializer_class(), column_field, operator_value, value)
    ...
    def list(self, request, *args, **kwargs):
        ...
        if self.filter_list_by_callback:
            filter_kwargs = self.filter_list_by_callback(queryset)
            if filter_kwargs:
                if 'or_kwargs' in filter_kwargs:
                    or_kwargs = filter_kwargs.pop('or_kwargs')
                    queryset = queryset.filter(
    ...
    def populate_select_and_prefetch_related_from_serializer_fields(
        ...
        for serializer_field_key, serializer_field in serializer_fields.items():
            ...
            for idx, field_name in enumerate(field_names):
                field = None
                meta = root_model._meta if idx == 0 else fields[idx -
                                                                1].related_model._meta
                try:
                ...
                if related_model:
                    ...
                    field_name_with_prefixes = field_name if len(
                        prefixes) == 0 else f"{'__'.join(prefixes)}__{field_name}"
                    if should_populate_select_related and field_class_name in ['OneToOneField', 'ForeignKey'] and field_name_with_prefixes not in self.select_related:
                        self.select_related.append(field_name_with_prefixes)

                    if should_populate_prefetch_related and field_class_name in ['ManyToManyField', 'ManyToOneField'] and field_name_with_prefixes not in self.prefetch_related:
    ...
    def populate_search_and_ordering_fields(self):
        if self.serializer_class and (not self.search_fields or not self.ordering_fields):
        ...
        if not id_to_add:
            ...
            for field in fields:
                ...
                if is_required:
                    ...
                    request = serializer.context['request'].data.get(
                        field_name, None)
                    if validated == None and request is not None:
                        ...
                        if type(request) is dict and 'label' in request:
                            ...
                            for related_field in related_model_fields:
                                related_field_name = getattr(
                                    related_field, 'name')

                                if related_field.related_model or related_field_name == 'id' or getattr(related_field, 'blank', False):
                                    continue

                                data = {
                                    related_field_name: request['label']
                                }
                                (created_instance, created) = related_model.objects.get_or_create(
                                    **data)
                                break
...
class GenericViewSet(DRFReactBySchemaBaseModelViewSet):
    serializer_list_class = None
    pagination_class = None if 'PAGINATION_MODE' not in settings.get('DRF_REACT_BY_SCHEMA', {}) or settings['DRF_REACT_BY_SCHEMA']['PAGINATION_MODE'] != 'server' else DRFReactBySchemaPagination
    ...
    for name, model in apps.all_models[app].items():
        ...
        if isinstance(model, type):
            ...
            if not viewset_name in dir():
                generated_class = type(viewset_name, (GenericViewSet,), {
                    'model_name': model_name,
                    'app': app,
                    'serializer_class': getattr(serializers, f"{model_name}Serializer"),
                    'serializer_list_class': getattr(serializers, f"{model_name}ListSerializer"),
                })
                ...
                for field in model._meta.get_fields():
                    if field.__class__.__name__ == 'ManyToOneRel':
                        ...
                        if getattr(serializers, f"{related_model_name}Serializer", None):
                            ...
                            target_field_name = 'pk' if field.target_field.name == 'id' else field.target_field.name
                            generated_nested_class = type(related_viewset_name, (GenericRelatedModelViewSet,), {
                                'model': field.related_model,
                                'app': app,
                                'serializer_class': getattr(serializers, f"{related_model_name}Serializer"),
                            ...
                            globals()[
                                related_viewset_name] = generated_nested_class
