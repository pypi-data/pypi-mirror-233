...
class EC:
    ...
    ***********************************************************************/
    ...
    def gcd(self, m: int, n: int) -> int:
        ...
        logging.warning(
            "The 'gcd' method is deprecated and will be discontinued in the future."
        )
    ...
    def egcd(self, m: int, n: int) -> tuple:
        ...
        logging.warning(
            "The 'egcd' method is deprecated and will be discontinued in the future."
        )
    ...
    def mmi(self, m: int, n: int) -> int:
        ...
        logging.warning(
            "The 'mmi' method is deprecated and will be discontinued in the future."
        )
    ...
    def is_point_on_curve(self, P: tuple):
        (x, y) = P
        return (y**2) % self.p == (x**3 + self.a * x + self.b) % self.p

    def dot(self, P: tuple, Q: tuple) -> tuple:
        """
        References: https://en.wikipedia.org/wiki/Elliptic_curve
        ...
        if P == Q:
            if not self.is_point_on_curve(P):
                raise ValueError("Invalid point: Not on the elliptic curve.")

            n = (3 * x_1**2 + self.a) % self.p
            d = (2 * y_1) % self.p
            ...
            x_3 = (s**2 - x_1 - x_1) % self.p
            y_3 = (s * (x_1 - x_3) - y_1) % self.p

            if not self.is_point_on_curve((int(x_3), int(y_3))):
                raise ValueError("Invalid point: Not on the elliptic curve.")

            return int(x_3), int(y_3)
        else:
            if not self.is_point_on_curve(P) or not self.is_point_on_curve(Q):
                raise ValueError("Invalid point: Not on the elliptic curve.")
    ...
    def trapdoor(self, G: tuple, k: int) -> tuple:
        """
        References: https://en.wikipedia.org/wiki/Trapdoor_function
