...
from tetra_model_zoo.video_classifier.model import KineticsClassifier
...
def normalize_base(video: torch.Tensor, mean: List[float], std: List[float]):
    """
...
def get_class_name_kinetics_400() -> str:
...
def recognize_action_kinetics_400(prediction: torch.Tensor) -> List[str]:
    ...
    Returns:
        classnames: class id from Kinetics-400 dataset is returned.
...
class KineticsClassifierApp:
    ...
    def predict(self, path: str) -> List[str]:
        """
        ...
        input_video = read_video_per_second(path)
