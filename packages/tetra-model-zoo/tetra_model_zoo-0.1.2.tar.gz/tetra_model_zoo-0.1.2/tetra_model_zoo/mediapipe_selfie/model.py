...
from tetra_model_zoo.mediapipe_selfie.utils import (
    build_state_dict,
    get_convert,
    get_probable_names,
)
...
class SelfieSegmentation(nn.Module):
    ...
    def __init__(self, image_type: Optional[str] = "square"):
        """
        Parameters:
    ...
    @staticmethod
    def from_pretrained(image_type: str = DEFAULT_WEIGHTS):
        """
        ...
        """
        front_net = SelfieSegmentation(image_type)
        destination_path = download_data(
            MEDIAPIPE_SELFIE_CKPT_MAP[image_type], MODEL_ID
        )
    ...
    def forward(self, x):
        """
        Parameters:
            x (image): Input image to be segmented.
