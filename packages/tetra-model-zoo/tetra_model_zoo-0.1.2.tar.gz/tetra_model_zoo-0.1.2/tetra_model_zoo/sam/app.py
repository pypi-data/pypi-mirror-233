...
from tetra_model_zoo.sam.model import SAMTetraWrapper
...
class SAMApp:
    ...
    @no_type_check
    def __init__(self, model: SAMTetraWrapper):
        self.orig_img_size = None
        self.image_embeddings = None
        self.sam_tetra_wrapper = model
        self.sam_encoder = self.sam_tetra_wrapper.get_sam_encoder()
    ...
    def prepare(self, input_image: np.ndarray, single_mask_mode=True):
        ...
        self.sam_decoder = self.sam_tetra_wrapper.get_sam_decoder(
    ...
    def postprocess_mask(self, generated_mask: torch.Tensor):
        ...
        return generated_mask > self.sam_tetra_wrapper.get_sam().mask_threshold
