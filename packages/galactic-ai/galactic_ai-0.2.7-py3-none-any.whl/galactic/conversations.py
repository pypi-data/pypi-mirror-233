...
def conversation_from_dicts(
    ...
    def _parse_messages(sample: dict):
        conversation = sample[input_column] if conversation_field is None else sample[input_column][conversation_field]
        conversation = list(map(lambda msg: {
            output_message_config.role_field: role_dict[msg[input_message_config.role_field]],
            output_message_config.content_field: msg[input_message_config.content_field],
...
def conversation_from_string(
    ...
    self.dataset = self.dataset.map(lambda x: {output_column: _parse_single(x[input_column])})
    return self
...
def convert_conversation_to_string(
    ...
    def _convert_single(conversation: list[dict]):
        ...
        for msg in conversation:
            role, content = msg[input_message_config.role_field], msg[input_message_config.content_field]
            result += {
    ...
    self.dataset = self.dataset.map(lambda x: {output_column: _convert_single(x[input_column])})
    return self
...
def get_conversation_length(
    ...
    self.dataset.map(
        lambda x: {output_column: len(x[input_column])}
    )
    return self
...
def get_conversation_speakers(
    ...
    self.dataset.map(
        lambda x: {output_column: [msg[input_message_config.role_field] for msg in x[input_column]]}
    )
    return self
...
def is_alternating(
    ...
    def _check_alternating(conversation: list[dict]):
        speakers = [msg[input_message_config.role_field] for msg in conversation]
        return all(speakers[i] != speakers[i + 1] for i in range(len(speakers) - 1))
...
def get_last_speaker(
    ...
    if isinstance(input_message_config, dict):
        input_message_config = MessageConfig(**input_message_config)
    self.dataset = self.dataset.map(
        lambda x: {output_column: x[input_column][-1][input_message_config.role_field]}
    )
    return self
...
def standardize_last_turn(
    ...
    def _standardize_single(conversation: list[dict]):
        ...
        while conversation[-1][input_message_config.role_field] != expected_role:
            conversation = conversation[:-1]
        return conversation
        
    self.dataset = self.dataset.map(lambda x: {output_column: _standardize_single(x[input_column])})
    return self
...
def get_shared_prefix(
    ...
    Operates on strings, not lists of messages.
    """
...
def add_initial_system_message(
    ...
    self.dataset.map(
        lambda x: {output_column: new_message + x[input_column]}
    )
    return self
...
def take_initial_system_message(
    ...
    def _take_system_msg_single(conversation: list[dict]):
        if conversation[0]['role'] == 'system':
            return {
                output_column: conversation[0] if output_type == 'dict' else conversation[0][input_message_config.content_field],
                input_column: conversation[1:] if remove_from_input else conversation
            }
        else:
            return {
                output_column: None,
    ...
    self.dataset = self.dataset.map(lambda x: _take_system_msg_single(x[input_column]))
    return self
...
def take_last_message(
    ...
    def _take_last_single(conversation: list[dict]):
        return {
            output_column: conversation[-1] if output_type == 'dict' else conversation[-1][input_message_config.content_field],
            input_column: conversation[:-1] if remove_from_input else conversation
        }
        
    self.dataset = self.dataset.map(lambda x: _take_last_single(x[input_column]))
    return self