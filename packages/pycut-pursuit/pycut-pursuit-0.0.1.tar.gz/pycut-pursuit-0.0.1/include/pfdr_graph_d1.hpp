...
public:
    ...
    #define NULH SCALAR /* use SCALAR flag for null hessian */
    ...
    /* warning: coor_weights cannot be changed from null to varying weights or
    ...
    { set_edge_weights(edge_weights, homo_edge_weight, this->coor_weights); }
...
private:
    ...
    Condshape compute_ga_shape(const real_t* coor_weights,
        Condshape hess_f_h_shape)
    {
        if (hess_f_h_shape == MULTIDIM || coor_weights){
            return MULTIDIM;
        }else{
            return MONODIM;
        }
    ...
    Condshape compute_w_shape(D1p d1p, const real_t* coor_weights,
        Condshape hess_f_h_shape)
    {
        if (d1p == D12 && (hess_f_h_shape == MULTIDIM || coor_weights)){
            return MULTIDIM;
        }else{
            return MONODIM;
        }
    ...
    Condshape compute_wd1_shape(D1p d1p, const real_t* coor_weights,
        Condshape hess_f_h_shape)
    {
        if (hess_f_h_shape == NULH){
            return SCALAR;
        }else if (d1p == D12 || (hess_f_h_shape == MONODIM && !coor_weights)){
            return MONODIM;
        }else{
            return MULTIDIM;
        }
    ...
    Condshape compute_thd1_shape(D1p d1p, const real_t* coor_weights,
        Condshape hess_f_h_shape)
    {
        if (d1p == D11 && (hess_f_h_shape == MULTIDIM || coor_weights)){
            return MULTIDIM;
        }else{
            return MONODIM;
        }
