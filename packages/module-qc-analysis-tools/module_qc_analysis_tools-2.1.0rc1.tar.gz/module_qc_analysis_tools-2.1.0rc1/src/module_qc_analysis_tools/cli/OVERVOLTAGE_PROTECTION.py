...
def main(
    ...
    for filename in sorted(allinputs):
        ...
        for inputDF in inputDFs:
            ...
            data.add_parameter("OVP_VINA", calculated_data["VinA"]["Values"][0])
            data.add_parameter("OVP_VIND", calculated_data["VinD"]["Values"][0])
            data.add_parameter("OVP_VREFOVP", calculated_data["VrefOVP"]["Values"][0])
            data.add_parameter("OVP_IINA", calculated_data["IinA"]["Values"][0])
            data.add_parameter("OVP_IIND", calculated_data["IinD"]["Values"][0])
