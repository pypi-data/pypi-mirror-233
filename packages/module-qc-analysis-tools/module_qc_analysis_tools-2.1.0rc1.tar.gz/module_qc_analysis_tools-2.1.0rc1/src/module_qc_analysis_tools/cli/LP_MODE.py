...
def main(
    ...
    log.info("")
    log.info(" =======================================")
    log.info(" \tPerforming LP MODE analysis")
    log.info(" =======================================")
    log.info("")
    ...
    for filename in sorted(allinputs):
        ...
        for inputDF in inputDFs:
            ...
            data.add_parameter("LP_VINA", calculated_data["VinA"]["Values"][0])
            data.add_parameter("LP_VIND", calculated_data["VinD"]["Values"][0])
            data.add_parameter("LP_VOFFS", calculated_data["Vofs"]["Values"][0])
            data.add_parameter("LP_IINA", calculated_data["IinA"]["Values"][0])
            data.add_parameter("LP_IIND", calculated_data["IinD"]["Values"][0])
            data.add_parameter("LP_ISHUNTA", calculated_data["IshuntA"]["Values"][0])
            data.add_parameter("LP_ISHUNTD", calculated_data["IshuntD"]["Values"][0])
            data.add_parameter(
                "LP_DIGITAL_FAIL", int(calculated_data["FailingPixels"]["Values"][0])
            )
