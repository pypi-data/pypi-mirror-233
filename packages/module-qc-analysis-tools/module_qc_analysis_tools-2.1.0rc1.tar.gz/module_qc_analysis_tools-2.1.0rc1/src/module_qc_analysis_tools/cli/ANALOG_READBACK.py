    ...
    for filename in sorted(allinputs):
        ...
        for inputDF in inputDFs:
            ...
            if inputDF._subtestType == "AR_VMEAS":
                ...
                data[chipname].add_parameter("AR_NOMINAL_SETTINGS", AR_values)
                ...
                # Add parameters for output file
                data[chipname].add_parameter("AR_VDDA_VS_TRIM", vdda)
                data[chipname].add_parameter("AR_VDDD_VS_TRIM", vddd)
                data[chipname].add_parameter("AR_ROSC_SLOPE", p1_list)
                data[chipname].add_parameter("AR_ROSC_OFFSET", p0_list)
                data[chipname].add_parameter("AR_ROSC_MAX_RESIDUAL", maxres_list)

                # Load values to dictionary for QC analysis
                tmpresults.update({"AR_VDDA_VS_TRIM": vdda})
                tmpresults.update({"AR_VDDD_VS_TRIM": vddd})
