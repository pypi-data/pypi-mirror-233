...
"""
###############################################################################################
General idea of how this analysis works (developer notes):
...
def main(
    ...
    for c in input_data["chip"]:
        ...
        failure_summary_elec = count_pixels(pix_fail, record_fail, test_names_elec)
        ...
        if (
            ...
            input_data_path = datadir + "/" + filepaths[params_0bias.get("input")[0]]
            input_data = read_json(Path(input_data_path))
            check_input_yarr_data(input_data, path=input_data_path)
            noise_hv = format_pixel_input(input_data)

            input_data_path = datadir + "/" + filepaths[params_0bias.get("input")[1]]
            input_data = read_json(Path(input_data_path))
            check_input_yarr_data(input_data, path=input_data_path)
        ...
        if params_xtalk.get("input")[0] in filepaths:
            input_data_path = datadir + "/" + filepaths[params_xtalk.get("input")[0]]
            input_data = read_json(Path(input_data_path))
            check_input_yarr_data(input_data, path=input_data_path)
            ...
            try:
                ...
                chip_id = input_data.get("RD53B").get("Parameter").get("ChipId")
                if module_type == "quad" and chip_id not in [12, 13, 14, 15]:
                    log.warning(
                        bcolors.WARNING
        ...
        if (
            ...
            input_data_path = datadir + "/" + filepaths[params_source.get("input")[0]]
            input_data = read_json(Path(input_data_path))
            check_input_yarr_data(input_data, path=input_data_path)
            occ_sourcescan = format_pixel_input(input_data)

            input_data_path = datadir + "/" + filepaths[params_source.get("config")[0]]
            input_data = read_json(Path(input_data_path))
            check_input_yarr_data(input_data, path=input_data_path, config=True)
        ...
        if np.any(occ_xtalk):
            ...
            for (
                ...
                if module_type != "quad" and label == "inneredge":
                    continue
        ...
        failure_summary_disc = count_pixels(pix_fail, record_fail, test_names_disc)
            ...
            for test in test_names_all:
                try:
                    outputArrays.update(
                        {
                            test
                            + "_indep": result_array_indep[get_fail_bit(test)]
                            .reshape((384, 400))
                            .transpose()
                        }
                    )
                    outputArrays.update(
                        {
                            test
                            + "_dep": result_array_indep[get_fail_bit(test)]
                            .reshape((384, 400))
                            .transpose()
                        }
                    )
        ...
        failure_summary_all = count_pixels(pix_fail, record_fail, test_names_all)

        for fname, nfail in failure_summary_all.items():
            if "DISCONNECTED_BUMPS_" in fname:
                data.add_parameter("PIXEL_FAILURE_" + fname, nfail.get("independent"))
            else:
        ...
        results.update({"ELECTRICALLY_FAILED": total_elec_failing})
        ...
        for etest, eresult in list(failure_summary_disc.items()):
            if eresult.get("dependent") == -1 and "SOURCE" not in etest:
                total_disc_failing = -1
        results.update({"DISCONNECTED_PIXELS": total_disc_failing})
        ...
        for etest, eresult in list(failure_summary_all.items()):
            if eresult.get("dependent") == -1 and "SOURCE" not in etest:
                total_failing = -1
        results.update({"FAILING_PIXELS": total_failing})
        data.add_parameter("PIXEL_FAILURE_FAILING_PIXELS", total_failing)

        # Print summary of analysis
        print_pixel_classification(failure_summary_all, test_type, output_dir, chipName)
