...
class MakeSvgInstance:
    ...
    def __drawAspect(self, r, ar, aspect_dict, aspect_id: str, color: str, isAxis=False, cut_by=8.4):
        ...
        if isAxis:
            ...
            degree_text = self.__dec2deg(aspect_dict['p1_abs_pos'] % 30, type="1")
                        
            dx = firstCut['x'] - start['x']
            dy = firstCut['y'] - start['y']

            offset_length = 0  # Distanza dal centro dell'asse
            offset_x = -dy * offset_length / math.sqrt(dx**2 + dy**2)
            offset_y = dx * offset_length / math.sqrt(dx**2 + dy**2)
            ...
            angle_from_horizontal = abs(math.atan2(dy, dx)) % (2 * math.pi)
            if 0 < angle_from_horizontal < math.pi:
                # Line is generally pointing right
                offset_x = offset_length
                offset_y = -offset_length
            elif math.pi <= angle_from_horizontal < 2 * math.pi:
                # Line is generally pointing left
                offset_x = -offset_length
                offset_y = offset_length
                
            # Calcola il punto intermedio tra start e firstCut
            shifted_x = start['x'] + degree_shift * (firstCut['x'] - start['x'])
            shifted_y = start['y'] + degree_shift * (firstCut['y'] - start['y'])

            # Applica l'offset alle coordinate midpoint_x e midpoint_y
            text_x = shifted_x + offset_x
            text_y = shifted_y + offset_y
            rotation_transform = f"rotate({angle}, {shifted_x}, {shifted_y})"
            line = f'''
                <g class="axis" id="{aspect_id}" style="stroke-width: {axis_stroke_width}; stroke-opacity: {axis_stroke_opacity}; transform: translate(0, 0);" stroke-linecap="round" >
                    ...
                    <text x="{text_x}" y="{text_y}" fill="{color}" text-anchor="middle" dominant-baseline="middle" transform="{rotation_transform}" style="font-size: .6em">{degree_text}</text>
                </g>
            '''
        else:
            if orb:
        ...
        if not isAxis:
            ...
            if (0 <= angle < 45) or (315 <= angle < 360):
                ...
                mid_x -= label_offset
            line += f'<text class="aspectDeg" id="{aspect_id}Deg" x="{mid_x}" y="{mid_y}" fill="#ffffff" text-anchor="middle" dominant-baseline="middle" style="opacity: 0; font-size: .6em">{str(round(orb))}°</text>'
        return line
    ...
    def __makeHouses(self, r):
        ...
        for i in range(xr):
            ...
            if self.chart_type == "Transit" or self.chart_type == "Composite":
                ...
                path = path + f'<text class="houseLabel" id="{house_by_index[i+1]} House Label Transit" style="fill: #000000; fill-opacity: {text_opacity}; font-size: .6em; font-family: Cinzel; font-weight: bold"><tspan x="{xtext}" y="{ytext}">{house_number}</tspan></text>'
            ...
            path = path + f'<text class="houseLabel" id="{house_by_index[i+1]} House Label" style="fill: #000000; fill-opacity: .8; font-size: .6em; font-family: Cinzel; font-weight: bold"><tspan x="' + \
    ...
    def __makePlanets(self, r):
        ...
        for e in range(len(keys)):
            ...
            output = output + f'<line id="{self.planets_settings[i]["name"].title()}GLine" x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" style="stroke: {self.planets_settings[i]["color"]}; stroke-width: 1px; stroke-opacity:.8;"/>'
            ...
            if not too_close:
                ...
                output += f'<text transform="rotate({rotate})" text-anchor="{textanchor}" style="fill: {self.planets_settings[i]["color"]}; font-size: .6em;">'
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            for e in range(len(t_keys)):
                ...
                output = output + f'<line id="{self.planets_settings[i]["name"].title()}GTransitLine" x1="'+str(x1)+'" y1="'+str(y1)+'" x2="'+str(x2)+'" y2="'+str(
                    y2)+'" style="stroke: #0096FF'+'; stroke-width: 1px; stroke-opacity: 1;"/>'
                ...
                output = output + f'<line id="{self.planets_settings[i]["name"].title()}GTransitLineInner" x1="'+str(x1_inner)+'" y1="'+str(y1_inner)+'" x2="'+str(x2_inner)+'" y2="'+str(y2_inner)+'" style="stroke: #0096FF'+'; stroke-width: 1px; stroke-opacity: 1;"/>'
    ...
    def __makeAxis(self, r, ar, source, cut_by, color, transit_axis, composite_axis):
        out = ''
        axis_stroke_opacity = '1'
        for axis in source:
            if (transit_axis and axis.name == 'Ascendant') or (not transit_axis and axis.name in {'Ascendant', 'Midheaven'}):
                ...
                arrow_top = f'''
                    <marker id="arrowhead{axis_data['label_short']}{color}" markerWidth="12" markerHeight="12" refX="6" refY="6" orient="auto-start-reverse">
                        <polyline points="3,4 6,6 3,8" style="fill: none; stroke: {color}; stroke-opacity: {axis_stroke_opacity}" />
                    </marker>
                '''
                if axis_data:
                    axis_id = f"{axis_data['label_short'].replace('Asc', 'Ascendant').replace('Mc', 'Midheaven')}G"
                    if transit_axis or composite_axis:
                        axis_id += 'Transit'
                    aspect_dict = {
                    ...
                    c = self.c4
                    offset = (int(self.houses_degree_ut[6]) / -1) + int(aspect_dict['p1_abs_pos'])
                    x1 = self.__sliceToX(0, r-c, offset) + c
                    y1 = self.__sliceToY(0, r-c, offset) + c
                    length = 7
                    x2 = self.__sliceToX(0, r+length-c, offset) - length + c
                    y2 = self.__sliceToY(0, r+length-c, offset) - length + c
                    tickColor = color if transit_axis or composite_axis else 'black'
                    out += f'''
                        <marker id="arrowhead{axis_id}{tickColor}Line" markerWidth="12" markerHeight="12" refX="6" refY="6" orient="auto-start-reverse">
                            <polyline points="3,4 6,6 3,8" style="fill: none; stroke: {tickColor}; stroke-opacity: {axis_stroke_opacity}" />
                        </marker>
                    '''
                    marker_arrow = f'marker-end: url(#arrowhead{axis_id}{tickColor}Line);'
                    out += f'<line id="{axis_id}Line" x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" style="stroke: {tickColor}; stroke-width: 1px; stroke-opacity:.8; {marker_arrow}" />'
        return out
    ...
    def makeTemplate(self):
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            transit_axis = True if self.chart_type == "Transit" else False
            composite_axis = True if self.chart_type == "Composite" else False
            td['transitRingAxes'] = self.__makeAxis(r, r-self.c1+54.5, self.t_user.axis_list, 9.7, '#0096FF', transit_axis, composite_axis)
            ...
            td['c3style'] = 'fill: %s; fill-opacity: 1; stroke: %s; stroke-width: 1px' % (
                self.colors_settings['paper_2'], self.colors_settings['zodiac_transit_ring_3'])
            ...
            td['makePatterns'] = ''
            td['makeAxis'] = self.__makeAxis(r, r-self.c1+18, self.user.axis_list, 8.4, '#ff0000', False, False)
            ...
            td['makeAxis'] = self.__makeAxis(r, (r-self.c1+30), self.user.axis_list, 8.4, '#ff0000', False, False)
