...
import crystalai
from crystalai.upload_progress import BufferReader
from crystalai.validators import (
...
class Engine:
    @classmethod
    def get(cls, args):
        engine = crystalai.Engine.retrieve(id=args.id)
        display(engine)

    @classmethod
    def update(cls, args):
        engine = crystalai.Engine.modify(args.id, replicas=args.replicas)
        display(engine)
    ...
    def generate(cls, args):
        ...
        resp = crystalai.Engine(id=args.id).generate(
...
class ChatCompletion:
    ...
    def create(cls, args):
        ...
        resp = crystalai.ChatCompletion.create(
            # Required
            model=args.model,
            engine=args.engine,
            messages=messages,
...
class Completion:
    ...
    def create(cls, args):
        ...
        resp = crystalai.Completion.create(
            engine=args.engine,
            model=args.model,
            n=args.n,
            max_tokens=args.max_tokens,
...
class Deployment:
    @classmethod
    def get(cls, args):
        resp = crystalai.Deployment.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = crystalai.Deployment.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = crystalai.Deployment.list()
        print(models)

    @classmethod
    def create(cls, args):
        models = crystalai.Deployment.create(
...
class Model:
    @classmethod
    def get(cls, args):
        resp = crystalai.Model.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = crystalai.Model.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = crystalai.Model.list()
        print(models)
...
class File:
    ...
    @classmethod
    def get(cls, args):
        resp = crystalai.File.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        file = crystalai.File.delete(args.id)
        print(file)

    @classmethod
    def list(cls, args):
        file = crystalai.File.list()
        print(file)
...
class Image:
    ...
    def create_variation(cls, args):
        with open(args.image, "rb") as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc="Upload progress")
        resp = crystalai.Image.create_variation(
            image=buffer_reader,
            size=args.size,
            n=args.num_images,
            response_format=args.response_format,
    ...
    def create_edit(cls, args):
        ...
        resp = crystalai.Image.create_edit(
            image=image_reader,
            mask=mask_reader,
            prompt=args.prompt,
            size=args.size,
...
class Audio:
    ...
    def transcribe(cls, args):
        ...
        resp = crystalai.Audio.transcribe_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
    ...
    def translate(cls, args):
        with open(args.file, "rb") as r:
            file_reader = BufferReader(r.read(), desc="Upload progress")
        resp = crystalai.Audio.translate_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
...
class FineTune:
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = crystalai.File.find_matching_files(
    ...
    def _get_or_upload(cls, file, check_if_file_exists=True):
        try:
            ...
            crystalai.File.retrieve(file)
            return file
        except crystalai.error.InvalidRequestError:
            pass
    ...
    def create(cls, args):
        ...
        resp = crystalai.FineTune.create(**create_args)
    ...
    @classmethod
    def get(cls, args):
        resp = crystalai.FineTune.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = crystalai.FineTune.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise crystalai.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = crystalai.FineTune.retrieve(id=args.id)["result_files"][0]
        resp = crystalai.File.download(id=result_file["id"])
        print(resp.decode("utf-8"))

    @classmethod
    def events(cls, args):
        if args.stream:
            raise crystalai.error.OpenAIError(
                message=(
        ...
        resp = crystalai.FineTune.list_events(id=args.id)  # type: ignore
        print(resp)
    ...
    def _stream_events(cls, job_id):
        def signal_handler(sig, frame):
            status = crystalai.FineTune.retrieve(job_id).status
            sys.stdout.write(
        ...
        events = crystalai.FineTune.stream_events(job_id)
        ...
        resp = crystalai.FineTune.retrieve(id=job_id)
        status = resp["status"]
        if status == "succeeded":
            ...
            sys.stdout.write(
                "\nJob failed. Please contact us through our help center at help.crystalcomputing.com if you need assistance."
            )
        sys.stdout.write("\n")

    @classmethod
    def cancel(cls, args):
        resp = crystalai.FineTune.cancel(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        resp = crystalai.FineTune.delete(sid=args.id)
        print(resp)
...
class FineTuningJob:
    ...
    def list(cls, args):
        has_ft_jobs = False
        for fine_tune_job in crystalai.FineTuningJob.auto_paging_iter():
            has_ft_jobs = True
            print(fine_tune_job)
        if not has_ft_jobs:
    ...
    def _maybe_upload_file(
        ...
        sys.stdout.flush()
        status = crystalai.File.wait_for_processing(resp["id"])
        if status != "processed":
            raise crystalai.error.OpenAIError(
    ...
    def create(cls, args):
        ...
        resp = crystalai.FineTuningJob.create(**create_args)
        print(resp)
        return

    @classmethod
    def get(cls, args):
        resp = crystalai.FineTuningJob.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = crystalai.FineTuningJob.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise crystalai.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = crystalai.FineTuningJob.retrieve(id=args.id)["result_files"][0]
        resp = crystalai.File.download(id=result_file)
        print(resp.decode("utf-8"))
    ...
    def events(cls, args):
        ...
        while has_more:
            resp = crystalai.FineTuningJob.list_events(id=args.id, after=after)  # type: ignore
    ...
    @classmethod
    def cancel(cls, args):
        resp = crystalai.FineTuningJob.cancel(id=args.id)
        print(resp)
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import crystalai.wandb_logger

        resp = crystalai.wandb_logger.WandbLogger.sync(
            id=args.id,
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument(
        "-e",
        "--engine",
        help="The engine to use. See https://platform.crystalcomputing.com/docs/engines for more about what engines are available.",
    )
    sub.add_argument(
        "-m",
        "--model",
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument(
        "-p",
        "--purpose",
        help="Why are you uploading this file? (see https://platform.crystalcomputing.com/docs/api-reference/ for purposes)",
        required=True,
    )
