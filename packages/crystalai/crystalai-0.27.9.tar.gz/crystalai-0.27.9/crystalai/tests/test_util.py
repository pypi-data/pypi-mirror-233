...
def api_key_file():
    saved_path = crystalai.api_key_path
    try:
        with NamedTemporaryFile(prefix="openai-api-key", mode="wt") as tmp:
            crystalai.api_key_path = tmp.name
            yield tmp
    finally:
        crystalai.api_key_path = saved_path


def test_openai_api_key_path(api_key_file) -> None:
    print("crystal_foo", file=api_key_file)
    api_key_file.flush()
    assert util.default_api_key() == "crystal_foo"
