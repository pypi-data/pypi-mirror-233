...
import crystalai
from crystalai import version
from crystalai.cli import api_register, display_error, tools_register, wandb_register
...
def main():
    ...
    crystalai.debug = True
    if args.api_key is not None:
        crystalai.api_key = args.api_key
    if args.api_base is not None:
        crystalai.api_base = args.api_base
    if args.organization is not None:
        crystalai.organization = args.organization
    if args.proxy is not None:
        crystalai.proxy = {}
        for proxy in args.proxy:
            if proxy.startswith('https'):
                crystalai.proxy['https'] = proxy
            elif proxy.startswith('http'):
                crystalai.proxy['http'] = proxy

    try:
        args.func(args)
    except crystalai.error.OpenAIError as e:
        display_error(e)
        return 1
