...
import crystalai
from crystalai.datalib.numpy_helper import numpy as np
...
def get_embedding(text: str, engine="text-embedding-ada-002", **kwargs) -> List[float]:
    ...
    return crystalai.Embedding.create(input=[text], engine=engine, **kwargs)["data"][0]["embedding"]
...
async def aget_embedding(
    ...
    return (await crystalai.Embedding.acreate(input=[text], engine=engine, **kwargs))["data"][0][
        "embedding"
    ]
...
def get_embeddings(
    ...
    data = crystalai.Embedding.create(input=list_of_text, engine=engine, **kwargs).data
...
async def aget_embeddings(
    ...
    data = (await crystalai.Embedding.acreate(input=list_of_text, engine=engine, **kwargs)).data
    return [d["embedding"] for d in data]
