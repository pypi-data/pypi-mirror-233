...
class KafkaProducer:
    def __init__(self,
        ...
        config = self._set_conn_config(host,
                                       username,
                                       password,
                                       mechanism,
                                       security_protocol)
    ...
    def _set_conn_config(host,
        ...
        if not any(['localhost' in host, 
                    '172.0.0.1' in host]):
            config.update({'sasl.username': username,
    ...
    def _add_msg_to_producer_buffer(self, key, value):
    ...
    def send(self, key, value):
        self._add_msg_to_producer_buffer(key, value)
        self.producer.flush()
