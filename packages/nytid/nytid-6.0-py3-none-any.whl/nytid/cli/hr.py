...
def summarize_user(user, course_events, salary=150):
    """
    ...
    for event in events:
        ...
        first_comma = event_type.find(",")
        course = event_type[first_comma + 2 :]
...
amanuensis_opt = typer.Option(
    help="Include amanuensis, i.e. generate " "mertid time sheets."
)
...
def cli_generate_timesheets(
    ...
    for user in users:
        ...
        for contract in user_contracts:
            contract_events = summarize_user(user, contract["events"])
        ...
        for event in removed_events:
            logging.warning(
                f"Event {event} was removed, but previously reported " f"for {user}."
            )
        if store:
            timesheets_dir = pathlib.Path(typerconf.get(TIMESHEETS_DIR_PATH))
            filename = f"{user}.{datetime.datetime.now().isoformat()}.json"
