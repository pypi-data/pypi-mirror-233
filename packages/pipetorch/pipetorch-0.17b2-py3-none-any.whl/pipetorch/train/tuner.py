...
class tuner:
    ...
    def run( self, cache_valid=True, xscale='log', xlabel=None, **kwargs ):
        graphx = []
        sloss = []
        validation_set = []
        mem_validation = 0
        ...
        #with plt_notebook():
        with Plot(xscale=xscale, xlabel=xlabel, **kwargs) as p:
            with self.trainer.train_mode:
                tq = tqdm(self.lrvalues, leave=False)
                for i, lr in enumerate(tq):
                    graphx.append(lr)
                    self.lrupdate(lr)
                    *X, y = self.next_train()
                    loss, pred_y = self.trainer._train_batch(*X, y=y)
                    with self.trainer.eval_mode:
                        loss = self.trainer.loss_dl(validation_set).cpu()
                    try:
