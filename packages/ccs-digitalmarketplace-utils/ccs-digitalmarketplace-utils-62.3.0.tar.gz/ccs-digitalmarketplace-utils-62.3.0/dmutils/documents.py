...
def validate_documents(files):
    ...
    """
    ID_TO_VALIDATION = {
        'groupOrConsortiumDetailsAttachment6URL': file_is_open_spreadsheet_format,
        'groupOrConsortiumDetailsAttachment4aURL': file_is_zip,
    }

    errors = {}

    for field, contents in files.items():
        validator = ID_TO_VALIDATION.get(field, file_is_open_document_format)

        if not validator(contents):
            errors[field] = validator.__name__
...
def file_is_open_spreadsheet_format(file_object):
    """Checks file extension and format as being ods."""
    extension = get_extension(file_object.filename)

    return extension == ".ods" and extension_matches_possible_file_formats(file_object, extension)
...
def generate_file_name(framework_slug, upload_type, supplier_id, service_id, field, filename, suffix=None):
    ...
    ID_TO_FILE_NAME_SUFFIX = {
        ...
        'groupOrConsortiumDetailsAttachment6URL': 'attachment-6-consortia-details',
        'groupOrConsortiumDetailsAttachment4aURL': 'attachment-4a-consortia-declaration',
    }
