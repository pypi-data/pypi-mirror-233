...
This code is forked from https://github.com/dlce-eva/python-newick and
...
def check_string(n, type_):
    if RP_PATTERN.search(n) or any(c in n for c in WHITESPACE):
        raise ValueError(
            '"{}" may not appear in {}'.format(RESERVED_PUNCTUATION.keys(), type_)
        )
...
class Node(object):
    ...
    def name(self, n):
        ...
        if (
            (not quoted)
            and self._auto_quote
            and any(
                char in n for char in "".join(RESERVED_PUNCTUATION) + QUOTE + WHITESPACE
            )
        ):
    ...
    def newick(self) -> str:
        ...
        if self.comments:
            if (
                self._length
                and len(self.comments) == 2
                and not self._colon_before_comment
            ):
    ...
    def _ascii_art(self, char1="\u2500", show_internal=True, maxlen=None):
        ...
        if self.descendants:
            ...
            prefixes = (
                [pad] * (lo + 1) + [pad + "\u2502"] * (hi - lo - 1) + [pad] * (end - hi)
            )
            mid = (lo + hi) // 2
            prefixes[mid] = (
                char1 + "\u2500" * (len(prefixes[mid]) - 2) + prefixes[mid][-1]
            )
    ...
    def ascii_art(self, strict: bool = False, show_internal: bool = True) -> str:
        ...
        return "\n".join(
            normalize(line)
            for line in self._ascii_art(show_internal=show_internal)[0]
            if set(line) != {" ", "\u2502"}
        )  # remove lines of only spaces and pipes
    ...
    def _postorder(self):
        stack = [self]
        descendant_map = {
            id(node): [n for n in node.descendants] for node in self.walk()
        }
    ...
    def prune(self, nodes: typing.List["Node"], inverse: bool = False):
        ...
        self.visit(
            ...
            # be pruned!
            lambda n: (
                (not inverse and n in nodes)
                or (inverse and n.is_leaf and n not in nodes)  # noqa: W504
            )
            and n.ancestor,
            mode="postorder",
        )
    ...
    def remove_redundant_nodes(
        self, preserve_lengths: bool = True, keep_leaf_name: bool = False
    ):
        """
...
def loads(
    s: typing.Union[str, typing.Iterable[str]], strip_comments: bool = False, **kw
) -> typing.List[Node]:
    """
    ...
    # Adds a final semicolon if it is missing
    if isinstance(s, str):
        s = s.strip()
        if not s.endswith(";"):
            s += ";"

    # Adds a dummy root name if it is missing
    if isinstance(s, str):
        if s.endswith(");"):
            s = s[:-2] + ")__ROOT__;"

    # Adds dummy node names if they are missing (the parsing routine requires them);
    # this is a bit of a hack, but it works: for every instance of "):", we insert
    # a dummy sequential name, which can later be easily removed (e.g., "):0.1" -> ")__DUMMY_1__:0.1"
    def insert_dummy_labels(newick_string: str):
        counter = 1
        while "):" in newick_string:
            newick_string = newick_string.replace("):", f")__DUMMY_{counter}__:", 1)
            counter += 1
        return newick_string

    if isinstance(s, str):
        s = insert_dummy_labels(s)
...
class NewickString(list):
        ...
        if not isinstance(s, list):
            ...
            while 1:
                try:
                    ...
                    if (
                        c == QUOTE
                    ):  # Start of quoted string - we read to the end immediately.
                        ...
                        while 1:
                            ...
                            if doublequote and c == QUOTE:  # The escaped quote.
                                ...
                                if lookahead == QUOTE:
                                    ...
                                    inquote = False
                                    self.append(
                                        Token("".join(n), TokenType.QWORD, level)
                                    )
                                    break
                        continue
                    ...
                    word.append(
                        c
                    )  # All other characters are just accumulated into a word.
                except StopIteration:
                    if inquote:
    ...
    ) -> typing.Generator["NewickString", None, None]:
        def checked(t):
            if t:
