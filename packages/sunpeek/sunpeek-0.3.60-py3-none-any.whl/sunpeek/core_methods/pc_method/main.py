...
from sunpeek.core_methods.pc_method import AvailablePCMethods, AvailablePCFormulae, OutputUnits
...
F_PIPES = 0.98
F_UNCERTAINTY = 0.90
F_OTHERS__FORMULA1 = 0.98
F_OTHERS__FORMULA2 = 0.99
...
class PCSettings:
    ...
    safety_others: Optional[float]  # has no single default, depends on & needs to be set by Formula
    ...
    default_settings = dict(
        ...
        safety_others=None,  # Has no single default, depends on & needs to be set by Formula
    ...
    def __init__(self,
                 ...
                 formula: Union[AvailablePCFormulae, int],
                 **kwargs):
        ...
        if self.safety_others is None:
            if formula == AvailablePCFormulae.one:
                self.safety_others = F_OTHERS__FORMULA1
            elif formula == AvailablePCFormulae.two:
                self.safety_others = F_OTHERS__FORMULA2
...
class PCMethod(ABC):
    ...
    def report_problems(self, check_mode: AlgoCheckMode) -> ProblemReport:
        ...
        if not self.plant.arrays:
            r.add_own(AlgoProblem(ProblemType.component_missing, self.plant, 'arrays',
                                  'To run a Performance Check analysis, you need to add arrays to the plant'))
        ...
        for array in self.plant.arrays:
            r = self.formula.report_problems(r, array, check_mode)
    ...
    def _create_output_object(self) -> results.PCMethodOutput:
        ...
        out.evaluation_mode = AvailablePCMethods.iso.value if self.mode.value == AvailablePCMethods.iso else AvailablePCMethods.extended.value
        ...
        if has_intervals:
            ...
            try:
                ...
                plant_out.mean_fluid_heat_capacity = None if no_fluid else self.plant.fluid_solar.get_heat_capacity(te_s)[0]
            finally:
