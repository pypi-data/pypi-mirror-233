...
def data_start_end(plant_id: int,
    ...
    plant = crd.get_plants(sess, plant_id=plant_id)
    idx = plant.time_index

    if idx is None:
        return smodels.PlantDataStartEnd(start=None, end=None)

    return smodels.PlantDataStartEnd(start=idx[0], end=idx[-1])
...
def delete_sensors(ids: Annotated[list[str] | None, Query()] = None,
                   plant_id: Union[int, str, None] = None,
                   sess: Session = Depends(session),
                   crd=Depends(crud),
                   ):
    ...
    for sensor in sensors_to_delete:
        # delete_sensor(id=sensor.id, sess=sess, crd=crd)
