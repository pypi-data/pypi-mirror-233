...
class Collector(AttrSetterMixin, ORMBase):
    ...
    kd = ComponentParam('', minimum=0, maximum=1)
    ...
    def update_parameters(self) -> None:
        ...
        - For statically-tested collectors only: Estimates `kd` using estimate_kd_Hess_and_Hanby(), if necessary.
        """
        # Parse & store current object parameters
        attribs = {'gross_length', 'area_gr', 'area_ap', 'a1', 'a2', 'a5', 'a8', 'kd', 'eta0b', 'eta0hem'}
        required_not_none = ['gross_length', 'area_gr', 'a1', 'a2', 'a5']
        p = dict()
        for k in attribs:
            p[k] = uu.parse_quantity(self.__getattribute__(k))
            if p[k] is None and k in required_not_none:
                raise CollectorDefinitionError(f'Collector parameter is None, but must be specified: "{k}".')

        if p['area_ap'] is not None and (p['area_ap'] > p['area_gr']):
            raise CollectorDefinitionError(f'Aperture area must be smaller than gross area. You provided '
                                           f'"area_ap"={p["area_ap"]}, "area_gr"={p["area_gr"]}.')
        ...
        if is_aperture and p['area_ap'] is None:
        ...
        if p['kd'] is None:  # Is applied to SST collectors only
            p['kd'], info = estimate_kd_Hess_and_Hanby(self.iam_method)
            self.calculated_parameters['kd'] = info
        self.kd = p['kd']

        if p['eta0hem'] is None:
            self.eta0hem, info = estimate_eta0hem(eta0b=p['eta0b'], kd=p['kd'])
            self.calculated_parameters['eta0hem'] = info

        if p['eta0b'] is None:
            self.eta0b, info = estimate_eta0b(eta0hem=p['eta0hem'], kd=p['kd'])
        ...
        aperture_keys = ['a1', 'a2', 'a5', 'a8', 'eta0b', 'eta0hem']
        if not self.aperture_parameters:
            self.aperture_parameters = {
                k: {'magnitude': p[k].magnitude, 'units': str(p[k].units)} if p[k] is not None else None
                for k in aperture_keys}

        # Add info text about calculated parameters
        for k, v in self.aperture_parameters.items():
            self.calculated_parameters.setdefault(k, '')
            self.calculated_parameters[k] += 'Converted from aperture to gross area by SunPeek.'

        # Do the area conversion
        conversion_factor = p['area_ap'] / p['area_gr']
        for k, v in self.aperture_parameters.items():
            self.__setattr__(k, None if v is None else conversion_factor * uu.parse_quantity(v))
    ...
    def _infer_test_type(self, test_type: Union[str, None]) -> str:
        ...
        return test_type
        # if test_type.lower() not in [s.lower() for s in self.VALID_TEST_TYPES]:
        #     raise ValueError(f'Parameter "test_type" must be one of {", ".join(self.VALID_TEST_TYPES)}.')

        # is_dynamic = test_type.lower() in [s.lower() for s in [self.t_types.dynamic.value, self.t_types.QDT.value]]
        # return 'QDT' if is_dynamic else 'SST'

    def _infer_test_reference_area(self, area: Union[str, None]) -> str:
        """Returns test reference area type (gross, aperture) based on user input.
        """
        ...
        #     raise ValueError(f'Parameter "test_reference_area" must be one of {", ".join(self.VALID_AREA_TYPES)}.')
        # is_gross = area.lower() in [s.lower() for s in [self.ref_a_types.area_gr, self.ref_a_types.gross]]
        # return 'gross' if is_gross else 'aperture'

    # def _set_collector_parameters(self, area_gr, area_ap, a1, a2, a5, kd, eta0b, eta0hem) -> None:
