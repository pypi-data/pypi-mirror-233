...
response_model=Union[List[smodels.Collector], smodels.Collector],
                   tags=["collectors"], 
...
@config_router.post("/collectors/new",
                    response_model=Union[smodels.Collector, List[smodels.Collector]],
                    tags=["collectors"],
                    status_code=201,
                    summary="Create a new collector or collectors")
def create_collector(collector: Union[
    ...
    for i, collector in enumerate(collectors_):
        ...
        if test_type in ['SST', "static"]:
            ...
            raise ConfigurationError(
                "Collector test_type parameter must be one of 'SST', 'static', 'QDT' or 'dynamic'")
