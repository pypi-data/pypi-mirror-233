...
class Plant(Component):
    ...
    sensor_slots = {
        ...
        'rh_amb': SensorSlot('rh_amb', st.float_0_100,
...
class Array(Component):
    ...
    def calc_area_gr_from_collector(self):
        ...
        """
        coll = self.collector
        if (self.area_ap is None) or (coll is None) or isinstance(coll, UninitialisedCollector):
            return None
    ...
    def collector(self, value):
        if isinstance(value, Collector):
            ...
            raise ConfigurationError("Collector must be a Collector object, or the name of an existing collector.")
