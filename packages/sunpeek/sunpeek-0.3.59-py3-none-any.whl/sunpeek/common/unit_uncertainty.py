...
def to_pint(s, unit=None, absolute_uncertainty=None, relative_uncertainty=None,
    ...
    use_relative_abs : `boolean`, optional
        ...
        of resistance thermometer uncertainty, typically given as :math:`a + r * \mid T \mid` according to
        `DIN EN 60751`_.
...
def check_quantity(q, unit='', min_limit=-np.Inf, max_limit=np.Inf, none_allowed=False, max_ndim=0):
    """
    Check that `q` is a valid scalar pint Quantity object.
    Also accepts pandas Series as input; in that case it coverts the Series to pint Quantity and returns it.
    ...
    #     try:
    #         unit = q.dtype.units.__str__()
    #     except AttributeError:
    #         raise TypeError('check_quantity accepts pd.Series as input only if dtype is some pint unit.')
    #     # q = Q(q.pint.m_as(unit).to_numpy(), unit)
