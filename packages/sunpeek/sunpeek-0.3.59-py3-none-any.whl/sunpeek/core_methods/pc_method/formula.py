...
from sunpeek.core_methods.pc_method import AvailablePCFormulae, OutputUnits
...
# TODO Use this for all formulae?
MAX_AOI = Q(80, 'deg')
...
class Formula(ABC):
    ...
    # array power.
    # Common to formulae 1 & 2
    max_deltat_collector = MAX_DELTA_T_COLLECTOR
    min_te_amb = MIN_TE_AMB
    max_wind_speed = MAX_WIND_SPEED

    @classmethod
    def create(cls, formula: Union[AvailablePCFormulae, int], use_wind: bool) -> 'Formula':
        if formula == AvailablePCFormulae.one:
            return Formula1(use_wind)

        elif formula == AvailablePCFormulae.two:
            return Formula2(use_wind)

        else:
            raise PCMethodError(f'Unknown formula number "{formula}". '
                                f'Valid formulae: {", ".join(map(str, AvailablePCFormulae))}.')
    ...
    def report_problems(self, r: ProblemReport, array: Array, check_mode: AlgoCheckMode) -> ProblemReport:
        ...
        if not is_valid_collector(array.collector, check_mode):
            r.add_missing_collector(array, 'collector')
        else:
            for attrib in ['a1', 'a2', 'a5', 'eta0b', 'kd']:
                if getattr(array.collector, attrib) is None:
                    r.add_missing_attrib(array.collector, attrib,
    ...
    def get_nan_mask(self, plant: Plant):
        """This method checks whether all sensors required to apply a specific formula are available.
    ...
    def _get_nan_mask_common(self, plant):
        """This method checks sensors common to both formulae 1 and 2.
        """
    ...
    def _calc_pc_restrictions_common(self, plant, resampler) -> pd.Series:
        """Checks the operating condition restrictions that are common to Formula 1 and Formula 2.
    ...
    def calc_estimated_power(self, array: Array, aggregator: Callable) -> pd.DataFrame:
        """Calculates the estimated specific power output of the collector based on the specific formula.
...
class Formula1(Formula):
    """ Implements formula 1 of the ISO 24194. See :class:`Formula` base class for more info.
    """
    id = 1
    ...
    def report_problems(self, r: ProblemReport, array: Array, check_mode: AlgoCheckMode) -> ProblemReport:
        ...
        for attrib in ['eta0hem']:
            if getattr(array.collector, attrib) is None:
                r.add_missing_attrib(array.collector, attrib,
                                     f'Collector coefficient "{attrib}" is required for formula 1 but '
    ...
    def calc_estimated_power(self, array, aggregator) -> pd.DataFrame:
        """Calculates the estimated power output of a collector array based on formula 1 in ISO 24194.
        """
...
class Formula2(Formula):
    """ Implements formula 2 of the ISO 24194. See :class:`Formula` base class for more info.
    """
    id = 2
    ...
    def calc_estimated_power(self, array, aggregator) -> pd.DataFrame:
        """Calculates the estimated specific power output of a collector array based on formula 2 in ISO 24194.
        """
        # Collector coefficients
        a1 = array.collector.a1
        a2 = array.collector.a2
