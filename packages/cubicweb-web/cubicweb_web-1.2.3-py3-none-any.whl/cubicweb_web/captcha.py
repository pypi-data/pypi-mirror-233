...
from cubicweb_web import ProcessFormError, formwidgets as fw
...
def pil_captcha(text, fontfile, fontsize):
    ...
    for num in range(100):
        ...
        draw.rectangle(
            (randint(0, x), randint(0, y), randint(0, x), randint(0, y)), fill=fill
        )
...
def captcha(fontfile, fontsize, size=5, format="JPEG"):
    ...
    """
    text = "".join(choice("QWERTYUOPASDFGHJKLZXCVBNM") for i in range(size))
...
class CaptchaWidget(fw.TextInput):
    def render(self, form, field, renderer=None):
        ...
        src = form._cw.build_url(
            "view",
            vid="captcha",
            t=int(time() * 100),
            captchakey=field.input_name(form),
        )
        img = tags.img(src=src, alt="captcha")
        img = '<div class="captcha">%s</div>' % img
        return img + super().render(form, field, renderer)

    def process_field_data(self, form, field):
        captcha = form._cw.session.data.pop(field.input_name(form), None)
        val = super().process_field_data(form, field)
        if val is None:
