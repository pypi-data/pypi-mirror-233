...
def dn_lookup(gather_args, model, dn_parent, storage):
    ...
    for item in gather_args["dn"].split("."):
        try:
            ...
            # TODO log for debug
            return []
    if current_pointer is None and not gather_args.get("allow_none"):
        return []
...
def key_lookup(gather_args, model, dn_parent, storage):
    if KEY_GATHER not in gather_args:
        try:
            return storage["context_data"].get(gather_args.get("key", gather_args.get("dn")))  # TODO set better debugging
        except Exception as e:
            raise InvalidModel(f"Error while doing key lookup dn {dn}, source: {source}, reason: {e}")
    else:
...
def gather_items(model, dn, storage):
    ...
    try:
        ...
        raise InvalidModel(f"Error while gathering data for on dn {dn}, source: {source}, reason: {e}")
...
def is_node(model):
    return model["type"] in ["dict", "list"] and all("nested" not in sub_model for sub_model in model.get("nested", {}).values())
