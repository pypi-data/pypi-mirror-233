...
class EauGrid:
  ...
  def __call__(self, x: int, y: int, *, offset: int | tuple[int, int] = 0) -> EauPosition:
    return self.cell(x, y, offset=offset)

  def x(self, num: int, *, offset: int = 0) -> EauLayout:
    if not self.cols:
    ...
    t = num * 2 - 1
    return EauLayout.Position(self.margin[0] + w * t, offset=offset)

  def y(self, num: int, *, offset: int = 0) -> EauLayout:
    if not self.rows:
    ...
    t = num * 2 - 1
    return EauLayout.Position(self.margin[2] + w * t, offset=offset)

  def cell(self, x: int, y: int, *, offset: int | tuple[int, int] = 0) -> EauPosition:
    ...
    return EauPosition(
      self.x(x, offset=offset[0] if isinstance(offset, tuple) else offset),
      self.y(y, offset=offset[1] if isinstance(offset, tuple) else offset)
    )
