...
from scpup.services import EauService
...
def eaucollidegroup(sprite: EauSprite, group: scpup.EauGroup) -> EauSprite | None:
  ...
  """
  for spr in group:
    collided = eaucollidesprite(sprite, spr)
    if collided:
      return spr
  return None
...
class EauSprite:
  ...
  def __init__(self,
    ...
    if len(paths) > 0:
      self.image, self.rect = scpup.load_image(
...
class EauSecuence:
  ...
  def __init__(
    ...
    *steps: EauSecuenceStep | tuple[int, int],
    ...
    if sound_path:
      loaded = audioService.load(id(self), sound_path)
    else:
      loaded = False
    self.has_sound = loaded
  ...
  def trigger(self, name: str) -> None:
    ...
    for s in self.secuences:
      if s.name == name:
        ...
        if s.has_sound:
          EauService.get('EauAudioService').play(id(s))
        break
