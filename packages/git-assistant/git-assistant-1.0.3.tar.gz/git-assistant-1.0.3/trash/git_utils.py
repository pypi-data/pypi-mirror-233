...
class git_assistant:
    ...
    def __init__(self, writer, repo_url=None, folder=None):
    ...
    def try_completion(self, context, message, temperature=0.7, max_tokens=512, request_timeout=100):
        try:
            self.writer.define_context(context=context)
            response = self.writer.ask(
                message=message, temperature=temperature, max_tokens=max_tokens, request_timeout=request_timeout
                )
        except:
            time.sleep(10)
            self.writer.define_context(context=context)
    ...
    def get_summary_concat(self,
        ...
        self.files['summary'] = {}
        self.summary_concat = ""
        for path, content in self.files['content'].items():
            ...
            response = self.try_completion(context, message=prompt, temperature=0, request_timeout=30)
    ...
    def get_global_summary(self,
        ...
        response = self.try_completion(context, message=prompt, temperature=0, max_tokens=3000, request_timeout=60)
    ...
    def generate_readme_structure(self,
        ...
        response = self.try_completion(context, message=prompt, temperature=0.7, request_timeout=60)
    ...
    def generate_readme(self,
        ...
        response = self.try_completion(context, message=prompt, temperature=0.7, max_tokens=3000, request_timeout=60)
