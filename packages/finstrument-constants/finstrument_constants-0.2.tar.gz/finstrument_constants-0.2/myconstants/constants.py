...
class FredSeriesBase:
    def __init__(self, _id, id_type, fred_dir: Path, start_years: list):
        self.fred_dir = fred_dir
        self.start_years = start_years
        self.symbol = _id
        fred_metadata = self.get_metadata(self.fred_dir)
        try:
        ...
        self.positive_correlations, self.negative_correlations, self.all_correlations = self.init_corrs(start_years)

    def get_metadata(self, fred_dir: Path):
        return pd.read_csv(fred_dir / 'fred_md_metadata.csv')

    def init_corrs(self, start_years):
        return (
...
class FredapiSeries(FredSeriesBase):
    def __init__(self, api_id: str, revised: bool, fred_dir: Path, start_years: list, fred_key):
        super().__init__(api_id, 'api_id', fred_dir, start_years)
        if not revised:
            self.name = self.name + ' (UNREVISED)'
        self.set_fred_series(revised, fred_key)

    def set_fred_series(self, revised, fred_key):
...
class FredmdSeries(FredSeriesBase):
    def __init__(self, fred_md_id: str, fred_dir: Path, start_years: list):
        super().__init__(fred_md_id, 'fred_md_id', fred_dir, start_years)
        self.set_fred_series()

    def set_fred_series(self):
        """For getting a series from the FRED-MD dataset"""
        md_data = pd.read_csv(self.fred_dir / 'FRED_MD/MD_2023-08-02.csv')
...
class Security:
    def __init__(self, symbol: str, stocks_dir: Path, start_years: list, securities_metadata: tuple):
        self.stocks_dir = stocks_dir
        self.start_years = start_years
        self.securities_metadata = securities_metadata
    ...
    def set_series_data(self):
        ...
        file_path = self.stocks_dir / f'yahoo_daily/parquets/{self.symbol}.parquet'
        try:
    ...
    def get_symbol_name_and_type(self) -> None:
        etf_metadata, stock_metadata, index_metadata = self.securities_metadata
