...
class AuthenticationManager:
    ...
    def delete_expired_token():
        ...
        token_filepath = os.path.join(auth_dir, AuthenticationManager.TOKEN_FILENAME)
        if os.path.exists(token_filepath):
            os.remove(token_filepath)

    def get_access_token_or_authenticate(self) -> Optional[str]:
            ...
            if not self._access_token:
                if not self._services:
                    ...
                    if self._auth_config is not None:
                        ...
                        self._services = [PassThroughService()]
    ...
    def _load_auth_config(self) -> None:
        config = ff.Client(insecure=self._insecure).get_auth_config()

        if config.WhichOneof("config") == "okta":
            okta_config = config.okta
            self._auth_config = OktaAuthConfig(
