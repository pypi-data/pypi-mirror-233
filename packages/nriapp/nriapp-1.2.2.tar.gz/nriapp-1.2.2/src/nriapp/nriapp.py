...
#       doc.add_run("Severity: ")
        list_country = ""
        length = len(data["companyName /Country"])
        for idx, i in enumerate(data["companyName /Country"]):
            list_country = list_country + i
            if length > idx + 1:
                list_country = list_country + ", "

        doc.add_run("Subsidiary - Country: " + list_country)
#        doc.add_run("Detection Sources: ")
        doc.add_run("Timeframe: " + data["firstActivity"] + " - " + data["lastActivity"])
        doc.add_run("CSIRT Severity: ")
        doc.add_run("Verdict: ")
        doc.add_run("Alert logs: ")
        doc.add_run("IOCs:")
        doc.add_run("")
        pass
    ...
    def timely_update_mdr(self, lookBackInDays=30):
        ...
        todate_list = usp.list_incidents(unit='hours', range=24, max=50)

        for i in todate_list:
