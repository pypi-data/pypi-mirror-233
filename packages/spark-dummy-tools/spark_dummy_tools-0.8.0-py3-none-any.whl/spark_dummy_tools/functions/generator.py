...
def generated_dummy_table_artifactory(spark=None,
    ...
    if path is not None:
        ...
        if str(output_type).lower() == "parquet":
            df3.coalesce(1).write.partitionBy(_partition_colum).mode("overwrite").parquet(path_directory)
        else:
            df3.coalesce(1).write.format('avro').partitionBy(_partition_colum).mode("overwrite").parquet(path_directory)
