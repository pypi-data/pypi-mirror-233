...
def Raster_Correction(input_path, output_path, limit=None, lowpass_kernel_size=5,
                      bilateral_win_size=3, bilateral_sigma_color=0.05, bilateral_sigma_spatial=3,
                      clip_percentiles=[2, 98], optical=False, scale='power', Vegetation_mask=None):
    """
    ...
    Args:
        ...
        bilateral_sigma_color (float): Standard deviation for color space in bilateral filter.
    ...
    for image_file in image_files:
        try:
            ...
            if optical==False:
                ...
                denoise_bilateral = restoration.denoise_bilateral(data, win_size=bilateral_win_size,
            ...
            # Apply Median Filter
            #median_filter = cv2.medianBlur(denoise_bilateral, median_kernel_size)
              # Apply low-pass filter
            if lowpass_kernel_size is not None:
                filtered = gaussian(denoise_bilateral, sigma=lowpass_kernel_size, preserve_range =True )  # Adjust sigma according to your desired smoothing strength
                
                # Clip the data using percentiles
                clipped_data = pct_clip(filtered)
                filtered_label='Low Pass Filter'
                # Normalize the clipped data to the range of 0 to 255
                normalized_data = (clipped_data - clipped_data.min()) / (clipped_data.max() - clipped_data.min()) * int(
                    clipped_data.max())
                corrected_data = exposure.rescale_intensity(normalized_data, out_range=(0, 255)).astype(np.uint8)
            elif lowpass_kernel_size is None and optical==False:
                filtered=cv2.medianBlur(denoise_bilateral, 5)
                filtered_label='MedianBlur Filter'
                ...
                # Clip the data using percentiles
                clipped_data = pct_clip(denoise_bilateral)
            ...
            if optical==True:
                corrected_data = exposure.equalize_adapthist(corrected_data, kernel_size=32, clip_limit=0.01, nbins=256)
                corrected_data = exposure.rescale_intensity(corrected_data, out_range=(0, 255)).astype(np.uint8)
                filtered=corrected_data
