...
def Auto_Variogram(data="", column_attribute="", latlon=False, aoi_shapefile="", pixel_size=20,num_chunks=10,overlap_percentage=0, out_fileName='interpolated_kriging', 
                   plot_folder='kriging_plots', geo_folder='geo_rasterFolder', smoothing_kernel=2, mask=None):
    """
    ...
    if latlon:
        x, y = utm_to_latlon(easting=geodata.x, northing=geodata.y, zone_number=10, zone_letter="N")
    else:
        x, y = geodata.x, geodata.y
    ...
    if aoi_shapefile:
        transform=get_transform_from_gdf(aoi, pixel_size)
    ...
    plt.colorbar(ax2.imshow(z_grid, origin="upper", cmap='Spectral'), ax=ax2)
...
def measure_displacement_from_camera(hls_url, alpha=0.1, save_output=False, output_filename=None, ssim_threshold=0.4, 
    ...
    stream_params1 = {"-input_framerate": stream1.framerate, "-livestream": True}
    stream_params2 = {"-input_framerate": stream2.framerate, "-livestream": True}
