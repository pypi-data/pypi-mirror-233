...
#from core_via2 import *
# from core_working_ import *
# from core_working_rw2 import *
...
def main_Human(ncomps=80, knn=30, v0_random_seed=10, run_palantir_func=False):
    ...
    plt.show()
    print(time.ctime())
    v0 = VIA(Xin, true_label, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=30,
             too_big_factor=0.3,
             ...
             neighboring_terminal_states_threshold=3, piegraph_arrow_head_width=0.1,
             edgebundle_pruning_twice=True)  # , embedding=tsnem)#, user_defined_terminal_group=['pDC','ERY1', 'ERY3', 'MONO1','mDC (cDC)','PRE_B2'] )# do_compute_embedding=True, embedding_type='via-mds')#,#embedding=adata_counts.obsm['X_umap'])  # *.4 root=1,
    # user_defined_terminal_group=['pDC','MONO1','MEGA1']
    v0.run_VIA()

    plot_sc_lineage_probability(v0, embedding=tsnem)
    plt.show()
    if q == 0:
        title_ = 'Via 1.0 Lineages '
    else:
        title_ = 'RW2 Q_mem = ' + str(q)
    f.suptitle(title_, fontsize=12)
    f.savefig('/home/shobi/Trajectory/Datasets/Toy3/RW2/' + title_ + '.png', facecolor='white', transparent=False)

    # plot_sc_lineage_probability(v0, embedding=tsnem, marker_lineages=[2])
    # plt.show()
    ...
    # f, ax = plot_scatter(embedding=via_mds1, labels=v0.true_label, title='dens sampling')
    plt.show()
    plot_atlas_view(via_object=v0, lineage_pathway=[2, 3, 5, 6, 9, 12, 16, 17], linewidth_bundle=0.5,
                     headwidth_bundle=1)
    ...
    f, ax, ax1 = plot_piechart_viagraph(via0=v0, linewidth_edge=3)
    ...
    plt.show()
    plot_atlas_view(via_object=v0, sc_labels_expression=df_magic['MPO'].tolist(), n_milestones=100,
                     extra_title_text='MPO')
    plt.show()

    via_streamplot(v0, embedding=tsnem, scatter_size=50, title='original tsne')
    plt.show()

    plot_trajectory_curves(v0, embedding=tsnem, draw_all_curves=False)
    plt.show()

    animate_streamplot(v0, embedding=v0.embedding, scatter_size=800, scatter_alpha=0.15, density_grid=1,
    ...
    animate_atlas(via_object=v0, hammerbundle_dict=v0.hammerbundle_milestone_dict,
    ...
    plot_viagraph(via_object=v0, plot_clusters=True, title='solo viagraph with bundling')
    plt.show()
    ...
    hammerbundle_milestone_dict = make_edgebundle_milestone(via_object=v0, global_visual_pruning=0.5, decay=0.7,
                                                            initial_bandwidth=0.02, milestone_labels=v0.labels)

    plot_atlas_view(hammerbundle_dict=hammerbundle_milestone_dict,
    ...
    hammerbundle_milestone_dict = make_edgebundle_milestone(via_object=v0, global_visual_pruning=0.5, decay=0.7,
                                                            initial_bandwidth=0.05, n_milestones=300)
    ...
    plt.show()
    plot_piechart_viagraph(via0=v0, type_data='gene', gene_exp=df_magic_cluster['GATA1'].values, title='GATA1',
                        cmap='coolwarm')
    plt.show()
    ...
    marker_genes_matrix_plot = {"ERY": ['GATA1', 'GATA2', 'ITGA2B'], "BCell": ['IGHD', 'CD22'],
                                ...
                                "MONO": ['CD14', 'SPI1', 'MPO', 'IL12RB1', 'IL13RA1', 'C3AR1', 'FCGR3A'],
                                'HSC': ['CD34']}
    ...
    '''
    v1 = VIA(Xin, true_label, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
    ...
    plot_trajectory_curves(v0, v1, embedding=tsnem)
    plt.show()
    # DRAW Lineage pathways
    plot_sc_lineage_probability(v0, v1, embedding=tsnem)
    plt.show()
...
def main_Toy(ncomps=10, knn=30, random_seed=41, dataset='Toy3', root_user=['M1'],
             cluster_graph_pruning=1, foldername="/home/user/Trajectory/Datasets/"):
    ...
    if dataset == 'Toy4':
        edgepruning_clustering_resolution = 0.15  # 0.15  # 1
    else:
        edgepruning_clustering_resolution = 0.05  # 0.15#0.15
    import umap
    ...
    for m in [1, 5, 10, 50, 100]:
        foldername_ = '/home/user/Trajectory/Datasets/Toy3/corr_tests/lineage_single_cell_bp_memory'
        list_filenames.append(foldername_+str(m))
    lineage_corr(marker_lineages=['5','7','9','10'], list_filenames=list_filenames)#['5','9','7','10']
    '''
    # self.labels = kmeans.labels_.flatten()
    for shell in [1]:  # [0.001,0.002,0.005,0.01,0.05,0.1,0,1,10,100,200,500,1000,2000]:
        for q in [100]:  # , 50, 100]:
            random_seed = 52
            print(f'rs {random_seed}')
            v0 = VIA(X_pca, true_label, edgepruning_clustering_resolution=edgepruning_clustering_resolution,
                     edgepruning_clustering_resolution_local=1,
                     knn=knn, user_defined_terminal_group=['M6', 'M4', 'M2', 'M8'],
                     ...
                     piegraph_edgeweight_scalingfactor=1.0, resolution_parameter=1, time_series=False,
                     time_series_labels=None, do_compute_embedding=False, embedding_type='via-atlas',
                     memory=q)  # , embedding=embedding)#, do_compute_embedding=True, embedding_type='via-umap') #, embedding=embedding)  # *.4 root=2,embedding=embedding user_defined_terminal_group=['M8','M6'] #embedding_type='via-mds', do_compute_embedding=True,, user_defined_terminal_group=['M6','M8','M2','M7']
            # time_series_labels = adata_counts.obs['true_time'].tolist()
            v0.run_VIA()
            print('v0.terminal clusters are:', v0.terminal_clusters)

            # df_lineages = pd.DataFrame(v0.single_cell_bp, columns=v0.terminal_clusters)
            # df_lineages.to_csv("/home/user/Trajectory/Datasets/Toy3/corr_tests/lineage_single_cell_bp_memory"+str(q)+'.csv')
            # df_lineages = pd.DataFrame(v0.single_cell_bp_rownormed, columns=v0.terminal_clusters)
            # df_lineages.to_csv("/home/user/Trajectory/Datasets/Toy3/corr_tests/lineage_single_cell_bp_rownormed_memory" + str(q) + '.csv')
            # print("saved df sc-bp")

        # feature_rep_fpath, rw_features_input = rw2_feature_representation(via_object=v0,   working_directory='/home/user/Trajectory/Datasets/Toy3/',                   n_dim=64)
        plot_differentiation_flow(via_object=v0,
                                      marker_lineages=v0.terminal_clusters,
                                      label_node=true_label, do_log_flow=True)
        rw_features_input = pd.read_csv(
            '/home/user/Trajectory/Datasets/Toy3/pc20_knn30rw2_195.csv')  # pc30_knn30kseq15krev15RW2_emd_122.csv')
        rw_features_input = rw_features_input.drop(['Unnamed: 0'], axis=1).values

        atlas_embedding = via_atlas_emb(via_object=v0, X_input=rw_features_input[:, 0:30], init_pos='via',
                                        min_dist=0.2)

        f1, ax = plot_scatter(embedding=atlas_embedding, labels=v0.single_cell_pt_markov, cmap='plasma', s=5,
                              alpha=0.5, edgecolors='None',
                              title='atlas', text_labels=False)
        f1.set_size_inches(10, 10)

        f2, ax = plot_scatter(embedding=atlas_embedding, labels=true_label, title='atlas celltype',
                              alpha=0.5, s=5)
        f2.set_size_inches(10, 10)
        plt.show()

        embedding_mds = via_mds(via_object=v0, diffusion_op=5, n_milestones=1000, k_project_milestones=2, k=10,
                            random_seed=random_seed)

        f1, ax = plot_scatter(embedding=atlas_embedding, labels=v0.single_cell_pt_markov, cmap='plasma', s=5,
                              alpha=0.5, edgecolors='None',
                              title='via-mds pseudotime', text_labels=False)
        f1.set_size_inches(10, 10)

        f2, ax = plot_scatter(embedding=atlas_embedding, labels=true_label, title='via-mds celltype',
                              alpha=0.5, s=5)
        f2.set_size_inches(10, 10)
        plt.show()

        ax = print(f'testing population area plot')
        plot_population_composition(via_object=v0,
                                    color_dict={'M1': 'blue', 'M2': 'red', 'M3': 'purple', 'M4': 'orange',
                                                'M5': 'yellow', 'M6': 'green', 'M7': 'pink', 'M8': 'gray'},
                                    n_intervals=10, fraction=True)
        plt.show()
        v0.embedding = atlas_embedding
        plot_atlas_view(via_object=v0, linewidth_bundle=0.5, sc_labels_expression=v0.single_cell_pt_markov,
                         add_sc_embedding=True, sc_scatter_size=2,
                         headwidth_bundle=0.5, n_milestones=30, size_scatter=2, extra_title_text='qmemory = ' + str(q))
        plot_atlas_view(via_object=v0, linewidth_bundle=0.5, lineage_pathway=v0.terminal_clusters,

                         headwidth_bundle=0.5, n_milestones=30, size_scatter=2, extra_title_text='qmemory = ' + str(q))

        df_genes = pd.DataFrame(adata_counts.obsm['X_pca'][:, 0:5], columns=['Gene0', 'Gene1', 'Gene2', 'Gene3',
                                                                             'Gene4'])  # dummy gene values for illustration
        get_gene_expression(via_object=v0, gene_exp=df_genes, marker_genes=['Gene0', 'Gene1', 'Gene2'], n_splines=4,
                            spline_order=3, optional_title_text='qmemory = ' + str(q))
        plt.show()
        #df_genes['parc'] = v0.labels
        #df_genes_cluster = df_genes.groupby('parc', as_index=False).mean()
        #print(df_genes_cluster)
        print('plot via graph gene expression')
        plot_viagraph(via_object=v0, df_genes=df_genes, gene_list=['Gene0', 'Gene1', 'Gene2', 'Gene3',
                                                                             'Gene4'])
        plt.show()
        plot_piechart_viagraph(via_object=v0, cmap_piechart='plasma', reference_labels=true_label,
                            size_node_notpiechart=0.3, highlight_terminal_clusters=True, pie_size_scale=0.3)
        plot_sc_lineage_probability(v0, embedding=v0.embedding)  # , marker_lineages=[5, 10])

        plt.show()


        df_genes = pd.DataFrame(adata_counts.obsm['X_pca'][:, 0:5],
                                columns=['Gene0', 'Gene1', 'Gene2', 'Gene3', 'Gene4'])

        if dataset_name == 'Toy3':
            print('gene trend heatmap')
            f, axlist = plot_gene_trend_heatmaps(via_object=v0, df_gene_exp=df_genes,
                                                 marker_lineages=v0.terminal_clusters)
        ...
        if dataset_name == 'Toy4':
            ...
            axlist[-1].set_xlabel("pseudotime", fontsize=20)
            plt.show()
        get_gene_expression(v0, gene_exp=df_genes,
                            marker_genes=['Gene0', 'Gene1', 'Gene2'])  # , marker_lineages=[8,9,12])
        plt.show()
        via_streamplot(via_object=v0, scatter_size=20)  # embedding
        via_streamplot(via_object=v0, scatter_size=20, labels=v0.single_cell_pt_markov,
                       title='labels of pt')  # embedding
        ...
        via_streamplot(via_object=v0, scatter_size=20, color_scheme='time', title='auto pt', cmap='viridis')
        color_dict = {'M1': 'blue', 'M2': 'red', 'M3': 'purple', 'M4': 'orange', 'M5': 'yellow', 'M6': 'green',
                      'M7': 'pink', 'M8': 'gray'}
        via_streamplot(via_object=v0, scatter_size=20, color_dict=color_dict, labels=v0.true_label)  # embedding
        plt.show()
        # v0.embedding = via_mds(via_object=v0)
        plot_atlas_view(via_object=v0, linewidth_bundle=0.5,
                         headwidth_bundle=0.5, n_milestones=30, size_scatter=2)
        plot_atlas_view(via_object=v0, lineage_pathway=v0.terminal_clusters, linewidth_bundle=0.5,
                         headwidth_bundle=0.5, n_milestones=30, size_scatter=2)
        plot_atlas_view(via_object=v0, lineage_pathway=v0.terminal_clusters[0:3], linewidth_bundle=0.5,
                         headwidth_bundle=1, n_milestones=30)
        plot_atlas_view(via_object=v0, lineage_pathway=[v0.terminal_clusters[0]], linewidth_bundle=0.5,
                         headwidth_bundle=1, n_milestones=30)
        plt.show()

        embedding = via_mds(via_object=v0, diffusion_op=5, n_milestones=1000, k_project_milestones=2, k=10,
                            random_seed=random_seed)
        df_genes = pd.DataFrame(adata_counts.obsm['X_pca'][:, 0:5], columns=['Gene0', 'Gene1', 'Gene2', 'Gene3',
                                                                             'Gene4'])  # dummy gene values for illustration
        get_gene_expression(via_object=v0, gene_exp=df_genes, marker_genes=['Gene0', 'Gene1', 'Gene2'], n_splines=4,
                            spline_order=3)
        plt.show()
        plot_scatter(embedding=embedding, labels=true_label)
        plt.show()
        # plot_piechart_viagraph(via_object=v0, cmap_piechart='plasma', reference_labels=[int(i[-1]) for i in true_label], size_node_notpiechart=3, highlight_terminal_clusters=False, pie_size_scale=3)
        plot_piechart_viagraph(via_object=v0, cmap_piechart='plasma', reference_labels=[int(i[-1]) for i in true_label],
                            size_node_notpiechart=0.3, highlight_terminal_clusters=True, pie_size_scale=0.3)
        plt.show()

        # plot_piechart_viagraph(v0)
        df_ids = pd.DataFrame()
        ...
        fig, ax = plot_atlas_view(via_object=v0, linewidth_bundle=0.5,
                                   headwidth_bundle=1, lineage_pathway=v0.terminal_clusters[0:4], dpi=300,
                                   n_milestones=20)

        fig.suptitle(title_, fontsize=14)
        fig.set_size_inches(15, 10)
        plt.show()
        # fig.savefig('/home/shobi/Trajectory/Datasets/Toy3/RW2/randomstate'+str(random_seed)+'/Edgebundles_notRowNormed_run2/'+'Edgebundle_'+ title_ + '.png', facecolor='white', transparent=False)
        fig, ax = plot_sc_lineage_probability(v0, embedding=embedding)
        fig.suptitle(title_, fontsize=14)
        fig.set_size_inches(15, 10)
        # fig.savefig('/home/shobi/Trajectory/Datasets/Toy3/RW2/randomstate'+str(random_seed)+'/Edgebundles_notRowNormed_run2/'+  title_ + '.png', facecolor='white',                    transparent=False)

        plt.show()
    e1 = via_atlas_emb(via_object=v0, init_pos='via',
                  random_state=v0.random_seed)  # , n_epochs=100, spread=1,                                                      distance_metric='euclidean', min_dist=0.1, saveto='',                                                      random_state=v0.random_seed)
    plot_scatter(e1, labels=v0.labels, title='via init')
    e2 = via_atlas_emb(
        via_object=v0)  # , n_epochs=100, spread=1,                                                      distance_metric='euclidean', min_dist=0.1, saveto='',                                                      random_state=v0.random_seed)
    plot_scatter(e1, labels=v0.labels, title='spectral init')
    plt.show()
    plot_trajectory_curves(via_object=v0, embedding=embedding)
    plt.show()
    ...
    v0.hammerbundle_milestone_dict = make_edgebundle_milestone(via_object=v0,
                                                               n_milestones=40)  # optional, but just showing how to recompute with different n_milestones
    plot_atlas_view(via_object=v0, lineage_pathway=v0.terminal_clusters, linewidth_bundle=0.5, headwidth_bundle=1,
                     cmap='plasma', text_labels=True, show_milestones=True, scale_scatter_size_pop=False)
    plot_atlas_view(via_object=v0, lineage_pathway=v0.terminal_clusters[0:2], linewidth_bundle=0.5, headwidth_bundle=1,
                     cmap='plasma', text_labels=True, show_milestones=False, scale_scatter_size_pop=True,
                     fontsize_labels=4)
    plot_atlas_view(via_object=v0, linewidth_bundle=0.5, headwidth_bundle=2)
    plot_atlas_view(via_object=v0, lineage_pathway=[v0.terminal_clusters[0]], linewidth_bundle=0.5, headwidth_bundle=1)
    plt.show()
    plot_sc_lineage_probability(v0, embedding=embedding,
                                marker_lineages=v0.terminal_clusters[0:2])  # [10,5,1,0,2]) #Toy3)
    plt.show()
    hammerbundle_milestone_dict = make_edgebundle_milestone(via_object=v0, global_visual_pruning=1,
                                                            initial_bandwidth=0.02, decay=0.7,
                                                            milestone_labels=v0.labels)
    print(f"{datetime.now()}\tPlot milestone hammer external")
    plot_atlas_view(via_object=v0, hammerbundle_dict=hammerbundle_milestone_dict,
    ...
    plot_atlas_view(via_object=v0, hammerbundle_dict=hammerbundle_milestone_dict,
                     ...
                     extra_title_text='Using VIA clusters for lineage pathways', headwidth_bundle=2.5,
    ...
    plt.show()
    '''
    print(f'{datetime.now()}\tFor random seed: {random_seed}, the first sc markov pt are',
          v0.single_cell_pt_markov[0:10])

    # plot_sc_lineage_probability(v0, embedding=embedding,marker_lineages=[5,10])
    # plt.show()
    ...
    plot_atlas_view(hammerbundle_dict=hammerbundle_milestone_dict,
                     ...
                     extra_title_text='Gene0 Expression', headwidth_bundle=0.15,
                     sc_labels_expression=adata_counts.obsm['X_pca'][:, 0].tolist(), text_labels=True,
    ...
    plot_piechart_viagraph(via_object=v0)
    plt.show()

    plot_viagraph(via_object=v0, plot_clusters=True, title='viagraph with bundling', fontsize=10)
    plt.show()
    ...
    animate_streamplot(v0, embedding, scatter_size=800, scatter_alpha=0.15, density_grid=1,
                        saveto='/home/user/Trajectory/Datasets/Toy3/test_framerates_may2023.gif', facecolor_='white',
                        cmap_stream='Blues')
    plt.show()
    ...
    animate_atlas(via_object=v0, hammerbundle_dict=v0.hammerbundle_milestone_dict,
                        ...
                        linewidth_bundle=2, cmap='rainbow', facecolor='white',
                        extra_title_text='test animation', alpha_scatter=0.1, size_scatter=10,
                        saveto='/home/user/Trajectory/Datasets/Toy3/animate_eb_may2023.gif')
    make_edgebundle_viagraph(via_object=v0, edgebundle_pruning=1)
    plot_viagraph(via_object=v0, plot_clusters=True, title='viagraph with bundling', fontsize=10)
    plt.show()
    ...
    plot_piechart_viagraph(v0, edge_color='gray')
    plt.show()
    ...
    # plot_sc_lineage_probability(via_coarse=v0, via_fine=v0, embedding=embedding)

    plt.show()

    v1 = VIA(adata_counts.obsm['X_pca'][:, 0:ncomps], true_label,
             edgepruning_clustering_resolution=edgepruning_clustering_resolution, edgepruning_clustering_resolution_local=1,
             knn=knn, too_big_factor=0.1, cluster_graph_pruning=cluster_graph_pruning,
    ...
    plot_trajectory_curves(v0, v1, embedding)
    plt.show()
    ...
    plt.show()
    plot_sc_lineage_probability(via_object=v0, via_fine=v1, embedding=embedding)
    plt.show()


def main_Bcell(ncomps=50, knn=20, random_seed=0, cluster_graph_pruning=.15,
               path='/home/shobi/Trajectory/Datasets/Bcell/'):
    ...
    root_user = [42]
    v0 = VIA(input_via, true_label, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
    ...
    v1 = VIA(adata_counts.obsm['X_pca'][:, 0:ncomps], true_label, edgepruning_clustering_resolution=0.15,
    ...
    plt.show()
    plot_trajectory_curves(via_coarse=v0, via_fine=v1, embedding=embedding)
    plt.show()

    plot_sc_lineage_probability(via_coarse=v0, via_fine=v1, embedding=embedding)
...
def main_EB_clean(ncomps=30, knn=20, v0_random_seed=24, cluster_graph_pruning=.15,
    ...
    for knn in [10, 20, 50]:
        for ncomps in [100]:
            ...
            if do_phate:
                ...
                f1.savefig('/home/shobi/Trajectory/Datasets/EB_Phate/phate_npc' + save_str + '.png', facecolor='white',
                           transparent=False, )
                plt.show()

            print('do v0')
            root_user = [1]
            v0 = VIA(input_data, time_labels, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
                     cluster_graph_pruning=cluster_graph_pruning, resolution_parameter=2,
                     too_big_factor=v0_too_big, root_user=root_user, dataset='EB', random_seed=v0_random_seed,
                     is_coarse=True, preserve_disconnected=True, do_compute_embedding=False, embedding_type='via-mds',
                     time_series=True, time_series_labels=time_labels, do_gaussian_kernel_edgeweights=False,
                     t_diff_step=1)  # *.4 root=1,
            v0.run_VIA()

            plot_piechart_viagraph(via_object=v0)
            df_ids = pd.DataFrame()
            df_ids['true_time'] = time_labels
            df_ids['pt'] = v0.single_cell_pt_markov
            correlation = df_ids['pt'].corr(df_ids['true_time'])
            print('corr via knn', knn, correlation)
            plt.show()
    ...
    v1 = VIA(input_data, time_labels, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
             cluster_graph_pruning=cluster_graph_pruning,
             too_big_factor=v1_too_big, root_user=root_user, is_coarse=False, x_lazy=0.95, alpha_teleport=0.99,
             preserve_disconnected=True, dataset='EB',
    ...
    plot_trajectory_curves(via_coarse=v0, via_fine=v1, embedding=Y_phate)
    plt.show()

    plot_sc_lineage_probability(via_coarse=v0, via_fine=v1, embedding=Y_phate)
    plt.show()
...
def main_EB(ncomps=30, knn=20, v0_random_seed=24):
    ...
    print('do v0')
    v0 = VIA(input_data, time_labels, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
             too_big_factor=v0_too_big, root_user=root_user, dataset='EB', random_seed=v0_random_seed, is_coarse=True,
             preserve_disconnected=True)  # *.4 root=1,
    v0.run_VIA()
    super_labels = v0.labels
    ...
    plot_trajectory_curves(via_coarse=v0, via_fine=v1, embedding=U)
    plt.show()

    plot_sc_lineage_probability(via_coarse=v0, via_fine=v1, embedding=U)
...
def main_mESC_timeseries(knn=40, cluster_graph_pruning=0.15, random_seed=0, knn_sequential=15,
                         edgepruning_clustering_resolution=0.5):
    root = [0.0]
    knn_sequential = 15
    knn_sequential_reverse = 15
    t_diff_step = 2
    U = pd.read_csv(
        '/home/shobi/Trajectory/Datasets/mESC/mESC_7000perDay_noscaling_meso_timeseries_viaumap_knn40_knnseq15_locallypruned.csv')
    U = U.values[:, 1:]
    ...
    print(f"jac std global {edgepruning_clustering_resolution}")
    v0 = VIA(adata.X, true_labels_numeric, edgepruning_clustering_resolution=edgepruning_clustering_resolution,
             edgepruning_clustering_resolution_local=1, knn=knn,
             cluster_graph_pruning=cluster_graph_pruning,
             ...
             alpha_teleport=0.99, time_series=True, time_series_labels=true_labels_numeric,
             edgebundle_pruning=cluster_graph_pruning, edgebundle_pruning_twice=True,
             knn_sequential_reverse=knn_sequential_reverse, knn_sequential=knn_sequential, t_diff_step=t_diff_step,
             do_compute_embedding=False, embedding_type='via-umap')
    ...
    e1 = via_mds(via_object=v0)
    plot_scatter(e1, labels=true_labels_numeric, title='via-mds')
    plt.show()

    U = via_umap(via_object=v0, n_epochs=150, min_dist=0.3)
    plot_scatter(U, labels=true_labels_numeric)
    plt.show()
    ...
    df_genes = pd.DataFrame(adata[:, marker_genes].X)
    df_genes.columns = marker_genes
    get_gene_expression(via_object=v0, gene_exp=df_genes)
    plt.show()
    '''
    ...
    plt.show()
    plot_trajectory_curves(via_coarse=v0, via_fine=v0, embedding=U, draw_all_curves=False)
    plt.show()


def main_mESC(knn=30, v0_random_seed=42, cluster_graph_pruning=.0, run_palantir_func=False):
    import random
    ...
    v0 = VIA(adata.X, true_label_str, edgepruning_clustering_resolution=0.3, edgepruning_clustering_resolution_local=1, knn=knn,
             cluster_graph_pruning=cluster_graph_pruning,
             too_big_factor=v0_too_big, resolution_parameter=2,
             root_user=root, dataset='mESC', random_seed=v0_random_seed,
             is_coarse=True, preserve_disconnected=False, pseudotime_threshold_TS=40, x_lazy=0.99,
             alpha_teleport=0.99, time_series=True, time_series_labels=true_label_int,
             edgebundle_pruning=cluster_graph_pruning, edgebundle_pruning_twice=False, do_compute_embedding=True,
             embedding_type='via-umap')  # visual_cluster_graph_pruning=1, max_visual_outgoing_edges=3,
    v0.run_VIA()
    plot_piechart_viagraph(via_object=v0)
    plot_trajectory_curves(via_coarse=v0, via_fine=v0, embedding=U)
    plt.show()
    via_streamplot(v0, embedding=U[:, 0:2], scatter_size=50, scatter_alpha=0.2, marker_edgewidth=0.01,
                   density_stream=1, density_grid=0.5, smooth_transition=1, smooth_grid=0.3)
    plt.show()
    ...
    plot_trajectory_curves(via_coarse=v0, via_fine=v0, embedding=U)
    plt.show()
    v1 = VIA(adata.X, true_label_str, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
             cluster_graph_pruning=cluster_graph_pruning,
    ...
    plot_trajectory_curves(via_coarse=v0, via_fine=v1, embedding=U)
    # plot_sc_lineage_probability(v1, U, knn_hnsw, v0.full_graph_shortpath, np.arange(0, n_umap))
    plt.show()


def main_scATAC_zscores(knn=20, ncomps=30, cluster_graph_pruning=.15):
    ...
    v0 = VIA(X_in, true_label, edgepruning_clustering_resolution=0.3, edgepruning_clustering_resolution_local=1, knn=knn,
             cluster_graph_pruning=cluster_graph_pruning,
             too_big_factor=0.3, root_user=root, dataset='scATAC', random_seed=random_seed,
             visual_cluster_graph_pruning=.15, max_visual_outgoing_edges=2, is_coarse=True,
             preserve_disconnected=False)  # *.4 root=1,
    v0.run_VIA()
    ...
    # v0.plot_piechart_viagraph( type_pt='gene', gene_exp=df_mean['AGATAAG'].values, title='GATA1')
    # plt.show()

    v1 = VIA(X_in, true_label, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
             cluster_graph_pruning=cluster_graph_pruning,
    ...
    for key in gene_dict:
        ...
        v1.plot_piechart_viagraph( type_pt='gene', gene_exp=df_mean[key].values, title=gene_dict[key])
        plt.show()
    '''
...
def main_scATAC_Hemato(knn=20):
    ...
    v0 = VIA(X_in, true_label, edgepruning_clustering_resolution=0.5, edgepruning_clustering_resolution_local=1, knn=knn,
             too_big_factor=0.3, root_user=root, dataset='scATAC', random_seed=random_seed,
             visual_cluster_graph_pruning=.15, max_visual_outgoing_edges=2, is_coarse=True,
             preserve_disconnected=False)  # *.4 root=1,
    v0.run_VIA()
    ...
    gene_dict_long = {'ENSG00000092067_LINE336_CEBPE_D_N1': 'CEBPE Eosophil (GMP/Mono)',
                      ...
                      'ENSG00000105610_LINE802_KLF1_D': 'KLF1 (MkP)',
                      'ENSG00000119919_LINE2287_NKX23_I': "NKX32", 'ENSG00000164330_LINE251_EBF1_D_N2': 'EBF1 (Bcell)',
                      'ENSG00000157554_LINE1903_ERG_D_N3': 'ERG', 'ENSG00000185022_LINE531_MAFF_D_N1': 'MAFF',
                      'ENSG00000124092_LINE881_CTCFL_D': 'CTCF', 'ENSG00000119866_LINE848_BCL11A_D': 'BCL11A',
                      'ENSG00000117318_LINE151_ID3_I': 'ID3', 'ENSG00000078399_LINE2184_HOXA9_D_N1': 'HOXA9',
    ...
    v0.get_gene_expression(gene_exp=df_genes)
    plt.show()
    plot_trajectory_curves(v0, v0, embedding)
    plt.show()
    gene_interest = 'ENSG00000123685_LINE392_BATF3_D'
    v0.plot_piechart_viagraph(data_type='gene', gene_exp=df_mean[gene_interest], title=gene_dict[gene_interest])
    plt.show()

    v1 = VIA(X_in, true_label, edgepruning_clustering_resolution=0.15, edgepruning_clustering_resolution_local=1, knn=knn,
             too_big_factor=0.1, root_user=root, is_coarse=False,
             preserve_disconnected=True, dataset='', visual_cluster_graph_pruning=.15, max_visual_outgoing_edges=2,
    ...
    plot_trajectory_curves(v0, v1, embedding)
    # draw trajectory and evolution probability for each lineage
    plot_sc_lineage_probability(v0, v1, embedding)
    plt.show()
    return


def via_wrapper(adata, true_label=None, embedding=None, knn=20, edgepruning_clustering_resolution=0.15, root=0,
                dataset='', random_seed=42,
                v0_toobig=0.3, v1_toobig=0.1, marker_genes=[], ncomps=20, preserve_disconnected=False,
                cluster_graph_pruning=0.15, draw_all_curves=True, piegraph_edgeweight_scalingfactor=1.5,
                piegraph_arrow_head_width=0.2, edgebundle_pruning=None, edgebundle_pruning_twice=False):
    v0 = VIA(adata.obsm['X_pca'][:, 0:ncomps], true_label,
             edgepruning_clustering_resolution=edgepruning_clustering_resolution, edgepruning_clustering_resolution_local=1, knn=knn,
             too_big_factor=v0_toobig, root_user=root, dataset=dataset, random_seed=random_seed, is_coarse=True,
             preserve_disconnected=preserve_disconnected,
             cluster_graph_pruning=cluster_graph_pruning, piegraph_arrow_head_width=piegraph_arrow_head_width,
             piegraph_edgeweight_scalingfactor=piegraph_edgeweight_scalingfactor, visual_cluster_graph_pruning=0.15,
             max_visual_outgoing_edges=2, edgebundle_pruning_twice=edgebundle_pruning_twice,
             edgebundle_pruning=edgebundle_pruning)  # *.4 root=1,
    v0.run_VIA()
    ...
    plot_trajectory_curves(via_coarse=v0, via_fine=v0, embedding=embedding, draw_all_curves=draw_all_curves)
    plt.show()
    ...
    v1 = VIA(adata.obsm['X_pca'][:, 0:ncomps], true_label,
             edgepruning_clustering_resolution=edgepruning_clustering_resolution, edgepruning_clustering_resolution_local=1, knn=knn,
    ...
    plot_trajectory_curves(via_coarse=v0, via_fine=v1, embedding=embedding, draw_all_curves=draw_all_curves)
...
def via_wrapper_disconnected(adata, true_label=None, embedding=None, knn=20, edgepruning_clustering_resolution=0.15,
                             root=[1], dataset='',
    ...
    v0 = VIA(adata.obsm['X_pca'][:, 0:ncomps], true_label,
             ...
             too_big_factor=v0_toobig, root_user=root, dataset=dataset, random_seed=random_seed, is_coarse=True,
             preserve_disconnected=preserve_disconnected,
             cluster_graph_pruning=cluster_graph_pruning)  # *.4 root=1,
    v0.run_VIA()
    ...
    # draw overall pseudotime and main trajectories
    plot_trajectory_curves(embedding, super_clus_ds_PCA_loc, v0.labels, v0.labels, v0.edgelist_maxout,
    ...
    # draw trajectory and evolution probability for each lineage
    plot_sc_lineage_probability(v0, embedding, knn_hnsw, v0.full_graph_shortpath,
                                          np.arange(0, len(true_label)),
                                          adata.X)
    '''
    plot_trajectory_curves(via_coarse=v0, via_fine=v0, embedding=embedding)
    plt.show()
    plot_sc_lineage_probability(via_coarse=v0, via_fine=v0, embedding=embedding)
    plt.show()
    if len(marker_genes) > 0:
...
def main_faced(cell_line='mcf7', cluster_graph_pruning=1.):
    def faced_histogram():
        import seaborn as sns
    ...
    knn = 20  # 20
    random_seed = 1
    edgepruning_clustering_resolution = .5  # normally use 0.5
    ...
    v0 = VIA(X_in, true_label, edgepruning_clustering_resolution=edgepruning_clustering_resolution, edgepruning_clustering_resolution_local=3,
             knn=knn, resolution_parameter=1,
             cluster_graph_pruning=cluster_graph_pruning,
    ...
    v1 = VIA(X_in, true_label, edgepruning_clustering_resolution=edgepruning_clustering_resolution, edgepruning_clustering_resolution_local=1,
             knn=knn,
             cluster_graph_pruning=cluster_graph_pruning,
             too_big_factor=0.05, root_user=root_user, is_coarse=False,
             preserve_disconnected=True, dataset='faced',
             visual_cluster_graph_pruning=1, max_visual_outgoing_edges=2, random_seed=random_seed,
    ...
    plot_sc_lineage_probability(v0, v1, embedding=embedding)

    plot_trajectory_curves(v0, v1, embedding=embedding)
    plt.show()

def load_datasets():
    #adata = datasets.zebrahub('/home/user/Trajectory/Testing/', use_velocity=False)
    adata = datasets.mouse_gastrulation_sala('/home/user/Trajectory/Testing/')
    #adata= datasets.toy_multifurcating()
    print(adata)
...
def main():
    ...
    if dataset == 'Human':
        ...
        main_Toy(ncomps=20, knn=30, random_seed=2, dataset='Toy3', foldername="/home/user/Trajectory/Datasets/Toy3/",
                 cluster_graph_pruning=1)
        ...
        main_Toy(ncomps=10, knn=30, random_seed=0, dataset='Toy4',
                 foldername="/home/shobi/Trajectory/Datasets/Toy4/")  # pc10/knn30/rs2 for Toy4
        ...
        # call VIA
        via_wrapper(adata, true_label, embedding, knn=15, ncomps=20, edgepruning_clustering_resolution=0.15, root=[42],
                    dataset='', random_seed=1, v0_toobig=0.3, v1_toobig=0.1, marker_genes=marker_genes,
                    draw_all_curves=True, piegraph_edgeweight_scalingfactor=1.0, piegraph_arrow_head_width=0.05,
