      ...
      # Make cluster-graph
        ...
        if (self.embedding is None) & (self.do_compute_embedding == True):
            if self.embedding_type == 'via-umap':
                    ...
                    for k_seq_i in [5]:
                        for k_mds_i in [10]:
                            for rs_i in [self.random_seed]:
                                ...
                                if self.time_series_labels is not None:
                                    f2, ax2 = plot_scatter(embedding=self.embedding, labels=self.true_label, title='via-mds',alpha=0.5)
                ...
                plot_scatter(embedding=self.embedding, labels=color_labels, title='via-force')
                plot_scatter(embedding=self.embedding, labels=self.true_label, title='via-force')
                plt.show()
            else:
                print(
        ...
        if use_pt_to_guide_graph==True:
            ...
            self.embedding = via_umap(X_input=self.data, graph=self.adjacency_pt_augmented, n_epochs=100, spread=1,
                                       distance_metric='euclidean', min_dist=0.3)  # usually min_dist default =0.1, for cd34 0.8
