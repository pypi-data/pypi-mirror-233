...
def get_page_url(obj, full_path=False, locale=None):
    ...
    elif obj['category'] == 'affiliate':
        url = url_for('{0}affiliate'.format(prefix), **kw)
    elif obj['category'] == 'app':
        url = url_for('{0}app'.format(prefix), **kw)
    else:
...
def get_timerange(timeframe, custom=None):
    ...
    elif timeframe in ['Last 24 hours', 'Last 24h']:
        start = "now-24h/m"
        end = "now/m"
    elif timeframe in ['Last 48 hours', 'Last 48h']:
        start = "now-48h/m"
        end = "now/m"
        interval = '2h'
    elif timeframe in ['Last 7 days', 'Last Week']:
        start = "now-6d/d"
        end = "now/d"
        interval = '1d'
        ...
        if not _errors:
            ...
            if not _errors:
                ...
                if total > 0 and (not resource_key or (resource_key and resource_key.get('key') != _.pop()._id)):
                    ...
                    if entity['category'] not in ['provider', 'app']:
                        entity['geo'] = []
