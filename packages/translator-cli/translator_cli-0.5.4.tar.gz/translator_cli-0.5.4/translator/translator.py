...
from translator.consts import ENGINES, APP_NAME, GPT_ENGINES, FIXER_ENGINE
...
class Translator:
    ...
    def translate(
        ...
        try:
            ...
            if engine == "deepl":
                ...
                r = self.translate_with_google(text, input_lang, output_lang)
            elif engine == "aws":
            ...
            if logger.level == logging.DEBUG:
                logger.exception(
                    "No more available engines to perform the translation. Leaving untranslated..."
                )
            else:
                logger.warning(
                    "No more available engines to perform the translation. Leaving untranslated..."
    ...
    def get_engine(self, engine: str | int) -> str:
        logger.debug(f"Getting engine {engine}")
        if isinstance(engine, str):
            ...
            if engine >= len(self.engine_list):
                raise NoAvailableEnginesError(f"There are no more engines to try")
            else:
        ...
        while translation is None:
            try:
                translation = GoogleTranslator(
                    source=src_lang, target=trg_lang
                ).translate(text)
    ...
    def fix_translation(self, original, translation, src_lang, dest_lang):
        """
        Given two structures of nested lists, where one is supposed to be the translated version
        of the other, check if there are any problems with the structures differing at any point.
        It if were so, fix the subtree by translating it the traditional way
        """
        i = 0
