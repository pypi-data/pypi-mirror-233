...
class Algo:
    ...
    def list(cls) -> Generator[str, None, None]:
        ...
        # TODO: move all lazy loading logics in registry class
        lazy_load = config.main_cfg["registry"].get("lazy_load", True)
        if lazy_load and not ikomia.ik_registry.is_all_loaded():
            ikomia.ik_registry.load_algorithms()

        for name in ikomia.ik_registry.get_algorithms():
            info = ikomia.ik_registry.get_algorithm_info(name)
    ...
    def create(
        ...
        python_plugin_directory = Path(ikomia.ik_registry.get_plugins_directory(), "Python")
    ...
    def path(self) -> Path:
        ...
        if self._path is None:
            ...
            if self._info.language == ikomia.utils.ApiLanguage.CPP:
                self._path = Path(ikomia.ik_registry.get_plugins_directory(), "C++", self.name)
            elif self._info.language == ikomia.utils.ApiLanguage.PYTHON:
                self._path = Path(ikomia.ik_registry.get_plugins_directory(), "Python", self.name)
            else:
    ...
    def _get_compatible_os_list(self) -> List[str]:
        ...
        """
        if self._info.os == ikomia.utils.OSType.ALL:
            return ["LINUX", "WINDOWS"]
        elif self._info.os == ikomia.utils.OSType.WIN:
            return ["WINDOWS"]
        elif self._info.os == ikomia.utils.OSType.LINUX:
            return ["LINUX"]
        else:
            return []
...
def get_platform_information() -> dict:
    ...
    """
    return {
        "python_version": get_local_python_version(),
        "os": [platform.system().upper()],  # 'LINUX', 'DARWIN', 'JAVA', 'WINDOWS'
