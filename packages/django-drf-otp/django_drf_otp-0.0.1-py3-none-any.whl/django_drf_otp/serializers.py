...
class OTPTokenSerializer(serializers.Serializer):
    ...
    def validate(self, attrs: Dict) -> Dict:
        ...
        try:
            ...
            raise ValidationError(detail=self.error_messages["no_active_account"], code="no_active_account")
...
class OTPTokenVerifySerializer(serializers.Serializer):
    ...
    def validate(self, attrs: Dict) -> Dict:
        try:
            verification_token = VerificationToken.objects.get(token=attrs["token"], is_active=True)
        except VerificationToken.DoesNotExist:
            raise ValidationError(detail=_("Invalid token."), code="invalid_token")

        from .services import OTPService

        verified = OTPService().verify_token(token=attrs["token"], extra_secret_chars=verification_token.secret_chars)
        if not verified:
        ...
        attrs["user"] = verification_token.user
        return attrs
