...
class QuickFactorTestor:
    ...
    def evaluate(self, cleaned_factor: pd.DataFrame, type="long_and_short", rebalance_freq="Monthly", days=[-1]):
        """_summary_

        Parameters
        ----------
        factor_data : pd.DataFrame with MultiIndex
            _description_
        """
        assert type == "long_and_short"
    ...
    def get_group_positions(self, cleaned_factor: pd.DataFrame, factor_quantile: int):
        ...
        weights = (tmp > 0) / np.repeat(tmp.sum(axis=1).values.reshape(-1, 1), self.n_assets, axis=1)
        return weights

    def get_long_short_positions(self, cleaned_factor: pd.DataFrame):
        long_positions = self.get_group_positions(cleaned_factor, 5)
        short_positions = self.get_group_positions(cleaned_factor, 1)
...
class QuickBackTestor:
    ...
    def _rewards_in_range(self, start_position, start_date, end_date):
        returns_slice = self._returns.loc[start_date:end_date].copy()
        returns_slice.iloc[0] = 0
        pnl = (returns_slice + 1).cumprod()
        rewards = pnl.dot(start_position).pct_change()
