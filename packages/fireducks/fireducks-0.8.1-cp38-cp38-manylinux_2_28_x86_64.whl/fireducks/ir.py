...
def table_corr(table, method, min_periods, numeric_only):
    return _build_op(
        Op_table_corr,
        [TableType],
        [
            table,
            method,
            create_i32_attr("min_periods", min_periods),
...
def series_corr(left, right, method, min_periods):
    # TODO: support chaining only on input
    return _build_op(
        Op_series_corr,
        [Ty_scalar],
        [left, right, method, create_i32_attr("min_periods", min_periods)],
    ).outs[0]
