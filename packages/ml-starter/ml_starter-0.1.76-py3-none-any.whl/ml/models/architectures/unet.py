...
class BasicBlock(nn.Module):
    def __init__(
        ...
        # Projects time to embedding space.
        self.mlp_time = (
            nn.Sequential(
        ...
        # Projects class embeddings to embedding space.
        self.mlp_class = (
            nn.Sequential(
                nn.Linear(class_c, class_c),
                get_activation(act),
                nn.Linear(class_c, out_c),
            )
    ...
    def forward(self, x: Tensor, t: Tensor | None, c: Tensor | None) -> Tensor:
        ...
        if c is not None:
            assert self.mlp_class is not None, "Class was provided but class embedding is None"
            cx = self.mlp_class(c)
            out = out + cx[..., None, None]
        else:
            assert self.mlp_class is None, "Class embedding is not None but no class was provided"
    ...
    def __init__(
        ...
        for idx, (in_c, out_c) in enumerate(zip(all_dims[:-1], all_dims[1:])):
            ...
            self.down_blocks.extend(
                nn.ModuleList(
                    [
                        BasicBlock(in_c, in_c, embed_dim, use_time, class_dim),
                        BasicBlock(in_c, in_c, embed_dim, use_time, class_dim),
        ...
        for idx, (in_c, out_c, skip_c) in enumerate(zip(all_dims[::-1][:-1], all_dims[::-1][1:], all_dims[:-1][::-1])):
            ...
            self.up_blocks.extend(
                nn.ModuleList(
                    [
                        BasicBlock(in_c + skip_c, in_c, embed_dim, use_time, class_dim),
                        BasicBlock(in_c + skip_c, in_c, embed_dim, use_time, class_dim),
        ...
        self.mid_blocks = cast(
            ...
            nn.ModuleList(
                [
                    BasicBlock(all_dims[-1], all_dims[-1], embed_dim, use_time, class_dim),
        ...
        self.out_blocks = cast(
            ...
            nn.ModuleList(
                [
                    BasicBlock(embed_dim, embed_dim, embed_dim, use_time, class_dim),
                    nn.Conv2d(embed_dim, in_dim, 1, bias=True),
                ]
            ),
        )

    def forward(self, x: Tensor, t: Tensor | None = None, c: Tensor | None = None) -> Tensor:
        x = self.init_embed(x)
        if t is None:
