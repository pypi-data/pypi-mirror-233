...
class Camera(BaseCamera):
    ...
    def acquire_one_frame(cap):
        ...
        if (cap is None):
            ...
            ret = False
        if not cap.isOpened():
            print("X... 1camera  not Opened(@real_cam)")
            time.sleep(0.7)
        if not cap.isOpened():
            print("X... 2camera  not Opened(@real_cam)")
            ...
            try: #----this catches errors of libjpeg with cv2.CAP_V4L2
                ...
                if frame is not None:
                    ...
                    if not sucres:
                        ...
                        frame[0:4,0:4] = [0,0,255]
                        print(f" OBTAIN{fg.red}{obtres}{fg.default} ", end = "" )
