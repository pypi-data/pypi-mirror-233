            ...
            if row==0:
                if (col==0):
                    ...
                    if NOW.second % 2 == 0:
                        ...
                        cv2.circle( frametgt, (10,10), int(self.fontScale*18), (0,0,255), -1 )

                    else:
                        cv2.circle( frametgt, (10,10), int(self.fontScale*18), (0,255,0), -1 )
...
# --------------------------------------------------------save avi ------------
# ------------------------------------------------- also ZMQ
    def save_avi(self, seconds =  1, name = "" ,  basecamera_string= None , mycodec = "DIVX"):
        """
        ...
        # -------------- timelapse if seconds>0: ******************************* MKV FOR LOOP
            ...
            if not self.aviopened_laps:
                me = socket.gethostname()
                filename = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = os.path.expanduser("~/DATA/"+ f"{me}_laps{seconds:04d}_{filename}.mkv" )
                for i in range(3):
                    print(f"{bg.red}{filename}{bg.default}")

                self.save_avi_info = 0 # count saved frames
            ...
            if (dt.datetime.now()-self.avi_last).total_seconds() > seconds:
                ...
                self.save_avi_info+=1

                # # pixel typing
                # for i in range(300,300+ self.save_avi_info):
                ...
                if basecamera_string is not None:
                    ...
                    overtext = f"{self.save_avi_info:06d} / {overtext}"
                ...
                print(f"i... saving timelaps  #{self.save_avi_info:06d}   [ {basecamera_string} ]" )
