...
def test_storage():
    ...
    my_storage = generic_heat_water_storage.HeatStorage(
        config=my_storage_config, my_simulation_parameters=my_simulation_parameters
    )

    thermal_demand_heating_water = cp.ComponentOutput(
        "FakeThermalDemandHeatingWater",
        "ThermalDemandHeatingWater",
        lt.LoadTypes.ANY,
        lt.Units.PERCENT,
    )

    thermal_demand_warm_water = cp.ComponentOutput(
        "FakeThermalDemandWarmWater",
        "ThermalDemandWarmWater",
        lt.LoadTypes.WATER,
        lt.Units.CELSIUS,
    )

    control_signal_choose_storage = cp.ComponentOutput(
        "FakeControlSignalChooseStorage",
