...
def test_example_component():
    ...
    my_example_component.thermal_energy_delivered_c.source_output = (
        thermal_energy_delivered_output
    )

    number_of_outputs = fft.get_number_of_outputs(
        [my_example_component, thermal_energy_delivered_output]
    )
    ...
    stsv.values[
        thermal_energy_delivered_output.global_index
    ] = 50  # fake thermal energy delivered input
    ...
    assert (
        my_example_component_config.initial_temperature
        == my_example_component.initial_temperature
    )
