...
def on_report_generated(cookies, files, chatbot):
    from void_terminal.toolbox import find_recent_files
...
def read_env_variable(arg, default_value):
    ...
    """
    from void_terminal.colorful import print亮红, print亮绿
...
def read_single_conf_with_lru_cache(arg):
    from void_terminal.colorful import print亮红, print亮绿, print亮蓝
    try:
        # 优先级1. 获取环境变量作为配置
        default_ref = getattr(importlib.import_module('void_terminal.config'), arg)   # 读取默认值作为数据类型转换的参考
...
def clip_history(inputs, history, tokenizer, max_token_limit):
    ...
    """
    import numpy as np
    from void_terminal.request_llm.bridge_all import model_info
...
def get_plugin_default_kwargs():
    """
    """
    from void_terminal.toolbox import get_conf, ChatBotWithCookies
