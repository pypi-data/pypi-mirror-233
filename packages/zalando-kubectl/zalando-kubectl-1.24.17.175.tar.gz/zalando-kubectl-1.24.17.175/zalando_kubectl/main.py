...
from clickclick import Action, info, print_table, OutputFormat
...
def completion(ctx, env: Environment, kubectl_arg):
    ...
    if not env.kubectl.exists():
        raise Exception("zkubectl: completions not yet available, run any zkubectl command to finish setting up")
...
@click.option("--privileged-access-api", required=False, help="Privileged Access API URL")
@click.pass_obj
...
@click.pass_obj
def credentials(env: Environment, cluster):
    kube_config.get_auth(env, cluster)


@click_cli.command("login")
@click.argument("cluster", required=False)
@click.option("--okta/--no-okta", default=False, help="Login with Okta", envvar="ZKUBECTL_USE_OKTA")
@click.option(
    "--force-refresh/--no-force-refresh",
    default=False,
    help="When using Okta, refresh the token regardless of its expiration time",
)
@click.pass_obj
def do_login(env: Environment, cluster, okta, force_refresh):
...
def encrypt(env: Environment, cluster, kms_keyid, strip):
    ...
    if cluster:
        ...
        cluster_metadata = registry.get_cluster_with_params(registry_url, api_server_url=utils.get_api_server_url(env))
...
def generate(
    ...
    try:
        ...
        if typ == "fabric":
            token = auth_token()
            gen.fabric(token, application, component, team, backendport, file, debug)

        elif typ == "ingress":
            if not host:
                raise click.UsageError("--host required for ingress")
            gen.ingress(application, component, team, host, backendport, scopes, ui, debug)
            ...
            raise click.UsageError("Wrong typ information found: {}".format(typ))
...
def cluster_update_status(env: Environment):
    ...
    registry_url = get_registry_url(env.config)
    api_server_url = utils.get_api_server_url(env)
...
def latest_socat_image():
    ...
    response = requests.get(
        "https://{}/v2/{}/{}/tags/list".format(docker_registry, team, artifact),
        timeout=10,
    ...
    return "{}/{}/{}:{}".format(docker_registry, team, artifact, sorted(image["tags"])[-1])
...
def generate_containers(target, ports, base_port, timeout):
    ...
    for i, port in enumerate(ports):
        ...
        remote_port = port[-1]
        containers.append(generate_container(target, local_port, remote_port, socat_image, timeout))
...
def generate_job(containers, name, timeout):
    return {
        ...
        "metadata": {
            ...
            "annotations": {"janitor/ttl": "{}s".format(timeout)},
        },
        "spec": {
...
@click.option("--okta/--no-okta", default=False, help="Login with Okta", envvar="ZKUBECTL_USE_OKTA")
@click.option("-i", "--incident", help="Opsgenie incident number or URL, [required] with --emergency", type=str)
@click.option(
    "-u",
    "--user",
    help="User that should be given access, defaults to the current user, [invalid] with --okta",
    required=False,
)
@click.option(
    "-t",
    "--timeout",
    help="Duration, in seconds, to wait for the request to be approved, only valid with --okta",
    default=600,
    required=False,
    type=int,
)
...
@click.pass_obj
def request_cluster_access(env: Environment, emergency, incident, user, reason, okta, timeout):
    """Request access to the cluster"""
    cluster_access_check_environment(env)

    if user and okta:
        raise click.UsageError("When using Okta, you cannot specify a username.")
    ...
    if emergency:
        ...
        if incident.startswith("https://"):
            ...
            reference_url = "https://zalando.app.opsgenie.com/incident/detail/" + incident
        elif re.fullmatch(r"\w+-\w+-\w+-\w+-\w+-\d+", incident):
            reference_url = "https://zalando.app.opsgenie.com/alert/show/" + incident
        else:
            raise click.UsageError("You must provide a valid incident number or ID, alert ID or an Opsgenie URL")
        make_emergency_access_request(env, reference_url, user, reason, okta, timeout)
    else:
        make_manual_access_request(env, user, reason, okta, timeout)


def make_emergency_access_request(env: Environment, reference_url, user, reason, okta, timeout):
    if okta:
        access_request.create_with_okta(env, "emergency", reference_url, " ".join(reason), timeout)
        return

    user = access_request_username(user)
    access_request.create(env, "emergency", reference_url, user, " ".join(reason))
    click.echo(
...
def make_manual_access_request(env: Environment, user_option, reason, okta, timeout):
    if okta:
        access_request.create_with_okta(env, "privileged", None, " ".join(reason), timeout)
        return

    user = access_request_username(user_option)
    access_request.create(env, "manual", None, user, " ".join(reason))
...
@click.argument("username_or_request_key", required=True)
@click.option("--okta/--no-okta", default=False, help="Login with Okta", envvar="ZKUBECTL_USE_OKTA")
@click.option("--yes/--no-yes", default=False, help="Approve the given access request without prompting again.")
@click.pass_obj
def approve_access_request(env: Environment, username_or_request_key, okta, yes):
    """Approve a manual access request

    USERNAME_OR_REQUEST_KEY is the username or request_key (when using --okta) to approve access for.
    """
    cluster_access_check_environment(env)

    if okta:
        access_request.approve_with_okta(env, username_or_request_key, yes)
        return

    username = username_or_request_key

    if not username:
        username = click.prompt("User that should receive access")

    existing = access_request.get_all(env).get(username)
    if not existing:
        ...
        if click.confirm("Manual access request from {user}: {reason}. Approve?".format(user=username, reason=reason)):
            updated_reason = access_request.approve(env, username)
            click.echo("Approved access for user {user}: {reason}".format(user=username, reason=updated_reason))


@cluster_access.command("list")
@click.option("--okta/--no-okta", default=False, help="Login with Okta", envvar="ZKUBECTL_USE_OKTA")
@click.option("--request-key", help="The request key that identifies your privileged access request")
@click.option(
    "-o", "--output", type=click.Choice(["text", "json"]), default="text", help="Command output format (default: text)"
)
@click.pass_obj
def list_cluster_requests(env: Environment, okta, request_key, output):
    """List all current pending/approved access requests for the cluster"""
    cluster_access_check_environment(env)

    if request_key and not okta:
        raise click.UsageError("Request Key is only valid when using Okta.")

    if okta:
        access_request.list_with_okta(env, request_key)
        return

    all_requests = access_request.get_all(env)
...
def main():
    ...
    if sys.stdin.isatty() and sys.stdout.isatty():
        ...
        # Only inform users to update zkubectl
            ...
            clickclick.error("Unable to check for updates: {}".format(e))
