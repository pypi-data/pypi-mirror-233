...
class ExternalBinary:
    ...
    def run(
        self, args, input=None, stdout=None, stderr=None, check=False, forward_context=True, forward_namespace=True
    ):
...
def get_api_server_url(env: Environment) -> str:
    try:
        # --minify returns only the information about the current context only, which works nicely with --context
        args = ["config", "view", "--minify", "-o", "jsonpath={.clusters[].cluster.server}"]
