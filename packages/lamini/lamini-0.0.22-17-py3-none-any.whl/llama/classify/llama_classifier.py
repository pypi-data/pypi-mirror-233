...
class BinaryLlamaClassifier:
    ...
    def predict_proba(self, text: list[str]):
        return [
            [
                probs[self.classifier.class_ids["negative"]],
                probs[self.classifier.class_ids["positive"]],
            ]
    ...
    def predict(self, text: list[str]):
        return [
            1 if prob[1] > prob[0] else 0 for prob in self.predict_proba(text)
        ]
...
class LlamaClassifier:
    ...
    def predict(self, text):
        ...
        return [list(self.class_ids.keys())[class_id] for class_id in winning_classes]
