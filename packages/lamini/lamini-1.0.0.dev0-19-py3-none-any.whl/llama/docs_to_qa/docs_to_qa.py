...
class DocsToQA:
    def __init__(
        ...
        if self.docs_path:
            self._chunk_docs(self.docs_path)
...
{user_prompt} [/INST]"""
    ...
    def _chunk_docs(self, docs_path, char_chunk_size=5000):
        ...
        if docs_path_type == ".csv":
        ...
        elif docs_path_type == ".jsonl" or docs_path_type == ".jsonlines":
            ...
            #     self.docs[doc_index] = chunk
            #     doc_index += 1
            self._get_chunks(text, char_chunk_size, doc_index)
        else:
    ...
    def load_questions(self, dirpath):
        ...
        if not self.docs:
            ...
            self.docs_path = open(f"{dirpath}/docs_path.txt").read()
            self._chunk_docs(self.docs_path)
    ...
    def _save_questions(self, dirpath=None, verbose=True):
        ...
        docs_filepath = f"{dirpath}/docs_path.txt"
        with open(docs_filepath, "w") as file:
            file.write(self.docs_path)
        if verbose:
...
def finetune_qa(qa_dirpath, docs_path=None, model_name=None, is_public=False, use_retrieval=True):
    """
...
def run_model(question, docs_path=None, model_name=None, doc_id=None, verbose=False):
    """
