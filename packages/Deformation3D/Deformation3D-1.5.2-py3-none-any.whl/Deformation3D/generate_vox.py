...
def get_stem_leafs(data_root, corn_name, leaf_num, normal=True):
    ...
    point_set = pc_normalize(point_set) if normal else point_set
...
def create_voxel_from_cloud(np_cloud, np_label, leaf_sheath_list, leaf_size=10):
    """
    从点云中创建体素网格
    :param np_cloud: numpy 格式点云
    :param np_label: numpy 格式点云 label
    :param leaf_sheath_list: 叶鞘点云列表
    :param leaf_size: 体素分辨率
    ...
    for i in range(len(point_cloud)):
        hx = math.ceil((point_cloud[i][0] - x_min) / leaf_size)
        hy = math.ceil((point_cloud[i][1] - y_min) / leaf_size)
        hz = math.ceil((point_cloud[i][2] - z_min) / leaf_size)
        key = f"{hx}:{hy}:{hz}"
    ...
    return voxel_grid_x_y_z, np.array(
        voxel_grid_l), voxel_grid_points, voxel_grid_points_label, leaf_sheath_voxel_points_str_list


def generate_cubic(voxel_grid_x_y_z, voxel_grid_l, voxel_grid_points, leaf_sheath_voxel_list, voxel_size):
    """
...
def create_veg_file(filepath, vertices_dict, element_list, element_label,
    ...
    vertices_count = len(vertices_dict)
    elements_count = len(element_list)
    ...
    for key in vertices_dict:
        ...
        VERTICES_value += f"{key+1} {value[0]} {value[1]} {value[2]}\n"
...
def create_obj_file(filepath, vertices_dict, face_list):
    """
    创建 obj 文件
    :param filepath:
    :param vertices_dict:
    ...
    for key in vertices_dict:
        value = [float(i) for i in vertices_dict[key].split(':')]
        VERTICES += f"v {value[0]} {value[1]} {value[2]}\n"
    ...
    for face_one in face_list:
        FACES += f"f {face_one[0]} {face_one[1]} {face_one[2]} {face_one[3]}\n"
...
def save_random_choice_leaf_vertices_improve(vertices_dict, vertices_label_list, filepath, per_num=-1):
    """
    叶尖（x，y 方向上 距 0 点的 1/4 作为叶尖）中随即点选取, 并保存
    :param vertices_dict: 顶点
    :param vertices_label_list: 顶点 label
    :param filepath: 保存的文件名
    ...
    for i in range(0, n + 1):
        ...
        for leaf_index in leaf_index_list:
            key = vertices_dict[leaf_index]
            value = [float(i) for i in key.split(':')]
...
def generate_one(data_root, veg_root, corn_name, leaf_num, voxel_size=0.02, normal=True,
                 MATERIALS = "\n*MATERIAL STEM\nENU, 1000, 100000, 0.01\n\n*MATERIAL LEAF\nENU, 1000, 10000000, 0.45\n"):
    """
    生成一个基础的模板
    :param data_root:
    ...
    # 体素网格生成
    voxel_grid_x_y_z, voxel_grid_l, voxel_grid_points, voxel_grid_points_label, leaf_sheath_voxel_points = \
    ...
    # 六面体生成
    vertices_dict, element_list, element_label, vertices_label_list, face_list, voxel_points_dist_list, leaf_sheath_voxel_index_list = \
        generate_cubic(voxel_grid_x_y_z, voxel_grid_l, voxel_grid_points, leaf_sheath_voxel_points, voxel_size)
    f_log("六面体生成")

    # veg 文件生成
    veg_filepath = os.path.join(veg_root, f"{os.path.splitext(corn_name)[0]}.veg")
    create_veg_file(veg_filepath, vertices_dict, element_list, element_label, MATERIALS=MATERIALS)
    f_log(f"veg 文件生成: 立方体个数： {len(element_list)}")

    # 叶子中随即点选取
    # fix: 包括茎
    random_leafs_filepath = os.path.join(veg_root, f"{os.path.splitext(corn_name)[0]}_random_leaf.bou")
    save_random_choice_leaf_vertices_improve(vertices_dict, vertices_label_list, random_leafs_filepath)
    f_log("叶子中随即点选取")
    ...
    create_obj_file(obj_filepath, vertices_dict, face_list)
    ...
    save_points_labels(points_label_filepath, voxel_grid_points_label, fmt='%d')
    ...
    # voxel_points_closest_index_filepath = os.path.join(
    #     veg_root, f"{os.path.splitext(corn_name)[0]}_voxel_points_closest_index.txt")
    save_voxel_points_parameters(voxel_points_dist_filepath, voxel_points_dist_list, fmt='%.6f')
    # save_voxel_points_parameters(voxel_points_closest_index_filepath, voxel_points_dist_list, fmt='%d')
    f_log("保每个体素内的插值参数")
