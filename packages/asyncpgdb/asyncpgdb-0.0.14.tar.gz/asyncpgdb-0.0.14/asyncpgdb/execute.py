...
def __execute__(func: Callable[..., Coroutine[Any, Any, T]]):
    error_id = f"asyncpgdb.execute.{func.__name__}.error"

    def handle_exception(exception: Exception):
        error_message = (
            f"{error_id}: {str(type(exception).__name__)} : {str(exception)}"
        )
        print(error_message)
        return None
    ...
    async def inner(*args, **kwargs):
        try:
            ...
            result = handle_exception(exception=exception)
        return result
...
def __iterate__(func: Callable[..., Coroutine[Any, Any, T]]):
    error_id = f"asyncpgdb.iterate.{func.__name__}.error"
