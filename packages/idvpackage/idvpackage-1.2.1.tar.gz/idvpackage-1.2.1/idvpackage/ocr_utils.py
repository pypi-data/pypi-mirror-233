...
def detect_photo_on_screen(client, image):
    ...
    for label in image_labels:
        ...
        if confidence >= confidence_threshold and any(keyword.lower() in description for keyword in keywords):
            flag = 'consider'
...
def data_consistency_check(data, front_id_text, back_id_text):
    ...
    if passport_data:
        if not fuzzy_match_fields(data.get('dob'),passport_data.get('passport_date_of_birth')):
            data_consistency['breakdown']['date_of_birth']['result'] = 'consider'
        
        if not fuzzy_match_fields(data.get('first_name'),passport_data.get('passport_given_name_en')):
            data_consistency['breakdown']['first_name']['result'] = 'consider'
        
        if not fuzzy_match_fields(data.get('gender'),passport_data.get('passport_gender')):
            data_consistency['breakdown']['gender']['result'] = 'consider'

        if not fuzzy_match_fields(data.get('last_name'),passport_data.get('passport_surname_en')):
            data_consistency['breakdown']['last_name']['result'] = 'consider'
        
        if not fuzzy_match_fields(data.get('nationality'),passport_data.get('passport_nationality')):
            data_consistency['breakdown']['nationality']['result'] = 'consider'
...
def data_validation_check(data):
    ...
    data_validation['breakdown']['document_expiration'] = expiry_result
...
## pending
def image_integrity_check(data, front_id_text, back_id_text, coloured, blurred, glare, missing_fields):
...
def visual_authenticity_check(data, front_id_text, back_id_text, selfie, facial_similarity):
    visual_authenticity = VISUAL_AUTHENTICITY

    if np.any(selfie):
        if facial_similarity>=0.60:
...
def form_final_data_document_report(data, front_id_text, back_id_text, coloured, selfie, facial_similarity, blurred, glare, missing_fields):
    try:
        document_report = {
            ...
            "breakdown": {
                ...
                "image_integrity": image_integrity_check(data, front_id_text, back_id_text, coloured, blurred, glare, missing_fields),
                "issuing_authority": {
                "breakdown": {
                ...
                "police_record": {},
                "visual_authenticity": visual_authenticity_check(data, front_id_text, back_id_text, selfie, facial_similarity),
                ## pending - to be filled by dev
                "check_id": "", 
...
def form_final_facial_similarity_report(selfie, facial_similarity, liveness_result):
    print(facial_similarity, liveness_result)
    facial_report = {
        "created_at": "",
        ## pending - to be filled by dev
    ...
    if np.any(selfie) and facial_similarity >= 0.60:
    ...
    print(f"image integrity res: {image_integrity_final_result}")
    ...
    print(f"final result res: {complete_final_result}")
