...
class VersionUpload(BaseModel):
    """
    Version data. See the VersionUpload schema for more info  # noqa: E501
    """
    version: StrictStr = Field(description="Version string of the version to be published")
    plugin_dependencies: Optional[Dict[str, List[PluginDependency]]] = Field(default=None, description="Map of each platform's plugin dependencies", alias="pluginDependencies")
    platform_dependencies: Optional[Dict[str, List[Annotated[str, Field(min_length=1, strict=True, max_length=3)]]]] = Field(default=None, description="Map of platforms and their versions this version runs on", alias="platformDependencies")
    description: Optional[StrictStr] = None
    files: Optional[Annotated[List[MultipartFileOrUrl], Field(min_length=1, max_length=3)]] = None
    channel: StrictStr = Field(description="Channel of the version to be published under")
    __properties: ClassVar[List[str]] = ["version", "pluginDependencies", "platformDependencies", "description", "files", "channel"]
    ...
    def from_json(cls, json_str: str) -> Self:
        """Create an instance of VersionUpload from a JSON string"""
    ...
    def to_dict(self):
        ...
        if self.plugin_dependencies:
            for _key in self.plugin_dependencies:
                if self.plugin_dependencies[_key]:
                    _field_dict_of_array[_key] = [
                        _item.to_dict() for _item in self.plugin_dependencies[_key]
                    ]
            _dict['pluginDependencies'] = _field_dict_of_array
        # override the default output from pydantic by calling `to_dict()` of each value in platform_dependencies (dict of array)
        _field_dict_of_array = {}
        if self.platform_dependencies:
            for _key in self.platform_dependencies:
                if self.platform_dependencies[_key]:
                    _field_dict_of_array[_key] = [
                        _item.to_dict() for _item in self.platform_dependencies[_key]
                    ]
            _dict['platformDependencies'] = _field_dict_of_array
    ...
    def from_dict(cls, obj: dict) -> Self:
        """Create an instance of VersionUpload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "files": [MultipartFileOrUrl.from_dict(_item) for _item in obj.get("files")] if obj.get("files") is not None else None,
