    ...
    def train(self,
        ...
        *args, **kwargs:
        ...
        for EPOCH in tqdm(range(self.epoch), desc="training", unit="epoch"):
            self.logger.info("Start training@EPOCH {}. Current params: lr = {}{}; batchsize = {} * {}; patience = {}/{}".format(EPOCH, self.lr, " * {}".format(self.lr_scheduler(EPOCH)) if self.lr_scheduler else "", self.batch_size(EPOCH) if callable(self.batch_size) else self.batch_size, self.mini_batch(EPOCH) if callable(self.mini_batch) else self.mini_batch, patience, self.patience)) if self.logger else None # INFO-LOG
            patience = patience - 1
    ...
    def train_epoch(self,
        ...
        for idx, packet in enumerate(train_loader):
            ...
            if countr == mini_batch:
                self.optimizer.step()
                self.logger.debug("Training iter {}, loss = {:.8f}, optimizer stepped.".format(idx, loss.data)) if self.logger else None # DEBUG-LOG
                countr = 0
            else:
                self.logger.debug("Training iter {}, loss = {:.8f}.".format(idx, loss.data)) if self.logger else None # DEBUG-LOG
        if countr != 0:
            self.optimizer.step()
