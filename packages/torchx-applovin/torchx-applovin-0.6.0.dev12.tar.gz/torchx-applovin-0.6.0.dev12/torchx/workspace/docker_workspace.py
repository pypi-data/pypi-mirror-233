...
COPY . .
"""
default_repo = "gcr.io/strategic-atom-700/torchx-dist-test"
        ...
        for role in app.roles:
            if role.image.startswith(HASH_PREFIX):
                if not image_repo:
                    ...
                    logger.info(f"image_repo unspecified using default image repo ({default_repo})")

                    # log that we are using dfl
                    # raise KeyError(
