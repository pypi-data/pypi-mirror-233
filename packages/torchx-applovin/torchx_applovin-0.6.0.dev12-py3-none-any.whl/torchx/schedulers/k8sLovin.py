...
def role_to_pod(name: str, role: Role, service_account: Optional[str]) -> "V1Pod":
    ...
    # rewrite below better
    if "node_selector" in resource.capabilities:
        node_selector = resource.capabilities["node_selector"]
    else:
        node_selector = None

    if "affinity" in resource.capabilities:
        affinity = resource.capabilities["affinity"]
    else:
        affinity = None

    if "tolerations" in resource.capabilities:
    ...
    **Compatibility**
    ...
    def _run_opts(self) -> runopts:
        ...
        opts.add(
            ...
            default="gcr.io/strategic-atom-700/torchx-dist-test"
        )
        opts.add(
            "queue",
            type_=str,
