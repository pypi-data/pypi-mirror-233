...
def mqtt_config_save(request):
	' Handle MQTT configuration form post ';L='mqtt_client_key';K='mqtt_client_cert';J='mqtt_port';E='mqtt_server';D=True;C=request
	if not C.form:return server.redirect(_G,status=STATUS_CODE_302)
	B=False;A=C.form.get(E,_A)
	if A:
		logging.debug(f"{utils.is_fqdn(A)}: '{A}'");F=A.split('.')
		if len(F)==4 and all(A.isdigit()for A in F):networking.save_network_config_key(E,A);B=D
		elif utils.is_fqdn(A):networking.save_network_config_key(E,A);B=D
	G=C.form.get(J,_A)
	if G:networking.save_network_config_key(J,int(G));B=D
	H=C.form.get(K,_A)
	if H:networking.save_network_config_key(K,H);B=D
	I=C.form.get(L,_A)
	if I:networking.save_network_config_key(L,I);B=D
	if B:return server.redirect(_F,status=STATUS_CODE_302)
