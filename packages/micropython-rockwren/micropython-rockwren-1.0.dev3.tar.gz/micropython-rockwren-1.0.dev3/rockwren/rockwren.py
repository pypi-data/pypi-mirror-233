...
class Device:
	...
	def __init__(A,name='RockwrenDevice',device_type=b'light'):A.name=name;A.device_type=device_type;A.state=_B;A.web=None;A.mqtt_client=None;A.listeners=[];A.apply_state();' HTML template path for use for controlling the device from the web ui. ';A.template='/lib/rockwren/controls.html'
	...
	def information(A):'\n        Return a json representation of the device information encoded as a str. Extended or overridden for each device\n        that has additional information.  Normally, best practice is to extended: get the information dictionary\n        from this function then add to it to provide addition information.\n        :return: device state as json\n        ';E='dns_server';D='gateway';C='subnet_mask';B='ip_address';return ujson.dumps({'device':{'name':A.name,'type':A.device_type,'rockwren_version':__version__,'unique_id':ubinascii.hexlify(machine.unique_id()),'platform':sys.platform,'python_version':sys.version,'implementation':str(sys.implementation)},'mqtt':{'server':rockwren_env.MQTT_SERVER,'port':rockwren_env.MQTT_PORT,'command-topic':A.mqtt_client.command_topic,'availability-topic':A.mqtt_client.availability_topic,'state-topic':A.mqtt_client.state_topic},'network':{'ssid':rockwren_env.SSID,B:rockwren_env.CONNECTION_PARAMS.get(B),C:rockwren_env.CONNECTION_PARAMS.get(C),D:rockwren_env.CONNECTION_PARAMS.get(D),E:rockwren_env.CONNECTION_PARAMS[E]}})
...
def fly(the_device):
	...
	while True:
		...
		try:set_global_exception(uasyncio.get_event_loop());rockwren_env.CONNECTION_PARAMS=networking.connect();ntptime.settime();E=mqtt_client.MqttDevice(B,rockwren_env.MQTT_SERVER,rockwren_env.CONNECTION_PARAMS,command_handler=B.command_handler,mqtt_port=int(rockwren_env.MQTT_PORT));E.run(uasyncio.get_event_loop());web.run(uasyncio.get_event_loop());uasyncio.get_event_loop().run_forever()
