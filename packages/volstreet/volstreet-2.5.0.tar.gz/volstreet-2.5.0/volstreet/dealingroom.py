...
from volstreet.exceptions import ScripsLocationError, IntrinsicValueError, APIFetchError
...
# Global variables
obj: None | SmartConnect = None
login_data: None | dict = None
...
def retry_angel_api(data_type=None, max_attempts=10, wait_increase_factor=1.5):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
                    ...
                    # Other errors
                        ...
                        if getattr(thread_local, "robust_handling", False):
                            if attempt == max_attempts:
                                ...
                                logger.info(
                                    f"Trying to re-login. Attempt {attempt} failed. "
                                )
                                seconds_to_day_end: int = (
                                    datetime(
                                        *currenttime().date().timetuple()[:3],
                                        hour=15,
                                        minute=29,
                                    )
                                    - currenttime()
...
def increase_robustness(func):
    """Use this decorator on threaded functions for which you want to increase robustness
    in terms of fetching data from angels api (fetch_book and fetch_ltp for now). This decorator
    will set a flag in the thread local storage that will be used by the retry_angel_api decorator
    in cases of unknown errors to re-login and retry the function if it fails."""
...
class Option:
    ...
    def fetch_ltp(self):
        return fetch_ltp("NFO", self.symbol, self.token)
    ...
    def fetch_iv(self, spot=None, t=None, effective_iv=False):
...
class Strangle:
    ...
    def fetch_ltp(self):
        return fetch_ltp("NFO", self.call_symbol, self.call_token), fetch_ltp(
            "NFO", self.put_symbol, self.put_token
        )

    def underlying_ltp(self):
        symbol, token = fetch_symbol_token(self.underlying)
        return fetch_ltp(self.underlying_exchange, symbol, token)

    def fetch_ivs(self, spot=None, t=None, effective_iv=False):
        spot = spot if spot is not None else self.underlying_ltp()
        t = (
            t
            if t is not None
    ...
    def fetch_total_ltp(self):
        ...
        ), fetch_ltp("NFO", self.put_symbol, self.put_token)
...
class Index:
    ...
    def fetch_expirys(self, symbol: str):
        ...
        # Bringing the expiry to 3:30 PM and filtering out expired expiry dates
        allexpiries = [
    ...
    def fetch_ltp(self, future=None):
        ...
        if isinstance(future, int):
            ...
            ltp = fetch_ltp("NSE", self.symbol, self.token)
        return ltp
    ...
    def fetch_atm_info(self, expiry="current", effective_iv=False):
        ...
        call_iv, put_iv, avg_iv = atm_straddle.fetch_ivs(
            spot=price, effective_iv=effective_iv
        )
        return {
    ...
    def overnight_straddle(
        ...
        avg_till=(15, 28),
        notification_url=None,
        strategy_tag="Overnight short straddle",
    ):
        ...
        def check_eligibility_for_overnight_straddle(effective_tte: float) -> bool:
            after_weekend = check_for_weekend(self.current_expiry)
            square_off_tte = (
                effective_tte - 1
            )  # Square off 1 day after because duration of the trade is 1 day

            logger.info(f"{self.name} current expiry tte is {effective_tte} days")

            if after_weekend:
                logger.info(f"{self.name} current expiry is after a weekend")
                return False

            if square_off_tte < 1:
                if is_next_day_expiry(effective_tte):
                    logger.info(
                        f"{self.name} current expiry is next day so the trade is eligible on next expiry"
                    )
                    return True

                logger.info(
                    f"{self.name} current expiry is today so the trade is not eligible"
                )
                return False

            logger.info(
                f"{self.name} current expiry has enough TTE to trade so the trade is eligible"
            )
            return True
        ...
        effective_time_to_expiry = time_to_expiry(
            self.current_expiry, effective_time=True, in_days=True
        )
        ...
        if take_avg_price:
            ...
            while currenttime().time() < time(*avg_till):
                _ltp = self.fetch_ltp()
        ...
        if eligible_for_short:
            ...
            sell_straddle = Straddle(
                strike=sell_strike, underlying=self.name, expiry=expiry_to_trade
            )
            call_iv, put_iv, iv = sell_straddle.fetch_ivs()
            iv = iv * 100 if iv is not None else None
            notifier(
                f"{self.name} Deploying overnight short straddle with {sell_straddle}. IV: {iv}, VIX: {vix}",
                notification_url,
                "INFO",
            )
        else:
            ...
            sell_straddle = None
            notifier(
                f"{self.name} No straddle eligible for overnight short. VIX: {vix}",
                notification_url,
                "INFO",
            )
        ...
        trade_data = load_json_data(
            f"{self.name}_overnight_positions.json",
            user_id=obj.userId,
            default_structure=dict,
            notification_url=notification_url,
        )
        trade_data = trade_data.get(self.name, {})
        buy_strike = trade_data.get("strike", None)
        buy_expiry = trade_data.get("expiry", None)
        buy_straddle = (
            Straddle(strike=buy_strike, underlying=self.name, expiry=buy_expiry)
        ...
        if buy_straddle is None and sell_straddle is None:
            notifier(f"{self.name} No trade required.", notification_url, "INFO")
        elif sell_straddle is None:  # only exiting current position
            notifier(
                f"{self.name} Exiting current position on {buy_straddle}.",
                notification_url,
                "INFO",
            )
            ...
            notifier(
                f"{self.name} Entering new position on {sell_straddle}.",
                notification_url,
                "INFO",
            )
            ...
            if buy_straddle == sell_straddle:
                notifier(
                    f"{self.name} Same straddle. No trade required.",
                    notification_url,
                    "INFO",
                )
                ...
                notifier(
                    f"{self.name} Buying {buy_straddle} and selling {sell_straddle}.",
                    notification_url,
                    "INFO",
                )
                call_buy_avg, put_buy_avg = place_option_order_and_notify(
        ...
        trade_data = {self.name: {"strike": sell_strike, "expiry": expiry_to_trade}}
        save_json_data(
            trade_data,
            f"{self.name}_overnight_positions.json",
            user_id=obj.userId,
        ...
        self.strategy_log[strategy_tag].append(trade_info_dict)
    ...
    def buy_weekly_hedge(
        ...
        notifier(
            f"{self.name} Buying weekly hedge with {instrument}. IV: {iv}",
            notification_url,
            "INFO",
        )
        call_buy_avg, put_buy_avg = place_option_order_and_notify(
    ...
    def biweekly_straddle(
        ...
        strike_offset=1,
        notification_url=None,
        strategy_tag="Biweekly straddle",
    ):
        """Sells the far expiry straddle."""

        # Loading current position
        square_up_position = load_json_data(
            f"{self.name}_biweekly_position.json",
            user_id=obj.userId,
            default_structure=dict,
            notification_url=notification_url,
        )
        square_up_position = square_up_position.get(self.name, {})
        buy_strike = square_up_position.get("strike", None)
        buy_expiry = square_up_position.get("expiry", None)
        buy_straddle = (
            Straddle(strike=buy_strike, underlying=self.name, expiry=buy_expiry)
            if buy_strike is not None and buy_expiry is not None
            else None
        ...
        sell_straddle = Straddle(strike, self.name, expiry)
        call_iv, put_iv, iv = sell_straddle.fetch_ivs()
        notifier(
            f"{self.name} Deploying biweekly straddle\n"
        ...
        position_to_save = {self.name: {"strike": strike, "expiry": expiry}}
        save_json_data(
            position_to_save,
            f"{self.name}_biweekly_position.json",
            user_id=obj.userId,
    ...
    def intraday_strangle(
        ...
        def position_monitor(info_dict):
            ...
            # Price deque
            n_prices = max(
                int(seconds_to_avg / sleep_time), 1
        ...
        def justify_stop_loss(info_dict, side):
            ...
            try:
                logger.debug(
                    f"Calling simulate_price for {self.name} {side} strangle. "
                ...
                simulated_option_price = bs.simulate_price(
                ...
                error_message = (
                    f"Error in justify_stop_loss for {self.name} {side} strangle: {e}\n"
        ...
        def process_stop_loss(info_dict, sl_type):
            ...
            if catch_trend:
                trend_thread = Thread(
                    ...
                    name=f"{self.name} {sl_type} trend catcher",
                )
                trend_thread.start()
        ...
        try:
            atm_iv_at_entry = self.fetch_atm_info()["avg_iv"]
        except Exception as e:
        ...
        if shared_info_dict["exit_triggers"]["convert_to_butterfly"]:
            ...
            notifier(f"{self.name}: Converted to butterfly", notification_url, "INFO")
            while currenttime().time() < time(*exit_time):
                sleep(3)
        ...
        self.strategy_log[strategy_tag].append(trade_log)
    ...
    def intraday_trend(
        ...
        def get_futures_basis(traded_price):
            try:
                ...
                notifier(
                    f"Error in getting futures prices: {e}", notification_url, "ERROR"
                )
        ...
        while entries < max_entries and currenttime().time() < exit_time:
            ...
            notifier(
                f"{stop_loss_message}{self.name} trender {entries+1} exiting.",
                notification_url,
                "CRUCIAL",
            )
...
def load_json_data(
    ...
    try:
        ...
        notifier(f"Error while reading {file_path}: {e}", notification_url, "ERROR")
        # Assuming logger is a pre-configured logging object
        logger.error(
...
def save_json_data(
    ...
    try:
        ...
        notifier(f"Error while writing to {file_path}: {e}", notification_url, "ERROR")
...
def load_combine_save_json_data(
    ...
    existing_data = load_json_data(
        file_name, user_id, default_structure, notification_url
    )
    if new_data is None or new_data == [] or new_data == {}:
        return
    combined_data = combine_data(existing_data, new_data)
    save_json_data(combined_data, file_name, user_id, notification_url)
...
def login(user, pin, apikey, authkey, webhook_url=None, re_login=False):
    ...
    if obj is None or obj.refresh_token is None or re_login:
        ...
        if login_data["message"] != "SUCCESS":
            for attempt in range(2, 7):
                sleep(10)
                notifier(f"Login attempt {attempt}.", webhook_url, "CRUCIAL")
                login_data = obj.generateSession(user, pin, authkey_object.now())
                if login_data["message"] == "SUCCESS":
                    break
                if attempt == 6:
                    notifier("Login failed.", webhook_url, "ERROR")
...
def fetch_book(book: str) -> list:
    ...
    if book == "orderbook":
        return fetch_data(obj.orderBook)
    elif book in {"positions", "position"}:
        return fetch_data(obj.position)
    else:
...
def place_option_order_and_notify(
    ...
    **kwargs,
    ...
    try:  # Remove this try-except block after testing for a few days
        handle_open_orders_lite(order_ids, order_book, sleep_interval=0.05)
    except Exception as e:
        logger.error(
            f"Error while handling open orders: {e}",
            exc_info=(type(e), e, e.__traceback__),
        )
        notifier(f"Faced error while handling open orders: {e}. ", webhook_url, "INFO")
    sleep(1)  # Critical to wait again
    order_book = fetch_book("orderbook")
...
def fetch_symbol_token(
    ...
    if expiry is None and strike is None and option_type is None:  # Cash segment
        if (
            name in ["NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY"] and future is None
...
def fetch_ltp(exchange_seg, symbol, token):
    price_data = obj.ltpData(exchange_seg, symbol, token)
    return price_data
...
def fetch_straddle_price(name, expiry, strike, return_total_price=False):
    ...
    call_ltp = fetch_ltp("NFO", call_symbol, call_token)
    put_ltp = fetch_ltp("NFO", put_symbol, put_token)
...
def calculate_iv(opt_price, spot, strike, tte, opt_type):
    try:
        ...
        logger.error(f"Error in calculate_iv: {e}")
        return None
