...
#   timestamp: 2023-10-04T08:59:34+00:00
...
class AsyncProcessorSidecar(BaseModel):
    ...
    workers: conint(ge=1, le=5) = Field(
        1,
        description="+label=Worker Count\n+usage=Number of workers to spawn for the processor\n+message=Number of workers should be between 1 and 5 per replica",
    )
...
class NATSInputConfig(BaseModel):
    ...
    consumer_name: constr(regex=r"^[a-zA-Z0-9\-_]{1,32}$") = Field(
        ...,
        description="+label=Consumer Name\n+usage=Consumer name of input NATS\n+message=Consumer name should only contain alphanumeric letters, dashes(-), and underscores(_) and should be between 1 and 32 characters long\n+sort=3",
    )
...
class AsyncServiceAutoscaling(BaseAutoscaling):
    metrics: Union[SQSQueueMetricConfig, NATSMetricConfig, KafkaMetricConfig] = Field(
        ...,
        description="+label=Autoscaling metrics\n+usage=Metrics to use for the autoscaler\n+sort=4",
    )
...
class ModelDeployment(BaseModel):
    ...
    resources: Optional[Resources] = None
    replicas: conint(ge=0, le=500) = Field(
        1,
...
class WorkerConfig(BaseModel):
    input_config: Union[SQSInputConfig, NATSInputConfig, KafkaInputConfig] = Field(
        ..., description="+label=Input Config\n+usage=Input Config"
    )
    output_config: Optional[
        Union[SQSOutputConfig, NATSOutputConfig, KafkaOutputConfig]
