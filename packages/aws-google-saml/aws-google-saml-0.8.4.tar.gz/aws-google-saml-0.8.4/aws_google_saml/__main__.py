...
import webbrowser, base64, botocore.session, boto3, configparser, os, getopt, sys, pkgutil
...
def get_html_page_contents():
    try:
        print(__name__)
        return pkgutil.get_data(__name__, 'authed.html').decode('utf-8')
    except:
        with open('aws_google_saml/authed.html') as htmlFile:
