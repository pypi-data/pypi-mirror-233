...
def start_onprem_workstation(workstation=None, config_dir=None, profile=None):
...
class OnPremWorkstationClient:
    ...
    def get_container_status(self, workstation_id, hostname) -> str:
        token = metaflowconfig.get_metaflow_token_from_config(
            "/Users/ukashyap/.metaflowconfig", "liqo"
        )
        with Connection(
            host=hostname,
            user="outerbounds",
    ...
    def start_container(self, conn: Connection, workstation: dict):
        ...
        if existing_container is not None and existing_container["State"] == "running":
            ...
            if workstation["spec"]["gpu"] == 0:
                container_run_command = f"docker run --name {workstation['instance_id']} -d -e METAFLOW_HOME='/root/.metaflowconfig/' --cpus='{workstation['spec']['cpu_cores']}' --memory='{workstation['spec']['memory_gb']}g'  {workstation['spec']['image']} sh -c 'tail -f /dev/null'"
            else:
                ...
                if len(available_device_ids) < reqd_gpus:
                    ...
                    container_run_command = f"docker run --name {workstation['instance_id']} -d -e METAFLOW_HOME='/root/.metaflowconfig/' --cpus='{workstation['spec']['cpu_cores']}' --memory='{workstation['spec']['memory_gb']}g' {gpu_sub_command} {workstation['spec']['image']} sh -c 'tail -f /dev/null'"
            self.exec_remote_command(
