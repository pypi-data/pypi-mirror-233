...
def check_cli_command_exists(
    cli_name: str, cli_check_command: List[str], migitation: str, narrator: Narrator
) -> CommandStatus:
    narrator.announce_section(f"{cli_name} installation")
    narrator.announce_check(f"{cli_name} installation")
    check_status = CommandStatus(
        cli_name,
        status=OuterboundsCommandStatus.OK,
        reason=f"{cli_name} is installed.",
        mitigation="",
    )
    try:
        # Check if cli can be executed from the command line
        subprocess.run(cli_check_command, capture_output=True, check=True)
        narrator.ok()
        return check_status
    except:
        check_status.update(
            status=OuterboundsCommandStatus.FAIL,
            reason=f"No installation for {cli_name} detected on the user's PATH",
            mitigation=migitation,
        )
        narrator.not_ok(
            reason=f"No installation for {cli_name} detected on the user's PATH"
        )
...
def check_kubectl_installed(narrator: Narrator) -> CommandStatus:
    return check_cli_command_exists(
        "kubectl",
        ["kubectl", "--help"],
        "Please install kubectl manually from https://kubernetes.io/docs/tasks/tools/#kubectl",
        narrator,
    )


def check_docker_cli_installed(narrator: Narrator) -> CommandStatus:
    return check_cli_command_exists(
        "docker", ["which", "docker"], "Please install docker CLI manually.", narrator
    )
...
def execute_checks(
    ...
    check_name_to_func: Dict[str, Callable[[Narrator], CommandStatus]] = {
        ...
        "docker": check_docker_cli_installed,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Check whether all on-prem workstation dependencies are installed correctly.",
)
@click.option("-v", "--verbose", is_flag=True, default=False, help="Verbose output")
def check(no_config, verbose, output, workstation=False, prem=False):
    narrator = Narrator(verbose)

    if not workstation and not prem:
        check_names = [
            "ob_metaflow",
            "ob_extension",
            "metaflow_config",
