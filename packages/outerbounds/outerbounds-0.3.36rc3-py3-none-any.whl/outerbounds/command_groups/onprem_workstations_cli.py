...
class OnPremWorkstationClient:
    ...
    def ensure_container_running(self, workstation_id: str):
        ...
        for ws in self.existing_workstations.values():
            if ws["instance_id"] == workstation_id and ws["spec"]["on_prem"] == True:
                ...
                # start_container_if_not_running('ws-1010', "54.188.27.182", ws)
                return
    ...
    def start_container_if_not_running(
        ...
        with Connection(
            ...
            # Ensure all keys under ~/.ssh are copied
            # files_to_copy = get_ssh_files_to_copy_to_container()
            # for f_to_copy in files_to_copy:
            #     self.copy_to_remote_inside_docker(
            #         conn,
            #         f_to_copy["FileName"],
            #         f"/tmp/{f_to_copy['FileNameOnContainer']}",
            #         "/root/.ssh",
            #         workstation_id,
            #     )
            #     if f_to_copy["DeleteFromHostAfterCopy"]:
            #         os.remove(f_to_copy["FileName"])

            # self.exec_remote_command(
            #     "chmod_ssh_dir",
            #     f"docker exec {workstation_id} chmod 700 /root/.ssh",
            #     conn,
            # )
            # self.exec_remote_command(
            #     "chown_root",
            #     f"docker exec {workstation_id} chown root:root /root/.ssh -R",
            #     conn,
            # )
    ...
    def start_container(self, conn: Connection, workstation: dict):
        ...
        if existing_container is not None and existing_container["State"] == "running":
            ...
            container_run_command = f"docker run {env_file_flags} --name {workstation['instance_id']} -d --cpus='{workstation['spec']['cpu_cores']}' --memory='{workstation['spec']['memory_gb']}g' {gpu_sub_command} {workstation['spec']['image']} sh -c 'tail -f /dev/null'"
            self.exec_remote_command(
...
def list_onprem_workstations(
    ...
    try:
        on_prem_client = OnPremWorkstationClient(
            ...
            profile,
            admin_mode=all,
            init_with_container_status=use_container_status,
        )
        if filter_by_status == "":
            on_prem_client.command_response.add_or_update_data(
...
def get_list_of_env_vars_for_container(env_file_loc: str) -> dict[str, str]:
    ...
    if not os.path.isfile(env_file_loc):
        click.secho(
            f"Env file {env_file_loc} does not exist or is not a file",
            fg="red",
            err=True,
        )
        return env_vars
...
def file_name_has_allowed_extensions(
    file_name: str, allowed_extensions: List[str]
) -> bool:
    _, ext = os.path.splitext(file_name)
    return ext in allowed_extensions


def get_ssh_files_to_copy_to_container():
    ...
    if os.path.isfile(ssh_config_path):
        with open(ssh_config_path, "r") as f:
            ...
            with NamedTemporaryFile(
                dir=expanduser("~/.metaflowconfig"), delete=False, mode="w"
            ) as ntf:
                for line in ssh_config_lines:
                    if "UseKeychain" in line or "AddKeysToAgent" in line:
                        continue

                    if "IdentityFile" in line:
                        # parse location of identity file
                        id_file = line.strip().split(" ")[1]
                        filename_on_container = f'obp_{generate_random_suffix(length=5)}_{id_file.split("/")[-1]}'
                        if not expanduser(id_file).startswith(ssh_dir):
                            files_to_copy.append(
                                {
                                    "FileName": expanduser(id_file),
                                    "DeleteFromHostAfterCopy": False,
                                    "FileNameOnContainer": filename_on_container,
                                }
                            )
                        line = line.replace(
                            id_file, f"/root/.ssh/{filename_on_container}"
                        )
                ...
                files_to_copy.append(
                    {
                        "FileName": ntf.name,
                        "DeleteFromHostAfterCopy": True,
                        "FileNameOnContainer": "config",
                    }
                )

    if os.path.exists(ssh_dir):
        for key in os.listdir(ssh_dir):
            fq_path = os.path.join(ssh_dir, key)
            if (
                os.path.exists(fq_path)
                and file_name_has_allowed_extensions(fq_path, [".pem", ""])
                and not fq_path.endswith(".ssh/config")
            ):
                files_to_copy.append(
                    {
                        "FileName": fq_path,
                        "DeleteFromHostAfterCopy": False,
                        "FileNameOnContainer": key,
                    }
                )
