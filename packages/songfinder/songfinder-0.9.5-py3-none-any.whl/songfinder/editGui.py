...
class EditGui(object):
    def __init__(
        ...
        tagsSubPanel.pack(side=tk.TOP, fill=tk.X)
        ...
        self._inputTitle = inputFrame.TextField(
            titleSubPanel, width=20, text="Titre :", packing=tk.LEFT, state=tk.DISABLED
        )
        self._inputAuthor = inputFrame.TextField(
            titleSubPanel, width=10, text="Auteur :", packing=tk.LEFT, state=tk.DISABLED
        )
        ...
        self._inputCapo = inputFrame.TextField(
            chordsSubPanel, width=3, text="Capo: ", packing=tk.LEFT, state=tk.DISABLED
        )
        ...
        self._inputTags = inputFrame.TextField(
            tagsSubPanel, width=30, text="Tags: ", packing=tk.LEFT, state=tk.DISABLED
        )

        self._buttonSaveSong = tk.Button(
            buttonSubPanel, text="Sauver", command=self._saveSong, state=tk.DISABLED
        )
        buttonCreateSong = tk.Button(
            buttonSubPanel,
        ...
        self._songTextField = tk.Text(
            textSubPanel, width=width, height=height, undo=True, state=tk.DISABLED
        )
        self._songTextFieldScroller = tk.Scrollbar(
            textSubPanel, command=self._songTextField.yview
        )
    ...
    def _saveSong(self, event=None):  # pylint: disable=unused-argument
        if self._dataBase and self._printedElement.etype == "song":
            ...
            # pylint: disable=no-member
            self._printedElement.turfNumber = self._inputNumTurf.get(1.0, tk.END)
            # pylint: disable=no-member
            self._printedElement.hymnNumber = self._inputNumCCLI.get(1.0, tk.END)
            # pylint: disable=no-member
                ...
                if error.errno == errno.ENOENT:
                    try:
                        tkMessageBox.showerror(
                            "Erreur",
                            'Le chemin du chant "%s" n\'est pas valide. '
    ...
    def _changeSongState(self, event=None):  # pylint: disable=unused-argument
        ...
        if (
            (self._songTextField.edit_modified() or not notModified)
    ...
    def _syntaxHighlighting(self):
        ...
        guiHelper.coloration(
            self._songTextField, "blue", settings.GENSETTINGS.get("Syntax", "newline")
        )
        ...
        for newslideSyntax in settings.GENSETTINGS.get("Syntax", "newslide"):
            guiHelper.coloration(
                self._songTextField, "blue", newslideSyntax
            )  # TclError None
    ...
    def _createNewSong(self, copiedSong=""):
        if self._dataBase:
            ...
            if newSong not in self._dataBase or (
                newSong in self._dataBase
                and tkMessageBox.askyesno(
                    "Création",
                    'Le chant "%s" existe déjà, '
                ...
                if not self._saveSong():
                    ...
                    tkMessageBox.showinfo(
                        "Confirmation", 'Le chant "%s" a été créé.' % newSong.nom
                    )
    ...
    def printer(
        ...
        if toPrint:
            ...
            if loadDiapo and hasattr(self._diapoList, "load"):
                self._diapoList.load([toPrint], wantedDiapoNumber=1)
        else:
            self._printerOff()
