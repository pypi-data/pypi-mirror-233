...
class SQLFactory():    
    @reloadconfig
    def __init__(self,sql_connection:SQLConnection = SQLConnection.PORTAL,connectiondata= None,logger = None, config_path=None, autocommit=None, timeout=None):
        ...
        if not logger:
            self.logger = EasyPostLogger(name='SQLFactory', logpath= globals()['C_DEFAULT_LOG_PATH'], config_path=C_DEFAULT_CONFIG_PATH)
        else:
            self.logger = logger
