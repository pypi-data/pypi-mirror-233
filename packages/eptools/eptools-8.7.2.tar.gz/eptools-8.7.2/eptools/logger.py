...
def createRotatingLogger(name, log_path, level=logging.DEBUG, normal_format = iformat, error_format = eformat):
    handler = RotatingFileHandler(os.path.join(log_path , 'info.log'),backupCount=200,maxBytes=5000000)
    debug_handler = RotatingFileHandler(os.path.join(log_path , 'debug.log'),backupCount=200,maxBytes=5000000)
    error_handler = RotatingFileHandler(os.path.join(log_path , 'error.log'),backupCount=200,maxBytes=5000000)
    handler.setFormatter(normal_format)
    debug_handler.setFormatter(normal_format)
    error_handler.setFormatter(error_format)
    handler.setLevel(logging.INFO)
...
class EasyPostLogger():
    ...
    If 
    """
    def __init__(self, name: str=None, logpath: str=None, config_path: str=None) -> None:
