...
def Pixel_sorting(time,data,point_nb):
        ...
        photon_time = rand.uniform(0,time,size = len(data) )
...
def sorting(time,data,point_nb):
    ...
    for i in range(dim[0]):
          for j in range(dim[1]):
               signal[i,j] = Pixel_sorting(time,data[i,j],point_nb)
    return(signal)