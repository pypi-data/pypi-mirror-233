...
class CommandLine():
    ...
    def __init__(self, inOpts=None):
        ...
        self.parser.add_argument('-m', '--max', nargs='?', default=8, type=int, action='store',
                                 help='max kMer size ')
        self.parser.add_argument('-c', '--cutoffProb', nargs='?', type=float, default=-4.0, action='store',
     #
        ...
        self.parser.add_argument('-P', '--Palindrome', type=bool, nargs='?', const=True, default=False, action='store',
...
class Genome():
    ...
    def __init__(self, min=1, max=8, pseudo=1):
        ...
        for k in range(min, max + 1): # kMer size
            newKcounts = {''.join(kmer):pseudo for kmer in itertools.product(Genome.bases, repeat=k)}
    ...
    def addSequence(self,seq):
        ...
        for s in self.counts:
            if (len(s) >2 ) and (len(s) %2 == 1): # it's odd, so build a composite sum by adding over all center bases.
                flank = len(s) // 2
                newSeq = s[:flank] + 'N' + s[flank+1:] # build the composite
                composites[newSeq] = sum((self.counts[s[:flank] + b + s[(flank + 1):]] for b in Genome.bases))
                # build left and right k-1mers and central k-2mer
                composites[newSeq[:-1]] = sum((self.counts[s[:flank] + b + s[(flank + 1):-1]] for b in Genome.bases))
                composites[newSeq[1:]] = sum((self.counts[s[1:flank] + b + s[(flank + 1):]] for b in Genome.bases))
                composites[newSeq[1:-1]] = sum((self.counts[s[1:flank] + b + s[(flank + 1):-1]] for b in Genome.bases))
...
def main(myCommandLine=None):
    ...
    for seq,count in thisGenome.counts.items():
        ...
        if 'N' in seq: # this is a composite
            p = seq.find('N')
            if len(seq) != (p*2+1): continue # skip this one, it is a part of a composite
        elif len(seq) % 2 == 1: continue # odd sequences print only if they are a composite

        outArray.append([seq, count, thisGenome.E(seq), thisGenome.Evalue(seq), thisGenome.Zscore(seq)])
    ...
    for seq, count, E, eVal, Z in outArray:
        ...
        if (seq in thisGenome.counts) and (Z <= cl.args.cutoffProb) :
            ...
            if rc(seq) != seq: # this must not be a palindrome
                if cl.args.Palindrome: continue # dont print non palindromes
            elif not cl.args.Palindrome: # it is a palindrome so if we are printing everything give it a flag
                rcFlag = 'RC palindrome'

            print('{0:8}\t{1:0d}\t{2:0.2f}\t{3:0.2e}\t{4:0.2f}\t{5:13}'.format(seq, count,E, eVal, Z, rcFlag))
