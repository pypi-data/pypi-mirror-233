...
class pipeline:
    ...
    def __init__(self, genome_infile: str, plasmid_infile: str, outfile: str, z_score: float, pseudo: int, kMax: int, kMin: int, palindrome: bool = False) -> None:
    ...
    def load_stealth(self,z_score: float, pseudo: int, kMax: int, kMin: int, palindrome: bool = False):
    ...
    def hide_motif(self,region: list[SeqFeature.SimpleLocation]):    
        ...
        pattern_remover = ChromatoSeq.patternConstrainer(self.stealth_motifs,codon_freq)
    ...
    def writeOut(self):
        ...
        print(f"Removed {removed_motifs} of {total_motifs} motifs from {plasmid_name}. {remaining_motifs} motifs remain.", file= sys.stderr)    
...
def main():
    ...
    parser.add_argument(
        "--outfile", "-o", default= None, type=str, action="store", help="output filename, default = STDOUT"
    )
    parser.add_argument(
        "--plasmid", "-p", type= str, action="store", help="input plasmid file in GenBank format", required = True
    )
    parser.add_argument(
        "--zScore",
        "-z",
    ...
    dummy = pipeline(genome_infile, plasmid_infile, outfile, z_score, pseudo, kMax, kMin, palindrome)
