import torch
import numpy as np
from .baseline_tester import test_flash_attn, test_torch_naive
...
def tune(device=None, log_m_start=4, log_m_stop=10, verbose=0):
  if device is None:
    device = "cuda:0"
  ...
  # print(problematic)
  map_to_best_version = torch.zeros(num_gears, len(head_dims), 2, log_m_stop - log_m_start, dtype=torch.int)
      ...
      for is_causal_idx, is_causal in enumerate( (False, True) ):
        for log_m_idx, log_m in enumerate(range(log_m_start, log_m_stop)):
          for head_dim_idx, head_dim in enumerate(head_dims):
            ...
            query = torch.randn(1, m, num_heads, head_dim, device=device, dtype=torch.half) #* 0.1
            key = torch.randn(1, n, num_heads, head_dim, device=device, dtype=torch.half)  #* 0.1
            value = torch.randn(1, n, num_heads, head_dim, device=device, dtype=torch.half)  #* 0.1
            out = torch.zeros(1, m, num_heads, head_dim, device=device, dtype=torch.half) #* 0.1
            ...
            for v in range(num_kernel_versions):
              problem = [v, gear, head_dim, is_causal, m]
            ...
            # print(best_v)
            map_to_best_version[gear, head_dim_idx, is_causal_idx, log_m_idx] = best_v
            runtime = test_warpat(
