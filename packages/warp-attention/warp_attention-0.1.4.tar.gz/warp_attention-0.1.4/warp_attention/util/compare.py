...
from .baseline_tester import test_torch_naive, test_torch_sdpa, test_flash_attn
from .warpat_tester import test_warpat
from .util import mae, attn_flop
...
def compare(save_path, b, m, k, h, is_causal, ns=None, gears=None, device=None):
  ...
  results["flash attention 2"] = do(b, m, k, h, ns, is_causal, test_flash_attn)
  results["torch sdpa"] = do(b, m, k, h, ns, is_causal, test_torch_sdpa)

  for gear in gears:
    results[f"warp attention gear{gear}"] = do(b, m, k, h, ns, is_causal, test_warpat, gear=gear, version=-1)
  ...
  plt.legend()
  plt.title(f"{device_name} head_dim={k} query_seq_len={m} {'causal' if is_causal else ''}")
