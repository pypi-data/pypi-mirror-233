...
try:
  ...
  def _pick_best_version(
    ...
    if _map_to_best_version is not None:
      ...
      log_m_idx = log_m - _map_to_best_version["log_m_start"]
      head_dim_idx = _map_to_best_version["head_dims"].index(head_dim)

      version = _map_to_best_version["map_to_best_version"][gear, head_dim_idx, int(is_causal), log_m_idx]
  ...
  def warp_attention_forward(
    ...
    if version == -1:
      version = _pick_best_version(m, head_dim, gear, is_causal)
