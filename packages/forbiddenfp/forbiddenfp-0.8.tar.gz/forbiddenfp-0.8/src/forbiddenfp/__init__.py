...
def replace(from_str: str, to_str: str) -> Callable[[str], str]:
    return lambda s: s.replace(from_str, to_str)


def split(sep: str) -> Callable[[str], str]:
...
@chainable
def apply_if_false(self: _P, func: _F, predicate: _Pred = truthful) -> _P:
