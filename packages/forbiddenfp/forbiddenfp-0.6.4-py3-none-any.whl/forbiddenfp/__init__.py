...
@chainable
def map_pred(self: _P, pred_and_val: Dict[_Pred, _R], default: Optional[_R] = None) -> Optional[_R]:
    for pred, val in pred_and_val.items():
        if pred(self):
            return val
    return default
...
@chainable
def match_pred(self: _P, pred_and_action: Dict[_Pred, _R], default: _F = as_func(None)) -> Optional[_R]:
    for pred, action in pred_and_action.items():
        if pred(self):
            return action(self)
    return default(self)
...
def all_iter(self: Iterable[_P], predicate: _Pred = truthful) -> bool:
    return all(predicate(i) for i in self)


@chain_as("any")
def any_iter(self: Iterable[_P], predicate: _Pred = truthful) -> bool:
    return any(predicate(i) for i in self)
...
def filter_key(self: Dict[_P, _P2], predicate: Callable[[_P], bool]) -> Iterable[Tuple[_P, _P2]]:
    return ((k, v) for k, v in self.items() if predicate(k))
...
def last(self: Sequence[_P], predicate: _Pred = as_func(True)) -> Optional[_P]:
    return next((i for i in reversed(self) if predicate(i)), None)


@chain_as("next")
def first(self: Iterable[_P], predicate: _Pred = as_func(True)) -> Optional[_P]:
    return next((i for i in self if predicate(i)), None)


@chain_as("sum")
def sum_iter(self: Iterable[_P], of: Callable[[_P], Number] = identity, predicate: _Pred = truthful) -> int:
    return sum(of(i) for i in self if predicate(i))
