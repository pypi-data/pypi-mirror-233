...
def _get_urea_values(cycle: dict, inputs: list, term_id: str):
    ...
    return [0] if len(inputs) == 0 and _is_term_type_complete(cycle, {'termType': 'fertiliser'}) else values
...
def _should_run(cycle: dict):
    ...
    urea_share = get_country_breakdown(country_id, LOOKUPS['inorganicFertiliser'][0])
    uan_share = get_country_breakdown(country_id, LOOKUPS['inorganicFertiliser'][1])
