...
def _get_crop_property_average(cycle: dict, inputs: list, property_id: str):
    values = [get_node_property_value_converted(input, property_id) for input in inputs]
...
def _should_run(cycle: dict):
    ...
    total_feed = get_feed(cycle.get('inputs', []), termTypes=[
    ...
    should_run = all([total_feed, enteric_factor, DE, NDF])
