...
class LLMModel:
                ...
                if file_name.endswith('.pickle'):
                    ...
                    with open(file_path, 'rb') as file:
                        if file_name == 'decoder.pickle':
                            self.decoder_data = pickle.load(file)
                        elif file_name == 'encoder.pickle':
                            self.encoder_data = pickle.load(file)
                elif file_name == 'model.pt':
                    # Handle the model.pt file as needed
                    # You may use PyTorch's torch.load() function here
                    self.model = GPTLanguageModel(vocab_size=self.config_data["vocab_size"],n_embd = self.config_data["n_embd"],block_size = self.config_data["block_size"],n_layer=self.config_data["n_layer"],n_head=self.config_data["n_head"],dropout=self.config_data["dropout"])
  ...
  def generate(self,TextInput):
      context = torch.tensor(self.encoder_data(TextInput), dtype=torch.long, device=device)
      generated_chars = self.decoder_data(self.model.generate(context.unsqueeze(0), max_new_tokens=150)[0].tolist())
