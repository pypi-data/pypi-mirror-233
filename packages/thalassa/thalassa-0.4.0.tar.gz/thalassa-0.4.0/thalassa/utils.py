...
def get_thalassa_ds(no_nodes: int, no_periods: int = 10, seed: int | None = None) -> xr.Dataset:
    # Sanity check
    assert no_nodes >= 3, "no_nodes cannot be < 3"
    assert no_periods >= 2, "no_periods cannot be < 2"

    # Resolve random generator
    rng = np.random.default_rng(seed=seed)

    nodes = rng.uniform(-80, 80, (no_nodes, 2))
    triface_nodes = scipy.spatial.Delaunay(nodes).simplices
    time_range = pd.date_range("2023-01-01", periods=no_periods, freq="d")
    data_vars = {
        "depth": (("node",), rng.uniform(0, 1000, (no_nodes,))),
        "elev": (("node", "time"), rng.normal(1, 0.3, (no_nodes, no_periods))),
        "lon": (("node",), nodes[:, 0]),
        "lat": (("node",), nodes[:, 1]),
        "triface_nodes": (("triface", "three"), triface_nodes),
    }
    coords = dict(
        node=range(len(nodes)),
        triface=range(len(triface_nodes)),
        time=time_range,
    )
