"""RDSSource: SageWorks Data Source accessible through RDS Database"""
import pandas as pd
...
class RDSSource(DataSourceAbstract):
    """RDSSource: SageWorks Data Source accessible through RDS Database

    Common Usage:
        my_data = RDSSource(data_uuid, database="sageworks")
        my_data.summary()
        my_data.details()
    ...
    def __init__(self, data_uuid, database="sageworks", force_refresh: bool = False):
        """RDSSource Initialization
        Args:
            data_uuid (str): Name of RDS Table
            database (str): RDS Database Name (default: sageworks)
        ...
        # All done
        self.log.debug(f"RDSSource Initialized: {self.data_catalog_db}.{self.table_name}")
    ...
    def exists(self) -> bool:
        ...
        if self.catalog_table_meta is None:
            self.log.info(f"RDSSource {self.table_name} not found in SageWorks Metadata...")
            return False
        return True
    ...
    def column_names(self) -> list[str]:
        """Return the column names for this RDS Table"""
        return [item["Name"] for item in self.catalog_table_meta["StorageDescriptor"]["Columns"]]

    def column_types(self) -> list[str]:
        """Return the column types of the internal RDSSource"""
    ...
    def query(self, query: str) -> pd.DataFrame:
        ...
        self.log.info(f"RDS Query successful (scanned bytes: {scanned_bytes})")
        return df
    ...
    def athena_test_query(self):
        """Validate that RDS Queries are working"""
        ...
        scanned_bytes = df.query_metadata["Statistics"]["DataScannedInBytes"]
        self.log.info(f"RDS TEST Query successful (scanned bytes: {scanned_bytes})")
    ...
    def outliers(self, scale: float = 1.5, use_stddev=False, recompute: bool = False) -> pd.DataFrame:
    ...
    def details(self, recompute: bool = False) -> dict[dict]:
        """Additional Details about this RDSSource Artifact
        Args:
            recompute(bool): Recompute the details (default: False)
        Returns:
            dict(dict): A dictionary of details about this RDSSource
        """
    ...
    def delete(self):
        ...
        if not self.exists():
            self.log.warning(f"Trying to delete a RDSSource that doesn't exist: {self.table_name}")
...
if __name__ == "__main__":
    """Exercise the RDSSource Class"""

    # Retrieve a Data Source
    my_data = RDSSource("abalone_data")

    # Verify that the RDS Data Source exists
    ...
    my_data = RDSSource("does_not_exist")
