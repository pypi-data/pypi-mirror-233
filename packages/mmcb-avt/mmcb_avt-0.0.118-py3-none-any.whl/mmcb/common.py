...
class Packet:
    ...
    * An outbound IV, IT, and return IV
    ...
    @staticmethod
    def _unpack_environmental_data(data):
        """
        This software is written to accumulate environmental data
        (temperature/humidity) from all Yoctopuce sensors that acquire those
        parameters. If there is only one sensor, there is little value in
        tagging the values with the sensor they came from.
        ...
        ----------------------------------------------------------------------
        returns : list of dict (unmodified input argument) or list of values
        (if just one sensor)
            e.g.
                [46.2, 46.0, ... 46.0, 46.0]
        ----------------------------------------------------------------------
    ...
    def write_json(self, session):
        ...
        """
        prefix = f'{session}_{self.model}_{self.serial_number}'
        filename = f'{prefix}_{self.channel}' if self.channel else prefix
        filename = f'{filename}.json'

        osvo, rsvo = list_split(self.set_voltage)
        omcu, rmcu = list_split(self.measured_current)
        omvo, rmvo = list_split(self.measured_voltage)
        omti, rmti = list_split(self.measured_timestamp)
        omte, rmte = list_split(self.measured_temperature)
        omhu, rmhu = list_split(self.measured_humidity)

        tstart = min(omti)

        # Convert current measurements to uA
        for var in [omcu, rmcu, self.hold_current]:
            var[:] = [x * 1000000 for x in var]

        # Convert timestamps to start from zero
        for var in [omti, rmti, self.hold_timestamp]:
            ref_time = min(var)
            var[:] = [x - ref_time for x in var]
            ...
            json.dump(
                {
                    ...
                    'ident': self.ident,
                    'itk_module_serial_number': self.itk_serno,
                    'test_start_timestamp_utc': datetime.datetime.utcfromtimestamp(tstart).isoformat(),

                    'outbound_set_voltage_V': osvo,
                    'return_set_voltage_V': rsvo,
                    'outbound_measured_current_uA': omcu,
                    'return_measured_current_uA': rmcu,
                    'outbound_measured_voltage_V': omvo,
                    'return_measured_voltage_V': rmvo,
                    'outbound_measured_timestamp_s': omti,
                    'return_measured_timestamp_s': rmti,
                    'outbound_measured_humidity_rhpc': self._unpack_environmental_data(omhu),
                    'return_measured_humidity_rhpc': self._unpack_environmental_data(rmhu),
                    'outbound_measured_temperature_degC': self._unpack_environmental_data(omte),
                    'return_measured_temperature_degC': self._unpack_environmental_data(rmte),

                    'hold_voltage_V': self.hold_voltage,
                    'hold_current_uA': self.hold_current,
                    'hold_timestamp_s': self.hold_timestamp,
                    'hold_temperature_degC': self._unpack_environmental_data(self.hold_temperature),
                    'hold_humidity_rhpc': self._unpack_environmental_data(self.hold_humidity),
                },
                outfile,
            )
...
def cache_read(device_types=None, uselog=False):
    """
    ...
    if cache is None:
        _logprint(uselog, logging.CRITICAL,
    ...
    for device_type, device_details in cache.items():
        ...
        for device in device_details[1]:
            ...
            _logprint(uselog, logging.INFO,
                      f'cache: {manufacturer} {model} serial number {snu} on {port}')

    if not found:
        _logprint(uselog, logging.CRITICAL,
                  'cache did not contain any matching devices')
        sys.exit()
...
def rate_limit(timestamp, duration):
    ...
    args
        timestamp : float
            value previously returned by time.time()
        duration : float
            nominal delay time in seconds required to not exceed the desired
            volts-per-second rate limit
    --------------------------------------------------------------------------
    returns : float
        value returned by time.time(), this value will be returned to this
        function at the next call as argument timestamp
    --------------------------------------------------------------------------
    """
    if timestamp is not None:
        tdiff = time.time() - timestamp
        if tdiff < duration:
