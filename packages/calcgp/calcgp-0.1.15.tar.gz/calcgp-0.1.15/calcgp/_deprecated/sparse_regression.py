...
class SparseGradient(SparseGPRBase):
    '''A sparsified model that predicts gradient values from observations of the function.
    '''
    def train(self, X_data, Y_data):
...
class SparseIntegral(SparseGPRBase):
    '''A sparsified model that predicts function values from observations of the gradient.
    '''
    def train(self, X_data, Y_data):
        if isinstance(X_data, Tuple):
            self.prior = Prior(sparse=True, mode=DataMode.MIX)
...
class SparseFunction(SparseGPRBase):
    '''A sparsified model that predicts function values from observations of the function.
    '''
    def train(self, X_data, Y_data):
        self.prior = Prior(sparse=True, mode=DataMode.FUNC)
