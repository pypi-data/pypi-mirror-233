...
dev_key = "Здесь должен быть ваш dev_key"
out_format = "json"
vendor = "edu"
...
class Student:
    __slots__ = (
        "auth_token", "student_num", "name", "full_school_name", "student_class", "current_period", "all_periods",
        "city")
    ...
    async def auto_fill_data(self) -> bool:
        """
...
Заполняемые поля: student_num, name, full_school_name, student_class, current_period, all_periods, city)
        ...
        """
        result = await self.get_rules()
        result_ = await self.get_periods()
    ...
    async def get_periods(self) -> list:
        """
    ...
    async def get_diary(self, start_date: datetime.date = None, end_date: None = None) -> list:
        ...
        if response['state'] == 200:
            ...
            if response['result']['students']:
                ...
                return all_lessons, all_homeworks
            else:
                raise exceptions.AbsenceStudentNumber()
        else:
            raise exceptions.ServerAnswerError(response['error'])
    ...
    async def _convert_to_message(self, data: dict) -> Message:
        ...
        return Message(id_, subject, full_date, self, text, user_from)
