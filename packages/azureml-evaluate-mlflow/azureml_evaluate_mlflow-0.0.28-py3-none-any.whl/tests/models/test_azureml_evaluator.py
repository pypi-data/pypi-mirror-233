...
def test_image_classifier_evaluation(image_mc_classification_model_uri, image_net):  # noqa: F811
    ...
    expected_metrics = compute_metrics(task_type=constants.Tasks.CLASSIFICATION, y_test=y, y_pred=predicted_labels)
...
def test_image_classifier_multilabel_evaluation(image_ml_classification_model_uri, fridge_multi_label):  # noqa: F811
    ...
    y_pred = model.predict(fridge_multi_label.features_data)
    y_pred_probs = y_pred[EvaluationMiscLiterals.IMAGE_OUTPUT_PROBS_COLUMN].to_list()
    y_labels = y_pred[EvaluationMiscLiterals.IMAGE_OUTPUT_LABEL_COLUMN].to_list()
    ...
    for probs, labels in zip(y_pred_probs, y_labels):
        ...
        for index, pred in enumerate(probs):
            if pred >= EvaluationDefaultSetting.MULTI_LABEL_PRED_THRESHOLD:
    ...
    expected_metrics = compute_metrics(
        task_type=constants.Tasks.CLASSIFICATION, y_test=y, y_pred=predicted_labels, **{"multilabel": True}
    )
    for metric_key, expected_metric_value in expected_metrics["metrics"].items():
        if np.isscalar(expected_metric_value) and np.isnan(expected_metric_value) == False:
            assert np.isclose(
