...
class AMLClassifierModel(AMLGenericModel):
    def __init__(self, model_meta, model_impl):
        ...
        if not hasattr(self._model_impl, "predict_proba"):
            raise MlflowException("Model implementation is missing required predict_proba method.")
...
def load_model(
    ...
    if not suppress_warnings:
        _warn_dependency_requirement_mismatches(local_path)  # Can this be common for pyfunc and azureml?
    ...
    try:
        ...
        try:
            _logger.info("Unable to load using aml flavor. Trying out pyfunc", str(e))
            if _support_kwargs_for_load():
