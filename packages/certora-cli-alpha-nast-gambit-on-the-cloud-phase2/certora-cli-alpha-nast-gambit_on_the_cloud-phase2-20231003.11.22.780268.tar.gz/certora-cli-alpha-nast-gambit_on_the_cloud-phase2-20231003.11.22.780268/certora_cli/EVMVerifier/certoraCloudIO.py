...
from EVMVerifier.certoraExtensionInfo import ExtensionInfoWriter
...
def validate_version_and_branch(branch: Optional[str], commit_sha1: Optional[str]) -> None:
    ...
    if not installed:
        if not (branch or commit_sha1):
            old_api_branch_message = "--cloud BRANCH or --staging BRANCH,"
            new_api_branch_message = "--server PLATFORM [--prover_version BRANCH,]"
            branch_msg = new_api_branch_message if Util.is_new_api() else old_api_branch_message
            raise Util.CertoraUserInputError(
                "No package is installed. You must use the certoraRun script with either "
                f"{branch_msg}"
    ...
    try:
        ...
        if "." in latest and "." in version:
            ...
            major_upgrade_msg = \
                ...
                f" This is a new major version. It is recommended to upgrade by running: {installation_cmd}" \
                f" This is a new major version, potentially breaking backward compatibility." \
                f" Refer to the official documentation before proceeding with the upgrade: docs.certora.com"

            recommended_minor_upgrade_msg = \
                f"You are using {package_name} version {version}; however, version {latest} is available."\
                f" It is recommended to upgrade by running: {installation_cmd}"

            beta_upgrade_message = \
                f"You are using beta version {package_name}; however, version {latest} is available. " \
            ...
            compare_cloud_versions_check(local_main, local_sub, local_patch, remote_main, remote_sub, remote_patch,
                                         package_name, recommended_minor_upgrade_msg, major_upgrade_msg,
                                         beta_upgrade_message)
...
def validate_version_and_branch_new_api(context: CertoraContext) -> None:
    """
    Gets the latest package version and compares to the local package version.
    If the major version is different - i.e. there is new breaking syntax, will raise an error.
    If a minor version is different, it just warns the user and recommends him to upgrade the package.
...
class CloudVerification:
    ...
    def prepare_auth_data(self, cl_args: str) -> Optional[Dict[str, Any]]:
        ...
        # Note: branch and git_hash are mutually exclusive the cannot both set in this method
        if Util.is_new_api():
            auth_data["branch"] = self.context.prover_version if self.context.prover_version else ''
        else:
        ...
        if Util.is_new_api():
            ...
            if self.context.settings is not None:
                for settings_exp in self.context.settings:
    ...
    def print_output_links(self) -> None:
        if Ctx.is_staging(self.context):
            print(f"Follow your job at {Util.SupportedServers.STAGING.value}")
            print(f"If the job does not yet appear at the dashboard, follow it here {self.statusUrl}")
        else:
            print(f"Follow your job at {Util.SupportedServers.PRODUCTION.value}")
    ...
    def __compress_and_upload_zip_files(self) -> bool:
        ...
        # create new zip file
        if self.context.mode == Util.Mode.TAC:
            # We zip the tac file itself
            result = compress_files(self.ZipFilePath, self.context.files[0], short_output=self.context.short_output)
        elif self.context.mode == Util.Mode.BYTECODE:
            ...
            result = compress_files(self.ZipFilePath, *files_list, short_output=self.context.short_output)
    ...
    def get_domain(self) -> str:
        ...
        if Util.is_new_api():
            ...
            if Ctx.is_staging(self.context):
                domain = Util.SupportedServers.STAGING.value
            else:
                domain = Util.SupportedServers.PRODUCTION.value
        return domain
