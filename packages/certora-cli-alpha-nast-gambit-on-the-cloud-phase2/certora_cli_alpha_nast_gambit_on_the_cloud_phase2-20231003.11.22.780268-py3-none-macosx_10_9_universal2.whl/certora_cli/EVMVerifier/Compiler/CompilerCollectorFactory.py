...
def get_extra_solc_args(contract_path: Path, context: CertoraContext) -> str:
    ...
    if is_new_api():
        extra_solc_args += __solc_args_to_str(context)
...
class CompilerCollectorFactory:
    ...
    def __init__(self, context: CertoraContext, compiler_args: list, optimize_map: Dict[str, str],
                 default_compiler: str, compiler_mappings: Dict[str, str], config_path: Path):
...
def __solc_args_to_str(context: CertoraContext) -> str:
    args = []
