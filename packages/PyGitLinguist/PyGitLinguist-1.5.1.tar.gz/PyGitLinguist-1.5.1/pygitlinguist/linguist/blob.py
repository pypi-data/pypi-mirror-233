import os
from .blob_helper import BlobHelper  # adjust the import according to your project structure
import chardet
...
class Blob:
    ...
    def from_path(self):
        with open(self.path, 'rb') as file:
            f = file.read()
            encoding_ = chardet.detect(f)['encoding']
            if encoding_:
                self.content = f.decode(encoding_)
        if not self.content:
