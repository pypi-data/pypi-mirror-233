...
class RESTObject(object):
    ...
    def __init__(self, manager, attrs, *, created_from_list=False):
        ...
        self.__dict__["_parent_attrs"] = self.manager.get_parent_attrs
...
class RESTManager(object):
    ...
    def __init__(self, csg: CSGenome, parent=None) -> None:
        ...
        """
        self.csgenome = csg
        self._parent = parent  # for nested managers, NOTE: might come in handy with country to systems
        # self._computed_path = self._compute_path()
        self._computed_path = None

    @property
    def get_parent_attrs(self):  # a getter since the private field
    ...
    @property
    def get_path(self):  # getter for getting the computed path
        return self._computed_path


class Inequality:
    """
    Inequality class provides support for filtering int, float, and date fields for 
    tables by some sort of inequality, such as all records with a date greater than
    than 2005-11-05.
    """
...
class ge(Inequality):
    """
    Specify that an int/float/date column is filtered such that it is greater than 
    or equal to the given value. For dates, use a string value in format "%Y-%m-%d".
    If a search is supplied a list of Inequalities, all Inequalities are applied
    to the filter.
    """
...
class le(Inequality):
    """
    Specify that an int/float/date column is filtered such that it is less than or
    equal to the given value. For dates, use a string value in format "%Y-%m-%d".
    If a search is supplied a list of Inequalities, all Inequalities are applied
    to the filter.
    """
...
class gt(Inequality):
    """
    Specify that an int/float/date column is filtered such that it is greater than 
    the given value. For dates, use a string value in format "%Y-%m-%d". If a search 
    is supplied a list of Inequalities, all Inequalities are applied to the filter.
    """
...
class Range(Inequality):
    """
    Specify that an int/float/date column is filtered such that it is between the
    two supplied values, inclusive. For dates, use a string value in format "%Y-%m-%d".
    """

    def __init__(self, lvalue: Union[int, float], rvalue: Union[int, float]):
        self.lvalue = lvalue
        self.rvalue = rvalue
...
class Order:
    """
    Order class provides support for ordering by certain columns of a table either
...
class desc(Order):
    """
    Specify that a column should be ordered in descending order
    """
...
class asc(Order):
    """
    Specify that a column should be ordered in ascending order (this is default)
    """
...
class CSGError(Exception):
    """Exception raised for errors in using the csgapi.
    ...
    def __init__(self, response: Dict | str):
        if isinstance(response, str):
            ...
            self.message = errors['message'] if 'message' in errors else errors
            self.response = response
            super().__init__(
                f'{self.message}\n{pprint.pformat(self.response, 4)}')
