...
__all__ = [
    "HasOrganization",
    "HasOrganizationManager",
]


class HasOrganization(SaveMixin, RESTObject):
    pass


# NOTE: even tho Delete Mixin is integrated, since has_processor is a look up table,
# our API does not support DELETE for it
class HasOrganizationManager(GetMixin, CreateMixin, UpdateMixin, ListMixin, RESTManager):
    _path = "has_organization"
    _obj_cls = HasOrganization
    _create_attrs = RequiredOptional(required=("organization", "contributor"))
    ...
    def get(self, uid=None, params=None):
        ...
        # in each obj
        return cast(HasOrganization, super(HasOrganizationManager, self).get(uid=uid, params=params))
