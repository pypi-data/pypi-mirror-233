...
class CopyrightFPD:
    ...
    def get_clutter(self, data, predictions):
        clutter_data = []
        for sentence, prediction in zip(data, predictions):
            if prediction == 'f':
                ...
                entities = self.declutter_model(sentence).ents
                for ent in entities:
                    sentence_text = sentence_text.replace(ent.text, '', 1)

            clutter_data.append(sentence_text)
        return clutter_data
