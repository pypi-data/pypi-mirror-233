...
class Plugin(with_metaclass(PluginMeta, models.Plugin)):
    ...
    def __del__(self):
        # Include deactivation for backwards compatibility
...
def from_path(fpath, **kwargs):
    ...
    if fpath.endswith('.py'):
        ...
        for plug in from_module(module=module, root=root, info=None, **kwargs):
            plugins.append(plug)
    else:
        info = parse_plugin_info(fpath)
        plugins.append(from_info(info, **kwargs))
    return plugins
    

def all_from_folder(folders, **kwargs):
    plugins = []
    for fpath in find_plugin_paths(folders):
        plugins += from_path(fpath, **kwargs)
    return plugins
...
def from_module(module, root, info, **kwargs):
    ...
    try:
        ...
        logger.error(f"Failed to load {module} @ {root} info={info}")
        raise
