...
def _run_plugin(
    ...
    plugin.unsubscribe(observer=layout)  # pyright: ignore
...
class StageProcessor:  # pylint: disable=too-few-public-methods
    ...
    def _get_stage_plugins(self) -> list[Plugin]:
        ...
        results = jmespath.search(
            ...
            data=self.context.consolidated_sbom.dict(by_alias=True),
        )
        ...
        with (Path(self.context.collect_root_dir) / "generic" / "_metadata_" / "_run_data_").open(
            ...
            for plugin_ref in set(self.plugin_ref_list):
                ...
                plugin_needed: bool = len(used_purl_types.intersection(instance.supported_purl_types)) > 0

                if len(instance.supported_purl_types) == 0 or plugin_needed:
                ...
                for supported in instance.supported_purl_types:
                    supported_purls = jmespath.search(expression=f"[? starts_with(@, 'pkg:{supported}')]", data=results)
                    layout.add_job(type(instance).__name__, total=len(supported_purls))
