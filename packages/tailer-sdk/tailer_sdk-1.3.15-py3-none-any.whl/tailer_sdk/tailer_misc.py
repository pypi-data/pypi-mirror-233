    ...
    try:
        ...
        for variable in result:
            try:
                ...
                payload = payload.replace("""\"{{""" + variable + """}}\"""", json.dumps(context_data["parameters"][variable]["value"], indent=4))
            except Exception:
                pass
        ...
        for variable in result:
            ...
            try:
                ...
                if context_data["parameters"][variable]["type"] in ["string", "integer", "float"]:
                    payload = payload.replace("""{{""" + variable + """}}""", json.dumps(context_data["parameters"][variable]["value"], indent=4).replace("\"",""))
                else:
