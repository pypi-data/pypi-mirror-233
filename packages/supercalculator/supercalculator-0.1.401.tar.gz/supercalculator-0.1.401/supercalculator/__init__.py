...
def multiplication(value1 : str, value2 : str) -> str:
    ...
    value1 = __remove_following_0(value1)
    value2 = __remove_following_0(value2)
...
def exponent(value : str, exp : str) -> str:
    ...
    Cannot have decimal exponents of negative exponents yet!!
...
def __calc_lim(parsed_equation : dict, x_goes_towards : str, side, precision : int = 50) -> str:
    ...
    lim_mprecise = __calculate(copy.deepcopy(parsed_equation), subtraction(x_goes_towards, "0."+"0"*(precision + 5)+"1"))

    lim_out = __remove_trailing_0(_round(lim, round(precision / 2)))
    lim_out_mprecise = __remove_trailing_0(_round(lim_mprecise, round(precision / 2)))
...
def limit(equation, x_goes_towards : str, precision : int = 100) -> str:
    ...
    try:
        ...
        out = __calculate(copy.deepcopy(parsed_equation), t_x_goes_towards, lim_x_vals=lim)
    ...
    if x_goes_towards[-1] == "+":
        ...
        if upper_lim[0] == "-" and upper_lim.split(".")[0][1:] == "0":
            upper_lim = "0."
        if lower_lim[0] == "-" and lower_lim.split(".")[0][1:] == "0":
            lower_lim = "0."
...
def __calculate(equation : dict, x_val : str = None, lim_x_vals : dict = {}) -> str:
    ...
    t = list(collections.OrderedDict(sorted(solved_vals.items())).items())[0][-1]
    return t
