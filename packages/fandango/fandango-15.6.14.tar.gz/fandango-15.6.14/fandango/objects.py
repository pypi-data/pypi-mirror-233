...
# Python 2-3 conundrums
try:
    from operator import isCallable, isSequenceType
except:
    isCallable = callable
    isSequenceType = isSequence
...
def dir_module(module):
    """
    instead of dir(module) that returns everything imported in a file
    this dirModule() method retuns only the methods explicitly declared
    in the module file
    """
    return [
        a
        for a, v in list(module.__dict__.items())
        if getattr(v, "__module__", "") == module.__name__
    ]

dirModule = dir_module


def find_module(module):
    """
    returns the path to a python module
    """
    import imp

    if "." not in module:
        try:
            r = imp.find_module(module)[1]
        except Exception as e:
            try:
                r = imp.find_module(module,['/usr/lib/python2.7/dist-packages'])[1]
            except:
                traceback.print_exc()
                raise e
        return r
...
def load_module(module, modulename=None):
    """
    Loads a python module either from source file or from module
    """
    if py3:
        import importlib
        if '/' in module or '.py' in module:
            if not modulename:
...
def get_module_name(o):
    """
    returns module name of an object
    """    
    import inspect
    if isString(o):
        o = locals().get(o,o)
...
def dir_classes(module, owned=False):
    v = [a for a, v in list(module.__dict__.items()) if isinstance(v, type)]
    if owned:
        return [a for a in dirModule(module) if a in v]
    else:
        return v
...
def get_code(obj, deep=True):
    import inspect
    try:
        f = inspect.getsource(obj)
        return Struct(
            {
                "header": ",".join(
                    s.strip() for s in f.split(":")[0].replace("\n", "").split(",")
                ),
                "doc": obj.__doc__,
                "source": f,
                "children": deep
                and dict(
                    ...
                    if not k.startswith("_") and isCallable(v)
                ),
            }
        )
    except:
        return Struct(header=str(obj), doc="", source="", children=None)
...
class Struct(object):
    ...
    Creator arguments should be either empty, a dictionary or keyword arguments.
    ...
    def load(self, *args, **kwargs):
        ...
        if isSequence(dct) and len(dct) and not isSequence(dct[0]):
            dct = dict.fromkeys(dct)  # isDictionary also matches items lists
        [
            setattr(self, str(k), v)
            for k, v in (dct.items() if hasattr(dct, "items") else dct)
        ]
        
    def translate(self, key):
        k = str(key)
        if not clmatch('[_a-zA-Z]*[a-zA-Z_0-9]*$',k):
            k = '_' + str(key)
        return k
    ...
    def __call__(self, *args, **kwargs):
        ...
        elif len(args) == 1 and isString(args[0]):
    ...
    def to_str(self, order=None, sep=","):
        ...
        return (
            self.__str__()
            if order is None
            else (sep.join("%s" % self[k] for k in order))
        )
...
def make_property(var, fget=_fget, fset=_fset, fdel=_fdel):
    ...
    return property(
        partial(fget, var=var),
        partial(fset, var=var),
        partial(fdel, var=var),
        doc="%s property" % var,
    )
...
def NamedProperty(name, fget=None, fset=None, fdel=None):  # ,doc=None):
    ...
    """
    return property(
        partial(fget, var=name) if fget else None,
        partial(fset, var=name) if fset else None,
        partial(fdel, var=name) if fdel else None,
        doc=name,
    )
...
def NewClass(classname, classparent=None, classdict=None):
    """
    Creates a new class on demand just by merging an existing class with
    a dict containing new methods:
...
class Object(object):
    ...
    Objects inheriting from fandango.Object will be able to call any __init__ method
    from any parent class disregarding of the inheritance order.
    """
        ...
        for base in klass.__bases__:
            if "call__init__" in dir(base) and (
                ...
                try:
                    ...
                    print(
                        "Unable to execute %s.__init__!: %s" % (base.__name__, str(e))
                    )
        return
...
class Singleton(object):
    ...
    The __new__ method is overridden to force Singleton behaviour,
    the Singleton is created for the lowest subClass.

    @warning although __new__ is overridden __init__ is still being called
    for each instance=Singleton(), this is way we replace it by __dumb_init
    """
...
if py3:
    def decorates(func_org):
        """
        This decorator applies functools.wraps and adds a list of
        decorator methods

        Without argument, it returns decorator list.
        """
        def fff(func_decorator=None):
            if func_decorator is not None:
                @functools.wraps(func_org)
                def func_decorated(*args,**kwargs):
                    return func_decorator(*args,**kwargs)
                if isinstance(func_org, FunctionType):
                    if not hasattr(func_org,'func_decorators'):
                        #func_org.func_decorators = list()
                        setattr(func_org,'func_decorators',list())
                    func_decorated.func_decorators = func_org.func_decorators
                    func_decorated.func_decorators.append(func_decorator)
                return func_decorated
            elif hasattr(func_org,'func_decorators'):
                return func_org.func_decorators
            else:
                return []
        return fff
else:
    decorates = functools.wraps


def decorate(f,decorator,once=True):
    """
    This method only make sense if it allows to decorate instantiated methods
    But, it doesn't, so it is useless
    """
    if once and decorator in getattr(f,'func_decorators',[]):
        return f
    decorator = decorates(f)(decorator) #(f))
    decorated = decorator(f)
    #if isinstance(f,MethodType):
        ## FORGOT IT, METHOD TYPES CANNOT BE OVERRIDEN IN PYTHON3!
        ##o = f.im_self or f.im_class # This fails in python3!
        #o = getattr(f,'__self__',type(f)) #Instance or type?
        #print('%s.%s => %s(%s)' % (o,f.__name__,decorator,f))
        #setattr(o,f.__name__,ff)
    return decorated


class Decorator(object):
    """
    Class from which decorators can inherit, thus providing a way to 
    know if a function is decorated or is a decorator by calling
    issubclass(Decorator) or isinstance(Decorator)
    """
...
def decorate_with_context(context_type,context_kwargs=None):
    def fff(f,context_type=context_type,context_kwargs=context_kwargs):
       context_kwargs = context_kwargs or {}
       @functools.wraps(f)
       def ff(*args,**kwargs):
          with context_type(**context_kwargs) as c:
              return f(*args,**kwargs)
       return ff
    return fff
...
class Decorated(object): 
    """
    Abstract class to provide an API to get all decorators
    applied to a python object and its methods
    """
    #@TODO not implemented, but used in fandango.qt
    pass
...
class Decorator(object):
    ...
    def new_wrapped_instance(cls, *args, **kwargs):
        ...
        i = object.__new__(
            type(cls.__name__ + "_" + func.__name__, (cls,), {"__doc__": func.__doc__})
        )
...
class Cached(Decorator):
    ...
    You can setup an expire date for the cache, so it is recalculated when
    time period passes.
    """

    __example__ = """
    
    e.g.: check_device_cached = Cached(check_device,depth=10,expire=3)
    ...
    def __init__(self, func=None, depth=10, expire=3.0, log=False, catched=False):
        self.log = log
        self._im = None
        self.cache = {}
        self.depth = depth
    ...
    def _log(self, msg, force=False):
        if isCallable(self.log):
            ...
            print('Cached(%s).execute() exception: \n%s' % (self.func, msg))

    @staticmethod
    def getCachedObject(obj, methods=[], depth=10.0, expire=3.0, catched=False):
        """@RISKY
...
def Queued(my_queue):
    ...
    def QueuedDecorator(method, use_id=False):
        def QueuedFunction(*args, **kwargs):
            ...
            if use_id:
                r = (str(method) + str(args) + str(sorted(kwargs.items())), r)
            my_queue.put(r)
