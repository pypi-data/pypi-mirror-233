...
def shell_command(*commands, **keywords):
    ...
    @param decode controls result conversion to str (utf-8 by default)
    ...
    decode = keywords.get("decode","utf-8" if fun.py3 else False)
    import subprocess
    ...
    if len(commands) > 1:
        for comm in commands[1:]:
            ps = subprocess.Popen(
                comm, shell=True, stdin=process[-1].stdout, stdout=subprocess.PIPE
            )
            process.append(ps)  # comm1 | comm2 | comm3 > result
    result = process[-1].communicate()
    result = (
        result and len(result) >= 1 and result[0]
    ) or None  # process returns a tuple, being stdout the first field

    try:
        result = bytes.decode(result,decode) if decode else result
    except:
        pass
...
def sendmail(subject, text, receivers, sender="", attachments=None, trace=False):
    ...
    if trace:
        print("Sending mail to %s" % receivers)
...
def get_memstats(units="m"):
    ...
    txt = txt.split("\n")
    memstats = dict(
        list(zip(txt[0].strip().split(), list(map(int, txt[1].strip().split()[1:]))))
    )
    return memstats
...
def get_memory_usage():
    ...
    return 1 - (old_div(mfree, stats["total"]))
...
def get_process_memory(pid=None, virtual=False):
    ...
    try:
        ...
        if psutil is not None:
            ...
            mem, units = (
                shell_command(
                    "cat /proc/%s/status | grep Vm%s"
                    % (pid, "Size" if virtual else "RSS")
                )
                .lower()
                .strip()
                .split()[1:3]
            ...
            units = (
                ("k" in units and 1e3)
                or ("m" in units and 1e6)
                or ("g" in units and 1e9)
                or 1
            )
...
def get_process_pid(include="", exclude="grep|screen|kwrite"):
    ...
    ps = shell_command(
        'ps ax | grep -E "%s"' % include
        + (' | grep -viE "%s"' % exclude if exclude else "")
    )
    if not ps:
    ...
    if len(pids) > 1:
        raise Exception(
            "Multiple PIDs found: please refine your search using exclude argument"
        )
    return pids[0]
...
def check_process(pid):
    try:
        return file_exists("/proc/%s" % pid)  # os.kill(pid,0)
    except:
        return False
...
def findfolders(target="", parent="", filter_=True, printout=False):
    ...
    if filter_:
        filter_folders = lambda fs: [
            f
            for f in fs
            if f not in ".svn tags branches".split()
            and (f.count("/") < 6 or not f in "trunk xpand doc".split())
        ]
    else:
        filter_folders = lambda fs: fs

    def get_folders(path):
        folders = [
            "%s/%s" % (path, f)
            for f in filter_folders(linos.listdir(path, folders=True))
            if not stat.S_ISLNK(os.lstat("%s/%s" % (path, f)).st_mode)
        ]
        for f in folders:
...
@fun.Cached(depth=1000, expire=300.0)
@log.Debugged
...
def ping(ips, threaded=False, timeout=1):
    ...
    def _ping(ip):
        return not subprocess.call(
            "ping -c 1 -w %d %s" % (int(max((1, timeout))), ip),
            shell=True,
...
def sysargs_to_dict(
    ...
    if args is None:
        args = sys.argv[1:]
    if trace:
        print("sysargs_to_dict(%s,%s)" % (args, defaults))
    ...
    while i < e:
        ...
        if lazy and splitter in a:
            ...
            # -o or -oXcV
            if multiflag and splitter not in a:
                a = list(a.strip("-"))
