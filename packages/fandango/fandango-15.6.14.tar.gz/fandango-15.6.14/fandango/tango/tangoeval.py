...
from fandango.tango.methods import *
from fandango.tango.search import *
...
class FloatFakeCall(float):
    # placeholder for method that just returns a fixed value
    def __call__(self):
        return self
...
def getTangoValue(obj, device=None):
    ...
    try:
        ...
        if p:
            device, host = p["device"], p["host"] + ":" + p["port"]
        else:
            host = get_tango_host()
        if type(obj) is str:
        ...
        try:
            domain, family, member = device.split("/")[-3:]
        except:
        ...
        Type = (
            Type
            if (Type not in (bool, None, type(None)) and ty != CmdArgType.DevState)
            else int
        )
...
class TangoEval(object):
    ...
    rewhat = (
        "(?:(?:\\.)(?P<what>quality|time|value|exception|delta|all|"
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.proxies = proxies or ProxiesDict()
        ...
        if attributes:
            ...
            if self.use_events:
                proxy = lambda a: CachedAttributeProxy(
                    a,
            ...
            self.attributes = fn.CaselessDefaultDict(proxy)

        # Keeps last values for each variable
        self.previous = fn.CaselessDict()
        # Keeps values from the last eval execution only
        self.last = fn.CaselessDict()
        self.cache_depth = cache
        self.cache = (
            fn.CaselessDefaultDict(lambda k: list()) if self.cache_depth else None
        )  # Keeps [cache]
        self.result = None
        ...
        if trace is not None:
            ...
            self._trace = fandango.log.TRACE_LEVEL >= 4
    ...
    def init_locals(self):
        ...
        # Tango DB methods
        self._defaults["NAMES"] = (
            lambda x: get_matching_attributes(x)
            if parse_tango_model(x).get("attribute")
            else get_matching_devices(x)
        )
        self._defaults["CHECK"] = (
            lambda x: read_attribute(x)
            if parse_tango_model(x).get("attribute")
            else check_device(x)
        )
    ...
    def find_macro(self, target):
        ...
        So, replaces FIND(*/*/*/*) with [attrname1,attrname2,attrname3,...]
        """
    ...
    def parse_formula(self, formula, _locals=None, macros=None):
        """
        ...
        if _locals and ("$(" in formula or "${" in formula):
            for l, v in list(_locals.items()):
                formula = formula.replace("$(%s)" % str(l), str(v))
                formula = formula.replace("${%s}" % str(l), str(v))
        
        if macros:
            macros = [t for t in self.macros for m in macros if m in t]
        else:
            macros = self.macros
            
        for macro_name, macro_exp, macro_fun in macros:
            matches = re.findall(macro_exp, formula)
            for match in matches:
                res = macro_fun(match)
                formula = formula.replace(macro_name % match, res)
                self.trace(
                    "TangoEval.parse_formula: Replacing %s with %s"
                    % (macro_name % match, res)
                )
        return formula
    ...
    def parse_variables(self, formula, _locals=None, parsed=False):
        ...
        idev, iattr, ival = (
            ...
            1,
            2,
        )  # indexes of the expression matching device,attribute and value
    ...
    def read_attribute(self, device, attribute="", what="", _raise=True, timeout=None):
        ...
        self.trace(
            "read_attribute(%s,%s,%s,%s,%s)"
            % (device, attribute, what, _raise, timeout)
        )
        if not attribute:
        ...
        try:
            if aname not in self.attributes:
                ...
                try:
                    ...
                    self.trace(
                        "unable to set %s proxy timeout to %s ms: %s"
                        % (device, timeout, except2str())
                    )
                dp.ping()
            ...
            if self.cache_depth and not any(
                get_attribute_time(v) == get_attribute_time(value)
                for v in self.cache[aname]
            ):
                while len(self.cache[aname]) >= self.cache_depth:
                    self.cache[aname].pop(-1)
                self.cache[aname].insert(0, value)
            if what == "all":
                if self.cache_depth:
                    try:
                        setattr(value, "delta", self.get_delta(aname))
                    except:
                        pass
                setattr(
                    value,
                    "exception",
                    isinstance(getattr(value, "value", None), PyTango.DevFailed),
                )
            elif what in ("value", ""):
                value = getTangoValue(value, device=device)
            elif what == "w_value":
                value = getattr(value, "w_value", None)
            elif what == "time":
                value = get_attribute_time(value)
            elif what == "exception":
                value = isinstance(getattr(value, "value", None), PyTango.DevFailed)
            elif what == "delta":
                value = self.get_delta(aname)
            else:
                value = getattr(value, what)
            self.trace(
                "read_attribute(%s/%s.%s) => %s" % (device, attribute, what, value)
            )
            ...
            if isinstance(e, PyTango.DevFailed) and what == "exception":
                ...
                raise e
            self.trace(
                "TangoEval: ERROR(%s.%s)! Unable to get %s for attribute %s/%s: %s"
                % (type(e), _raise, what, device, attribute, except2str(e))
            )
    ...
    def update_locals(self, dct=None):
        self._locals["now"] = self._locals["t"] = FloatFakeCall(time.time())
    ...
    def get_delta(self, target):
        ...
        if self.cache and target in self.cache:
            ...
            if var_name in self.previous:
                ...
                cache = [
                    self.read_attribute(device, attribute, "all"),
                    self.previous[var_name],
                ]
            else:
                cache = []
        delta = 0 if not cache else (cache[0].value - cache[-1].value)
        self.trace(
            "get_delta(%s); cache[%d] = %s; delta = %s"
            % (target, len(cache), [v.value for v in cache], delta)
        )
        return delta
    ...
    def unsubscribeEvents(self, model = None):
        self.trace("TangoEval.unsubscribeEvents({})".format(model))
        for k,proxy in self.attributes.items():
            try:
                if not model or clmatch(model,k):
                    proxy.removeListener(self)
            except:
                self.trace(traceback.format_exc())
    ...
    def eval(self, formula=None, previous=None, _locals=None, _raise=FAILED_VALUE):
        ...
        for x in [
            ...
            "is",
            "now",
        ]:  # Check for case-dependent operators
        ...
        self.formula = self.parse_formula(
            self.formula
        )  # Replacement of FIND(...), env variables and comments.
        variables = self.parse_variables(
            self.formula, parsed=True
        )  # Extract the list of tango variables
        self.trace(">" * 80)
        ...
        targets = [
            (
                device + (attribute and "/%s" % attribute) + (what and ".%s" % what),
                device,
                attribute,
                what,
            )
        ...
        for target, device, attribute, what in targets:
            ...
            try:
                ...
                r = (
                    _raise
                    if not any(
                        d == device and a == attribute and w == "exception"
                        for t, d, a, w in targets
                    )
                    else FAILED_VALUE
                )
                if target in previous:
                    ...
                    self.previous[var_name] = self.read_attribute(
                        device, attribute or "State", what, _raise=r
                    )
...
class DeprecatedCachedAttributeProxy(PyTango.AttributeProxy):
    ...
    def __init__(self, name, keeptime=1000.0, fake=False):
        self.keeptime = keeptime
    ...
    def set_cache(self, value, t=None):
        ...
        self.last_read_value = (
            hasattr(value, "value") and value or fakeAttributeValue("", value)
        )

    def read(self, cache=True):
        now = time.time()
        if not cache or (now - self.last_read_time) > (old_div(self.keeptime, 1e3)):
            self.last_read_time = now
            try:
                self.last_read_value = (
                    None if self.fake else PyTango.AttributeProxy.read(self)
                )
            except Exception as e:
                self.last_read_value = e
        if isinstance(self.last_read_value, Exception):
            raise self.last_read_value
        else:
