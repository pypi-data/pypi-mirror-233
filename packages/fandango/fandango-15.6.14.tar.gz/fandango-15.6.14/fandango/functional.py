...
# alnum matching valid filenames without path, alphanumeric + "-_.*"
re_name = "(?:[a-zA-Z0-9-_\*\.]|(?:\.\*))(?:[a-zA-Z0-9-_\*\.\+]|(?:\.\*))*"
re_no_alnum = "[^a-zA-Z0-9-_]"
re_no_quotes = "(?:^|$|[^'\"a-zA-Z0-9_\./])"
...
def filtersmart(seq, filters):
    ...
    """
    seq = seq if isSequence(seq) else (
        list(seq.keys()) if isMapping(seq) else (seq,))
    if isString(filters):
    ...
    for f in filters:
        if f.startswith("+"):
            comp.append(parse(f[1:]))
        elif f.startswith("!"):
            neg.append(parse(f[1:]))
        else:
            raw.append(parse(f))
    ...
    return [
        s
        for s in seq
        if (not any(matchCl(n, s) for n in neg))
        and any(matchCl(r, s) for r in raw)
        and (not comp or all(matchCl(c, s) for c in comp))
    ]
...
RAW_TIME = "^(?:P)?([+-]?[0-9]+[.]?(?:[0-9]+)?)(?: )?(%s)$" % ("|").join(
    TIME_UNITS
)  # e.g. 3600.5 s

re_date = "[-]".join((reint, reint, reint))
rehour = "%s[:]%s" % (reint, reint) + "(?:[:]%s)?" % refloat
re_date += "(?:[ T]%s)?" % rehour  # as of ISO 8601
...
def call(args=None, kwargs=None, locals_=None, debug=False):
    ...
    kwargs, locals_, export = kwargs or {}, locals_ or {}, ""
    # args = map(str.strip,args)
    f, args = args[0], args[1:]
    print('fandango.call(%s,%s,%s)' % (f, args, f in locals_))
    ...
    if not isCallable(f):
        ...
        if f == "help":
            if args and args[0] in locals_:
                ...
                lines = []
                lines.append(locals_["__doc__"])
                lines.append("\nAvailable functions:")
                lines.append(
                    "\n".join(
                        sorted(
                            "\t" + getCode(v).header
                            for v in filter(isCallable, list(locals_.values()))
                        )
                    )
                )
                lines.append("")
    ...
    if all(isString(a) for a in args):
        ...
        args = list(map(str2type, args))
        kwargs = dict((k.split("=", 1)) for k in kwargs)
        kwargs = dict(
            list(zip(list(kwargs.keys()), list(map(str2type, list(kwargs.values())))))
        )
    ...
    if isDictionary(target):
        ...
        # Parse: method()
            ...
            # Parse: $VAR = #code
                ...
                and re.match(
                    "[A-Za-z\._]+[A-Za-z0-9\._]*$", target.split("=", 1)[0].strip()
                )
            ):
