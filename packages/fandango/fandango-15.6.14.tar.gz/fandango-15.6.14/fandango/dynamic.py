...
class DynamicDSImpl(PyTango.LatestDeviceImpl,Logger):
    ...
    def __init__(
        self, cl=None, name=None, _globals=None, _locals=None, useDynStates=True
    ):
    ...
    def prepare_DynDS(self):
        ...
        """
        try:
            self.warning('always_executed_hook():prepare_DynDS()')
            ...
            for a,v in self.dyn_values.items():
                try:
                    ...
                    self.info('prepare_DynDS({}): first eval: {}'.
                            format(a,r))
    ...
    def get_DynDS_properties(self, db=None):
        ...
        for method, target, config in (
            ...
            self.info(
                "In get_DynDS_properties: %s(%s) properties are: %s"
...
class DynamicDSAttrs(DynamicDSImpl):
    ...
    def check_changed_event(
        ...
        try:
            ...
            self.info(
                "In check_changed_event(%s,%s,%s): %s vs %s?"
            ...
            if v is None:
                ...
                try:
                    v, new_value = (float(v) if v is not None else None), float(
                        new_value
                    )
                except Exception as e:
                    self.debug(str(e))
    ...
    def read_dyn_attr(self, attr, fire_event=True):
        ...
        try:
            self.info(
                "DynamicDS(%s)::read_dyn_atr(%s) => evalAttr()"
                % (self.get_name(), aname)
            )
    ...
    def push_dyn_attr(
        ...
        try:
            ...
            self.info(
                "push_dyn_attr(%s,%s,changed=%s)=%s(%s))\n%s"
                % (
                    aname,
    ...
    def evalAttr(self, aname, WRITE=False, VALUE=None, _locals=None, push=False):
        ...
        try:
            ...
            (self.debug if et < 1.0 else self.warning)(
...
class DynamicDSHelpers(DynamicDSAttrs):
    ...
    def get_attr_formula(self, aname, full=False):
        ...
        if aname in self.dyn_values:
            ...
            try:
                aname, formula, compiled = next(
                    (
                        (k, self.dyn_values[k].formula, self.dyn_values[k].compiled)
                        for k in self.dyn_values
    ...
    def get_attr_date(self, aname, value, default=None):
        ...
        if tt is not None:
            if not isinstance(tt, (int, float)):
                tt = fn.ctime2time(tt)
            self.info("get_attr_date from value = %s" % fn.time2str(tt))
            return tt
            ...
            self.info("get_attr_date from now()")
            return fn.now()
    ...
    def get_attr_cache(self, aname):
        ...
        try:
            ...
            if (
                ...
                and self.KeepTime
                and updated
                and time.time() < (updated + (old_div(self.KeepTime, 1e3)))
            ):
        ...
        try:
            if not device:
                ...
                if device in devs_in_server:
                    ...
                    listener = self._locals.get('ATTRIBUTE','')
                    ...
                    if write:
                        ...
                        self._proxies[full_name].write(wvalue)
                        result = wvalue
                    else:
                        attrval = self._proxies[full_name].read()
            ...
            try:
                ...
                if full_name not in self._external_listeners:
                    ...
                    _log(
                        "info",
                        "\t%s.listeners: %s"
                        % (full_name, self._external_listeners[full_name]),
                    )
                ...
                if device == self.get_name():
                    ...
                    if device in devs_in_server:
                        ...
                        if cmd.lower() == "state":
                            ...
                            result = getattr(devs_in_server[device], cmd)(argin)
                    else:
...
class DynamicDS(DynamicDSHelpers):
    ...
    def always_executed_hook(self):
        self.debug("In DynamicDS::always_executed_hook(TAU=%s)" % ft.TAU)
        try:
            self._hook_epoch = time.time()  # Internal debugging
            if not self._prepared:
                self.prepare_DynDS()  # This code is placed here because its proper execution cannot be guaranteed during init_device()
...
def CreateDynamicCommands(ds, ds_class):
    ...
    for dev in devs:
        ...
        for name, formula in lines:  # ds.dyn_comms.items():
            ...
            name = (
                [n for n in list(ds_class.cmd_list.keys()) if n.lower() == name.lower()]
                or [name]
            )[0]
            return_type = PyTango.CmdArgType.names.get(
                formula.split("(")[0], PyTango.DevString
            )
            itype = (
