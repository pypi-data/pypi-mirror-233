...
class TServer(Object):
    ...
    def __init__(self, name="", host="", parent=None):
        if not name or "/" not in name:
            raise_(Exception, "TServer_WrongServerName_%s" % name)
        self.info = None
    ...
    def ping(self, dname=None):
        ...
        try:
            ...
            return None
        wait(0.01)  # this wait allows parallel threads
        try:
            proxy.ping()
        except:
    ...
    def get_server_level(self):
        ...
        if self.controlled and self.host:
            return self.host, self.level
        else:
            return "", 0
    ...
    def get_device_list(self):
        ...
        [
            result.extend(v)
            for c, v in list(self.classes.items())
            if c.lower() != "dserver"
        ]
        return result
        ...
        for klass in self.classes:
            for device in self.classes[klass]:
                try:
                    ...
                    self.log.warning(
                        "Unable to read %s state: %s" % (device, str(e)[:100] + "...")
                    )
    ...
    def get_all_status(self):
        ...
        try:
            ...
            print(
                "Unable to read all Status from %s: %s"
                % (self.name, str(e)[:100] + "...")
            )
            return result
...
class ServersDict(CaselessDict, Object):
    ...
    def __init__(
        ...
        self.db = (
            get_database() if not tango_host else get_database(*(tango_host.split(":")))
        )
        ...
        if loadAll:
            ...
            self.load_by_class(klass)
        elif devs_list:
            self.load_from_devs_list(devs_list)
        elif devices:
            self.load_from_devs_list(devices)
        elif servers_list:
        ...
        # elif pattern: self.load_from_servers_list(self.db.get_server_list(pattern))
            ...
            hosts = (
                type(hosts) is str
                and ("," in hosts and hosts.split(",") or [hosts])
                or hosts
            )
            for h in hosts:
                self.load_by_host(h)
            ...
            if name.count("/") >= 2 and not name.startswith("dserver/"):
                ...
                if servers_list:
                    self.load_from_servers_list(
                        [d.replace("dserver/", "") for d in servers_list], check=False
                    )
                    self.log.info("%d servers loaded" % len(servers_list))
                    return len(servers_list)
                else:
                    self.log.warning(
                        "No server matches with %s (family=%s)." % (server_name, family)
                    )
                    return 0
        return self
    ...
    def load_by_host(self, host):
        ...
        self.log.info(
            "%d servers assigned in DB to host %s: %s" % (len(servers), host, servers)
        )
        if servers:
            self.load_from_servers_list(servers)
        return self
    ...
    def load_from_devs_list(self, devs_list):
        ...
        if type(devs_list) is str:
            devs_list = devs_list.split(",")
        if type(devs_list) not in [list, set, tuple]:
            devs_list = [devs_list]
        servers_list = set()
        for d in devs_list:
        ...
        self.log.info(
            "Loading %d servers matching %s devices"
    ...
    def load_from_servers_list(self, servers_list, check=True):
        ...
        for s in servers_list:
            try:
                self.log.debug(
                    "loading from %s/%s server" % (get_tango_host(self.db), s)
                )
                ...
                self.log.warning(
                    "exception loading %s server: %s" % (s, str(e)[:100] + "...")
                )
                print(traceback.format_exc())
        self.log.debug(
            "load_from_servers_list(%d) took %f seconds"
    ...
    def get_db_device(self):
        ...
        dev = [d for d in list(self.proxies.keys())[:]
            if "sys/database/" in d.lower()]
        if dev:
            return self.proxies[dev[0]]
        else:
    ...
    def get_devs_from_db(self, dev_name):
        ...
        if dev_name.count("/") < 2:
            raise Exception("ThisIsNotAValidDeviceName")
    ...
    def get_hosts_from_db(self, filt="*"):
        ...
        return sorted(
            set(name.split(".")[0] for name in self.get_db_device().DbGetHostList(filt))
        )
    ...
    def get_device_server(self, device):
        ...
        try:
            ...
            self.log.error(
                "Impossible to retrieve server for device %s: %s"
                % (device, str(e)[:100] + "...")
            )
    ...
    def get_device_class(self, device, server=""):
        """This method gets the server related to a device; if it is not in the dict gets it from the Database."""
        for s in [self[server]] if server else list(self.values()):
            for klass, devs in list(s.classes.items()):
                if device in devs:
                    return klass
        self.log.warning(
            "get_class_for_device doesnt provide information for servers not loaded yet."
        )
        return None
    ...
    def get_device_host(self, device):
        ...
        for s in list(self.values()):
            if device in s.get_device_list():
                return s.host
        self.log.warning(
            "get_device_host doesnt provide information for servers not loaded yet."
        )
        return None
    ...
    def get_host_servers(self, host):
        ...
        return [
            ss.name
            for ss in list(self.values())
            if ss.host.lower() == host.lower().split(".")[0].strip()
        ]
    ...
    def get_host_level_servers(self, host, level=0, controlled=True):
        ...
        return [
            ...
            for ss in list(self.values())
            if ss.host.lower() == host.lower().split(".")[0].strip()
            and (ss.level == level or (not ss.controlled and not controlled))
        ]
    ...
    def get_class_devices(self, klass):
        ...
        result = set()
        [
            result.update(s.classes[klass])
            for s in list(self.values())
            if klass in s.classes
        ]
        return list(result)
    ...
    def get_all_classes(self):
        ...
        result = set()
        for s in list(self.values()):
            [result.add(c) for c in list(s.classes.keys())]
        return list(filter(bool, result))

    def get_all_hosts(self):
        """It returns all hosts containing servers."""
        return list(filter(bool, set(s.host for s in list(self.values()))))

    def get_all_devices(self):
        """It returns all devices contained in servers."""
        return list(fun.chain(*[s.get_device_list() for s in list(self.values())]))

    def get_server_states(self, update=False):
        result = {}
        if update:
            self.update_states()
        [result.__setitem__(s.name, s.state) for s in list(self.values())]
        return result

    def get_server_tree(self):
        """@todo It returns a dictionary with the shape {'server':{'class':'device'}}"""
        result = dict.fromkeys(list(self.keys()))
        for k in result:
    ...
    def get_host_overview(self, host="", as_text=False):
        ...
        if not host:
            ...
            if not result:
                self.log.warning("No servers has been loaded for host %s" % host)
        if as_text:
            from . import arrays

            result = "\n".join(
                "\t".join(map(str, l)) for l in arrays.tree2table(result)
            )
        return result
    ...
    def states(self, servers=[], class_type="", asynch=True, timeout=60.0):
        ...
        servers = (
            servers
            or (class_type and self.get_class_servers(class_type))
            or list(self.keys())
        )
        ...
        try:
            ...
            def try_device(sdict, server):
                try:
                    ...
                    if value is None:
                        self.log.debug(
                            "%s.ping() ... failed!" % (server)
                        )  # LOG DOESNT WORK IN THREADS
                    else:
                        self.log.debug(
                            "%s.ping() ... Alive" % (server)
                        )  # LOG DOESNT WORK IN THREADS
            ...
            while len(threads):
                for s_name, thread in list(threads.items()):
                    if not thread.isAlive():
                        result[s_name] = thread.result
                        self.log.debug(
                            "%s thread finished! (%s)" % (s_name, thread.result)
                        )
                        threads.pop(s_name)
                    wait(0.01)
            ...
            for s_name in servers:
                ...
                if state is None:
                    self.log.debug("%s.ping() ... failed!" % (admin))
                elif state == PyTango.DevState.FAULT:
                    self.log.debug("%s.ping() ... ZOMBIE!!" % s_name)
                else:
                    self.log.debug("%s.ping() ... Alive" % s_name)
                result[s_name] = state
        return result
    ...
    def start_servers(self, servers_list=None, host="", wait=10.0):
        ...
        if servers_list is None:
            servers_list = sorted(
                list(self.keys()), key=(lambda s: s in self and self[s].level or 0)
            )
        ...
        for s_name in servers_list:
            ...
            if not host:
                try:
                    if s_name in self:
                        self[s_name].init_from_db(
                            self.db
                        )  # Updating info from DB even if it was already initialized (it may be out-of-sync)
                    self[s_name].get_server_level()
                except Exception as e:
                    self.log.warning(
                        "start/stop_servers(%s): Unable to retrieve host/level information: %s"
                        % (s_name, e)
                    )
        ...
        for level, host, s_name in sorted(full_servers_list):
            if not host:
                self.log.error(
                    "Host has not been defined; server  %s cannot be started" % s_name
                )
                continue
            ...
            target = self[
                s_name
            ].name  # Using the True name as returned by DbGetDeviceInfo()
            dserver = ("dserver/" if "dserver" not in s_name else "") + s_name
            dserver = (
                get_tango_host(self.db) + "/" if ":" not in dserver else ""
            ) + dserver
            ...
            try:
                ...
                print(
                    "Exception StartingServer %s: %s" % (s_name, str(e)[:100] + "...")
                )
                self.log.error(
                    "Exception StartingServer %s: %s" % (s_name, str(e)[:100] + "...")
                )
            ...
            if not done:
                self.log.warning(
                    "The server %s Start couldnt be verified after %s seconds"
                    % (s_name, (t1 - t0))
                )
            else:
                self.log.info(
                    "The server %s Start verified after %s seconds"
                    % (s_name, (t1 - t0))
                )
            try:
                self.get_host_starter(host).UpdateServersInfo()
            except Exception as e:
                self.log.error("Unable to update %s Starter: %s" % (host, e))
        return done

    def start_all_servers(self):
        return self.start_servers(
            sorted(list(self.keys()), key=(lambda s: s in self and self[s].level or 0))
        )
    ...
    def stop_servers(self, servers_list=None):
        ...
        for server_name in servers_list:
            if fun.clmatch('starter/',server_name):
                # Ignore starters
                continue
            ...
            if server.ping() is not None:
                ...
                try:
                    ...
                    self.log.error(
                        "Exception in server_Stop(%s): %s"
                        % (server.name, str(e)[:100] + "...")
                    )
            else:
                ...
                try:
                    ...
                    self.log.warning(
                        "Exception in server_Kill(%s): may be not running"
                        % (server.name)
                    )
        ...
        for host in hosts:
            if host:
                try:
                    ...
                    self.log.warning(
                        "Unable to contact with Starter in host %s: %s" % (host, e)
                    )
        return done

    def stop_all_servers(self):
        return self.stop_servers(
            sorted(
                list(self.keys()), key=(lambda s: s in self and (-self[s].level) or 0)
            )
        )

    def restart_servers(self, servers_list=None, wait=15.0):
        """Performs stop_servers followed by start_servers."""
        if servers_list is None:
            servers_list = list(self.keys())
    ...
    def kill_servers(self, servers_list):
        ...
        for server_name in servers_list:
            ...
            try:
                ...
                self.log.error(
                    "Exception in kill_servers(%s): %s"
                    % (server_name, str(e)[:100] + "...")
                )
    ...
    def kill_os(self, name):
        ...
        for n in name:
            ...
            greps.append(
                subprocess.Popen(
                    command, shell=True, stdin=prev.stdout, stdout=subprocess.PIPE
                )
            )
            prev = greps[-1]
    ...
    def get_report(self):
        ...
        hosts = list(map(str, set(v.host for v in list(self.values()))))
    ...
    def get_server_level(self, server_name):
        ...
        if server_name in self:
            self[server_name].update_level(host, level)
...
class ComposersDict(ServersDict):
    def load_attributes(self):
        ...
        if not hasattr(self, "attributes"):
            self.attributes = dicts.CaselessDict()
        self.attributes.clear()
        for d in self.get_all_devices():
            attrs = list(
                self.db.get_device_property(d, ["DynamicAttributes"])[
                    "DynamicAttributes"
                ]
            )
            if not attrs:
                attrs = list(self.db.get_device_property(d, ["AlarmList"])["AlarmList"])
            for l in attrs:
                l = l.split("#")[0].strip()
                if not l:
                    continue
                elif "=" in l:
                    self.attributes[d + "/" + l.split("=")[0].strip()] = l.split(
                        "=", 1
                    )[-1].strip()
                elif ":" in l:
                    self.attributes[d + "/" + l.split(":")[0].strip()] = l.split(
    ...
    def get_attributes(self, device=None, attribute=None):
        ...
        result = CaselessDict(
            (n, f)
            for n, f in list(self.attributes.items())
            if (not device or fun.matchCl(device, n.rsplit("/", 1)[0], terminate=True))
            and (
                not attribute
                or fun.matchCl(attribute, n.rsplit("/", 1)[-1], terminate=True)
            )
        )
        return result
    ...
    def get_property(self, *args):
        ...
        if len(args) == 2:
            devs = [d for d in devs if fun.matchCl(args[0], d)]
        r = dict(
            (d, list(fun.toList(self.db.get_device_property(d, [property])[property])))
            for d in devs
        )
    ...
    def set_formula(self, attribute, formula, update=True):
        dev, attr = attribute.rsplit("/", 1)
        new, prop = [], list(
            self.db.get_device_property(dev, ["DynamicAttributes"])["DynamicAttributes"]
        )
        found = False
        for p in prop:
            if not attr.lower() == p.split("=")[0].strip().lower():
                ...
                new.append(
                    ...
                    % (
                        attr,
                        formula,
                        "#" + comment if comment and "#" not in formula else "",
                    )
                )
                print(new[-1])
                found = True
