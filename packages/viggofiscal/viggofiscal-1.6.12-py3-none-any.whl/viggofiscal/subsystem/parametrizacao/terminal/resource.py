...
class Terminal(entity.Entity, db.Model):
    ...
    attributes = ['domain_org_id', 'serie_fiscal_id', 'portador_id',
                  'descricao']
    ...
    serie_fiscal_id = db.Column(
        db.CHAR(32), db.ForeignKey('serie_fiscal.id'), nullable=False)
    ...
    domain_org = orm.relationship(
        'DomainOrg', backref=orm.backref('terminal_domain_org'))
    ...
    __table_args__ = (
        UniqueConstraint(
            'domain_org_id', 'serie_fiscal_id',
            name='terminal_domain_org_id_serie_fiscal_id_uk'),
        UniqueConstraint(
            'domain_org_id', 'descricao',
            name='terminal_domain_org_id_descricao_uk'),)

    def __init__(self, id, domain_org_id, serie_fiscal_id, portador_id,
                 descricao,
