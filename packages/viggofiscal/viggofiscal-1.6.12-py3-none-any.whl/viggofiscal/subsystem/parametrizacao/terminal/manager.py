...
class Create(operation.Create):
    ...
    def do(self, session, **kwargs):
        ...
        portador_data = {
            'descricao': descricao,
            'domain_id': self.entity.domain_org_id,
            'tipo': 'INTERNO',
            'padrao': False}
...
class Analisar(operation.Create):
    ...
    def do(self, session, **kwargs):
        considera_caixa = kwargs.pop('considera_caixa', False)
        terminal_id = kwargs.get('id', '')
        operadores = kwargs.get('operadores', [])
        operadores_em_uso = []
        if len(operadores) > 0:
            if considera_caixa:
                operadores_em_uso = self.manager.\
                    get_operadores_em_uso_com_caixa(
...
class Manager(manager.CommonManager):
    ...
    def get_operadores_em_uso_com_caixa(self, session, terminal_id, operadores):
        user_ids = [op.get('user_id', None) for op in operadores]
        query = """
            SELECT u."name", t.descricao, c.numero
            FROM "user" u
                LEFT JOIN terminal_operador to2 ON to2.user_id = u.id
                LEFT JOIN terminal t ON t.id = to2.terminal_id
                LEFT JOIN caixa c ON c.terminal_id = t.id AND c.status = 'ABERTO'
            WHERE to2.terminal_id <> '{terminal_id}' AND u.id IN {user_ids};
        """  # noqa: E501
        query = query.format(
