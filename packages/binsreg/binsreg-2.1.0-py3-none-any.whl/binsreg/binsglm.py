...
from binsreg.binsregselect import binsregselect  # .binsregselect to build package
from binsreg.funs import *   # .funs to build package
...
def binsglm(y, x, w=None, data=None, at=None, dist = 'Gaussian', link = None, deriv=0, nolink = False,
            ...
            nbins=None, binspos="qs", binsmethod="dpi", nbinsrot=None,
            pselect=None, sselect = None,
            samebinsby=False, randcut=None,
    ...
    **optimize : 
    ...
    if not nolink and deriv>1:
        raise Exception("Derivative should be no greater than 1 when the conditional mean is requested.")
    ...
    for i in range(ngroup):
            ...
            if not fewobs:
                ...
                if selection=="J":
                    binselect = binsregselect(y_sub, x_sub, w_sub, deriv=deriv,
                                                bins=dots, binspos=binspos, nbins=nbins_all,
                                                binsmethod=binsmethod, nbinsrot=nbinsrot,
                                                vce=vce, cluster=cluster_sub, randcut=randcut1k,
        ...
        if ciON:
            ...
            if not nolink:
                ...
                fit_0 = binsreg_pred(basis_0, model_ci, type = "xb", deriv=0, wvec=eval_w)[0]
                pred_ci_0   = linkinv_1(fit_0)

                if asyvar or deriv==0:
                    ci_pred_se  = pred_ci_0 * ci_se
                    if deriv == 0: ci_pred_fit = linkinv(ci_fit)
                    if deriv == 1: ci_pred_fit = pred_ci_0 * ci_fit
                else:
                    if eval_w is not None:
        ...
        if cbON:
            ...
            if not nolink:
                ...
                fit_0 = binsreg_pred(basis_0, model_cb, type = "xb", deriv=0, wvec=eval_w)[0]
                pred_cb_0 = linkinv_1(fit_0)

                if asyvar or deriv==0:
                    cb_pred_se  = pred_cb_0 * cb_se
                    if deriv == 0: cb_pred_fit = linkinv(cb_fit)
                    if deriv == 1: cb_pred_fit = pred_cb_0 * cb_fit
                else:
                    if eval_w is not None:
    ...
    if not noplot:
        ...
        for i in range(ngroup):
            ...
            if data_by.dots is not None:
                ...
                if not legendoff:
                    binsplot = binsplot + geom_point(aes(x='x', y='fit', colour='group'), data_by.dots.loc[index], size=2)
                    #binsplot = binsplot + geom_point(data=data_by.dots.loc[index], mapping = aes(x=x, y=fit, colour=group), shape=bysymbols[i], size=2)
                else:
                    binsplot = binsplot + geom_point(aes(x='x', y='fit'), data_by.dots.loc[index], colour=bycolors[i], size=2)
            ...
            if data_by.line is not None:
                ...
                if not legendoff:
                    binsplot = binsplot + geom_line(aes(x='x', y='fit', colour='group'), data_by.line.loc[index])
                    # binsplot = binsplot + geom_line(aes(x='x', y='fit', colour='group'), data_by.line.loc[index], linetype=bylpatterns[i])
                else:
                    binsplot = binsplot + geom_line(aes(x='x', y='fit'), data_by.line.loc[index], color=bycolors[i])
                    # binsplot = binsplot + geom_line(aes(x='x', y='fit'), data_by.line.loc[index], colour=bycolors[i], linetype=bylpatterns[i])
            if data_by.poly is not None:
                ...
                binsplot = binsplot + geom_line(aes(x='x', y='fit'), data_by.poly.loc[index], color=bycolors[i])
                # binsplot = binsplot + geom_line(aes(x='x', y='fit'), data_by.poly.loc[index], colour=bycolors[i], linetype=bylpatterns[i])            
            ...
            if data_by.ci is not None:
                ...
                binsplot = binsplot + geom_errorbar(aes(x='x', ymin='ci_l', ymax='ci_r'), data_by.ci.loc[index],color=bycolors[i], width = 0.05)
                # binsplot = binsplot + geom_errorbar(aes(x='x', ymin='ci_l', ymax='ci_r'), data_by.ci.loc[index], alpha=1, col=bycolors[i], linetype=bylpatterns[i])
    ...
    opt = options_glm(dots=dots_by, line=line_by, ci=ci_by, cb=cb_by,
                    polyreg=polyreg, deriv=deriv, dist=dist,link=link,
                    binspos=position, binsmethod=selectmethod,
                    N_by=N_by, Ndist_by=Ndist_by, Nclust_by=Nclust_by,
                    nbins_by=nbins_by, byvals=byvals)
    
    out = binsglm_output(bins_plot=binsplot, data_plot=data_plot, cval_by=cval_by,
