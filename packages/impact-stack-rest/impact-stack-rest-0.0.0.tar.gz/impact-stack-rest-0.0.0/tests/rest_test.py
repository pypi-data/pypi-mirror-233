...
class ClientTest:
    ...
    def test_request_to_root_url(self, mock_request):
        ...
        assert mock_request.mock_calls == [
            mock.call("GET", "https://base.example.org/", timeout=2),
    ...
    def test_request_using_path_parts(self, mock_request):
        ...
        assert mock_request.mock_calls == [
            mock.call("GET", "https://base.example.org/part/escaped%3F%2F%3D", timeout=2),
            mock.call().raise_for_status(),
            mock.call().json(),
        ]
    ...
    def test_request_using_path(self, mock_request):
        ...
        assert mock_request.mock_calls == [
            mock.call("GET", "https://base.example.org/test=?path", timeout=2),
            mock.call().raise_for_status(),
            mock.call().json(),
        ]
    ...
    def test_sets_timeout(self, mock_request):
        """Test that the Client sets a default timeout for all requests."""
        client = rest.Client("https://example.com", request_timeout=42)
        client.get()
        assert mock_request.mock_calls[0].kwargs["timeout"] == 42
