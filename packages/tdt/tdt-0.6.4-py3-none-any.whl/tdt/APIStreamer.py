...
class APIStreamer:
    ...
    def reset(self):
        ...
        if self.verbose:
            print(f"{self.nchan} channels in {self.buff_size} sample buffer at {self.fs:.2f} Hz")
            ...
            try:
                ...
                if self.curr_index != self.prev_index:
                    ...
                    if self.nchan < 4:
                        ...
                        self.curr_time = (self.curr_looptime + self.curr_loop * self.buff_size / self.nchan) / self.device_fs
                    ...
                    if self.new_data is not None:
                        with self.data_lock:
                            ...
                            self.ts = self.curr_time - self.history_seconds + np.arange(0, self.data.shape[1]) / self.fs
