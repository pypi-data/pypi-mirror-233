...
class AirLoopEquipment_auto():
	"""A piece of equipment serving an air-loop. This is generalized to be able to contain any type of air loop equipment."""
...
class CADObjectId_auto():
	...
	def programIdRef(self):
		"""ID reference to unique CAD program defined in DocumentHistory/ProgramInfo element."""
...
class Data_auto():
	...
	@property
	def IndependantValue(self):
		return self.get_child('IndependantValue')


	@property
	def IndependantValues(self):
		return self.get_children('IndependantValue')


	def add_IndependantValue(self,
		):
		return self.add_child('IndependantValue',
			)


	@property
	def DependantValue(self):
		return self.get_child('DependantValue')


	@property
	def DependantValues(self):
		return self.get_children('DependantValue')


	def add_DependantValue(self,
		):
		return self.add_child('DependantValue',
			)
...
class Equation_auto():
	"""The Equation element allows data for n-dimensional algebraic data to be entered. The Expression element should contain an independent variable followed by an = sign and an equation containing dependant variables. The independent and dependant variables are then defined by their respective tags. The only mathematical operations allowed are ^ (power), + (addition), - (subtraction), / (division), and * (multiplication). Example: z=x+2*y^2 would have an IndependantVariable with a name = z, and two DependantVariables with names of x and y. Descriptions would be included for each variable, along with unit and dataType information identifying the variable and its role. """
	...
	@property
	def IndepentantVariable(self):
		return self.get_child('IndepentantVariable')


	@property
	def IndepentantVariables(self):
		return self.get_children('IndepentantVariable')


	def add_IndepentantVariable(self,
		unit=None,
		dataType=None,
		):
		return self.add_child('IndepentantVariable',
			unit=unit,
			dataType=dataType,
			)
	...
	@property
	def DependantVariable(self):
		return self.get_child('DependantVariable')


	@property
	def DependantVariables(self):
		return self.get_children('DependantVariable')


	def add_DependantVariable(self,
		unit=None,
		dataType=None,
		minValue=None,
		maxValue=None,
		):
		return self.add_child('DependantVariable',
			unit=unit,
			dataType=dataType,
			minValue=minValue,
			maxValue=maxValue,
...
class PointData_auto():
	"""The PointData element allows for graph or tabular information to be entered. The data type and units of the independent and dependant variables are defined in their respective elements. Each Data element represent a data point, with each Value element representing the value of a variable - starting with the independent variable, then each dependant variable listed in the order defined."""
