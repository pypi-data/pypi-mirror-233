...
from typing import IO, AnyStr, TypeAlias, cast
...
class Path(pathlib.Path, pathlib.PurePosixPath):
    ...
    def access(
        ...
        *,
        ...
        See Also:
            `Real Effective IDs.
        <https://stackoverflow.com/questions/32455684/difference-between-real-user-id-effective-user-id-and-saved-user-id>`_
        """
        if not self.exists():
            return None
        return os.access(
    ...
    def find_up(self, function="is_file", name="__init__.py", uppermost=False):
        """Find file or dir up.
...
class Passwd:
    ...
    def from_login(cls):
        ...
        try:
            user = Path("/dev/console").owner() if pproj.MACOS else os.getlogin()
        except OSError:
