...
class header_wolf():
    """Header of WolfArray"""
    # In case of a mutliblock, the header have informations
...
class Ops_Array(wx.Frame):
    ...
    def __init__(self, parentarray, mapviewer=None):
        ...
        self.active_array = self.parentarray
    ...
    def set_GUI(self):
        ...
        super(Ops_Array, self).__init__(None, title=_('Operators'), size=(370, 450),
    ...
    def Onupdatepal(self, event):
        ...
        if curarray.mypal.automatic != auto or curarray.alpha != oldalpha or ret or auto != curarray.mypal.automatic:
...
class WolfArray(Element_To_Draw, header_wolf):
    ...
    def __init__(self,
                 ...
                 mold=None,                         # initialize from a copy a the mold object --> must be a WolArry
        ...
        self.rgb = None         # cumpy array with colorize values
        ...
        if isinstance(srcheader, header_wolf):
            ...
            # self.array.data = self.nullvalue
    ...
    def export_geotif(self,outdir='',extent = ''):
        ...
        band.WriteArray(arr.data.transpose())
        band.FlushCache()
    ...
    def import_geotif(self, fn:str='', which = None, crop=None):
        ...
        if which is None:
            ...
            names = [raster.GetRasterBand(which+1) for which in range(nb)]
            ...
            if nb>1:
                if self.wx_exists :
                    ...
                    which = names.index(dlg.GetSelection())+1
                    dlg.Destroy()
                else:
                    which =1
            else:
    ...
    def read_txt_header(self):
        ...
        if self.filename.endswith('.flt'):
            ...
            if not os.path.exists(self.filename + '.txt'):
                logging.info(_('File {os.path.curdir} does not exist -- Retry!'))
                return
    ...
    def mask_reset(self):
        """ Unmask everything
        """
        # FIXME Call this unmask or mask_clear
...
class WolfArrayMB(WolfArray):
    ...
    def __getitem__(self, block_key) -> WolfArray:
        """Access a block of this multi-blocks array."""
        return self.myblocks[block_key]
    ...
    def add_block(self, arr: WolfArray):
        ...
        # FIXME Remove nb_blocks and make it a property over len(self.myblocks)
        self.nb_blocks += 1
