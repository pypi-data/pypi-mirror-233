    ...
    def __init__(self, path=None, use_cache=True):
        ...
        if self._expiry:
            ...
            for k, v in self._expiry.items():
                if self.get_time_created(k) < reset_data_time:
...
def fetch_data(dataset: str, time_saved=None):
    ...
    res = finlab.utils.requests.post(url, params)
    ret = res.json()
...
def get(dataset: str, save_to_storage: bool = True, force_download=False):
    ...
    if url_data is not None and df is not None and len(df) != 0 and not force_download:
        ...
        merge_success = False
        try:
            df = pd.concat([df, short_df]).pipe(lambda df: df[~df.duplicated()])\
                [short_df.columns]\
...
def indicator(indname, adjust_price=False, resample='D', market:Union[str, finlab.market_info.MarketInfo]='TW_STOCK', **kwargs):
