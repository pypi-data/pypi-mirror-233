...
import sys # for test
sys.path.append("/root/dev/jjuke/jjuke") # for test
from modules.diffusion.common import default, make_beta_schedule
...
class DDIMSampler(DiffusionBase):
    def __init__(
            self,
            betas: np.ndarray,
            ddim_discretize_method: str = "uniform",
    ...
    def ddim_reverse_sample(self, denoise_fn, x_t, t):
        ...
        if self.ddim_eta != 0.0:
            print("Warn! Reverse ODE only for deterministic path, but 'ddim_eta={}'".format(self.ddim_eta))
    ...
    def sample_progressive(self, denoise_fn: Callable[[Tensor, Tensor], Tensor],
                           shape: Union[Sequence, Size], noise: Tensor = None) -> Tensor:
        """
        Returns:
        ...
        for i, step in enumerate(indices):
            ...
            sample = self.ddim_sample(denoise_fn=denoise_fn, x_t=sample, t=t, s=s)
            yield sample
    

    def forward(self, denoise_fn: Callable[[Tensor, Tensor], Tensor],
                shape: Union[Sequence, Size], noise: Tensor = None, num_samples=1) -> Tensor:
        """
        Returns:
            Tensor: (num_samples, *shape)
        """
        ...
        if num_samples > 1:
            for i, sample in enumerate(self.sample_progressive(denoise_fn=denoise_fn, shape=shape, noise=noise)):
                if i in sample_indices:
                    sample_list.append(sample)
            sample_list.append(sample)
            return torch.stack(sample_list)
        else:
            return next(self.sample_progressive(denoise_fn=denoise_fn, shape=shape, noise=noise))
