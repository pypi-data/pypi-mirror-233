...
class KarrasSampler(DiffusionBase):
    def __init__(
            ...
            rho: float = 7.,
            sampler: str = "heun", # ["heun|dpm|ancestral"]
            s_churn: float = 0.,
            s_tmin:float = 0.,
        ...
        assert sampler in "heun|dpm|ancestral".split("|"), "{} is not in 'heun, dpm, ancestral'.".format(sampler)
        ...
        ramp = np.linspace(0, 1, self.n_steps)
