...
from modules.diffusion.common import normal_kl, discretized_gaussian_log_likelihood, default
...
class DDPMSampler(DiffusionBase):
    ...
    def p_sample(self, denoise_fn: Callable[[Tensor, Tensor], Tensor],
        ...
        out = self.p_mean_variance(denoise_fn=denoise_fn, x_t=x_t, t=t)
    ...
    def forward(self, denoise_fn: Callable[[Tensor, Tensor], Tensor], shape: Union[Sequence, Size],
                noise: Tensor = None, num_samples=1) -> Tensor:
        """
        Returns:
        ...
        sample_list = []
        for i, sample in enumerate(self.sample_progressive(denoise_fn=denoise_fn, shape=shape, noise=sample)):
