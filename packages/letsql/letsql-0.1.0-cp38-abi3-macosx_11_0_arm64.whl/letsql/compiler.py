...
from letsql._vendor.ibis import NA
from letsql._vendor.ibis.expr.operations.udf import InputType
from letsql._vendor.ibis.formats.pyarrow import PyArrowType
...
def datafusion_column(value):
    return df.expr.Expr.column(value)


def datafusion_literal(value):
    if not isinstance(value, pa.Scalar):
        value = pa.scalar(value)
    return df.expr.Expr.literal(value)
...
def literal(op, **_):
    ...
    return datafusion_literal(arrow_scalar)
...
def column(op, **_):
    return df.expr.Expr.column(f'"{op.name}"')
...
def count_star(_, **__):
    return df.functions.count(datafusion_literal(1))
...
def negate(op, **kw):
    return datafusion_literal(-1) * translate(op.arg, **kw)
...
def cot(op, **kw):
    ...
    return datafusion_literal(1.0) / df.functions.tan(x)


@translate.register(ops.Radians)
def radians(op, **kw):
    return (
        translate(op.arg, **kw) * datafusion_literal(math.pi) / datafusion_literal(180)
    )
...
def degrees(op, **kw):
    return (
        translate(op.arg, **kw) * datafusion_literal(180) / datafusion_literal(math.pi)
    )
...
def null_if_zero(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.nullif(arg, datafusion_literal(0))
...
def zero_if_null(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.coalesce(arg, datafusion_literal(0))
...
def pi(_, **__):
    return datafusion_literal(math.pi)


@translate.register(ops.E)
def e(_, **__):
    return datafusion_literal(math.e)
...
def scalar_udf(op, **kw):
    if (input_type := op.__input_type__) == InputType.PYARROW:
        ...
        fun = getattr(df.functions, op.__func_name__)
        args = map(partial(translate, **kw), op.args)
        return fun(*args)
    else:
...
def regex_replace(op, **kw):
    ...
    pattern = translate(op.pattern, **kw)
    replacement = translate(op.replacement, **kw)
    return df.functions.regexp_replace(
        arg, pattern, replacement, datafusion_literal("g")
    )
...
def string_find(op, **kw):
    ...
    if (op_start := op.start) is not None:
        ...
        return df.functions.coalesce(
            df.functions.nullif(pos + start, start), datafusion_literal(0)
        ) - datafusion_literal(1)

    return df.functions.strpos(arg, pattern) - datafusion_literal(1)
...
def string_contains(op, **kw):
    ...
    return df.functions.strpos(haystack, needle) > datafusion_literal(0)
...
def extract_year(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("year"), arg)


@translate.register(ops.ExtractMonth)
def extract_month(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("month"), arg)


@translate.register(ops.ExtractDay)
def extract_day(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("day"), arg)


@translate.register(ops.ExtractQuarter)
def extract_quarter(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("quarter"), arg)


@translate.register(ops.ExtractMinute)
def extract_minute(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("minute"), arg)


@translate.register(ops.ExtractHour)
def extract_hour(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("hour"), arg)
...
def extract_day_of_the_year(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("doy"), arg)
...
def extract_day_of_the_week_index(op, **kw):
    arg = translate(op.arg, **kw)
    return (
        df.functions.date_part(datafusion_literal("dow"), arg) + datafusion_literal(6)
    ) % datafusion_literal(7)
...
def date(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_trunc(datafusion_literal("day"), arg)


@translate.register(ops.ExtractWeekOfYear)
def extract_week_of_year(op, **kw):
    arg = translate(op.arg, **kw)
    return df.functions.date_part(datafusion_literal("week"), arg)
