...
from letsql._vendor.ibis import literal as L
from letsql._vendor.ibis.expr import datatypes as dt

from .base import assert_series_equal, default_series_rename
...
@pytest.mark.parametrize(
    ...
    [
        ...
        param(
            ...
            decimal.Decimal("1.1"),
            marks=[
                pytest.mark.notyet(raises=Exception),
            ],
            id="decimal-big",
        ),
        param(
            ...
            marks=[
                pytest.mark.broken(raises=ValueError),
            ],
            id="decimal-infinity+",
        ),
        param(
...
@pytest.mark.parametrize(
    ...
    [
        param(
            ...
            lambda t: (t.double_col + 0.05).round().astype(int),  # to make it round up
            id="round",
        ),
        param(
            lambda t: t.double_col.add(0.05).round(3),
            lambda t: (t.double_col + 0.05).round(3),
            id="round-with-param",
            marks=[
                pytest.mark.notimpl(raises=com.UnsupportedOperationError),
            ],
        ),
        param(
...
def test_backend_specific_numerics(con, df, alltypes, expr_fn, expected_fn):
...
def test_binary_arithmetic_operations(alltypes, df, op):
    ...
    expected = default_series_rename(expected.astype("float64"))
