...
from letsql._vendor.ibis.expr.types.core import Expr, _FixedTextJupyterMixin
from letsql._vendor.ibis.expr.types.generic import literal
...
if TYPE_CHECKING:
    ...
    from letsql._vendor.ibis.selectors import IfAnyAll, Selector
    from letsql._vendor.ibis.expr.types.groupby import GroupedTable

_ALIASES = (f"_ibis_view_{n:d}" for n in itertools.count())


def _ensure_expr(table, expr):
    from letsql._vendor.ibis.selectors import Selector
...
class Table(Expr, _FixedTextJupyterMixin):
    ...
    def __dataframe__(self, nan_as_null: bool = False, allow_copy: bool = True):
        from letsql._vendor.ibis.expr.types.dataframe_interchange import IbisDataFrame
    ...
    def __pyarrow_result__(self, table: pa.Table) -> pa.Table:
        from letsql._vendor.ibis.formats.pyarrow import PyArrowData

        return PyArrowData.convert_table(table, self.schema())

    def __pandas_result__(self, df: pd.DataFrame) -> pd.DataFrame:
        from letsql._vendor.ibis.formats.pandas import PandasData
    ...
    def __interactive_rich_console__(self, console, options):
        from letsql._vendor.ibis.expr.types.pretty import to_rich_table
    ...
    def __getitem__(self, what):
        ...
        └────────────────┴───────────────┴───────────────────┴─────────────┘
        """
        from letsql._vendor.ibis.expr.types.generic import Column
        from letsql._vendor.ibis.expr.types.logical import BooleanValue
    ...
    def aggregate(
        ...
        **kwargs: ir.Value,
        ...
        └────────┴────────────┴──────────┘
        """
        import letsql._vendor.ibis.expr.analysis as an
    ...
    def info(self) -> Table:
        ...
        └───────────────────┴─────────┴──────────┴───────┴───────────┴───────────┴───┘
        """
        from letsql._vendor.ibis import literal as lit
