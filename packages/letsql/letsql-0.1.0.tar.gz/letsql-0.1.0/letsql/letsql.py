...
import letsql._internal as datafusion
...
from letsql._vendor.ibis.backends.base import (
    BaseBackend,
    CanCreateDatabase,
    CanCreateSchema,
)
from letsql._vendor.ibis.util import gen_name, normalize_filename
from letsql.compiler import translate

try:
    from letsql._internal import ExecutionContext as SessionContext
except ImportError:
    from letsql._internal import SessionContext

try:
    from letsql._internal import SessionConfig
except ImportError:
    SessionConfig = None
...
class Backend(BaseBackend, CanCreateDatabase, CanCreateSchema):
    ...
    def register(
        ...
        **kwargs: Any,
        ...
        elif first.startswith("postgresql://"):
            self._context.deregister_table(table_name)
            self._context.register_remote_table(table_name, source)
            return self.table(table_name)
        else:
