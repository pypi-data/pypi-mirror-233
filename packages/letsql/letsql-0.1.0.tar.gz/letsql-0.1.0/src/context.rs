...
use crate::datasource::provider::RemoteTable;
...
impl PySessionContext {
    ...
    fn register_remote_table(&self, name: &str, connection: &str, py: Python) -> PyResult<()> {
        ...
        let result = wait_for_future(py, remote_table).map_err(DataFusionError::from)?;

        self.ctx
            .register_table(name, Arc::new(result))
            .map_err(DataFusionError::from)?;
