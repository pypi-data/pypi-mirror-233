import pandas as pd
from evergreenlib import ExcelParser, TxtParser
import numpy as np
...
class GetCustomers:
    def __init__(self, filepath, value):
        ...
        self.Mobility_Filepath = r'V:\Findep\Incoming\test\DevOps\References\Payment Terms\Status report 2.0.xlsm'
        self.MobilityPtShtName = 'Клиенты'

        self.AS_Filepath = r'V:\Findep\Incoming\test\DevOps\References\Payment Terms\Срок отсрочки платежа 210723.xlsx'
    ...
    def further_process(self):
        ...
        merged_df_with_ws = pd.merge(merged_df_with_retail, self.ASPT_opt, left_on='KUNNR',
                                     ...
                                     how='left', sort=False)
        cols_to_drop = ['Сейлз', 'Менеджер', 'Клиент', 'ИНН_x', 'SAP код', 'Номер Договора аренды',
                        'Дата Договора аренды', 'ЭДО', 'Статус клиента', 'Контрагент_x', 'ИНН_y', 'КПП_x',
                        'Код дилера_x', 'Контрагент.Код SAP_x', 'Основной договор_x', 'Ведение взаиморасчетов_x',
                        'Sap_code_and_channel_x', 'Контрагент_y', 'ИНН', 'КПП_y', 'Код дилера_y',
                        'Контрагент.Код SAP_y', 'Основной договор_y', 'Ведение взаиморасчетов_y',
...
if __name__ == '__main__':
    parse_data = GetCustomers(filepath=r'V:\Findep\Incoming\test\DevOps\References\KNA1 22082023.txt',
                              value='KUNNR')

    print(parse_data.further_process().to_clipboard())
