from evergreenlib import TxtParser, GetVendors, GetCustomers, MasterData, Translated
import numpy as np
import pandas as pd
...
class Initials:
    ...
    def make_ap_df(self):
        ...
        self.AP_df = self.AP_df[~self.AP_df['Rap Account'].isin(['76.02.00', '60.01.00', '', None])]

        self.AP_df = self.AP_df[~self.AP_df['Account'].isin([np.nan, "", None, 'nan', "Account", "None"])]
        self.AP_df['Amount in LC'] = self.trailing_negative_numbers_(self.AP_df['Amount in LC'])
        self.AP_df[['Doc. Date', 'Pstng Date', 'Entry Date']] = self.AP_df[
            ['Doc. Date', 'Pstng Date', 'Entry Date']].applymap(
        ...
        self.AP_df['Corp Account'] = np.where(self.AP_df['Account'].isin(
            set(self.AP_df['Account'])), self.AP_df['Account'], self.AP_df['Offst.acct'])
        self.AP_df['Corp Account Name'] = self.AP_df.loc[:, 'Corp Account'].apply(
            lambda x: self.mydict2.get(x).get("Local Account Name") if self.mydict2.get(x) is not None else None)
        ...
        self.AP_df['Category'] = self.AP_df['Corp Account'].apply(
            lambda x: self.mydict2.get(x).get("reporting_line")
        ...
        self.AP_df["Company_code"] = np.select(conds, choices, self.AP_df["Vendor"])
        ...
        self.AP_df['Deferred_Payment_Days'] = self.AP_df.apply(lambda row: self.get_days(row), axis=1)
        ...
        self.AP_df['Param'] = pd.cut(self.AP_df['Overdue days'], bins=bins, labels=group_names)
        self.AP_df = self.AP_df[self.AP_df['Category'] != 'Other AR']
        self.AP_df['Company_name'] = self.AP_df.apply(
            lambda row: self.translated.get(row['Company_code'], {}).get("Translated", row['Company_name']),
            axis=1
        )
        self.AP_df = self.AP_df[self.AP_df['Category'].isin(['AP','Other AP'])]
        return self.AP_df
    ...
    def make_ar_df(self):
        ...
        self.AR_df = self.AR_df[~self.AR_df['Rap Account'].isin(['76.02.00', '60.01.00', '', None])]

        self.AR_df['Amount in LC'] = self.trailing_negative_numbers_(self.AR_df['Amount in LC'])
        self.AR_df[['Doc. Date', 'Pstng Date', 'Entry Date']] = self.AR_df[
            ['Doc. Date', 'Pstng Date', 'Entry Date']].applymap(
            lambda x: x.split(".")[2] + "-" + x.split(".")[1] + "-" + x.split(".")[0]).astype('datetime64[ns]')
        self.AR_df['Entry date and time'] = self.AR_df['Entry Date'].astype(str) + " " + self.AR_df['Time']
        self.AR_df['Entry date and time'] = self.AR_df['Entry date and time'].astype('datetime64[ns]')

        self.AR_df['DocumentNo'] = self.AR_df['DocumentNo'].str.split(".").str.get(0)
        ...
        self.AR_df['Corp Account Name'] = self.AR_df.loc[:, 'Corp Account'].apply(
            lambda x: self.mydict2.get(x).get("Local Account Name") if self.mydict2.get(
                x) is not None else None)

        self.AR_df['Doc. Date'] = np.where(self.AR_df['Doc. Date'].dt.year == 2200,
        ...
        self.AR_df['Category'] = self.AR_df['Corp Account'].apply(
            lambda x: self.mydict2.get(x).get("reporting_line")
            if self.mydict2.get(x) is not None else None)
        # self.AR_df = self.AR_df[self.AR_df['Category'] != 'not applicable for ARAP']
        conds = [
            (self.AR_df["Customer"].str.len() == 0) & (self.AR_df["Vendor"].str.len() != 0),
            (self.AR_df["Customer"].str.len() == 0) & (self.AR_df["Vendor"].str.len() == 0),
        ]
        choices = [self.AR_df["Vendor"], "not applicable"]

        self.AR_df["Company_code"] = np.select(conds, choices, self.AR_df["Customer"])
        ...
        self.AR_df['Deferred_Payment_Days'] = self.AR_df.apply(lambda row: self.get_days(row), axis=1)
        ...
        self.AR_df['Param'] = pd.cut(self.AR_df['Overdue days'], bins=bins, labels=group_names)
        ...
        self.AR_df = self.AR_df[self.AR_df['Category'].isin(['AR', 'Other AR'])]
...
if __name__ == '__main__':
    obj1 = Initials(
        AR_filepath=r'V:\Findep\Incoming\test\DevOps\ARAP Project\Project\Accounting Input Data\YTD07_2023\AR07YTD2023_02Aug2023.txt',
        report_date='07/31/2023'
    )
    print(obj1.make_ar_df().to_clipboard())
