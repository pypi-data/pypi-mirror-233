from evergreenlib import ExcelParser, MasterData, ExcelParser_retain_duplicates
import pandas as pd
import numpy as np

CoA_object = MasterData().further_process()

mydict2_object = CoA_object.set_index('Local Account Code').T.to_dict()
...
class Df1Parser:
    def __init__(self, filepath, sht, index, report_date):
        ...
        self.df1 = ExcelParser_retain_duplicates(filepath, sht, index).read_data()
    ...
    def further_process_df1(self):
        ...
        self.df1['Category'] = self.df1['Corp Account'].apply(
            lambda x: mydict2_object.get(x).get('reporting_line') if mydict2_object.get(x) else x)
        self.df1['Corp Account Name'] = self.df1['Corp Account'].apply(
            lambda x: mydict2_object.get(x).get('Local Account Name') if mydict2_object.get(x) else x)
...
class Df3Parser:
    ...
    def further_process_df1(self):
        ...
        self.df1['Category'] = self.df1['Corp Account'].apply(
            lambda x: mydict2_object.get(x).get('reporting_line') if mydict2_object.get(x) else x)
        self.df1 = self.df1[self.df1['Category'].isin(["Other AP","AP","Other AR","AR"])]
        self.df1['Corp Account Name'] = self.df1['Corp Account'].apply(
            lambda x: mydict2_object.get(x).get('Local Account Name') if mydict2_object.get(x) else x)
        self.df1['Transaction Type'] = 'HML'
        #
...
if __name__ == '__main__':
    # x = Df1Parser(
    #     filepath=r'V:\Findep\Incoming\test\DevOps\ARAP Project\Project\Accounting Input Data\YTD07_2023\07YTD2023_60_62.xlsx',
    #     sht='TDSheet', index='Счет', report_date='06/30/2023')
    ...
    print(z.further_process_df1().to_clipboard())
