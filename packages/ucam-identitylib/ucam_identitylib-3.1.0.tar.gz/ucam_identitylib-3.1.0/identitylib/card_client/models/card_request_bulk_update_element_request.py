...
class CardRequestBulkUpdateElementRequest(BaseModel):
    """
    Request to update a single card request as part of a bulk update.
    """
    ...
    identifiers: Optional[conlist(CardIdentifierSummaryRequest)] = None
    __properties = ["action", "fields", "id", "identifiers"]

    @validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('update', 'add', 'start', 'refresh', 'abandon', 'make', 'requeue', 'complete', 'set_hold', 'release_hold', 'cancel'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CardRequestBulkUpdateElementRequest:
        """Create an instance of CardRequestBulkUpdateElementRequest from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CardRequestBulkUpdateElementRequest:
        """Create an instance of CardRequestBulkUpdateElementRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CardRequestBulkUpdateElementRequest.parse_obj(obj)

        _obj = CardRequestBulkUpdateElementRequest.parse_obj({
            "action": obj.get("action"),
            "fields": CardRequestUpdateUpdateRequest.from_dict(obj.get("fields")) if obj.get("fields") is not None else None,
            "id": obj.get("id"),
            "identifiers": [CardIdentifierSummaryRequest.from_dict(_item) for _item in obj.get("identifiers")] if obj.get("identifiers") is not None else None
        })
        return _obj
