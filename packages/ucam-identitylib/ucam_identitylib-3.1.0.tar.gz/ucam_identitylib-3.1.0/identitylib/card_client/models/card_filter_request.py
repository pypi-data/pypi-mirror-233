...
class CardFilterRequest(BaseModel):
    ...
    CardFilterRequest
    """
    identifiers: conlist(constr(strict=True, min_length=1), max_items=50, min_items=1) = Field(..., description="Identifiers to filter by, in the format {value}@{scheme}. {scheme} may be omitted in which case the scheme is assumed to be CRSid")
    institution: Optional[constr(strict=True, min_length=1)] = Field(None, description="Filter by the institutions that cardholders belong to")
    status: Optional[StrictStr] = Field(None, description="Status to filter by, if omitted cards of all statuses are returned  * `ISSUED` - Issued * `REVOKED` - Revoked * `RETURNED` - Returned * `EXPIRED` - Expired * `UNACTIVATED` - Unactivated")
    updated_at__gte: Optional[datetime] = Field(None, description="Filter updatedAt by IsoDateTime greater than")
    updated_at__lte: Optional[datetime] = Field(None, description="Filter updatedAt by IsoDateTime less than")
    __properties = ["identifiers", "institution", "status", "updated_at__gte", "updated_at__lte"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CardFilterRequest:
        """Create an instance of CardFilterRequest from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CardFilterRequest:
        """Create an instance of CardFilterRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CardFilterRequest.parse_obj(obj)

        _obj = CardFilterRequest.parse_obj({
            "identifiers": obj.get("identifiers"),
            "institution": obj.get("institution"),
            "status": obj.get("status"),
            "updated_at__gte": obj.get("updated_at__gte"),
            "updated_at__lte": obj.get("updated_at__lte")
        })
        return _obj
