...
class RFIDConfiguration(BaseModel):
    ...
    RFIDConfiguration
    """
    configuration: conlist(Dict[str, Any]) = Field(...)
    id: StrictStr = Field(...)
    __properties = ["configuration", "id"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RFIDConfiguration:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> RFIDConfiguration:
        """Create an instance of RFIDConfiguration from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RFIDConfiguration.parse_obj(obj)

        _obj = RFIDConfiguration.parse_obj({
