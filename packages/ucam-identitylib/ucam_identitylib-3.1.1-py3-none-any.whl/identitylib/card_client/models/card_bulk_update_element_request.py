...
class CardBulkUpdateElementRequest(BaseModel):
    """
    Request to update a single card as part of a bulk update.
    """
    action: StrictStr = Field(..., description="The action to apply to this card.  * `cancel` - Cancel * `refresh` - Refresh")
    id: constr(strict=True) = Field(...)
    note: Optional[constr(strict=True, min_length=1)] = None
    __properties = ["action", "id", "note"]

    @validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('cancel', 'refresh'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CardBulkUpdateElementRequest:
        """Create an instance of CardBulkUpdateElementRequest from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CardBulkUpdateElementRequest:
        """Create an instance of CardBulkUpdateElementRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CardBulkUpdateElementRequest.parse_obj(obj)

        _obj = CardBulkUpdateElementRequest.parse_obj({
