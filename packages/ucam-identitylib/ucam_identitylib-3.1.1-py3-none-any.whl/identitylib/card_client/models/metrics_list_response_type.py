...
class MetricsListResponseType(BaseModel):
    """
    MetricsListResponseType
    """
    issued_at: conlist(IssuedAtType) = Field(..., alias="issuedAt")
    revoked_at: conlist(RevokedAtType) = Field(..., alias="revokedAt")
    revoked_at_notes: conlist(RevokedAtNotesType) = Field(..., alias="revokedAtNotes")
    __properties = ["issuedAt", "revokedAt", "revokedAtNotes"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MetricsListResponseType:
        """Create an instance of MetricsListResponseType from a JSON string"""
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in issued_at (list)
        _items = []
        if self.issued_at:
            for _item in self.issued_at:
                if _item:
                    _items.append(_item.to_dict())
            _dict['issuedAt'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in revoked_at (list)
        _items = []
        if self.revoked_at:
            for _item in self.revoked_at:
                if _item:
                    _items.append(_item.to_dict())
            _dict['revokedAt'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in revoked_at_notes (list)
        _items = []
        if self.revoked_at_notes:
            for _item in self.revoked_at_notes:
                if _item:
                    _items.append(_item.to_dict())
            _dict['revokedAtNotes'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> MetricsListResponseType:
        """Create an instance of MetricsListResponseType from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MetricsListResponseType.parse_obj(obj)

        _obj = MetricsListResponseType.parse_obj({
            "issued_at": [IssuedAtType.from_dict(_item) for _item in obj.get("issuedAt")] if obj.get("issuedAt") is not None else None,
            "revoked_at": [RevokedAtType.from_dict(_item) for _item in obj.get("revokedAt")] if obj.get("revokedAt") is not None else None,
            "revoked_at_notes": [RevokedAtNotesType.from_dict(_item) for _item in obj.get("revokedAtNotes")] if obj.get("revokedAtNotes") is not None else None
        })
        return _obj
