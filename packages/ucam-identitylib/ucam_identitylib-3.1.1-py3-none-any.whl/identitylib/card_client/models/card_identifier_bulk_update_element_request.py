...
class CardIdentifierBulkUpdateElementRequest(BaseModel):
    """
    Request to update a single card identifier as part of a bulk update.
    """
    action: StrictStr = Field(..., description="The action to apply to this card identifier.  * `restore` - Restore * `repair` - Repair * `soft_delete` - Soft Delete * `hard_delete` - Hard Delete")
    id: constr(strict=True) = Field(...)
    __properties = ["action", "id"]

    @validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('restore', 'repair', 'soft_delete', 'hard_delete'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CardIdentifierBulkUpdateElementRequest:
        """Create an instance of CardIdentifierBulkUpdateElementRequest from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CardIdentifierBulkUpdateElementRequest:
        """Create an instance of CardIdentifierBulkUpdateElementRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CardIdentifierBulkUpdateElementRequest.parse_obj(obj)

        _obj = CardIdentifierBulkUpdateElementRequest.parse_obj({
