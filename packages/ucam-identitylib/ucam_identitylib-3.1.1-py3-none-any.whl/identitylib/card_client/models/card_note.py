...
class CardNote(BaseModel):
    ...
    CardNote
    """
    card_id: constr(strict=True) = Field(..., alias="cardId")
    created_at: datetime = Field(..., alias="createdAt", description="When this record was created")
    created_by: Optional[StrictStr] = Field(None, alias="createdBy", description="The identifier of the user who created the note - not known for legacy data")
    id: constr(strict=True) = Field(...)
    text: StrictStr = Field(..., description="The text of the note against a given card")
    __properties = ["cardId", "createdAt", "createdBy", "id", "text"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CardNote:
        """Create an instance of CardNote from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CardNote:
        """Create an instance of CardNote from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CardNote.parse_obj(obj)

        _obj = CardNote.parse_obj({
