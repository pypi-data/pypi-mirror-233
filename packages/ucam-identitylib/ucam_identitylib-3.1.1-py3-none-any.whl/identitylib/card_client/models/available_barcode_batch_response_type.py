...
class AvailableBarcodeBatchResponseType(BaseModel):
    ...
    """
    created: conlist(StrictStr) = Field(...)
    invalid: conlist(AvailableBarcodeBatchInvalid) = Field(...)
    __properties = ["created", "invalid"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AvailableBarcodeBatchResponseType:
        """Create an instance of AvailableBarcodeBatchResponseType from a JSON string"""
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in invalid (list)
        _items = []
        if self.invalid:
            for _item in self.invalid:
                if _item:
                    _items.append(_item.to_dict())
            _dict['invalid'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> AvailableBarcodeBatchResponseType:
        """Create an instance of AvailableBarcodeBatchResponseType from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AvailableBarcodeBatchResponseType.parse_obj(obj)

        _obj = AvailableBarcodeBatchResponseType.parse_obj({
            "created": obj.get("created"),
            "invalid": [AvailableBarcodeBatchInvalid.from_dict(_item) for _item in obj.get("invalid")] if obj.get("invalid") is not None else None
        })
        return _obj
