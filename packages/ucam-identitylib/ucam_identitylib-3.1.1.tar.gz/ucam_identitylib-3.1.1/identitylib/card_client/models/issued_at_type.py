...
class IssuedAtType(BaseModel):
    ...
    __properties = ["count", "date", "requestorDomain"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> IssuedAtType:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> IssuedAtType:
        """Create an instance of IssuedAtType from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return IssuedAtType.parse_obj(obj)

        _obj = IssuedAtType.parse_obj({
            "count": obj.get("count"),
            "var_date": obj.get("date"),
            "requestor_domain": obj.get("requestorDomain")
        })
        return _obj
