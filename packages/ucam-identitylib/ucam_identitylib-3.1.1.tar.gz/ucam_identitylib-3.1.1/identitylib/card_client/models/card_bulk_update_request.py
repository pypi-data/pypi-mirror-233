...
class CardBulkUpdateRequest(BaseModel):
    ...
    """
    updates: conlist(CardBulkUpdateElementRequest) = Field(...)
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CardBulkUpdateRequest:
        """Create an instance of CardBulkUpdateRequest from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CardBulkUpdateRequest:
        """Create an instance of CardBulkUpdateRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CardBulkUpdateRequest.parse_obj(obj)

        _obj = CardBulkUpdateRequest.parse_obj({
            "updates": [CardBulkUpdateElementRequest.from_dict(_item) for _item in obj.get("updates")] if obj.get("updates") is not None else None
        })
        return _obj
