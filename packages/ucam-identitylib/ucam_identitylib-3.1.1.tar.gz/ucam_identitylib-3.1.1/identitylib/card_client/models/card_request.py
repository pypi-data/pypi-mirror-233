...
class CardRequest(BaseModel):
    ...
    CardRequest
    """
    affiliations: conlist(Dict[str, Any]) = Field(..., description="The affiliations of the card request - the cardholders associations fetched from the University identity systems.")
    attributes: Optional[Dict[str, Any]] = Field(None, description="The attributes of the card - describing what is present on the physical version of the card. This is an untyped block of JSON and the structure may change between card types, as different card types will allow different data to be printed on them. The attributes of temporary cards will not be populated as data relating to what has been printed on temporary cards has not been recorded.")
    back_visualization_link: StrictStr = Field(..., alias="backVisualizationLink")
    can_update: StrictBool = Field(..., alias="canUpdate")
    cancel_reason: Optional[StrictStr] = Field(None, alias="cancelReason", description="Reason the card request was CANCELLED")
    card_request_version: Optional[conint(strict=True, le=2147483647, ge=-2147483648)] = Field(None, alias="cardRequestVersion", description="Auto-incremented version number of the card request")
    card_type: Optional[StrictStr] = Field(None, alias="cardType", description="The type of the card  * `MIFARE_PERSONAL` - Personal * `MIFARE_TEMPORARY` - Temporary")
    cardholder_status: Optional[StrictStr] = Field(None, alias="cardholderStatus", description="An approximation of the status of the cardholder at the point this card request has been raised. Only used to allow filtering of card requests, rather than as a deterministic indication of a cardholder's status.")
    college_institution_id: Optional[StrictStr] = Field(None, alias="collegeInstitutionId", description="The college institution id used to set the card request scarf  * `NO_COLLEGE` - no-college * `CAIUS` - gonville * `CATH` - st-catherines * `CHRISTS` - christs * `CHURCH` - churchill * `CLARE` - clare * `CLAREH` - clare-hall * `CORPUS` - corpus-christi * `DARWIN` - darwin * `DOWN` - downing * `EDMUND` - st-edmunds * `EMM` - emmanuel * `FITZ` - fitzwilliam * `GIRTON` - girton * `HOM` - homerton * `HUGHES` - hughes-hall * `JESUS` - jesus * `JOHNS` - st-johns * `KINGS` - kings * `LCC` - lucy-cavendish * `MAGD` - magdalene * `NEWH` - murray-edwards * `NEWN` - newnham * `PEMB` - pembroke * `PET` - peterhouse * `QUEENS` - queens * `RIDLEY` - ridley-hall * `ROBIN` - robinson * `SEL` - selwyn * `SID` - sidney-sussex * `TRIN` - trinity * `TRINH` - trinity-hall * `WESLEY` - wesley * `WESTCOT` - westcott * `WESTMIN` - westminster * `WOLFC` - wolfson")
    created_at: Optional[datetime] = Field(..., alias="createdAt", description="When this record was created")
    destination: Optional[StrictStr] = Field(None, description="The location the physical card will be delivered to once created")
    error_reason: Optional[StrictStr] = Field(None, alias="errorReason", description="ERROR raised by card request")
    front_visualization_link: StrictStr = Field(..., alias="frontVisualizationLink")
    hold_reason: Optional[StrictStr] = Field(None, alias="holdReason", description="Reason the card request was placed on HOLD")
    id: constr(strict=True) = Field(...)
    identifiers: conlist(CardIdentifierSummary) = Field(...)
    issue_number: Optional[conint(strict=True, le=2147483647, ge=0)] = Field(None, alias="issueNumber", description="The issue number of the card")
    requestor: Optional[StrictStr] = Field(None, description="Origin of the card request")
    rfid_data: conlist(Dict[str, Any]) = Field(..., alias="rfidData")
    self_link: StrictStr = Field(..., alias="selfLink")
    updated_at: Optional[datetime] = Field(..., alias="updatedAt", description="When this record was updated")
    updated_by: Optional[StrictStr] = Field(None, alias="updatedBy", description="The identifier of the client who last updated this record")
    workflow_state: StrictStr = Field(..., alias="workflowState", description="* `PENDING` - Pending * `HOLD` - Hold * `CANCELLED` - Cancelled * `CREATING_TODO` - ToDo * `CREATING_INPROGRESS` - InProgress * `CREATING_INVERIFICATION` - InVerification * `CREATING_DONE` - Done * `PENDING_CRSID_REQUIRED` - PendingCRSidRequired * `PENDING_PHOTO_REQUIRED` - PendingPhotoRequired * `PENDING_DESTINATION_REQUIRED` - PendingDestinationRequired * `PENDING_EXPIRY_DATA_REQUIRED` - PendingExpiryDataRequired")
    __properties = ["affiliations", "attributes", "backVisualizationLink", "canUpdate", "cancelReason", "cardRequestVersion", "cardType", "cardholderStatus", "collegeInstitutionId", "createdAt", "destination", "errorReason", "frontVisualizationLink", "holdReason", "id", "identifiers", "issueNumber", "requestor", "rfidData", "selfLink", "updatedAt", "updatedBy", "workflowState"]
    ...
    def card_type_validate_enum(cls, value):
        ...
        if value not in ('MIFARE_PERSONAL', 'MIFARE_TEMPORARY'):
            raise ValueError("must be one of enum values ('MIFARE_PERSONAL', 'MIFARE_TEMPORARY')")
        return value

    @validator('college_institution_id')
    def college_institution_id_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('NO_COLLEGE', 'CAIUS', 'CATH', 'CHRISTS', 'CHURCH', 'CLARE', 'CLAREH', 'CORPUS', 'DARWIN', 'DOWN', 'EDMUND', 'EMM', 'FITZ', 'GIRTON', 'HOM', 'HUGHES', 'JESUS', 'JOHNS', 'KINGS', 'LCC', 'MAGD', 'NEWH', 'NEWN', 'PEMB', 'PET', 'QUEENS', 'RIDLEY', 'ROBIN', 'SEL', 'SID', 'TRIN', 'TRINH', 'WESLEY', 'WESTCOT', 'WESTMIN', 'WOLFC', '', 'null'):
    ...
    @validator('workflow_state')
    def workflow_state_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('PENDING', 'HOLD', 'CANCELLED', 'CREATING_TODO', 'CREATING_INPROGRESS', 'CREATING_INVERIFICATION', 'CREATING_DONE', 'PENDING_CRSID_REQUIRED', 'PENDING_PHOTO_REQUIRED', 'PENDING_DESTINATION_REQUIRED', 'PENDING_EXPIRY_DATA_REQUIRED'):
    ...
    def to_dict(self):
        ...
        if self.cancel_reason is None and "cancel_reason" in self.__fields_set__:
            _dict['cancelReason'] = None

        # set to None if college_institution_id (nullable) is None
        # and __fields_set__ contains the field
        if self.college_institution_id is None and "college_institution_id" in self.__fields_set__:
            _dict['collegeInstitutionId'] = None
        ...
        # and __fields_set__ contains the field
        if self.error_reason is None and "error_reason" in self.__fields_set__:
            _dict['errorReason'] = None

        # set to None if hold_reason (nullable) is None
        # and __fields_set__ contains the field
        if self.hold_reason is None and "hold_reason" in self.__fields_set__:
            _dict['holdReason'] = None

        # set to None if issue_number (nullable) is None
        # and __fields_set__ contains the field
        if self.issue_number is None and "issue_number" in self.__fields_set__:
            _dict['issueNumber'] = None

        # set to None if requestor (nullable) is None
        # and __fields_set__ contains the field
        if self.requestor is None and "requestor" in self.__fields_set__:
    ...
    def from_dict(cls, obj: dict) -> CardRequest:
        ...
        _obj = CardRequest.parse_obj({
            ...
            "back_visualization_link": obj.get("backVisualizationLink"),
            "can_update": obj.get("canUpdate"),
            "cancel_reason": obj.get("cancelReason"),
            "card_request_version": obj.get("cardRequestVersion"),
            "card_type": obj.get("cardType"),
            "cardholder_status": obj.get("cardholderStatus"),
            "college_institution_id": obj.get("collegeInstitutionId"),
            "created_at": obj.get("createdAt"),
            "destination": obj.get("destination"),
            "error_reason": obj.get("errorReason"),
            "front_visualization_link": obj.get("frontVisualizationLink"),
            "hold_reason": obj.get("holdReason"),
            "id": obj.get("id"),
            "identifiers": [CardIdentifierSummary.from_dict(_item) for _item in obj.get("identifiers")] if obj.get("identifiers") is not None else None,
            "issue_number": obj.get("issueNumber"),
            "requestor": obj.get("requestor"),
            "rfid_data": obj.get("rfidData"),
