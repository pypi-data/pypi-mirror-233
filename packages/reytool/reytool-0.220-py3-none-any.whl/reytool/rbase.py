...
from typing import Any, List, Tuple, Iterable, Type, Literal, Optional, Union, NoReturn
...
def warn(*infos: Any, exception: Type[BaseException] = UserWarning, stacklevel: int = 3) -> None:
    """
    Throw `warning`.
...
def exc(exception: Type[BaseException] = AssertionError, value: Optional[Any] = None, frame: int = 2) -> NoReturn:
    """
    Throw `exception`.
    ...
    if exception == AssertionError:
        if value_name is None:
            ...
            text = "parameter '%s' use error" % value_name
...
def get_exc(title: Optional[str] = None) -> Tuple[str, Type[BaseException], BaseException, TracebackType]:
    """
...
def check_most_one(*values: Any) -> None:
    ...
    if none_count > 1:
        ...
        if vars_name is not None:
            ...
            vars_name_str = ""
        raise ValueError(f"at most one of parameters{vars_name_str} is not None")
