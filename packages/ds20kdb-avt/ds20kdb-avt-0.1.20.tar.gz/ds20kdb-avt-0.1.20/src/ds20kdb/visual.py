...
class DrawWafer:
    ...
    # wafer plot area
    img = PIL.Image.new(mode='RGBA', size=(side_px, side_px))
    ...
    def __init__(self, wafer_lot, wafer_number, sipm_groups):
    ...
    def _apply_mask(self):
        ...
        """
        # image: mask
        mask_img = PIL.Image.new(mode='1', size=(self.side_px, self.side_px))
        mask_draw = PIL.ImageDraw.Draw(mask_img)
        # construct mask
        # extent defines the mask diameter
        mask_draw.ellipse(
            (
    ...
    def _draw_sipm(self, location, fill_colour, text_colour):
        """
        ...
        label = f'c{col:02} r{row:02}'
        left, top, right, bottom = self.draw.textbbox((x_centre_px, y_centre_px), label)
        xdiff = (right - left) // 2
        ydiff = (bottom - top) // 2
        self.draw.text(
            (x_centre_px - xdiff, y_centre_px - ydiff),
            label,
            fill=text_colour,
            anchor='mm',
        )
    ...
    def _draw_sipms(self, sipm_groups):
        ...
        # draw SiPMs
        for location, (fill_colour, text_colour) in sipms_to_draw.items():
            self._draw_sipm(location, fill_colour, text_colour)
    ...
    def _draw_wafer(self, sipm_groups):
        ...
        -----------------------------------------------------------------------
        """
        # generic SiPM structures outside the valid locations
        rect = itertools.product(
            range(self.minc - 2, self.maxc + 3),
            range(self.minr - 1, self.maxr + 2),
        )
        gold = itertools.chain(
            # top right
        ...
        groups_common_to_all_wafers = [
            {
                'name': 'rect',
                'locations': rect,
                'sipm_colour': self.colour.sipm_generic,
                'text_colour': None,
            },
            {
                'name': 'gold',
                'locations': gold,
                'sipm_colour': self.colour.sipm_gold,
                'text_colour': None,
            },
        ]
        all_sipm_groups = itertools.chain(groups_common_to_all_wafers, sipm_groups)
    ...
    def _draw_wafer_ident(self):
        ...
        ident_cent_x = self.half_side_px
        ident_cent_y = self.sipm_v_size_px
        left, top, right, bottom = self.draw.textbbox(
            (ident_cent_x, ident_cent_y), label
        )
        # draw text as close as possible to SiPM centre
        xdiff = (right - left) // 2
        ydiff = (bottom - top) // 2
        self.draw.text(
            (ident_cent_x - xdiff, ident_cent_y - ydiff),
            label,
            fill='white',
            anchor='mm',
        )
    ...
    def save(self):
        ...
        -----------------------------------------------------------------------
        """
        self.img.save(f'wafer_{self.wafer_lot}_{self.wafer_number:02}.png')
