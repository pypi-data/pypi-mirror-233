...
def parse_payload(input_data: Dict, client: CogniteClient) -> Dict:
    ...
    for i, well_data in enumerate(input_wells):
        if len(well_data["input_logs"].keys()) == 0:
            try:
                bytes_data = client.files.download_bytes(
                    external_id=well_data["input_file_reference"]
                )
            ...
            warnings.warn(
                "Sending input logs directly to the model will soon be deprecated. Migrate to sending a reference to input files in CDF using the 'input_file_reference' field to suppress this warning",
                DeprecationWarning,
                stacklevel=2,
            )
...
def service(data: Dict, secrets: Dict, platform: str = "cdf") -> Dict:
    ...
    try:
        ...
        required_output_curves = ast.literal_eval(
            str(c.info["prediction"]["metadata"]["required_output"])
        )
        is_response_valid = response_validation(
            response=y, required_output=required_output_curves
        )
        ...
        if not completed_successfully:
            error_message += (
                f" See the traceback for more details:\n{traceback.format_exc()}"
            )
