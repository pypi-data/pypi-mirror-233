...
def response_validation(response: List[Dict]) -> bool:
    ...
    Args:
        response (Dict): response from predict function

    Returns:
        bool: whether or not response structure is valid
    """
    required_output_fields = {"well_name", "DEPTH"}
    validated_response = []
    for res in response:
        validated_response.append(required_output_fields.issubset(res.keys()))
