...
def _check_model(model: Any) -> None:
    ...
    y = predict(x, model, secrets)
    assert service_test.prediction_check(y)


def _upload_input_to_cdf_files(input_data: Dict, client: CogniteClient) -> str:
    content = str(json.dumps(input_data))
    id = uuid.uuid4().hex
    external_file_id = f"mlops_demo_input_test_{id}.binary"
...
def _delete_input_file_from_cdf_files(
    file_reference: str, client: CogniteClient
) -> None:
    client.files.delete(external_id=file_reference)


def _check_model_input_file_reference(model_elements: Any) -> None:
    x = service_test.prediction_input_file_reference
    x_to_upload = service_test.prediction_input["input"][0]["input_logs"]
    client = get_client(
    ...
    external_file_id = _upload_input_to_cdf_files(input_data=x_to_upload, client=client)
    x["input"][0]["input_logs"] = {}  # type: ignore
    x["input"][0]["input_file_reference"] = external_file_id  # type: ignore
    x["return_file"] = False
    try:
        y_pred = predict(x, model_elements, secrets)
        _delete_input_file_from_cdf_files(
            file_reference=external_file_id, client=client
        )
        ...
        raise AssertionError(
            "Failed to make predictions using CDF input file reference"
        ) from e
    service_test.prediction_check(y_pred)
...
def test_predict_with_input_file_reference():
    model = initialization("model_artifact", secrets=secrets)
    _check_model_input_file_reference(model)
