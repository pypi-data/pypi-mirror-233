...
def draw_corr(df: pd.DataFrame, target_col: str, methods: list = ['pearson', 'spearman', 'kendall'], num_top_features: int = 10):
    """
...
def df_scatter_plot(df: pd.DataFrame, folder_path: str = None, smiles_col: str = "SMILES"):
    """
    ...
    for column in df_numeric.columns:
        if column in [smiles_col, "id"]:
            ...
            try:
                ...
                sns.scatterplot(x=df.index, y=df[column], alpha=0.5, label='Scatter Plot', palette='Paired', edgecolor=None)
                plt.title(f"Scatter Plot of {column}")
                plt.grid(color='lightgray', linestyle='--', linewidth=0.5)  # Add grid with light gray color and dashed lines
