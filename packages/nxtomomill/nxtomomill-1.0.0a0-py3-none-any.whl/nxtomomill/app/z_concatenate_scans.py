...
.. code-block:: bash
     complete here with the output of --help
"""
...
def get_arguments(argv):
    ...
    if args.pixel_size_m is not None:
        ...
        with h5py.File(args.file_list[0], "r") as h5f:
...
def main(argv):
    ...
    for i_stage in range(args.first_stage, args.last_stage + 1):
        ...
        if dummy_frame is None:
            if len(nxt.instrument.detector.data) > 0:
                ...
                with h5py.File(output_file, mode="w") as h5s:
    ...
    with h5py.File(output_file, "r+") as output_target:
