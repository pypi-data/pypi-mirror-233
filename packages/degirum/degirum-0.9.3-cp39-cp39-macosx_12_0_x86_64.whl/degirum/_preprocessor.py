...
from .CoreClient import ModelParams as ModelParams
...
class _Image_Preprocessor:
    ...
    def __init__(
        ...
        *,
        ...
        """
        Constructor
            - `model_params`: model parameters object returned from CoreClient.SyncRuntime.params;
    ...
    def _apply_pad_method(self, image, colorspace="RGB"):
        ...
        h = self._model_params.InputH[self._model_input]
        w = self._model_params.InputW[self._model_input]
        ...
        if self._pad_method == "stretch" and (iw != w or ih != h):
            ...
            coord_conv = lambda x, y: (
                iw * min(1, max(0, (x - dx) / (w - 2 * dx))),
                ih * min(1, max(0, (y - dy) / (h - 2 * dy))),
            )
...
class _Preprocessor_CV(_Image_Preprocessor):
    ...
    def forward(self, image):
        ...
        if self.image_format == "JPEG":
            ...
            return dict(
                raw_result=buf.tobytes(),  # save to byte buffer
...
class _CommonPreprocessor:
    ...
    def forward(self, data):
        ...
        return dict(raw_result=data.tobytes())
...
class _TensorPreprocessor(_CommonPreprocessor):
    ...
    def forward(self, data):
        ...
        """
        shape = (
            self._model_params.InputN[self._model_input],
            self._model_params.InputH[self._model_input],
            self._model_params.InputW[self._model_input],
            self._model_params.InputC[self._model_input],
        )
        if not isinstance(data, numpy.ndarray):
            raise DegirumException(
        ...
        if numpy.shape(data) != shape:
            raise DegirumException(
                f"Shape of tensor passed as the input #{self._model_input} does not match to model parameters. Expected tensor shape is {shape}."
            )
        return dict(raw_result=data.tobytes())
...
class _PreprocessorAggregator:
    ...
    def _image_property_iter(self):
        ...
        try:
            ...
            raise DegirumException("There is no Image inputs in model")
        yield from iterator
    ...
    def image_backend(self, val: str):
        ...
        for i, p in enumerate(self._preprocessors):
            ...
            if val == p.image_backend:
                continue
            self._preprocessors[i] = create_image_preprocessor(
                self._model_parameters, image_backend=val
            )
            # reassign previous property values to new backend
            self._preprocessors[i].resize_method = p.resize_method
            self._preprocessors[i].pad_method = p.pad_method
            self._preprocessors[i].fill_color = p.fill_color
            self._preprocessors[i].input_colorspace = p.input_colorspace
            self._preprocessors[i].crop_percentage = p.crop_percentage
