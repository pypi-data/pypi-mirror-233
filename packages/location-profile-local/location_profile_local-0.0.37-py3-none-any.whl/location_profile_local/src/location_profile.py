...
class LocationProfiles(GenericCRUD):
    ...
    def get_last_location_id_by_profile_id(self, profile_id: int) -> int:
        ...
        #location_id = self.select_one_by_id(table_name="location_profile_view", select_clause_value="location_id", id_column_name="profile_id", id_column_value=profile_id)
        location_id = self.select_multi_by_where(table_name="location_profile_view", select_clause_value="location_id", where=f"profile_id = {profile_id}",limit=1,order_by="start_timestamp desc")
    ...
    def get_location_ids_by_profile_id(self, profile_id: int) -> List[Location]:
        ...
        logger.start(GET_LOCATION_IDS_BY_PROFILE_ID_METHOD_NAME,
                     object={'profile_id': profile_id})
        location_ids = self.select_multi_by_id(table_name="location_profile_view", select_clause_value="location_id",
