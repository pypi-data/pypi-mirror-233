...
def get_custom_albumentations_file(img_path):
    """Get custom albumentations file if it exists in the image path"""
    # Check if there is a file called "albumentations.yml" or "albumentations.yaml" in the image path
    custom_albumentations_file = None
    yml_file_path = os.path.join(img_path, "albumentations.yml")
    yaml_file_path = os.path.join(img_path, "albumentations.yaml")

    if os.path.exists(yml_file_path):
        print("*** albumentations.yml found")
        custom_albumentations_file = yml_file_path
    elif os.path.exists(yaml_file_path):
        print("*** albumentations.yaml found")
        custom_albumentations_file = yaml_file_path
    else:
        print("*** no albumentations.yml or albumentations.yaml found")

    return custom_albumentations_file


def build_yolo_dataset(cfg, img_path, batch, data, mode="train", rect=False, stride=32):
    """Build YOLO Dataset"""

    custom_albumentations_file = get_custom_albumentations_file(img_path)
    ...
    return YOLODataset(
        ...
        custom_albumentations_file=custom_albumentations_file,
        classes=cfg.classes,
        data=data,
