...
class Albumentations:
    ...
    def __init__(self, p=1.0, custom_albumentations_file=None):
        ...
        try:
            ...
            if custom_albumentations_file is not None:
                LOGGER.info(
                    f"{prefix}loading albumentations from {custom_albumentations_file}"
                )
                T = self.load_albumentations_from_yaml(custom_albumentations_file)
            else:
                T = [
                    A.Blur(p=0.01),
                    A.MedianBlur(p=0.01),
                    ...
                    A.RandomGamma(p=0.0),
                    A.ImageCompression(quality_lower=75, p=0.0),  # default transforms
                ]
    ...
    def load_albumentations_from_yaml(self, yaml_path):
        import yaml
...
def v8_transforms(dataset, imgsz, hyp, custom_albumentations_file=None, stretch=False):
    ...
    return Compose(
        [
            ...
            Albumentations(
                p=1.0, custom_albumentations_file=custom_albumentations_file
            ),
        ...
        if augment:  # Resize and crop
            ...
            if auto_aug:
                ...
                if any((hsv_h, hsv_s, hsv_v)):
                    T += [
                        A.ColorJitter(*hsv2colorjitter(hsv_h, hsv_s, hsv_v))
