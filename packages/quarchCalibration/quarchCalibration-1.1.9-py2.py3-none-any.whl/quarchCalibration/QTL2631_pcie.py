...
def getFixtureData(device,channel):
    ...
    if (channel == "+12V V"):
        ...
        return parseFixtureData(data,16,17)
    elif (channel == "+3.3V V"):
        return parseFixtureData(data,33,16)
    elif (channel == "+3.3V dV"):
        return parseFixtureData(data,49,17)
    elif (channel == "+3.3Vaux V"):
        return parseFixtureData(data,66,16)
    elif (channel == "+3.3Vaux dV"):
        return parseFixtureData(data,82,17)
    elif (channel == "+12Vaux V"):
        return parseFixtureData(data,99,16)
    elif (channel == "+12Vaux dV"):
        return parseFixtureData(data,115,17)


class QTL2631_pcie (PowerModule):
    ...
    '+12V_VOLT_MULTIPLIER'       : '0xA105',
    '+12V_VOLT_OFFSET'           : '0xA106',
    '+12V_CURR_MULTIPLIER'       : '0xA107',
    '+12V_CURR_OFFSET'           : '0xA108',
    #'POWER_1_LEAKAGE_MULTIPLIER'   : '0xA109',	-- Not used in External Shunt Mezzanine
    '+3.3V_VOLT_MULTIPLIER'       : '0xA10A',
    '+3.3V_VOLT_OFFSET'           : '0xA10B',
    '+3.3V_CURR_MULTIPLIER'       : '0xA10C',
    '+3.3V_CURR_OFFSET'           : '0xA10D',
    #'+3.3V_LEAKAGE_MULTIPLIER'   : '0xA10E',	-- Not used in External Shunt Mezzanine
    '+3.3Vaux_VOLT_MULTIPLIER'       : '0xA10F',
    '+3.3Vaux_VOLT_OFFSET'           : '0xA110',
    '+3.3Vaux_CURR_MULTIPLIER'       : '0xA111',
    '+3.3Vaux_CURR_OFFSET'           : '0xA112',
    #'+3.3Vaux_LEAKAGE_MULTIPLIER'   : '0xA113',	-- Not used in External Shunt Mezzanine
    '+12Vaux_VOLT_MULTIPLIER'       : '0xA114',
    '+12Vaux_VOLT_OFFSET'           : '0xA115',
    '+12Vaux_CURR_MULTIPLIER'       : '0xA116',
    '+12Vaux_CURR_OFFSET'           : '0xA117',
    'CALIBRATION_COMPLETE'          : '0xA118'
    }

    CAL_CONTROL_SETTINGS = {
    '+12V_CALIBRATION_CONTROL_SETTING'	: '0x00F0',	#set full averaging
    '+3.3V_CALIBRATION_CONTROL_SETTING'	: '0x00F0',	#set full averaging
    '+3.3Vaux_CALIBRATION_CONTROL_SETTING'	: '0x00F0',	#set full averaging
    '+12Vaux_CALIBRATION_CONTROL_SETTING'	: '0x00F0',	#set full averaging
    '+12V_SHUNT'                         : 0.002,
    '+3.3V_SHUNT'                         : 0.005,
    '+3.3Vaux_SHUNT'                         : 0.02,
    '+12Vaux_SHUNT'                         : 0.0004
    ...
    host_switchbox_mapping = {'12V':'A','+12V_IN':'1','+3.3V_IN':'2','+3.3Vaux_IN':'3','+12Vaux_IN':'4'}
    ...
    load_switchbox_mapping = {'LOAD':'A','+12V_OUT':'1','+3.3V_OUT':'2','+3.3Vaux_OUT':'3','+12Vaux_OUT':'4'}
    ...
    def __init__(self,dut):
        ...
        self.name = "Gen5 AIC x16 PAM Fixture with AUX Power"
        self.dut = dut
        ...
        self.calibrations["+12V"] = {
            "Voltage":self.QTL2631_pcie_VoltageCalibration(self,"+12V"),
            "DeltaVolts":self.QTL2631_pcie_deltaVoltCalibration(self,"+12V")
            }
        # populate +3.3V channel with calibrations
        self.calibrations["+3.3V"] = {
            "Voltage":self.QTL2631_pcie_VoltageCalibration(self,"+3.3V"),
            "DeltaVolts":self.QTL2631_pcie_deltaVoltCalibration(self,"+3.3V")
            }
        # populate +3.3Vaux channel with calibrations
        self.calibrations["+3.3Vaux"] = {
            "Voltage":self.QTL2631_pcie_VoltageCalibration(self,"+3.3Vaux"),
            "DeltaVolts":self.QTL2631_pcie_deltaVoltCalibration(self,"+3.3Vaux")
            }
        # populate +12Vaux channel with calibrations
        self.calibrations["+12Vaux"] = {
            "Voltage":self.QTL2631_pcie_VoltageCalibration(self,"+12Vaux"),
            "DeltaVolts":self.QTL2631_pcie_deltaVoltCalibration(self,"+12Vaux")
            }

        self.verifications = {}
        # populate +12V channel with verifications
        self.verifications["+12V"] = {
            "Voltage":self.QTL2631_pcie_VoltageVerification(self,"+12V"),
            "Current":self.QTL2631_pcie_deltaVoltVerification(self,"+12V")
            }
        self.verifications["+3.3V"] = {
            "Voltage":self.QTL2631_pcie_VoltageVerification(self,"+3.3V"),
            "Current":self.QTL2631_pcie_deltaVoltVerification(self,"+3.3V")
            }
        self.verifications["+3.3Vaux"] = {
            "Voltage":self.QTL2631_pcie_VoltageVerification(self,"+3.3Vaux"),
            "Current":self.QTL2631_pcie_deltaVoltVerification(self,"+3.3Vaux")
            }
        self.verifications["+12Vaux"] = {
            "Voltage":self.QTL2631_pcie_VoltageVerification(self,"+12Vaux"),
            "Current":self.QTL2631_pcie_deltaVoltVerification(self,"+12Vaux")
            }
    ...
    class QTL2631_pcieCalibration (Calibration):
    ...
    class QTL2631_pcie_VoltageCalibration (QTL2631_pcieCalibration):
    ...
    class QTL2631_pcie_deltaVoltCalibration (QTL2631_pcieCalibration):
        ...
        def __init__(self,powerModule,thisChannel):
            ...
            self.relErrorLimit = 1                  # 1%
            self.test_min = 10                      # 10uV
            self.test_max = 50000                   # 50000uV
            if self.test_max > (self.powerModule.CAL_CONTROL_SETTINGS[self.thisChannel + "_SHUNT"]*4)*1000000:  # if test_max is going to push the Keithely over 4 amps
                self.test_max = (self.powerModule.CAL_CONTROL_SETTINGS[self.thisChannel + "_SHUNT"]*4)*1000000    # limit test max to whatever the keithley can source
            self.test_steps = 20
            self.units = "uV"
            self.scaling = 16
            self.multiplier_signed = False
        ...
        def setRef(self,value):
            ...
            # i = V/R = (value in uV/1000000) / Resistance
            self.powerModule.calInstrument.setReferenceCurrent((value/1000000)/self.powerModule.CAL_CONTROL_SETTINGS[self.thisChannel + "_SHUNT"])

        def readRef(self):

            # The Keithley sources resistance in Amps, the calibration board has a 5mOhm resistor on it
            # we need to convert the amp value back into a voltage in uV
            # V= IR = current*resistance*1000000           
            return self.powerModule.calInstrument.measureLoadCurrent()*self.powerModule.CAL_CONTROL_SETTINGS[self.thisChannel + "_SHUNT"]*1000000

        def readVal(self):

            return getFixtureData(self.powerModule.dut,self.thisChannel + " dV")
    ...
    class QTL2631_pcie_VoltageVerification (QTL2631_pcieCalibration):
    ...
    class QTL2631_pcie_deltaVoltVerification (QTL2631_pcieCalibration):
