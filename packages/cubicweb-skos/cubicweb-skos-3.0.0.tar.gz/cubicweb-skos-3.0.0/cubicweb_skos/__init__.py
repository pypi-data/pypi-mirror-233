...
def register_skos_concept_rdf_list_mapping(reg):
    ...
    reg.register_attribute_equivalence("Concept", "definition", "skos:definition")
...
def register_skos_concept_rdf_output_mapping(reg, with_labels=True):
    ...
    reg.register_relation_equivalence(
        "Concept", "broader_concept", "Concept", "skos:broader"
    )
    reg.register_relation_equivalence(
        "Concept", "broader_concept", "Concept", "skos:narrower", reverse=True
    )
    reg.register_relation_equivalence(
        "Concept", "related_concept", "Concept", "skos:related"
    )
    # may be a concept instead of an ExternalUri if the concept is locally known, but that is
    # handled by the importer
    reg.register_relation_equivalence(
        "Concept", "exact_match", "ExternalUri", "skos:exactMatch"
    )
    reg.register_relation_equivalence(
        "Concept", "close_match", "ExternalUri", "skos:closeMatch"
    )
...
LABELS_RDF_MAPPING = {
    ...
    "alternative_label": "skos:altLabel",
