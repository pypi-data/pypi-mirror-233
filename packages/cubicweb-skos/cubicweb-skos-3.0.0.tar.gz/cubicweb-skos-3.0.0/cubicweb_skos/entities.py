...
def _add_concept(scheme, label, language_code, kind="preferred", **kwargs):
    cnx = scheme._cw
    concept = cnx.create_entity("Concept", in_scheme=scheme, **kwargs)
    cnx.create_entity(
        "Label", label=label, language_code=language_code, kind=kind, label_of=concept
    )
...
class ConceptScheme(AnyEntity):
    ...
    def add_concept(self, label, language_code="en", kind="preferred", **kwargs):
...
class Concept(AnyEntity):
    ...
    def labels(self):
        ...
        labels = {}
        for lab in self.preferred_label:
            labels[lab.short_language_code] = lab.label
            labels[lab.language_code] = lab.label
        return labels

    def label(self, language_code=None, default_language_code="en"):
        if language_code is None:
            language_code = self._cw.lang
        for possible_lang in (
            language_code,
            language_code[:2],
    ...
    def add_concept(self, label, language_code="en", kind="preferred"):
        """Add a sub-concept to this concept"""
        return _add_concept(
            self.scheme, label, language_code, kind, broader_concept=self
        )
...
class ConceptSchemeRDFPrimaryAdapter(AbstractRDFAdapter):
    ...
    def warm_caches(self):
        ...
        cnx = self._cw
        scheme.complete()
        concept_rql = _select_attributes(
            "Any CS,C WHERE C in_scheme CS, CS eid %(cs)s", "C", self.concept_attributes
        )
        ...
        for rtype in ("exact_match", "close_match"):
            rset = cnx.execute(
                "Any X,C,XU WHERE C %s X?, C in_scheme CS, CS eid %%(cs)s, "
                "X cwuri XU" % rtype,
                {"cs": scheme.eid},
            )
            cache_entities_relations(concepts, rset, rtype, "subject")
        for rtype in ("broader_concept", "related_concept"):
            rset = cnx.execute(
                "Any C1,C2 WHERE C1 %s C2, C1 in_scheme CS, C2 in_scheme CS, "
                "CS eid %%(cs)s" % rtype,
                {"cs": scheme.eid},
            )
...
def cache_entities_relations(entities, rset, rtype, role, entity_col=1, target_col=0):
    ...
    * related entities are in `target_col` column of the rset
    ...
    cache_key = f"{rtype}_{role}"
    for entity, subrset in _split_rset(rset, entity_col):
        entity._cw_related_cache[cache_key] = (
            subrset,
            tuple(subrset.entities(target_col)),
        )
        try:
