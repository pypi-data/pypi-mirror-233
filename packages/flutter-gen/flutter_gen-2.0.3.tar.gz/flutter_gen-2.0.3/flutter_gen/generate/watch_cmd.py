...
class WatchCommand(Command):
    ...
    def run(self):
        ...
        GenerateRepoCommand().run()
        # Start watch
        thread_mobx = multiprocessing.Process(target=self.build_runner)
        thread_mobx.start()
        thread_image = multiprocessing.Process(
            target=self.monitor,
            args=("./assets/images", self.gen_image, ["*.jpg", "*.png", "*.svg"]),
        )
        thread_image.start()
        ...
        thread_router = multiprocessing.Process(
            target=self.monitor,
            args=("./lib/scenes", self.gen_router, ["app_pages.dart"]),
        )
        thread_router.start()
        thread_repo = multiprocessing.Process(
            target=self.monitor,
            args=("./lib/data/repositories", self.gen_repo, ["*_repository.dart"]),
        )
        thread_repo.start()
        thread_localization = multiprocessing.Process(
            target=self.monitor,
            args=("./assets/i18n", self.gen_localization, ["*.json"]),
        )
