...
def gen(init, inp, ele, Natom_list, nsys, name, system_weight, spg, Natom_weight=[], cellbounds=None, rcut=0.5, form='vasp'):
    ...
    for i in range(nsys):
        ...
        if init == 1:
            ...
            sys = gen_pyxtal(ele, Natom_dist[i], spacegroups=spg, volume=None, cellbounds=cellbounds, rcut=rcut, form=form)
        if sys == False:
            continue
        else:
            if form == 'extxyz':
...
def gen_ase(ele, Natom, volume=None, cellbounds=None, N=1, rcut=0.5, form='vasp'):
...
def gen_pyxtal(ele, Natom, spacegroups=0, volume=None, cellbounds=None, filename=None, rcut=0.5, form='vasp'):
    if cellbounds == None:
    ...
    while True:
        if spacegroups == 0:
            ...
            spg = spacegroups
        try:
            my_crystal.from_random(3, spg, ele, Natom)
        except:
            continue
