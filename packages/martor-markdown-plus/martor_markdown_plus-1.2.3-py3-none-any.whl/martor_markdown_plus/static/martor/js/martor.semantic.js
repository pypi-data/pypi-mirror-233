...
(function ($) {
    ...
    $.fn.martor = function() {
        ...
        this.each(function(i, obj) {
            ...
            var markdownToTable = function(editor) {
                console.log('D')
                ...
                if (editor.selection.isEmpty()) {
                    ...
                    editor.focus();
                    editor.selection.moveTo(curpos.row+3, curpos.column+40);
                }else {
            ...
            var markdownToAdmonitionNote = function(editor) {
                var originalRange = editor.getSelectionRange();
                if (editor.selection.isEmpty()) {
                    var curpos = editor.getCursorPosition();
                    editor.session.insert(curpos, '!!! note\n    ');
                    editor.focus();
                    editor.selection.moveTo(curpos.row+1, curpos.column+5);
                }else {
                  var range = editor.getSelectionRange();
                  var text = editor.session.getTextRange(range);
                  editor.session.replace(range, '!!! note\n    '+text);
                  originalRange.end.column += 5;
                  editor.focus();
                  editor.selection.setSelectionRange(originalRange);
                }
            ...
            var markdownToLink = function(editor) {
                ...
                if (editor.selection.isEmpty()) {
                    var curpos = editor.getCursorPosition();
                    editor.session.insert(curpos, ' [](https://){:target="_blank"} ');
                    editor.focus();
                    editor.selection.moveTo(curpos.row, curpos.column+2);
                }else {
                  var range = editor.getSelectionRange();
                  var text = editor.session.getTextRange(range);
                  editor.session.replace(range, '['+text+'](https://){:target="_blank"} ');
                  editor.focus();
                  editor.selection.moveTo(
            ...
            // Trigger Click
            $('.markdown-table[data-field-name='+field_name+']').click(function(){
                markdownToTable(editor);
            });
            $('.markdown-admonition-note[data-field-name='+field_name+']').click(function(){
                markdownToAdmonitionNote(editor);
            });
