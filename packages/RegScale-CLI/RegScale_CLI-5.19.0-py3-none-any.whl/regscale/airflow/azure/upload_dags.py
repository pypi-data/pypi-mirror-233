...
def upload_dag_to_blob_storage(
    ...
    :param str file_path: Path to the DAG file
    ...
    try:
        ...
        blob_client.upload_blob(file_path.read_bytes())
...
def retrieve_dags_to_upload(
    path: Union[str, Path], file_extension: str = ".py"
) -> List[Tuple[Path, str]]:
    """Retrieve DAGs to upload to Azure Blob Storage
    :param str path: Path to the DAGs folder
...
def upload_dags_to_blob_storage(
    ...
    if connection_string is None or container_name is None:
        logger.error(
            "Please provide the connection string and container name for Azure Blob Storage."
        )
        if exit_on_failure:
            sys.exit(1)
