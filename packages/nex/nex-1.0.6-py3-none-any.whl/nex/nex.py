""" The main module of nex Python package.
    For documentation, see 
    https://www.neuroexplorer.com/docs/reference/scripting/index.html
"""
...
def _GetListOfNumbers(varWithListOfNumbers, varName):
    """ Internal function. Make sure that passed parameter can be converted to a list of numbers. """
    try:
...
class NexVar:
    ...
    def PreThresholdTime(self) -> float:
        """Returns the pre-threshold time (in seconds) of a waveform variable."""
        return self._Get('PreThresholdTime')

    def SetPreThresholdTime(self, preThresholdTimeInSeconds:float):
        """Sets the pre-threshold time of a waveform variable."""
        callPars = {}
        callPars['functionName'] = 'SetPreThresholdTime'
        callPars['parameters'] = [{'varId':self.varId}, {'PreThresholdTime':preThresholdTimeInSeconds}]
        values = []
    ...
    def FragmentCounts(self) -> List[float]:
        """Returns fragment counts (number of data points in each fragment) of a continuous variable."""
        return self._Get('FragmentCounts')

    def Intervals(self) -> List[List[float]]:
        """
        Returns intervals of an interval variable as a list.
        The first element of the returned list is the list of all interval start times in seconds.
        The second element of the returned list is the list of all interval end times in seconds.
        """
    ...
    def WaveformValues(self) -> List[List[float]]:
        ...
        the second - values of the second waveform, etc. Values are in milliVolts.
        """
    ...
    def SetContVarTimestampsAndValues(self, timestamps:List[float], values:List[float]):
        ...
        timestamps = _GetListOfNumbers(timestamps, 'timestamps')
        values = _GetListOfNumbers(values, 'values')
...
class NexDoc:
    ...
    def _CheckIfValidDoc(self):
        """Internal method. Checks if a document is valid and throws error if a document is not valid."""
        if not self:
    ...
    def GetNumResValue(self, row:int, col:int) -> float:
        """Returns the specified numerical results value."""
    ...
    def GetAllNumericalResults(self) -> List[List[float]]:
        ...
        The first element contains the values of the first column of the numerical results.
        The second element contains values of the second column of the numerical results, etc.
        """
        self._CheckIfValidDoc()
    ...
    def GetAllAnalysisParameters(self) -> str:
        """Returns all analysis parameters as a JSON string consisting of pairs {parName : parValueAsString}."""
        return self._Get('AllAnalysisParameters')
    
    def GetSelVarNames(self) -> List[str]:
        """Returns selected variable names as a list of strings."""
        self._CheckIfValidDoc()
...
def Dialog(*arg) -> int:
    ...
    Returns 0 if a user pressed Cancel button, otherwise, returns 1.
...
# FUNCTIONS THAT CALL PYTHON INTERNAL FUNCTIONS
...
# repeat doc member functions as global nex functions
...
# end of repeat doc member functions as global nex functions
