...
import click
from provider.terraform import Terraform
from output.inventory import Inventory


@click.group()
@click.version_option("1.0.0", prog_name="state2inventory")
def cli() -> None:
...
def terraform(state_file: click.File, output_file: click.File, key: str) -> None:
    ...
    try:
        inventory: Inventory = provider.parse()

        output = inventory.generate()
        if len(output) > 1:
