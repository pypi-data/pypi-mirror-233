...
class TransactionSet:
        ...
        for claim in self.claims:
            for service in claim.services:
                datum = TransactionSet.serialize_service(
                    self.check_information, self.financial_information, self.payer, claim, service
                )
    ...
    def serialize_claim(
        ...
        # --------------------
        datum = {
            "Service Dates": claim.claim.service_period_start.date + " - " + claim.claim.service_period_end.date,
            "Claim Number": claim.claim.claim_number,
            "Payer": payer.organization.name,
            "Check/EFT Number (Check/EFT Date)": check_information.check_number + ' (' + financial_information.transaction_date + ')',
            "Patient Name (Patient Control Number) (ID)": claim.patient.last_first_name + ' (' + claim.patient.identification + ') (' + claim.patient.identification_code + ')',
            "Patient Amt": '',
    ...
    def build(cls, file_path: str) -> "TransactionSet":
        ...
        return TransactionSet(
            check_information, interchange, financial_information, claims, organizations, file_path
        )
