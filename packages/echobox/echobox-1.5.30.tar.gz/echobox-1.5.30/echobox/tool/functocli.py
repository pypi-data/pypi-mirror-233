...
from echobox.tool import datatypes
...
class AppSepc(object):
    ...
    def run(self):
        kwargs = self.kwargs
        if self.common_args_option.get('run_app_config', {}).get('with_method_will_call', False):
            self.common_kwargs['method_will_call'] = self.method_will_call
...
def get_cls_parent_methods_for_exclude(cls):
    ...
    for parent_cls in cls.__bases__:
        ...
        parent_methods = {key: val for key, val in parent_methods.items() if
                          not val.__dict__.get('keep_method_for_cli', False)}
...
def build_argparser_for_func(method_name, method_spec):
    ...
    for arg_name in args:
        ...
        if default_values is not None and arg_name in default_values:
            if len(arg_name) == 1:
                parser.add_argument('-' + arg_name, help=help, default=default_values[arg_name], nargs=1,
                                    required=False)
            else:
                parser.add_argument('--' + arg_name, help=help, default=default_values[arg_name], nargs=1,
                                    required=False)
            ...
            if len(arg_name) == 1:
                parser.add_argument('-' + arg_name, help=help, nargs=1, required=True)
            else:
                parser.add_argument('--' + arg_name, help=help, nargs=1, required=True)
