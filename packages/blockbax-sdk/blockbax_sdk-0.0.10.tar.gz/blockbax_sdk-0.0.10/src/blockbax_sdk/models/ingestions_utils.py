...
from .series import IngestionRequest, SeriesRequest
...
class Batch:
    ...
    def create_series_request(self) -> SeriesRequest:
        return SeriesRequest(
            series=[
                IngestionRequest(
                    ingestionId=ingestion_id,
                    measurements=[m.to_request() for m in measurements],
                )
...
class Batcher:
    ...
    def next(self, force: bool = False) -> Iterable[SeriesRequest]:
...
def create_series_batches(
    ...
    for ingestion in ingestions:
        external_id_result = split_ingestion_id_into_external_ids(ingestion.id)
        for measurement in ingestion.measurements:
            timestamp = datetime.min  # send as soon as posible if no date is provided
            ...
            if external_id_result is not None:
                ...
                measurements[timestamp][0].setdefault(subject_external_id, []).append(
                    (ingestion.id, measurement)
                )
            else:
                measurements[timestamp][1].append((ingestion.id, measurement))

    batcher = Batcher()

    for timestamp, ingested_measurments in collections.OrderedDict(
        ...
        ) = ingested_measurments

        # Add measurements per subject to ensure that the same measurements per subject for this timestamp are ingesteed together
        for (
            _,
