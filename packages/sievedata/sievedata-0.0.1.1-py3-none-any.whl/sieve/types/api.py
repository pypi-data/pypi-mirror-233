...
from ..api.constants import JOB_ID, JOB_SOURCE_NAME, JOB_SOURCE_TYPE, JOB_STATUS, MODEL_ID, MODEL_NAME, PROJECT_LAYER_ITERATION_TYPE, PROJECT_LAYER_MODELS, MODEL_VERSION
...
class SieveModel:
    ...
    def dict(self) -> Dict:
        ...
        if self.id != None and self.name != None and self.version != None:
            return {
                MODEL_ID: self.id,
        ...
        if self.name != None and self.version != None:
            return {
                MODEL_NAME: self.name,
                MODEL_VERSION: self.version
            }
...
class SieveLayer:
    ...
    @property
    def dict(self) -> Dict:
        plm = [model.dict for model in self.models]
        return {
            PROJECT_LAYER_ITERATION_TYPE: self.iteration_type.value,
            PROJECT_LAYER_MODELS:  plm
        }
