    ...
    <div :class="[$style['mission-leg-wrapper__content']]">
      ...
      <div class="flex flex-col mb-4 lg:mb-0">
        <div class="flex items-start gap-x-4 w-full">
          ...
          <div class="w-5/12">
            <ULabel required label-text="Time:" />
            <UFlatPickr
          ...
          <div class="w-32">
            <ULabel :required="false" label-text="Zone:" />
            <USelectWrapper
          ...
          <div class="w-7/12">
            ...
            <UFlatPickr
              ...
              :config="{
                ...
                altFormat: 'Y-m-d',
                dateFormat: 'Y-m-d',
                minDate: new Date(missionFormModel.legs[legIndex].departure_datetime),
                minuteIncrement: 1
              }"
...
import UFlatPickr from '@/components/ui/form/UFlatPickr.vue'
...
const computedMinimumDepartureTime = computed(() => {
  ...
  const legDepartureTime = getLeg(props.legIndex)?.departure_datetime as string
  const legArrivalTime = getLeg(props.legIndex - 1)?.arrival_datetime as string
  ...
  return props.legIndex === 0 || departureDate !== arrivalDate ? null : `${h}:${m}`
})

const computedMinimumArrivalTime = computed(() => {
  const getLeg = (index: number) => missionFormModel.value?.legs?.[index]
  const legDepartureTime = getLeg(props.legIndex)?.departure_datetime as string
  const legArrivalTime = getLeg(props.legIndex)?.arrival_datetime as string
...
const onCrewUpdate = (value) => {
  ...
  if (missionFormModel.value.legs.length) {
    missionFormModel.value.legs = missionFormModel.value.legs.map((el) =>
      el.pob_crew ? el : { ...el, pob_crew: value }
    )
  }
}
...
const onChangeDepartureLocation = (
  ...
  if (!airport.is_lat_lon_available) {
    missionFormModel.value.legs[props.legIndex].departure_datetime_is_local =
...
const onChangeArrivalLocation = (
  ...
  if (!airport.is_lat_lon_available) {
    missionFormModel.value.legs[props.legIndex].arrival_datetime_is_local =
      airport.is_lat_lon_available
  }
