...
def netMetrics(net:AdvNet, xArr:np.ndarray, yArr:np.ndarray, returnAll:bool = False, 
    ...
    method:
        - Can be 'r2', 'aae', or 'sse'
        - 'r2' returns the R^2 fit of the model's predictions to the data.
        - 'aae' returns the average-ansolute error (gives inverse so that for both metrics,
          the model 'score' will always increase with accuracy).
        - 'sse' returns the inverse of the sum-squared-error (gives inverse so that for both metrics,
          the model 'score' will always increase with accuracy).
    """
...
def r2d2(yHat:np.ndarray, yTrue:np.ndarray):
    """
    Returns the R^2 value of the model values (yHat) against the true
    y data (yTrue).
    """
    ...
    return (np.sum((yHat - np.mean(yTrue)) ** 2) / np.sum((yTrue - np.mean(yTrue)) ** 2))
    
    # # Calculate Residue
    # SSres = np.sum((np.array(yHat) - yTrue) ** 2)

    # # Regression
    # SSreg = np.sum((np.array(yHat) - np.mean(yTrue)) ** 2)
