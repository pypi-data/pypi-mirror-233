...
sample_type.assign_property(
	prop                 = 'diff_time',   # Mandatory value
	section              = '',            # Name of the section
	ordinal              = 1,             # Position of property
	mandatory            = True,
...
The `get_samples()` method, by default, returns fewer attributes to make the download process faster.
However, if you want to include specific attributes in the results, you can do so by using the `attrs` parameter.
...
- Not downloaded properties (e.g `parents`, `children`) will not be removed upon `save()` unless explicitly done by the user.
