    ...
    def export(self, fileName=None, toBytes=False, copy=False):
        ...
        try:
            self.setExportMode(True, {
                'antialias': self.params['antialias'], 'background': self.params['background'], 'painter': painter,
...
otherExpts = [
    [QT_TRANSLATE_NOOP('MainWindow', 'Temperature, PT100 Sensor'), ('7.1', 'pt100')],
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, lang, app, tr_eyes, tr_qt):
        ...
        self.stepper_timer = QtCore.QTimer()
        self.stepper_timer.timeout.connect(self.updateStepperMotor)
        self.stepper_timer.start(10)
    ...
    def updateEditor(self, text):
        if self.expName[1] == 'editor':
            text = text.replace('from . eyes17 import eyes', '#from . eyes17 import eyes')
            text = text.replace('p = eyes17.eyes.open()',
                                '#p = eyes17.eyes.open() #Uncomment when running script in standalone mode')
    ...
    def locateDevices(self):
        L = {}
        try:
            L = self.getFreePorts(p.H.portname)
    ...
    def newStepperController(self):
        from .layouts import oscilloscope_widget
        if p.connected:
            self.stepperDialog = oscilloscope_widget.DIOSTEPPER(self, total=200, device=p)
            self.stepperDialog.launch()

    def updateStepperMotor(self):
        if self.stepperDialog != None:
            if self.stepperDialog.isVisible():
                self.stepperDialog.read()
    ...
    def scope_help(self, e):
        ...
        if self.expName != 'scope':
            explib = importlib.import_module('.scope',package='eyes17GUI')
            try:
                try:
    ...
    def callExpt(self, details):
        ...
        explib = importlib.import_module('.'+module_name,package='eyes17GUI')
        try:
            try:
                self.expWidget.timer.stop()  # Stop the timer loop of current widget
            except:
    ...
    def makeMenu(self):
        ...
        icon = QtGui.QIcon(os.path.join(imagePath, "eyes17-icon.png"))
        ...
        if ok:
            ...
            for e in languages:
                ...
                if lang not in self.targetLanguages:
                    continue
                cb = QCheckBox(
                    self.tr("Export screenshot in {name} ({localname}).").format(name=e.name, localname=e.localName))
...
You can customize the way they are used to build the path."""
    ...
    def screenshotPlot(self, tmpFileName=None):
        ...
        try:
            ...
            QtWidgets.QMessageBox.warning(self, self.tr('Missing Plot'),
                                          self.tr('Unable to locate a plot. Please try to right click and export'))
        ...
        try:
            ...
            ScreenShotDir = '~/'
        bw = self.conf['ScreenTheme']['BackGround']
        ScreenShotPath = os.path.join(ScreenShotDir,
                                      self.safeFileName(self.title + "-small-screen-" + bw, 'svg').lower())
        path = ""
        if not tmpFileName:
            # ask interactively for a path if an automatic temporary file
            # name is not set
            path, _filter = QtWidgets.QFileDialog.getSaveFileName(self, 'Save File', ScreenShotPath,
                                                                  'SVG(*.svg);;PNG(*.png)')
        if path or tmpFileName:
            self.setConfig('DEFAULT', 'ScreenShotDir', os.path.dirname(path))
            self.conf.read(cnf)
            ...
            if path[-4:] == '.png':
                ...
                val, ok = QtWidgets.QInputDialog.getInt(self, "Set Width",
                                                        self.tr('Enter Width(px). Height will be autoset'), 800, 50,
                                                        4000)
                if ok:
