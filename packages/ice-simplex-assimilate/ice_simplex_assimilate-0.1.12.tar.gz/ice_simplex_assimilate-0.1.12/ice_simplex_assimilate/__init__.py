...
def post_process_sample(sample: NDArray[np.uint32], height_bounds: HeightBounds) -> (
    ...
    area /= area.sum()  # renormalize
    if (remainder := 1 - area.sum()) < 0:
        warnings.warn(f"Area sum is greater than 1.0 by {-remainder}. Clipping to legal bounds.")
        area[area.argmax()] += remainder * 1.0001  # subtract the remainder from the largest component
        print(area.sum())
        print(area.sum() - 1)
...
def post_process_ensemble(ensemble: NDArray[np.uint32], height_bounds: HeightBounds) -> (NDArray[np.float32], NDArray[np.float32]):
    ...
    out = vectorized_pps(ensemble, height_bounds=height_bounds)
    # TODO: debug aggregate area > 1.0
    valid_area = (out[0].sum(axis=1) <= 1.0)
    diffs = 1.0 - out[0].sum(axis=1)
    assert (out[0].sum(axis=1) <= 1.0).all()
