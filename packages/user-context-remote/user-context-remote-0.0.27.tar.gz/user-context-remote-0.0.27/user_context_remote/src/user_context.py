...
class UserContext:
    ...
    def __new__(cls, email: str = None, password: str = None, userJWT: str = None):
        if cls._instance is None:
            cls._instance = super(UserContext, cls).__new__(cls)
            if email is not None and password is not None:
                cls._instance._initialize(user_identfier=email, password=password)
            else:
                cls._instance._initialize(userJWT=userJWT)
        return cls._instance

    def _initialize(self, user_identfier: str = None, password: str = None, userJWT: str = None):
        ...
        if (user_identfier is not None and password is not None):
            self.userJWT = None
            data = self._authenticate_by_email_and_password(user_identfier, password)
        else:
            self.userJWT = userJWT
            data = self._authenticate_by_userJWT(userJWT=userJWT)
    ...
    # TODO Split to two methods one using username+ password and second using jwt
    # TODO username -> username_or_email
    def login(user_identfier: str = None, password: str = None, userJWT: str = None):
        # logger.start(object={"username": username, "passowrd": password})
        if UserContext._instance is None:
            if userJWT is not None:
                UserContext._instance = UserContext(userJWT=userJWT)
            else:
                if user_identfier is None:
                    user_identfier = os.getenv("PRODUCT_USERNAME")
                if password is None:
                    password = os.getenv("PRODUCT_PASSWORD")
                if user_identfier is None or password is None or user_identfier == "" or password == "":
                ...
                UserContext._instance = UserContext(
                    email=user_identfier, password=password)
    ...
    def _authenticate_by_email_and_password(self, user_identfier: str, password: str) -> str:
        ...
        try:
            ...
            data = {"user_identfier": user_identfier, "password": password}
            ...
            self.userJWT = output.json()["data"]["token"]
            # logger.end(object={"user_userJWT": user_userJWT })
            return output
        # TODO e -> exception
    ...
    def _authenticate_by_userJWT(self, userJWT: str) -> str:
    ...
    # TODO: We should also get the email from user_userJWT
...
# #logger = #logger.create_#logger(object=obj)
    ...
    # Commented as we get the decoded user_userJWT from the authentication service and the user-context do not have access to th JWT_SECRET_KEY
    # def get_user_json_by_user_userJWT(self, userJWT: str) -> None:
    #     if userJWT is None or userJWT == "":
    #         raise Exception(
    #             "Your .env PRODUCT_NAME or PRODUCT_PASSWORD is wrong")
    #     #logger.start(object={"userJWT": userJWT})
    #     try:
    #         secret_key = os.getenv("JWT_SECRET_KEY")
    #         if secret_key is not None:
    #             decoded_payload = jwt.decode(userJWT, secret_key, algorithms=[
