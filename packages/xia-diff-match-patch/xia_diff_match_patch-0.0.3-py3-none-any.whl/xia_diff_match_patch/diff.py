...
def apply_patches(source_value, *args):
    dmp = DiffMatchPatch()

    patched = source_value
    diffs = [dmp.diff_main(source_value, arg, True) for arg in reversed(args)]
    for diff in diffs:
        patch = dmp.patch_make(patched, diff)
        patched, _ = dmp.patch_apply(patch, patched)
...
class DiffMatchPatch:
    ...
    def patch_addContext(self, patch, text):
        ...
        while (text.find(pattern) != text.rfind(pattern) and (self.Match_MaxBits ==
                                                              0 or len(
                    pattern) < self.Match_MaxBits - self.Patch_Margin -
                                                              self.Patch_Margin)):
