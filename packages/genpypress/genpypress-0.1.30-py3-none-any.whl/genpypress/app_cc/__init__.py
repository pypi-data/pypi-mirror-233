...
_PATTERN_REPLACE_VIEW = f"({_REPLACE}|{_CREATE})" + _WS + _VIEW
...
_re_replace_view = re.compile(_PATTERN_REPLACE_VIEW, re.I)
...
def _is_create_script(insides: str, file: str):
    if m := _re_create.search(insides):
        ...
        # pokud je to skript který zakládá view, nic nedělej
        if _re_replace_view.search(insides):
            return False, None
