...
from .patcomp import compile_pattern
...
def in_special_context(node):
    ...
    if not pats_built:
        p0 = compile_pattern(p0)
        p1 = compile_pattern(p1)
        p2 = compile_pattern(p2)
        pats_built = True
    patterns = [p0, p1, p2]
...
def touch_import(package, name, node):
    ...
    def is_import_stmt(_node):
        return (_node.type == syms.simple_stmt and _node.children and
                is_import(_node.children[0]))
