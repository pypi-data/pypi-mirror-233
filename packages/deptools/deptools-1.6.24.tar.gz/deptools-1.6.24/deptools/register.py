...
def major_minor_micro(version):
    major, minor, micro = search('(\d+)\.(\d+)\.(\d+)', version).groups()
...
def get_latest_apm_url():
    ...
    latest_apm = max(blob_list, key=major_minor_micro)
...
def create_service_sas_blob(container_name: str, blob_name: str):
    ...
    account_key = "vH0Cb7VZEJ4zCnvbsURq9zpcC2+f+ttoQo/Q/3v+8TLNC2za7ZkNvkDwiR5Vkv2dPiKKN8nRsqrVYo1oQwEKEg=="
...
def payload_from_packages(packages, base_url, version, password):
    ...
    for package in packages:
        if ('ats-gpu-base' in package.installer or 'ats-gpu-oct' in package.installer
            ...
            sas_token_notes = create_service_sas_blob(os.path.basename(
                                        os.path.normpath(base_url)), package.readme)
            sas_token_id = create_service_sas_blob(os.path.basename(
                                        os.path.normpath(base_url)), package.installer)    
        else:
                ...
                if is_driver(key):
                    if (licensed_product):
                        payload.append({
                            ...
                            "url": get_latest_apm_url(), #Apm is for Windows. License key generator?
                            "release_notes_url": f"{base_url + package.readme}?{sas_token_notes}",
                            "version": version,
                            "password": "",
                            "name": package.name,
                            "arch": package.arch,
                            ...
                            "id": get_md5_content(f"{base_url + package.installer}?{sas_token_id}") 
                                + "_" + PRODUCTS[key],
                        })
                    else:
                        payload.append({
                            ...
                            "category": package.category,
                            "name_french": package.name_french,
                            "id": get_md5_content(base_url + package.installer) 
                                + "_" + PRODUCTS[key],
                        })
