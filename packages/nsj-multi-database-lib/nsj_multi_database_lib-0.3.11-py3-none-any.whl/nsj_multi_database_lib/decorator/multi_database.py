    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if hasattr(g, 'profile') and "authentication_type" in g.profile and g.profile["authentication_type"] == "access_token":
                    ...
                    raise Exception(
                        f'Usuário não configurado no banco do multibancos')
                ...
                if request.method.upper() == 'GET':
                    ...
                    msg = f"Faltando propriedade obrigatória no corpo da requisição: {e}."

                get_logger().warning(msg)
                return (format_json_error(msg), 400, {"Content-Type": "application/json; charset=utf-8"})
            except Exception as e:
                msg = f"Erro desconhecido: {e}."
                get_logger().exception(msg, e)
                return (format_json_error(msg), 500, {"Content-Type": "application/json; charset=utf-8"})
