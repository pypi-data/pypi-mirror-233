...
def main(search,catagory):
    ...
    def extract_info(i):
        ...
        if filter(item):
            res.append(item)
    with open ("jackett_config.json","r") as config:
        jackett_config=json.load(config)
    url = f"{jackett_config['url']}/api/v2.0/indexers/all/results?apikey={jackett_config['api_key']}&Query="
    with open("qbit_config.json", "r") as config:
        qbit_config = json.load(config)
