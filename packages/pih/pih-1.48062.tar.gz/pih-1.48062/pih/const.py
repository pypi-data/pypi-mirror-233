...
class SETTINGS(Enum):
    ...
    PLAIN_FORMAT_AS_DEFAULT_LOGIN_LIST: StorageValue = StorageValue("PLAIN_FORMAT_AS_DEFAULT_LOGIN_LIST", ["bar", "rob", "ptyu"])
    ...
    HOSPITAL_WORK_DAY_START_TIME: TimeStorageValue = TimeStorageValue("HOSPITAL_WORK_DAY_START_TIME", "8:00")
    ...
    TIME_TRACKING_FOR_KAG: StorageValue = StorageValue("TIME_TRACKING_FOR_KAG",  ["035", "058", "064", "101-0-", "125", "131", "134", "139", "156", "177", "199", "124"])
    ...
    CHILLER_ALERT_TEMPERATURE: FloatStorageValue = FloatStorageValue(
        "CHILLER_ALERT_TEMPERATURE", 17.0)
    
    CHILLER_MAX_TEMPERATURE: IntStorageValue = IntStorageValue(
        "CHILLER_MAX_TEMPERATURE", 17)
    
    CHILLER_MIN_TEMPERATURE: IntStorageValue = IntStorageValue(
        "CHILLER_MIN_TEMPERATURE", 10)
    
    CT_ROOM_MAX_TEMPERATURE: IntStorageValue = IntStorageValue(
        "CT_ROOM_MAX_TEMPERATURE", 24)
    
    CT_ROOM_MIN_TEMPERATURE: IntStorageValue = IntStorageValue(
        "CT_ROOM_MIN_TEMPERATURE", 20)
    
    CT_ROOM_MAX_HUMIDITY: IntStorageValue = IntStorageValue(
        "CT_ROOM_MAX_HUMIDITY", 60)
    
    CT_ROOM_MIN_HUMIDITY: IntStorageValue = IntStorageValue(
...
class PARAM_ITEMS:
    ...
    TELEPHONE_NUMBER: ParamItem = ParamItem(FIELD_NAME_COLLECTION.TELEPHONE_NUMBER, "Телефонный номер")
...
class Events(Enum):
    ...
    #
    IT_NOTIFY_ABOUT_CREATE_USER: EventDescription = EventDescription("Добрый день, отдел Информационных технологий.\nДокументы для нового пользователя: {name} готовы!\nОписание: {description}\nЛогин: {login}\nПароль: {password}\nТелефон: {telephone_number}\nЭлектронная почта: {email}\nДополнительная информация: {inforamtion}", LogMessageChannels.IT, LogMessageFlags.NOTIFICATION.value, (PARAM_ITEMS.NAME, ParamItem("description", ""), ParamItem("login", ""), ParamItem("password", ""), PARAM_ITEMS.TELEPHONE_NUMBER, ParamItem("email", ""), ParamItem("information", "")))

    IT_NOTIFY_ABOUT_CREATE_MARK: EventDescription = EventDescription("Добрый день, отдел Информационных технологий.\nКарта доступа для новой персоны: {name} готова!\nТелефон: {telephone_number}\nНомер карты доступа: {tab_number}\nГруппа доступа: {group_name}", LogMessageChannels.IT, LogMessageFlags.NOTIFICATION.value, (PARAM_ITEMS.NAME, PARAM_ITEMS.TELEPHONE_NUMBER, PARAM_ITEMS.TELEPHONE_NUMBER, ParamItem("group_name", "")))

    IT_NOTIFY_ABOUT_CREATE_TEMPORARY_MARK: EventDescription = EventDescription("Добрый день, отдел Информационных технологий.\nВременная карта доступа для персоны: {name} готова!\nНомер карты: {tab_number}\nТелефон: {telephone_number}", LogMessageChannels.IT, LogMessageFlags.NOTIFICATION.value, (PARAM_ITEMS.NAME, ParamItem("tab_number", ""), PARAM_ITEMS.TELEPHONE_NUMBER))
    ...
    IT_TASK_AFTER_CREATE_USER: EventDescription = EventDescription("Добрый день, {it_user_name}.\nНеобходимо создать почту для пользователя: {name}\nАдресс электронной почты: {mail}\nПароль: {password}\nДополнительная информация:{information}", LogMessageChannels.IT, LogMessageFlags.TASK.value, (ParamItem(
        "it_user_name", ""), PARAM_ITEMS.NAME, ParamItem("mail", ""), ParamItem("password", ""), ParamItem("information", "")))
    ...
    ACTION_WAS_DONE: EventDescription = EventDescription(
        "Совершено действие {} (\"{}\").\nДействие совершил: {} ({}).\nПараметры действия: {}.\nПринудительное действие: {}.", LogMessageChannels.DEBUG, LogMessageFlags.SAVE, [ParamItem(FIELD_NAME_COLLECTION.ACTION_DESCRIPTION, ""), ParamItem(FIELD_NAME_COLLECTION.ACTION_NAME, ""), ParamItem(FIELD_NAME_COLLECTION.NAME, ""), ParamItem(FIELD_NAME_COLLECTION.LOGIN, ""), ParamItem(FIELD_NAME_COLLECTION.PARAMETERS, ""), ParamItem(FIELD_NAME_COLLECTION.FORCED, "")])
...
class Actions(Enum):
    ...
    VALENTA_SYNCHRONIZATION: ActionDescription = ActionDescription(
        "VALENTA_SYNCHRONIZATION", ("valenta", "валента"), "Синхронизация Валенты", "Совершить синхронизацию для Валенты", False, True, forcable=True)

    TIME_TRACKING_REPORT: ActionDescription = ActionDescription(
        "TIME_TRACKING_REPORT", ("tt", "урв"), "Отчеты по учёту рабочего времени", "Создать", False, False, "начало и конец периода для создания отчета. Например \"1-10\", начиная с 1 дня и кончая 10 дня до настоящего дня")
    
    ATTACH_SHARED_DISKS: ActionDescription = ActionDescription(
        "ATTACH_SHARED_DISKS", ("attach", ), "Присоединить сетевые диски", "Присоединить", False, True)
    
    ACTION: ActionDescription = ActionDescription(
        "ACTION", ("action", ), "Неспециализированное действие", None, False, True, forcable=True)
...
class EMAIL:
    ...
    EXTERNAL_MAIL: str = f"mail.pacifichosp@{MAIL_RU_NAME}"
    EXTERNAL_MAIL_SERVER: str = f"imap.{MAIL_RU_NAME}"
    MAILER_DAEMON: str = f"mailer-daemon@corp.{MAIL_RU_NAME}"
