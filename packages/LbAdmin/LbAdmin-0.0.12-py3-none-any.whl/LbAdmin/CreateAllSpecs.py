...
def main():
    ...
    parser.add_argument("-r", "--release",
                      help="Release number"
                           "[default: %default]",
                      default="1")
    parser.add_argument("--require-base-package",
                      help="require the real package with checksum instead of the RPM with symlink",
                      default=False,
                      action='store_true')
    ...
    for p in platforms:
        ...
        output = rpmname + ".spec"
        f = createSpec(p, args.external_list_json, output, release=args.args.require_base_package,
                        rpmname=rpmname, rpmroot=args.buildroot, return_required=True)
