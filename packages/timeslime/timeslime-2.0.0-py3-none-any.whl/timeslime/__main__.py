...
DATABASE = SqliteDatabase(join(DATABASE_DIRECTORY, "timeslime.db"))
CONSOLE = Console()
...
def config(weekly_working_hours, timeslime_server, username, password):
...
def start(start_time):
    ...
    if configuration_service.timeslime_server:
        ...
        synchronize_service = SynchronizeService(DATABASE, configuration_service)
        synchronize_service.sync()
    time_service = TimeService(DATABASE)
...
Example:
    ...
    report_service = ReportService(DATABASE, configuration_service)
    try:
