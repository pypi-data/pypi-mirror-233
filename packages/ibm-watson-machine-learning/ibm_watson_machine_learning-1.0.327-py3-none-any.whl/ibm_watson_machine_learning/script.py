    ...
    def _create_asset(self, script_meta, file_path, extension='.py'):
        ...
        if not self._client.WSD:
            ...
            if lang == 'R':
                ...
                if self._client.CPD_version >= 4.7:
                    ...
                    for sw_spec in self._client.software_specifications.get_details()['resources']:
                        if 'life_cycle' in sw_spec['metadata']:
                            sw_configuration = sw_spec['entity'].get('software_specification', {}).get(
                                'software_configuration', {})
                            if sw_configuration.get('platform', {}).get('name') == 'r' and len(
                                    sw_configuration.get('included_packages', [])) > 1:
                                if SpecStates.RETIRED.value in sw_spec['metadata']['life_cycle']:
                                    retired_rscript_sw_specs.append(sw_spec['metadata']['name'])
                                elif SpecStates.CONSTRICTED.value in sw_spec['metadata']['life_cycle']:
                                    constricted_rscript_sw_specs.append(sw_spec['metadata']['name'])
                                elif SpecStates.DEPRECATED.value in sw_spec['metadata']['life_cycle']:
                                    deprecated_rscript_sw_specs.append(sw_spec['metadata']['name'])
                                else:
                ...
                retired_sw_spec_ids = [self._client.software_specifications.get_id_by_name(sw_name)
                                       for sw_name in retired_rscript_sw_specs]
                constricted_sw_spec_ids = [self._client.software_specifications.get_id_by_name(sw_name)
                ...
                unsupported_dict = {SpecStates.RETIRED.value: retired_sw_spec_ids,
                ...
                for key in unsupported_dict:
                    if script_meta[u'software_spec_uid'] in unsupported_dict[key]:
                        warnings.warn(f"Provided software spec is {key} for R scripts. "
