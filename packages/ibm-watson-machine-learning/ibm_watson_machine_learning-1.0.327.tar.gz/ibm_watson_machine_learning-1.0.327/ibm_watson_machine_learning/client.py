...
from ibm_watson_machine_learning.utils import version, CPDVersion
    ...
    def __init__(self, wml_credentials, project_id=None, verify=None):
        ...
        if 'instance_id' not in self.wml_credentials.keys():
            ...
            if 'icp' == self.wml_credentials[u'instance_id'].lower() or 'openshift' == self.wml_credentials[
                ...
                if response_get_wml_services.status_code != 200:  # retry with endpoint for cpd 4.8 and higher
                    response_get_wml_services = requests.get(
                        f"{self.wml_credentials['url']}/ml/wml_services/v2/version",
                        headers={'X-WML-User-Client': 'PythonClient'})
                ...
                if response_get_wml_services.status_code == 200:
                    ...
                    if wml_version:
                        ...
                        if not self.wml_credentials.get('version'):
                            self.wml_credentials['version'] = wml_version
                        elif self.wml_credentials.get('version') != wml_version:
                            warn(
                                f"The provided version: {self.wml_credentials.get('version')} "
                                f"is different from the current CP4D version: {wml_version}. "
                                f"Correct the wml_credentials with proper CP4D version number.")

                        if self.wml_credentials['version'] not in CPDVersion.supported_version_list:
                ...
                if 'version' not in self.wml_credentials:
                    raise WMLClientError(Messages.get_message(CPDVersion.supported_version_list, message_id="version_not_provided"))

                # Note to be removed while wml local support removal
                if self.wml_credentials[u'version'].lower() in ['1.1', '2.0']:
                    self.CAMS = True
                ...
                if self.wml_credentials[u'version'].lower() in CPDVersion.supported_version_list:
                    self.CPD_version.cpd_version = self.wml_credentials[u'version'].lower()
                    self.CAMS = True
                    os.environ["DEPLOYMENT_PRIVATE"] = "icp4d"
                    self.ICP_PLATFORM_SPACES = True

                    if 'bedrock_url' not in self.wml_credentials and self.CPD_version != 3.5:
                        if 4.0 <= self.CPD_version < 4.7:
                        ...
                        self.wml_credentials['bedrock_url'] = '.'.join(
                            [bedrock_prefix] + self.wml_credentials['url'].split('.')[1:])
