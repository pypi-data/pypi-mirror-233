...
class Model(WMLResource):
    ...
    .. hint::
        To use the Model class with LangChain, use the :func:`WatsonxLLM <ibm_watson_machine_learning.foundation_models.extensions.langchain.WatsonxLLM>` wrapper.
        ...
        **Example**
        ...
        if isinstance(prompt, list):
            ...
            try:
                ...
                if response_scoring.status_code >= 400 and response_scoring.status_code <= 599:
                    raise WMLClientError(f'Request failed with: {response_scoring.text} ({response_scoring.status_code})')

            except Exception as e:
                warnings.warn(f"Response failed with error: '{str(e)}' for {payload}")
                return {
                    'results': [{'generated_text': f"Response failed with error '{str(e)}' on prompt '{payload['input']}'", 'stop_reason': 'ERROR'}]
                }
