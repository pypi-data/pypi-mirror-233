...
class OptimProblem:
    ...
    def __init__(self, model: Callable[..., Dict], specifications: Spec,
        ...
        self.wrapper = Wrapper(model, specifications, parameters,
                               resultsHandler)

    def run(self, ftol=1e-5, disp=True, maxiter=500,nbParetoPts=5,otherpts=None,
        ...
        :param method: SLSQP or LeastSquare according problems
...
def SLSQP(wrapper: Wrapper, x0, options,disp):
    """
    Solves the optimization problem with SLSQP method from
    ...
    # options = {'func': None, 'maxiter': 100, 'ftol': 1e-06, 'iprint': 1,
    # 'disp': False, 'eps': 1.4901161193847656e-08}
    # ftol : float Precision goal for the value of f in the stopping criterion.
    # eps : float  Step size used for numerical approximation of the Jacobian.
    # disp : bool  Set to True to print convergence messages. If False,
    # verbosity is ignored and set to 0.
    ...
    if (not result.success):
        if (result.status != 9):  # not if maxiteration # TODO gérer
            # une exception
            #  Iteration limit exceeded    (Exit mode 9)
            # TRY Random initial point !
            print("WARNING : Optimization doesn't converge... Trying "
                  "random inital guess")
            import numpy
...
def LSSQ(wrapper: Wrapper, x0, options):
    ...
    result = scipyOpt.least_squares(fun=wrapper.f_val, x0=x0, bounds=bounds,
                                    jac=wrapper.f_grad_using_make_jvp,
                                    ftol=options['ftol'],
    ...
    returns the value of each elements of 'outputs' obtained for each 
    optimization
    use iter.print() or noload.gui.plotIterations.plotXY(iter) to see the 
    results
    '''
