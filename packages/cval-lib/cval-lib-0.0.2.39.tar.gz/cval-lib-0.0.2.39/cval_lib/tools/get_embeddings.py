...
def save_bbox_for_cluster(
    ...
    if os.path.exists(path_to_bboxes):
        shutil.rmtree(path_to_bboxes)
    os.makedirs(path_to_bboxes)
    allfiles = glob.glob(
        os.path.join(path_to_images, 'images', pattern),
    )
    jpgs_dict = {get_before_last_dot(os.path.basename(x)): x.strip() for x in allfiles}
    for id_file, path_to_img in jpgs_dict.items():
        filename = os.path.join(
            path_to_images,
            f'labels/{id_file}.txt',
        )
        if os.path.exists(filename):
            ...
            for i, line in enumerate(bbox):
                ...
                image.crop((x1, y1, x2, y2)).save(
                    os.path.join(
                        path_to_bboxes,
                        f'{id_file}_{i}_{label}_{normalize_pattern(pattern)}',
                    )
                )
...
class CustomDataset(Dataset):
    def __init__(
            self,
            path_to_dir: Union[Path, str, os.PathLike] = '',
            pattern: str = '*.jpg',
    ):
        super(CustomDataset, self).__init__()
        allfiles = glob.glob(os.path.join(path_to_dir, pattern))
...
class Matcher(Dataset):
    def __init__(
        ...
        allfiles = sorted(glob.glob(os.path.join(path_to_dir, pattern)))
        self.images = allfiles
        self.images_class = [os.path.basename(x).split('_')[-2] for x in allfiles]
...
def train_siam(
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save(model.state_dict(), f'end_pool.pt')
        scheduler.step()


def get_embeddings(
        full_path: Union[Path, str, os.PathLike],
        dim: Tuple[int, int],
        device: object(),
        batch_size: int = 10,
        shuffle: bool = True,
    ...
    emb, files = generate_embeddings(model, test_loader, device)
    return emb, files
