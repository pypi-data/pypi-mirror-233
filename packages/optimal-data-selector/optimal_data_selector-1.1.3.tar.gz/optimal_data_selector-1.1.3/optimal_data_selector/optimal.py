def OptimalDataSelector(predictor=None,target=None,combination=10,random_state=0,train_size=0.7,bs_problem='class',time_forecast=True,scaling=None,acc_forecast=True,boost=None,randomness=True,load_path=None,asking=True):
    ...
    Parameters:
        ...
        asking        : bool, True --> [True,False]
            Set it to False, and the function will run without asking to save the data structure.
            When set to True, the function will ask you to save the data structure. The default value is True.
    ...
    if load_path == None:
      ...
      elif bs_problem == 'reg' and boost == None and predic:
          start1=time.time()
          for i in range(random_state,random_state+combination):
              x_train1,x_test1,y_train1,y_test1=train_test_split(X,Y,train_size=train_size,random_state=i,shuffle=randomness)
              x_train.append([x_train1,y_train1,x_test1,y_test1])
