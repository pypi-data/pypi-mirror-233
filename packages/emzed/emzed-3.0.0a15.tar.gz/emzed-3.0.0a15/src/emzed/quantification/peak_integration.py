...
def integrate(
    ...
    **model_extra_args,
    ...
    if post_fixes is None:
        post_fixes_peaks = peak_table.supported_postfixes(needed_columns_peaks)
    else:
        missing_peak_columns = []
        for post_fix in post_fixes:
            for name in needed_columns_peaks:
                if name + post_fix not in peak_table.col_names:
                    missing_peak_columns.append(name + post_fix)

    needed_columns_chromatograms = ["chromatogram", "peakmap"]
    if post_fixes is None:
        post_fixes_chromatograms = peak_table.supported_postfixes(
            needed_columns_chromatograms
        )
    else:
        missing_chromatogram_columns = []
        for post_fix in post_fixes:
            for name in needed_columns_chromatograms:
                if name + post_fix not in peak_table.col_names:
                    missing_chromatogram_columns.append(name + post_fix)
    ...
    if post_fixes is None:
        if not post_fixes_chromatograms and not post_fixes_peaks:
            raise ValueError(
                "given table is neither valid peaks and/or chromatogram table"
            )
        post_fixes = post_fixes_chromatograms + post_fixes_peaks
    else:
        if missing_chromatogram_columns and missing_peak_columns:
            raise ValueError(
                "given table is neither valid peaks and/or chromatogram table"
            )
    ...
    indices = list(range(len(peak_table)))
    ...
    for postfix in postfixes:
        for index, row in zip(indices, peak_table):
            ...
            if chromatogram is not None:
                model = current_peak_shape_model._fit(
                    chromatogram.rts, chromatogram.intensities, model_extra_args
                )
            else:
