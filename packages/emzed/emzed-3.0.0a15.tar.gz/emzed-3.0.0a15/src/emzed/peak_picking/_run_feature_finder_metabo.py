...
def run_feature_finder_metabo(
    ...
    extract_ms_chromatograms=False,
    ...
    **parameters,
    ...
    :param extract_ms_chromatograms:  also extract ms chrotograms if present in peakmap
    ...
    if not run_feature_grouper and any(k.startswith("ffm_") for k in parameters.keys()):
        warnings.warn(
    ...
    if not peak_tables:
        tab = _setup_final_table(
            [], peak_map, run_feature_grouper, extract_ms_chromatograms, []
        )
    else:
...
def _run_feature_finder_metabo(
    ...
    **parameters,
    ...
    chromatogram_rows = []
    if extract_ms_chromatograms:
        for chromatogram in peak_map.ms_chromatograms:
    ...
    table = _setup_final_table(
        rows, peak_map, run_feature_grouper, extract_ms_chromatograms, chromatogram_rows
    )
...
run_feature_finder_metabo.__doc__ = _setup_doc_string()
...
def _setup_final_table(
    rows, peak_map, run_feature_grouper, extract_ms_chromatograms, ms_chromatograms
):
    column_names = [
        "feature_id",
        "mz",
    ...
    if extract_ms_chromatograms:
        column_names += ["precursor_mz", "chromatogram_type", "chromatogram"]
        column_types += [MzType, str, MSChromatogram]

        # extend columns of existing rows:
