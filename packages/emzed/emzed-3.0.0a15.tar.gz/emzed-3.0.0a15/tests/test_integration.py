...
@pytest.mark.parametrize(
    "method", ["no_integration", "emg", "emg_robust", "asym_gauss", "linear", "sgolay"]
)
...
def test_mixed_peak_and_chromatogram_integration(data_path, regtest):
    peak_table = emzed.io.load_table(data_path("peaks_and_chromatograms_mixed.table"))

    # integrate peaks from peak picker and ms_chromatograms from peakmap file:
    integrated_peaks = integrate(peak_table[763:775], "emg_robust", n_cores=3)

    # mitigate numerical differences on different ci servers:
    integrated_peaks.set_col_format("area", "%.0e")
    integrated_peaks.set_col_format("rmse", "%.0e")
...
def test_ms_chromatogram_integration(data_path, regtest):
    ...
    integrated_peaks = integrate(t[10:20], "emg_robust", n_cores=3)
...
def test_chromatogram_integration(data_path, regtest):
    ...
    integrated_peaks = integrate(t, "emg_robust", n_cores=3)
