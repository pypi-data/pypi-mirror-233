...
class SelectionListGui(object):
    def __init__(
        self, frame, screens=None, diapoList=None, searchFuntion=None, printer=None
    ):
        ...
        self._saveButton = tk.Button(
            lisButtonSubPanel, text="Sauver", command=self._saveSet, state=tk.DISABLED
        )
        self._deleteSetButton = tk.Button(
            lisButtonSubPanel, text="Suppr", command=self._deleteSet, state=tk.DISABLED
        )

        self._upButton = tk.Button(
            lisButtonSubPanel, text="Monter", command=self._up, state=tk.DISABLED
        )
        self._downButton = tk.Button(
            lisButtonSubPanel, text="Descendre", command=self._down, state=tk.DISABLED
        )
        self._deleteElemButton = tk.Button(
            lisButtonSubPanel, text="Suppr", command=self._deleteElem, state=tk.DISABLED
        )
        ...
        addMediaButton = tk.Button(
            lisButtonSubPanel, text="Ajouter\nMedias", command=self._addMedia
        )
        if vlc == 0:
            addMediaButton.config(state=tk.DISABLED)
        addImageButton = tk.Button(
            lisButtonSubPanel, text="Ajouter\nImages", command=self._addImage
        )
        addVerseButton = tk.Button(
            lisButtonSubPanel, text="Ajouter\nVersets", command=self._addVers
        )
        ...
        self.getSetList()
        if settings.GENSETTINGS.get("Parameters", "autoload"):
            if self._setList:
    ...
    def _selectSong(self, event=None):  # pylint: disable=unused-argument
        ...
        if self._diapoList is not None:
            self._diapoList.load(self.list(), wantedElementNumber=self._curSelection)
    ...
    def _addMedia(self):
        ...
        if medias:
            for media in medias:
                ...
                anwser = 1
                mediaType = settings.GENSETTINGS.get(
                    "Extentions", "audio"
                ...
                if media and (extention not in mediaType):
                    anwser = tkMessageBox.askyesno(
                        ...
                        "Voulez-vous continuer malgré tout ?" % (mediaFile, extention),
                    )
                if media and anwser:
                    self.addElementToSelection(
                        elements.Element(mediaFile, "media", media)
                    )
    ...
    def _deleteSet(self):
        if tkMessageBox.askyesno(
            "Confirmation",
            "Etes-vous sur de suprrimer " 'la liste:\n"%s" ?' % str(self._set),
        ):
        ...
        if classPaths.PATHS.sets:
            if not self._set:
                ...
                if self._setSelection.get():
                    self._set.path = self._setSelection.get()
                extention = settings.GENSETTINGS.get("Extentions", "liste")[0]
                saveSetName = tkFileDialog.asksaveasfilename(
        ...
        if element and self._searchFuntion and not element.exist() and element.nom:
            ...
            if searchResults and searchResults[0].etype == element.etype:
                tkMessageBox.showinfo(
                    ...
                    'il va être échangé par "%s".'
                    % (element.nom, searchResults[0].nom),
                )
