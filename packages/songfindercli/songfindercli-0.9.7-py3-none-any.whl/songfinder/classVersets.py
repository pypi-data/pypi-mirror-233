...
class class_versets(tk.Frame, object):
    def __init__(self, fenetre, papa, **kwargs):
        ...
        self.bouton_select = tk.Button(
            self, text="Sélectionner", command=self.papa.selectVers, state=tk.NORMAL
        )
    ...
    def parse(self, event=None):  # pylint: disable=unused-argument
        ...
        chemin = os.path.join(
            self.chemin_bibles,
            self.version + settings.GENSETTINGS.get("Extentions", "bible")[0],
        )
    ...
    def affiche_chap1(self, event=None):  # pylint: disable=unused-argument
        if self.liste_livres.curselection():
            self.livre = (
                int(self.liste_livres.curselection()[0]) + self.testament.get() * 39
            )
    ...
    def affiche_vers2(self, event=None):  # pylint: disable=unused-argument
        ...
        for i, vers in enumerate(
            self.bible[self.livre][self.chap2][
                self.vers1 * (self.chap1 == self.chap2) :
            ]
        ):
    ...
    def affiche_text(self, event=None):  # pylint: disable=unused-argument
        if self.liste_vers2.curselection():
            self.verset = 1
            self.vers2 = int(self.liste_vers2.curselection()[0]) + self.vers1 * (
                self.chap1 == self.chap2
            )
        ...
        if self.chap1 == self.chap2:
            for i, text in enumerate(
                self.bible[self.livre][self.chap1][self.vers1 : self.vers2 + 1]
            ):
                self.text.insert(
                    tk.INSERT, "  " + str(self.vers1 + i + 1) + "  " + text.text + ""
                )
        else:
            ...
            for i, text in enumerate(
                self.bible[self.livre][self.chap2][: self.vers2 + 1]
            ):
