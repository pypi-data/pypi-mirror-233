...
class FPSMonitor(Monitor):
    ...
    def get_FPS_info(self):
        ...
        if G.device.sdkversion >= 24:
            self.get_FPS_surface_view(surface_list_info)
        else:
            self.get_FPS_surface_view_low_version(surface_list_info)
        """
    ...
    def get_frame_rate(self):
        try:
            info:str = G.device.adb.raw_shell("dumpsys gfxinfo {} framestats ".format(G.device.package)).decode()
            data_list = []
            activity_infos = G.device.get_activity()
            fps = 0
            for activity in activity_infos:
                if activity in info:
                    tmp_res = info.split(activity)
                    include_info = None
                    for i in tmp_res:
                        if "---PROFILEDATA---" in i:
                            include_info = i
                    if include_info:
                        frametimes = include_info.split("---PROFILEDATA---")[1]
                        frametimes = frametimes[0:]
                        total = len(frametimes)
                        jank = 0
                        for j in data_list:
                            data_list = j.split(",")
                            ...
                            if times > 16.67:
                                jank += 1
                        fps_cur = int(total / (total + jank) * 60)
                        fps = max(fps, fps_cur)
            G.device.adb.raw_shell("dumpsys gfxinfo {} reset ".format(G.device.package))
            return fps
        except:
            traceback.print_exc()
            return 0
    ...
    def get_FPS_surface_view(self, surface_info):
        ...
        if self.m_surface_view_name is None:
            self.m_surface_view_name = self.get_surfaceview_activity()
        logger.info("获取到的surface_view信息是：{}".format(self.m_surface_view_name))

    def get_FPS_surface_view_low_version(self, surface_info):
        for i in surface_info.split(os.linesep):
            if G.device.package not in i:
                continue
            if not self.m_surface_view_name or "BLAST" in i:
    ...
    def get_surfaceview_activity(self):
        ...
        try:
            ...
            if activity_line:
                ...
                if not activity_name.__contains__(G.device.package):
                    logger.error(
        ...
        while G.stop_event.is_set():  # 停止了循环会停止
            ...
            try:
                ...
                if not FPS_info_FPS:
                    logger.info("未获取到数据")
                    logger.info("尝试换一种方式")
                    gfxinfo_res = self.get_frame_rate()
                    if not gfxinfo_res:
                        ...
                        FPS_info_FPS, FPS_info_lag_number, FPS_full_number = {time.time(): [0 for i in
                                                                                            range(gfxinfo_res)
                                                                                            ]}, [], gfxinfo_res
