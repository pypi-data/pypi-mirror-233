...
class ImportCustomerFile(BaseNModel, DictModel):
    ...
    dict_properties = ['file_public_url', 'file_storage_filename', 'import_settings']
    
    @staticmethod
    def get(merchant_acct):
        return ImportCustomerFile.query(ancestor=merchant_acct.create_ndb_key()).get()
    ...
    @staticmethod
    def define_customer_default_password(merchant_acct, default_password='123456'): 
        import_customer_file = ImportCustomerFile.get(merchant_acct)   
    ...
    def confirm_status(merchant_acct): 
        import_customer_file = ImportCustomerFile.get(merchant_acct)   
        ConfirmedImportCustomerFile.create(import_customer_file)        
