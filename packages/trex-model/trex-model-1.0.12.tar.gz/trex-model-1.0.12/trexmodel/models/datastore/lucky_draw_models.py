        ...
        if lucky_draw_ticket_spending_currency>0:
            if sales_amount> merchant_acct.lucky_draw_ticket_spending_currency:
                ...
                if merchant_acct.lucky_draw_ticket_is_recurring_scheme:
                    number_of_ticket = int(sales_amount/merchant_acct.lucky_draw_ticket_spending_currency)
                else:
                    number_of_ticket = 1
            else:
                number_of_ticket = 0
    ...
    def patch_prize_image_base_url(self):
        ...
        for prize in drawed_details.get('prizes'):
            if prize.get('prize_type') == 'voucher':
                ...
                '''
                if prize.get('image_url').startswith('/static'):
                    prize['image_url'] = '%s%s' % (program_conf.IMAGE_BASE_URL, prize.get('image_url'))
                '''
                if index>0:
        ...
        if won_prize:
            if won_prize.get('prize_type') == 'voucher':
                '''
                if won_prize.get('image_url').startswith('/static'):
                    won_prize['image_url'] = '%s%s' % (program_conf.IMAGE_BASE_URL, won_prize.get('image_url'))
                '''
