    ...
    if data_type == 'dict':
        for key, value in data.items():
            ...
            if isinstance(value, (dict, list)):
                ...
                cemir_print(value, depth + 1)
            else:
                if isinstance(value, (str, int, bool, float)):
                    value_output = f"{color_codes[type(value).__name__.lower()]}{value}{color_codes['reset']}"
                else:
                    value_output = str(value)
                print(f"\t" * depth + f"{key_output}: {value_output} ({type(value).__name__})")
    elif data_type == 'list':
        for i, item in enumerate(data):
            item_output = f"{color_codes[type(item).__name__.lower()]}{item}{color_codes['reset']}"
            print(f"\t" * depth + f"{color_codes['key']}{i}:{color_codes['reset']} {item_output} ({type(item).__name__})")
            if isinstance(item, (dict, list)):
                cemir_print(item, depth + 1)
    else:
        if data_type in color_codes:
            color_code = color_codes[data_type]
