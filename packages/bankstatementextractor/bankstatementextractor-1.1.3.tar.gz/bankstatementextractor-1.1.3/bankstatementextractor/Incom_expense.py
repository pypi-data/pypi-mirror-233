...
class Incom_expense:   
    ...
    def income_detection(self,pdf_path):
            ...
            model_name = 'places'
            id = pd.Series(pd.to_numeric(os.listdir('models/'+model_name), errors='coerce'),).dropna(

            ).sort_values(ascending=False).iloc[0]  

            places_categories = pd.read_csv('models'+'/categories.csv',)['0']
            # categories
            places_model = tf.keras.models.load_model('models/'+model_name+'/'+str(id))
            ...
            # .sort_values()
            words_categories = pd.read_csv('models'+'/categories.csv',)['0']
            # categories
            words_model = tf.keras.models.load_model('models/'+model_name+'/'+str(id))
        ...
        # try:
            ...
            with open("global_percentiles.json", "r") as f:
            ...
            model = tf.keras.models.load_model(
                'models/lean/0.9384675110399379')
            labels = pd.read_csv('models/'+'lean'+'/labels')['0']
