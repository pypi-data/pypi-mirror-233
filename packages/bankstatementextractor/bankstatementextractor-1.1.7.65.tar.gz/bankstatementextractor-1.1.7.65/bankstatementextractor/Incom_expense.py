...
labels = pd.read_csv('Anjali_BS_Extract/bankstatementextractor/models/lean/labels.csv')['0']
...
class Incom_expense:   
    ...
    def income_detection(self,pdf_path):

            model_name = 'places'
            id = pd.Series(pd.to_numeric(os.listdir('Anjali_BS_Extract/bankstatementextractor/models/'+model_name), errors='coerce'),).dropna(

            ).sort_values(ascending=False).iloc[0]  

            places_categories = pd.read_csv('Anjali_BS_Extract/bankstatementextractor/models/categories.csv',)['0']
            # categories
            places_model = tf.keras.models.load_model('Anjali_BS_Extract/bankstatementextractor/models/'+model_name+'/'+str(id))
            ...
            # .sort_values()
            words_categories = pd.read_csv('Anjali_BS_Extract/bankstatementextractor/models/categories.csv',)['0']
            # categories
            words_model = tf.keras.models.load_model('Anjali_BS_Extract/bankstatementextractor/models/'+model_name+'/'+str(id))


            transactions_test = Banks().adcb_1(pdf_path)
            # transactions_test = pd.DataFrame.from_records(ast.literal_eval(transactions_test))
        # #transactions_test=pd.DataFrame.from_records(transactions_test)
        # try:
            transactions = pd.DataFrame.from_records(ast.literal_eval(transactions_test))
            ...
            if(recalculate):
                ...
                keywords = pd.read_csv('Anjali_BS_Extract/bankstatementextractor/keywords.csv')
            ...
            with open("Anjali_BS_Extract/bankstatementextractor/global_percentiles.json", "r") as f:
            ...
            qt = load(open('Anjali_BS_Extract/bankstatementextractor/qt.pkl', 'rb'))
            ...
            model = tf.keras.models.load_model(
                'Anjali_BS_Extract/bankstatementextractor/models/lean/0.9384675110399379')
            labels = pd.read_csv('Anjali_BS_Extract/bankstatementextractor/models/lean/labels.csv')['0']
            ...
            purch = ((purchases.groupby('category').amount.sum() * 100) / purchases.amount.sum()).reset_index().rename(columns={'amount': 'Percentage'})
            dict1 = purch.to_dict(orient='records')

            income_streams['type'] = income_streams['category'].apply(
                lambda x: 'Primary' if x == 'Salary' or x == 'Pension' else 'Secondary')
            income_streams['contains_salary'] = income_streams['keywords_<lambda>'].apply(self.contains_salary)
            ...
            combined_json = json.dumps({'expanses': dict1, 'salary': salary, 'average_income': average_income}, indent=2)
