...
class TimeEncodingBuilder():
    ...
    def getImage(self, data: np.array, tolist: bool = False):
        for encoder in self._encoders:
            self._encoded += encoder.getEncoded(data)
        
        return self._encoded if tolist else np.stack(self._encoded, axis=-1)
    
    def getEncoded(self, data: np.array, tolist: bool = False):
        for encoder in self._encoders:
            self._encoded += encoder.getEncoded(data)

        return self._encoded if tolist else np.concatenate(self._encoded)
