...
def main():
    ...
    while True:
        ...
        if event == 'Analysis Start':
            ...
            # 被験者が選択した範囲を切り出す
            first_frame_cropped = first_frame_gray[roi[1]:roi[1] + roi[3], roi[0]:roi[0] + roi[2]]
            ...
            cv2.rectangle(roi_draw_image, rectangle1, rectangle2, (0, 0, 255), thickness=10)
            ...
            cropped_th_img_path = os.path.join(directory, f"{file_name}_crop_threshold.png")
            ...
            out = cv2.VideoWriter(output_path, fourcc, fps, (roi[2], roi[3]), isColor=False)
            ...
            time_count = np.arange(1, range_frames + 1) / (fps * 60)  # Divide by 60 to convert seconds to minutes
            ...
            #ヒートマップ
            fig4 = plt.figure(figsize=(roi[2]//10, roi[3]//10))  # Adjust size as needed
            ...
            excel_file_name = os.path.join(directory, f"{file_name}_pixel_intensities.xlsx")
            ...
            with pd.ExcelWriter(excel_file_name) as writer:
                df.to_excel(writer, index=False, sheet_name='Delta Pixel Intensity', startrow=0, startcol=0)
