...
# Once we move over to ObjectLists, rename to run_plural_mapi_request and remove the old one
def run_plural_mapi_request_list(
    query: str,
    query_function: str,
    ...
    future = connection.pool.submit(
        _threaded_plural_mapi_request_list,
        headers=connection.request_header,
        endpoint=connection.endpoint,
        query=query,
        variables=variables,
...
# Once we move over to ObjectLists, rename to _threaded_plural_mapi_request and remove the old one
def _threaded_plural_mapi_request_list(
    headers: Dict[str, str],
    endpoint: str,
    query: str,
    variables: Optional[Dict[str, Any]],
    ...
    return _deserialize_response_list(response, query_function, model_type)
...
# Once we move over to ObjectLists, rename to _deserialize_response_list and remove the old one
def _deserialize_response_list(
    response: Dict[str, Any],
    query_function: str,
    model_type: Type[ModelT],
    paginated: bool = False,
    ...
    try:
        ...
        object_type = ObjectType.from_model_type(model_type)
        return ObjectList([model_type.from_mapi_response(i) for i in query_response], object_type)
    except (KeyError, TypeError) as e:
        raise MAPIException(status=HTTPStatus.INTERNAL_SERVER_ERROR,
                            message='Internal Server Error: Malformed response data') from e
