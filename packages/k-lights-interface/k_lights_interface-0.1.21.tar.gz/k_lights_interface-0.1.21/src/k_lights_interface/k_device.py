...
class KDevice:
    ...
    def set_channel_unsafe(self, ch_name: Literal["red", "green", "blue", "amber", "cyan", "lime"], value: float) -> bool:
        """Sets the channel to the given value.

        Args:
            ch_name (Literal["red","green","blue", "amber","cyan","lime"]): The channel to set
            value (float): The value to set the channel to

        Returns:
            bool: Whether the command was successful
        """
        if ch_name not in self.capabilities.channel_ordering:
            logger.info(f"Device {self.name} does not have channel {ch_name}")
            return False
        if value < 0 or value > 100:
            logger.info(f"Error: Channel value {value} out of range [0, 100]")
            return False
        index = self.capabilities.channel_ordering.index(ch_name)
        output_vals = [0]*self.capabilities.number_of_led_colors
        output_vals[index] = value
        message = kprot.DriverMessage()
        message.set_emitter_channels = kprot.SetEmitterChannelsMessage(output_value=output_vals, verify_outputs=False)
        ret, _ = self.k_transport.execute_command(message, with_response=True)
        return ret

    def set_channels_unsafe(self, output_dict: Dict[Literal["red", "green", "blue", "amber", "cyan", "lime"], float]) -> bool:
        formatted_outputs = [0]*self.capabilities.number_of_led_colors
        for ch_name, value in output_dict.items():
            if ch_name not in self.capabilities.channel_ordering:
                logger.info(f"Device {self.name} does not have channel {ch_name}")
                return False
            if value < 0 or value > 100:
                logger.info(f"Error: Channel value {value} out of range [0, 100]")
                return False
            formatted_outputs[self.capabilities.channel_ordering.index(ch_name)] = value
        message = kprot.DriverMessage()
        message.set_emitter_channels = kprot.SetEmitterChannelsMessage(output_value=formatted_outputs, verify_outputs=False)
        ret, _ = self.k_transport.execute_command(message, with_response=True)
        return ret


# ---------------------------------------------------------------------------- #
#                       FUNCTIONS FOR DEVICE SETTINGS                          #
# ---------------------------------------------------------------------------- #
