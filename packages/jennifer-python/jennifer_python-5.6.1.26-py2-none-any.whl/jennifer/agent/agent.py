            ...
            try:
                ...
                if cmd == b'agentcheck_env':
                    ...
                    try:
                        ...
                        if debug_log:
                            _log('diagnostics', 'agentcheck_env', 'gc', traceback.format_exc())

                    data = Agent.get_environment_variables()
                    if data is not None:
                        if debug_log:
                            _log('diagnostics', 'agentcheck_env', 'get_environment_variables')

                        import jennifer.hooks
                        import jennifer.agent.class_selector as class_selector
                        ...
                        data['jennifer.dynamic_profile_count'] = str(class_selector.get_hooked_count())
                        data['jennifer.current_pid'] = str(os.getpid())

                        self.send_to_master('agentcheck_env', data)

                    if debug_log:
                        _log('diagnostics', 'send_to_master')
                    continue
        ...
        try:
            ...
            with self.active_object_meter_lock:
                ax_list = [t.to_active_service_dict(current_time, current_cpu) for t in
                           self.active_object_meter.values()]

            self.send_to_master('active_service_list', {
                'active_services': ax_list,
            })
        except Exception as e:
            _log('exception', 'process_agent_metric', e)
    ...
    def send_to_master(self, cmd, params):
        try:
            ...
            if e.errno != errno.EPIPE:  # 2.7 backward compatibility
                _log('exception', 'send_to_master', e)
            self.masterConnection = None
