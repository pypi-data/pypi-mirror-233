...
import random as rd
import imagehash
def load_images(image_path) -> array:
...
def adjust_img_color(image, number_new=1, color_value=0.5, color_value_end=1.5) -> image:
    list_img = []
...
def resize(image, size) -> image:
    return cv2.resize(image, size)

def save(name: str, file) -> Tuple[bool, Union[None, Exception]]:
    try:
