...
def get_supply_matrix_planning_all_year(factories,products ):
    ...
    return matrix_plan_supply_max, matrix_plan_supply_min
...
def get_dict_product(products):

    dict_index_product = {}
    dict_weight_product = {}

    for index in range(len(products)):
        dict_index_product[index] = products[index]["productCode"]
        dict_weight_product[index] = products[index]["weight"]

    return dict_index_product, dict_weight_product
...
def process_data_plan(data_json_plan):
    ...
    data_plan['supply_matrix_planning_max'], data_plan['supply_matrix_planning_min'] = get_supply_matrix_planning_all_year(factories,products)
    ...
    dict_index_product, data_plan['dict_weight_product'] = get_dict_product(products)
    ...
    data_plan['safety_stock'] = get_inventory_safety(depots,products,dict_index_product, 'safetyStock' )
    data_plan['inventory'] = get_inventory_safety(depots, products, dict_index_product, 'inventory' )
    data_plan['handlingCost_in']  = []
    for index_depot in range(len(depots)):
        data_plan['handlingCost_in'] .append( depots[index_depot]['handlingInCost'])

    data_plan['handlingCost_out'] = []
    for index_depot in range(len(depots)):
        data_plan['handlingCost_out'].append( depots[index_depot]['handlingOutCost'])


    distane_f_h = np.empty((len(factories) , len(depots)), np.int_)
    mat_config_FD = np.empty((len(factories) , len(depots)), np.int_)
    IB_transport_cost = np.empty((len(factories) , len(depots)), np.int_)
    ...
    for i in range(len(factories)):
        for j in range(len(depots)):
            ...
            for loc_pair in FD:
                if loc_pair['factoryCode'] == data_plan['dict_index_factory'][i] and loc_pair['depotCode'] == data_plan['dict_index_depot'][j]:
            ...
            for pair in data_json_plan['priceByLevelService']['IBtranspCost']:
                if pair['factoryCode'] == data_plan['dict_index_factory'][i] and pair['depotCode'] == data_plan['dict_index_depot'][j]:
                    IB_transport_cost[i,j] = pair['price']


    distane_h_c = np.empty((len(depots),len(customers)), np.int_)
    mat_config_DC = np.empty((len(depots),len(customers)), np.int_)
    OB_transport_cost = np.empty((len(depots),len(customers)), np.int_)

    DC = data_json_plan['distances']['DC']
    for i in range(len(depots)):
        for j in range(len(customers)):
            for loc_pair in DC:
                if loc_pair['depotCode'] == data_plan['dict_index_depot'][i] and loc_pair['customerCode'] == data_plan['dict_index_customer'][j]:
            ...
            for pair in data_json_plan['priceByLevelService']['OBtranspCost']:
                if pair['depotCode'] == data_plan['dict_index_depot'][i] and pair['customerCode'] == data_plan['dict_index_customer'][j]:
    ...
    return data_plan
# process_data_plan(data_json_plan)
...
def create_simulation_uniformlly(each_factory_idx, each_sku_idx, supply_matrix_planning, days_plan ):
    ...
    '''
    supply_max_each_day = round(supply_matrix_planning[each_factory_idx][each_sku_idx]/365)
    supply_min_each_day = round(supply_matrix_planning[each_factory_idx][each_sku_idx]/365)
...
def create_database_supply( supply_matrix_past, data_encode_supply , data_json_plan, days_plan = 365):
    '''
...
def create_database_demand( demand_matrix_past, data_encode_demand , data_json_plan, days_plan = 365):
    '''
    tạo database mô phỏng năng lực sản xuất của nhà máy theo từng sản phẩm theo từng ngày.
    '''
    # trong past data
    customer_code_arr = data_encode_demand['list_customercode_code']
    print("customer_code_arr", customer_code_arr)
    sku_arr = data_encode_demand['list_sku_code']
    print("sku_arr", sku_arr)
    ...
    for each_customer_code in customer_code_in_json:
        ...
        print("NPP",each_customer_code,each_customer_idx  )
        # nếu không có KH trong past data
        if each_customer_code not in customer_code_arr:
            for each_sku_code in sku_code_in_json:
                each_sku_idx = data_plan['dict_product_index'][each_sku_code]
                print("each_sku_code", each_sku_code, each_sku_idx)
        ...
        # khi KH có trong past data
            ...
            for each_sku_code in sku_code_in_json:
                ...
                # mã hàng có trong past data
                    ...
                    print(each_customer_code,each_sku_code, "demand_vector_past", demand_vector_past)
                    print(each_customer_idx, each_sku_idx)
...
# demand_customer_forcasting = create_database_demand( demand_matrix, data_encode_demand , data_json_plan, days_plan = 365)

def simulation_plan(data_json_plan, past_data,days_plan = 365 ):
    ...
    data_simulation['supply_max_factory_forcasting'], data_simulation['supply_min_factory_forcasting'] = \
                    create_database_supply( supply_matrix, data_encode_supply , data_json_plan, days_plan = 365)
...
def write_file(data_json_plan, past_data ):
    data_plan = process_data_plan(data_json_plan)
    demand_matrix = past_data['demand_matrix']
    data_encode_demand = past_data['data_encode_demand']
    ...
    supply_matrix = past_data['supply_matrix']
    data_encode_supply = past_data['data_encode_supply']
    # Convert NumPy arrays in data_plan to Python lists
    data_plan['supply_matrix_planning_max'] = ndarray_to_list(data_plan['supply_matrix_planning_max'])
    data_plan['supply_matrix_planning_min'] = ndarray_to_list(data_plan['supply_matrix_planning_min'])

    data_plan['demand_matrix_planning'] = ndarray_to_list(data_plan['demand_matrix_planning'])

    supply_max_factory_forcasting, supply_min_factory_forcasting = create_database_supply( supply_matrix, data_encode_supply , data_json_plan, days_plan = 365)
    # Save supply data to a compressed ZIP file
    with zipfile.ZipFile("plan_data_NM.zip", mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zip_file:
        dumped_JSON = json.dumps({"supply_max_matrix": ndarray_to_list(supply_max_factory_forcasting),\
                                "supply_min_matrix": ndarray_to_list(supply_min_factory_forcasting),\
                                "infor_dict": data_plan},
    ...
    with zipfile.ZipFile("plan_data_NPP.zip", mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zip_file:
        dumped_JSON = json.dumps({"demand_matrix": ndarray_to_list(demand_customer_forcasting), "infor_dict": data_plan},
                                ensure_ascii=False, indent=4)
