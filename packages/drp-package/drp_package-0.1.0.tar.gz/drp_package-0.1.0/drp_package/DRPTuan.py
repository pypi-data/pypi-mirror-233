...
def create_fake_data(s,num_products, num_factories, num_depots, num_physic_depots, num_days_in_priods):
    ...
    fake_data['weight_product_cost'] = [10]*num_products
...
class Input:
    ...
    def draw_limit_in(self):
        ...
        print("weight_product",data['weight_product'] )


        for dep_idx in range(data['num_depots']):
            for item_idx in range(data['num_products']):
                capacity_N[dep_idx][item_idx] = data['matrix_demand'][ dep_idx, item_idx] *data['weight_product'][item_idx]
...
def add_constraint_limitIn(model, data, N):
    ...
    for physic_depot_id in range(len(data['cluster_physic_depot'])):
        cluster = data['cluster_physic_depot'][physic_depot_id]
        capacity_luong_hang = sum([N[fac_idx, dep_idx, item_idx]*data['weight_product'][item_idx] for item_idx in range(data['num_products']) \
...
def create_coefficent(data, product_idx,fac_idx, dep_idx ):
    ...
    cost_vanchuyen = data['weight_product_cost'][product_idx]*float(data['distane_f_h'][fac_idx][dep_idx])
...
class Output:
    ...
    def draw_check_limitIn(self):
        ...
        for fac_idx in range(data['num_factories']):
            for dep_idx in range(data['num_depots']):
                for item_idx in range(data['num_products']):
                    capacity_N[fac_idx][dep_idx][item_idx] = N_all[fac_idx, dep_idx, item_idx] *data['weight_product'][item_idx]
...
def optimize(data, limit_time_s = 180, num_of_solution=11000):
...
class ORModel:
    def __init__(self, input):
        ...
        # self.model =
    def solve(self, limit_time_s = 180, num_of_solution=11000):
        data= self.data
        self.out_put = optimize(data, limit_time_s = 180, num_of_solution=11000)
...
class DrpTuan:
    ...
    def solve_drp_ngay(self, param= "or_model", limit_time_s = 180, num_of_solution=11000):
        self.create_model(param)
        if param == "or_model":
            self.output= self.model.solve(limit_time_s = 180, num_of_solution=11000)
...
# problem.solve_drp_ngay("or_model",60,2)
