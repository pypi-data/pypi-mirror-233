...
def get_fake_data(s, num_products, num_depots,num_customers, num_physic_depots, num_priods):
    ...
    fake_data['price_product'] = [1]*num_products
...
"""2. Tổng lượng hàng xuất đi luôn bé hơn hoặc bằng lượng hàng tồn kho:
$$ \sum_j X_i^j D_j^{kt} \le \text{Inventory}_i^k $$
"""

def add_constraint_inventory(model, data,x):
    for day in range(data['num_priods']):
        for dep_idx in range(data['num_depots']):
            for product_idx in range(data['num_products']): # sản phẩm

                model.Add(sum( [x[dep_idx, cus_idx] *  data['matrix_demand'][day][cus_idx][product_idx] for cus_idx in range(data['num_customers'])]) \
                        <= data['inventory'][day][dep_idx][product_idx]  )
...
def add_constraint_limitOut(model, data,x):
    for day in range(data['num_priods']):
        for physic_depot_id in range(data['num_physic_depots']):
            ...
            # weight_requets = sum([data['matrix_demand'][day,:,prod_id]*data['weight_product'][prod_id] for prod_id in range(data['num_products']) ])
            ...
            demand_of_physicDepot = sum([ x[dep_idx, cus_idx]*sum([data['matrix_demand'][day,cus_idx,prod_id]*\
                                                                   data['weight_product'][prod_id] for prod_id in range(data['num_products']) ]) \
...
def create_coefficent(data,dep_idx,cus_idx, M ):
    ...
    cost_vanchuyen = float(sum(sum(data['matrix_demand'][:,cus_idx,sku_id])*data['price_product'][sku_id] \
                                       for sku_id in range(data['num_products'])))*data['distane_h_c'][dep_idx][cus_idx]
                ...
                for cus_idx in range(data['num_customers']):
                    for item_idx in range(data['num_products']):
                        capacity_N[priod][dep_idx][cus_idx][item_idx] = output[dep_idx, cus_idx] *\
                                                                        data['matrix_demand'][priod][cus_idx][item_idx]*data['weight_product'][item_idx]
...
def optimize(data, limit_time_s, num_of_solution, cost_penalty):
    ...
    add_constraint_inventory(model, data,x)
    ...
    elif solution_printer.best_solution:
        print(f"Minimum of objective function: {1 - solver.ObjectiveValue()}\n")
        for dep_idx in range(data['num_depots']):
            for cus_idx in range(data['num_customers']):
...
class ORModel:
    def __init__(self, input):
        ...
        # self.model =
    def solve(self, limit_time_s=180, num_of_solution=5, cost_penalty=100_000):
        data= self.data
...
class DrpNgayBackUp:
    ...
    def solve_drp_ngay(self, param= "or_model", limit_time_s=180, num_of_solution=5, cost_penalty=100_000):
        ...
        if param == "or_model":
            self.output= self.model.solve(limit_time_s=180, num_of_solution=5, cost_penalty=100_000)

    def help(self):
        # numpy.sort(a, axis=-1, kind=None, order=None)
        s="DrpNgayBackUp.input\nDrpNgayBackUp.output\nDrpNgayBackUp.model"
        s+= "\nmain( data_model = None, file_name = None)"
        s+="\nParameters\n data_model: kiểu dữ liệu được truyền vào. \nfile_name: đọc từ file. \n Default: create faked data\nsolve_drp_ngay(self, param= , limit_time_s=180, num_of_solution=5, cost_penalty=100_000)"
        print(s)

input= Input()
A = DrpNgayBackUp(input)

# A.solve_drp_ngay("or_model",60,5,100)
