...
def get_fake_data(s, num_products, num_depots,num_customers, num_physic_depots, num_priods):
    ...
    fake_data['IB_transport_cost'] = np.copy(distane_h_c)
...
def create_coefficent(data,dep_idx,cus_idx, M ):
    ...
    cost_vanchuyen = float(sum(sum(data['matrix_demand'][:, cus_idx, sku_id]) \
                               for sku_id in range(data['num_products']))) * data['IB_transport_cost'][dep_idx][cus_idx]
