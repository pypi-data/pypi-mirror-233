...
class Agent(client_type):
    ...
    def __init__(self, spec, bb_addr):
        ...
        self.promises = PromiseSet(spec)
    
    def attribute(self, name):
        try: 
            if name in self.spec.attributes:
                attr_value = self.spec.attributes[name]
                try:
                    result = parse_promise(attr_value)
                    if result != None:
                        raise Exception(f"Agent '{self.spec.name}' is trying to access value from attribute '{name}' before Promise fulfillment")
                ...
                raise KeyError(f"could not find attribute '{name}'; valid attributes are: {list(self.spec.attributes.keys())}")
    ...
    async def await_data(self):
        self.post_status_update(AgentState.AWAITING_PROMISE, "awaiting {} promises".format(len(self.promises.promises)))
            ...
            if len(msgs) > 0:
                for msg in msgs:
                    ...
                    if msg_type == "result":
                        ...
                        attr_name, exists = self.promises.contains(msg.result.promise)

                        if exists:
                            self.promises.promises[attr_name] = msg.result.promise
                            self.spec.attributes[attr_name] = msg.result.promise.data

                        if self.promises.all_fulfilled():
