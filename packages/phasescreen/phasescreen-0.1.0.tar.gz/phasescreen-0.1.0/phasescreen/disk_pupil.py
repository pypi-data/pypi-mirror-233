...
if __name__=="__main__":
    from matplotlib.pylab import plt 
    from phasescreen.base import PhaseImageMaker 
    import zernpol 
    screen = DiskScreenMaker( center=(.3, 0), diameter=2, phase_angle=np.pi/8.0, obscuration_diameter=0.2  ).make_screen( [800,800], 300.0 )
    
    pm = PhaseImageMaker( screen )
    # plt.imshow( pm.make_phase( [0,1.0, 0.5, 0.2] ) )
    # plt.imshow( pm.make_phase( np.random.normal( size=(30,))) ) 
    plt.imshow( pm.make_phase( {zernpol.zernpol('tip') : 2.0} )  )
