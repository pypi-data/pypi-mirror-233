...
__all__ = [
    ...
    "blank_check_company_timeseries",
...
def futures_market_session_type(sessionsymbol):
    return session_type(sessionsymbol)
...
def first_quoted_date(symbol, datetimeformat="iso", format="", tz="UTC"):
    if format != '':
    ...
    if len(r.content) == 0:
        return None
    return decode_date(r.content, datetimeformat, tz=tz)


def last_quoted_date(symbol, datetimeformat="iso", format="", tz="UTC"):
    if format != '':
        datetimeformat = format # Hanlde previous calling style - to be deprecated
...
def second_last_quoted_date(symbol, datetimeformat="iso", format="", tz="UTC"):
    if format != '':
        datetimeformat = format # Hanlde previous calling style - to be deprecated
...
def first_notice_date(symbol, datetimeformat="iso", format="", tz="UTC"):
    if format != '':
        datetimeformat = format # Hanlde previous calling style - to be deprecated
...
###############
def blank_check_company_timeseries(
    symbol,
    padding_setting=PaddingType.NONE,
    start_date="1800-01-01",
    end_date="2999-01-01",
    ...
    r = get_api_data("blankcheckcompany", str(symbol), parameters)
    validate_api_response(r, symbol)
    if timeseriesformat == "numpy-recarray":
        return create_numpy_recarray(
            r, numpy_recarray, datetimeformat=datetimeformat, timezone=timezone
