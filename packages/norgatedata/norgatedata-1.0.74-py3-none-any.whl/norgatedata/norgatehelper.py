...
def get_api_data(
    ...
    while trycount < maxretries and not success:
        ...
        try:
            ...
            if maxretries > 1:
                logger.warn(
                    "Unable to obtain data from Norgate Data - perhaps the app Norgate Data Updater (NDU) has not been started on this PC?  Attempt "
                    + str(trycount)
                    + "/"
                    + str(maxretries)
                    + ".  Retrying... "
                ...
                logger.warn(
                    "Unable to obtain valid status from Norgate Data - perhaps the app Norgate Data Updater (NDU) has not been started on this PC?"
                )
...
def create_pandas_dataframe(
    ...
    if datetimeformat == "datetime": 
        # Note, this actually emits date objects on M8D/datetimes64[D] objects
        npdates = npdates.astype(datetime.datetime) 
    elif datetimeformat == "date":
        npdates = npdates.astype(datetime.date)
    indicatorType = []
    ...
    if timezone is not None and timezone != "" and timezone != "naive":
        pdf.index = pdf.index.tz_localize(timezone)
    pdf.name = symbol
    pdf.index.name = "Date"
...
def version_checker(currentversion, package):
    ...
    if not (os.path.isfile(jsonfile)) or (
        os.path.isfile(jsonfile) and (time() - os.path.getmtime(jsonfile) > 60 * 60) or newversion == "0"
    ):  # Check once an hour
        newversion = get_latest_pypi_version(package)
        # Write it out, with some retries
        for i in range(1,10):
            try:
                ...
                logger.Warning("Unable to write to file " + jsonfile + " Error is: " + str(e))
                logger.Warning("This will be retried")
    ...
    if newversion > currentversion:
        ...
        logger.warn("Recent change log can be found at: https://pypi.org/project/" + package + "/#recent-version-history")
    ...
    if not os.path.exists(path):
        try:
            ...
            if exc.errno == EEXIST and os.path.isdir(path):
                return
            logger.error("Folder " + path + "does not exist and unable to create folder - consider using environment variable NORGATEDATA_ROOT to specify a different folder" )
            sys.exit()

    # Check folder is writeable, with a random filename
    checkfilename = path + "/" + "checkfile.tmp." + str(random.randint(1,99999999))
    try:
        ...
        logger.error("Unable to write to folder " + path + " " + str(e))
        logger.error("... consider using environment variable NORGATEDATA_ROOT to specify a different folder that is writeable")
        sys.exit()

def decode_date(content, datetimeformat="iso", format=None, tz='UTC'):
    """
    ...
    if type(content) == bytes:
        ...
        dateval = content.strftime("%Y-%m-%d")
    elif type(content) == datetime.date:
        dateval = content.strftime("%Y-%m-%d")
    elif type(content) == np.datetime64:
        dateval = content.strftime("%Y-%m-%d")
    else:
        logger.error("Unknown date type provided - " + str(type(content)));
    try:
        ...
        if datetimeformat == "pandas-timestamp":
            return pd.Timestamp(dateval, tz=tz)
