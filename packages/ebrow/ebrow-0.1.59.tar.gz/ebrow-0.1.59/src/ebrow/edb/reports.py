...
class Report:
    ...
    def __init__(self, parent, ui, settings):
        ...
        self._ui.chkLOSfilter_3.clicked.connect(self._setClassFilter)
    ...
    def updateTabReport(self):
        ...
        if self._defaultPreface is None:
...
'''.format(self._settings.readSettingAsString("stationName"),
              ...
              self._dataSource.totalsUnclassified()
              )

        # self._defaultPreface = self._defaultPreface.replace("\n", "")
        preface = self._settings.readSettingAsString("preface")
    ...
    def _toggleCheckAll(self):
        ...
        self._ui.chkLOSfilter_3.setChecked(self._ui.chkAll_3.isChecked())
    ...
    def _fillImportList(self):
        ...
        for fileName in fileList:
            ...
            if img.name.startswith('autoshot'):
                ...
                evTuple = (imgFile, htmlFile, commFile, detFile, powFile, img2dFile, img3dFile)
    ...
    def _buildCoverageTable(self):
        ...
        rdfDict = dict()
        rdfDict['1'] = [':/rts', 'Meteor scatter detection report', ':/logo']

        # section 1 report coverage, cannot be excluded
        rdfDict['2'] = ['Report coverage']
    ...
    def _buildSiteInfosTable(self):
        ...
        if not self._ui.chkSiteInfosExc.isChecked():
            ...
            rdfDict['2'] = ['Station name: ', self._settings.readSettingAsString('stationName'),
                            self._settings.readSettingAsString('logoPath')]
            ...
            rdfDict['7'] = ['Antenna type: ', self._settings.readSettingAsString('antenna'),
                            'Azimuth: ', str(self._settings.readSettingAsString('antAzimuth')) + '°',
                            'Elevation: ', str(self._settings.readSettingAsString('antElevation')) + '°']
            ...
            rdfDict['12'] = ['e-mail: ', self._settings.readSettingAsString('email')]
            rdfDict['13'] = ['Notes: ', self._settings.readSettingAsString('notes')]
    ...
    def _buildDailyCountsTable(self):
        if not self._ui.chkDailyExc.isChecked():
            ...
            dailyDf = self._dataSource.dailyByClassification(df, self._classFilter,
    ...
    def _buildHourlyCountsTable(self):
        if not self._ui.chkHourlyExc.isChecked():
            ...
            hourlyDf = self._dataSource.makeDaily(df, self._parent.fromDate, self._parent.toDate, dtRes='H',
    ...
    def _buildTenMinCountsTable(self):
        if not self._ui.chk10minExc.isChecked():
            ...
            tenDf = self._dataSource.makeDaily(df, self._parent.fromDate, self._parent.toDate, dtRes='10T',
                                               filters=self._classFilter)
    ...
    def _reportXLSX(self):
        ...
        # TODO: openpyxl is able to embed images also, besides data
        # but I need to know a way to get the Workbook created by to_excel()
        # to continue work on it, or otherwise reopen the xslx as Workbook
        # and continue working on it, adding the images and comments.
        self._parent.busy(False)
...
<html lang="en">
    <head>
        <title>Echoes Report System</title>
...
'''
        ...
        if self._isChecked('HI LOS') == 'checked':
            self._div[self.DIV_DAILY] += '\t\t\t\t\t\t\t<th>Headed</th>\n'
...
'''
    ...
    def _buildEventHtml(self, evTuple):
        if self._ui.chkExpEvExc.isChecked():
            return
        (imgFile, htmlFile, commFile, detFile, powFile, img2dFile, img3dFile) = evTuple
        ...
        if commFile is not None:
            commFilePath = srcFilePath / commFile
            with open(commFilePath, 'r') as f:
        ...
        htmlDoc += '''\t\t\t\t<div class="box">
                    <h2 class="graph">Events gallery, event#{0}</h2>
                    <p>{1}</p>
                    <hr>
                    <h3>Screenshot</h3>
                        <img src="{2}" alt="{3}">
                    <hr>
'''.format(id, comment, imgFilePath.as_posix(), imgFilePath.name)
        ...
        if powFile is not None:
            ...
            htmlDoc += '''\t\t\t\t\t<h3>Total power profile</h3>
                        <img src="{0}" alt="{1}">
                    <hr>
'''.format(powFilePath.as_posix(), powFilePath.name)
        ...
        if img2dFile is not None:
            ...
            htmlDoc += '''\t\t\t\t\t<h3>2D data plotting</h3>
                        <img src="{0}" alt="{1}">
                    <hr>
'''.format(img2dFilePath.as_posix(), img2dFilePath.name)
        ...
        if img3dFile is not None:
            ...
            htmlDoc += '''\t\t\t\t\t<h3>3D data plotting</h3>
                        <img src="{0}" alt="{1}">
                    <hr>
'''.format(img3dFilePath.as_posix(), img3dFilePath.name)

        if df is not None:
            htmlDoc += '''\t\t\t\t\t<h3>Event details</h3>
                        <table class="blueTable" style="table-layout: fixed;word-wrap: break-word;">
                            <thead>
                                <tr>
'''
            for col in df.columns:
...
'''.format(
        ...
        divReceiver = '''\t\t\t\t<div class="box">
                    ...
                    <dl>
                        ...
                        <dd>{6}</dd>
                        <dt>Software</dt>
                        <dd>Echoes v.{7} and its Data Browser v.{8}</dd>
                        <dt>e-mail</dt>
                        <dd>{9}</dd>
                        <dt>Notes:</dt>
                        <dd>{10}</dd>
...
'''.format(df.loc['7', 'B'], df.loc['7', 'D'], df.loc['7', 'F'],
           df.loc['8', 'B'], df.loc['9', 'B'], df.loc['10', 'B'],
           df.loc['11', 'B'], '0.51', self._parent.version,
           df.loc['12', 'B'], df.loc['13', 'B'])  # TODO: the echoes version must be embedded in DB
...
'''
        ...
        divEvents = '''\t\t\t\t<div class="box">
                    <h2 class="events">Exported events images gallery</h2>
                    <ul>
'''
        for evTuple in self._events:
            ...
            divEvents += '\t\t\t\t\t\t<li><a href="{0}" title="{1}" {4}>Screenshot {2}, {3}</a></li>\n'.format(
    ...
    def _activeFiltersHtml(self):
        html = '''\t\t\t\t<h4>Active filters:</h4>
                <ol>
                    ...
                    <li> 
                        <input type="checkbox" id="he" name="he" value="Headed echo" {2} onclick="return false;">
                        <label for="he">Headed echo</label>
                    </li>
                    <li> 
                        <input type="checkbox" id="esd" name="esd" value="Fake events (ESD)" {3} onclick="return false;">
                        <label for="esd">Fake events (ESD)</label>
                    </li>
                    <li> 
                        <input type="checkbox" id="car1" name="car1" value="Fake events (Carrier type 1)" {4} onclick="return false;">
                        <label for="car1">Fake events (Carrier type 1)</label>
                    </li>
                    <li> 
                        <input type="checkbox" id="car2" name="car2" value="Fake events (Carrier type 2)" {5} onclick="return false;">
                        <label for="car2">Fake events (Carrier type 2)</label>
                    </li>
                    <li> 
                        <input type="checkbox" id="sat" name="sat" value="Fake events (RX saturation)" {6} onclick="return false;">
                        <label for="car2">Fake events (RX saturation)</label>
                    </li>
                    <li> 
                        <input type="checkbox" id="long" name="long" value="Fake events (too long lasting)" {7} onclick="return false;">
                        <label for="car2">Fake events (too long lasting)</label>
                    </li>
                </ol>
'''.format(self._isChecked('OVER'), self._isChecked('UNDER'), self._isChecked('HI LOS'), self._isChecked('FAKE ESD'),
           self._isChecked('FAKE CAR1'),
