...
class DatasetCollector(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'image': image.to(torch.device('cuda' if torch.cuda.is_available() else 'cpu')), 'label': torch.tensor(class_idx).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))}
...
class Generator(nn.Module):
    ...
    def forward(self, inputs):
        ...
        label_output = label_output.to(device)
        label_output = label_output.view(-1, 1, 4, 4)

        # converting latent 512x1x1 to 512x4x4
        latent_output = self.latent(noise_vector)
        latent_output = latent_output.to(device)
...
class Discriminator(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Ensure both image and label are on the same device
        device = img.device
        label = label.to(device)
        img = img.to(device)
        ...
        label_output = self.label_condition_disc(label.to(img.device))
...
class cGAN():
    def __init__(self) -> None:
        ...
        self.criterion_generator = nn.BCELoss().to(self.device)
        self.criterion_discriminator = nn.BCELoss().to(self.device)
