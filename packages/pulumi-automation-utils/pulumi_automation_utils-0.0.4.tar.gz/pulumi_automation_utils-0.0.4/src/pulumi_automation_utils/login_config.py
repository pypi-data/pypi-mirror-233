    ...
    def login_to_azure(self):
        if not self.logged_in:
            if self.get_login_mode() is LoginMode.INTERACTIVE:
                try:
                    send_os_command("az account show")
                except subprocess.CalledProcessError as e:
                    print("command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))
                    try:
                        send_os_command("az logout")
                    except subprocess.CalledProcessError as e:
                        print("command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))
                try:
                    output = json.loads(sanitize_stdout_for_json(send_os_command("az login --use-device-code")))
                    ...
                    print("command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))
                    exit()
                # TODO: Get the subscription from the output and set it in the login params
            else:
