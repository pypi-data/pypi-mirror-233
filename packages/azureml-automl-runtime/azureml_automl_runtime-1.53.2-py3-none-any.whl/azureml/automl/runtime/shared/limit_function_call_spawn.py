...
class EnforceLimits:
    ...
    def execute_multiple(
    ...
    ) -> Tuple[Optional[List[Optional[T]]], Optional[BaseException], float]:
        """
        Execute the functions in different subprocesses with the resource constraints applied.

        :param working_dir: The working directory to use for temporary files
        :param functions: The functions to execute
        :param args: List of positional args. args[i] are the positional args for the function at functions[i]
        :param kwargs: List of keyword args. kwargs[i] are the keyword args for the funtion at functions[i]
        :return: List of value/error/execution time tuple
        """
        if args is None:
            args = [()]
        if kwargs is None:
        ...
        # Extends args and kwargs to be the same length as functions
        for _ in range(len(args), len(functions)):
            args.append(())

        for _ in range(len(kwargs), len(functions)):
            kwargs.append({})
        ...
        limit_functions, all_args = [], []

        # Only log types to prevent possibility of leaking PII
        for func, arg, kwarg in zip(functions, args, kwargs):
            logger.info(
                "Calling function {} with argument types: {}, {}".format(
                    func, [x.__class__ for x in arg], {k: kwarg[k].__class__ for k in kwarg}
                )
            )
            limit_functions.append(EnforceLimits.subprocess_func)
            all_args.append((func,) + single_args + arg)
        ...
        try:
            ...
            result = spawn_client.multiple_run_in_proc(
                working_dir,
                timeout,
                limit_functions,
                all_args,
