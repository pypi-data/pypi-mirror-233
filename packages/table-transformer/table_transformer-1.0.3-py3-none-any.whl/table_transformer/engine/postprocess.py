...
def apply_class_thresholds(bboxes,
                           labels, scores, class_names, class_thresholds):
    """
    Filter out bounding boxes
    whose confidence is below the confidence threshold for
    its associated class label.
    """
    # Apply class-specific thresholds
    indices_above_threshold = [idx for idx, (score, label) in enumerate(
        zip(scores, labels))
...
def objects_to_cells(table,
                     objects_in_table,
                     tokens_in_table, class_map, class_thresholds):
    """
    Process the bounding boxes produced by the table
    ...
    table_structures = objects_to_table_structures(table,
                                                   objects_in_table,
                                                   tokens_in_table,
                                                   class_map,
                                                   class_thresholds)
    ...
    if (len(table_structures['columns']) < 1
            or len(table_structures['rows']) < 1):
        cells = []  # None
        confidence_score = 0
    else:
        cells, confidence_score = table_structure_to_cells(table_structures,
                                                           tokens_in_table,
                                                           table['bbox'])
...
def objects_to_table_structures(table_object,
                                objects_in_table,
                                tokens_in_table,
                                class_names, class_thresholds):
    """
    Process the bounding boxes produced by the table structure
    recognition model into
    a *consistent* set of table structures (rows, columns,
    supercells, headers).
    This entails resolving conflicts/overlaps, and ensuring the boxes
    meet certain alignment
    ...
    columns = [obj for obj in objects_in_table
               if class_names[obj['label']] == 'table column']
    rows = [obj for obj in objects_in_table
            if class_names[obj['label']] == 'table row']
    headers = [obj for obj in objects_in_table
               if class_names[obj['label']] == 'table column header']
    supercells = [obj for obj in objects_in_table
                  if class_names[obj['label']] == 'table spanning cell']
    for obj in supercells:
        obj['subheader'] = False
    subheaders = [obj for obj in objects_in_table
    ...
    columns = refine_columns(columns,
    ...
    table_object['row_column_bbox'] = [column_rect[0], row_rect[1],
    ...
    if len(rows) > 0 and len(columns) > 1:
        table_structures = refine_table_structures(
            table_object['bbox'],
            table_structures, tokens_in_table, class_thresholds)

    return table_structures


def refine_rows(rows, score_threshold):
    """
...
def refine_columns(columns, score_threshold):
    """
...
def nms_by_containment(container_objects,
    ...
    """
    Non-maxima suppression (NMS) of objects based on shared containment
    of other objects.
    """
    ...
    packages_by_container, _, _ = slot_into_containers(
        container_objects,
        package_objects=[], overlap_threshold=overlap_threshold,
    ...
    final_objects = [obj for idx, obj in enumerate(container_objects)
                     if not suppression[idx]]
    return final_objects


def slot_into_containers(container_objects, package_objects,
                         overlap_threshold=0.5,
                         unique_assignment=True, forced_assignment=False):
    """
    ...
    for package_num, package in enumerate(package_objects):
        ...
        for container_num, container in enumerate(container_objects):
            ...
            match_scores.append({'container': container,
        ...
        if forced_assignment or best_match_score['score'] >= overlap_threshold:
            container_assignments[
                best_match_score['container_num']].append(package_num)
        ...
        if not unique_assignment:  # slot package into all eligible slots
            for match_score in sorted_match_scores[1:]:
                if match_score['score'] >= overlap_threshold:
                    container_assignments[
                        match_score['container_num']].append(package_num)
                    package_assignments[package_num].append(
...
def remove_objects_without_content(page_spans, objects):
    """
    Remove any objects (these can be rows, columns, supercells, etc.)
    that don't
...
def extract_text_from_spans(spans, join_with_space=True,
                            remove_integer_superscripts=True):
    """
    ...
    # Force the span at the end of every line within a block to
    # have exactly one space
    # unless the line ends with a space or ends with a non-space
    # followed by a hyphen
    line_texts = []
    line_span_texts = [spans_copy[0]['text']]
    for span1, span2 in zip(spans_copy[:-1], spans_copy[1:]):
        if (not span1['block_num'] == span2['block_num'] or
                not span1['line_num'] == span2['line_num']):
...
def refine_table_structures(table_bbox, table_structures, page_spans,
                            class_thresholds):
    """
    ...
    # Process supercells
    supercells = [elem
                  for elem in table_structures['supercells']
                  if not elem['subheader']]
    subheaders = [elem
                  for elem in table_structures['supercells']
                  if elem['subheader']]
    supercells = apply_threshold(
        supercells, class_thresholds["table spanning cell"])
    ...
    # agreement
    # with rows and columns first; if supercells still overlap after this
    # operation,
...
def nms(objects, match_criteria="object2_overlap", match_threshold=0.05,
        keep_higher=True):
    """
    A customizable version of non-maxima suppression (NMS).

    Default behavior: If a lower-confidence object overlaps more than 5% of
    its area
    ...
    keep_higher: if True, keep the object with the higher metric;
    otherwise, keep the lower
    """
    if len(objects) == 0:
        return []
    ...
    for object2_num in range(1, num_objects):
        ...
        for object1_num in range(object2_num):
            if not suppression[object1_num]:
                ...
                intersect_area = object1_rect.intersect(
                    object2_rect).get_area()
                try:
...
def align_headers(headers, rows):
    ...
    for row_num in header_row_nums:
        if row_num == last_row_num + 1:
            ...
            # This ignores any subsequent rows in the table labeled
            # as a header.
...
def align_supercells(supercells, rows, columns):
    """
    For each supercell, align it to the rows it intersects 50% of the
    height of,
    and the columns it intersects 50% of the width of.
    Eliminate supercells for which there are no rows and columns it
    intersects 50% with.
    """
    aligned_supercells = []
    ...
    for supercell in supercells:
        ...
        supercell['header'] = False
        if (len(intersecting_data_rows) > 0
                and len(intersecting_header_rows) > 0):
        ...
        for row_num in intersecting_rows:
            if row_bbox_rect is None:
                ...
                row_bbox_rect = row_bbox_rect.include_rect(
                    rows[row_num]['bbox'])
        if row_bbox_rect is None:
            continue
        ...
        if (len(intersecting_rows) > 0 and len(intersecting_cols) > 0
                and (len(intersecting_rows) > 1
                     or len(intersecting_cols) > 1)):
            ...
            if ('span' in supercell and supercell['header'] and
                    len(supercell['column_numbers']) > 1):
                for row_num in range(0, min(supercell['row_numbers'])):
                    new_supercell = {'row_numbers': [row_num],
                                     'column_numbers': supercell[
                                         'column_numbers'],
                                     'score': supercell['score'],
                                     'propagated': True}
                    new_supercell_columns = [columns[idx]
                                             for idx in supercell[
                                                 'column_numbers']]
                    new_supercell_rows = [rows[idx]
                                          for idx in supercell['row_numbers']]
                    bbox = [min([column['bbox'][0]
                                 for column in new_supercell_columns]),
                            min([row['bbox'][1]
                                 for row in new_supercell_rows]),
                            max([column['bbox'][2]
                                 for column in new_supercell_columns]),
                            max([row['bbox'][3]
                                 for row in new_supercell_rows])]
...
def header_supercell_tree(supercells):
    ...
    any row above it.
    The cells in the header form a tree, but a supercell with more than
    one supercell in a row
    above it means that some cell has more than one parent, which is not
    allowed. Eliminate
    any supercell that would cause this to be violated.
    """
    header_supercells = [supercell for supercell in supercells
...
def table_structure_to_cells(table_structures, table_spans, table_bbox):
    ...
    for column_num, column in enumerate(columns):
        for row_num, row in enumerate(rows):
            ...
            cell = {'bbox': list(cell_rect), 'column_nums': [column_num],
                    'row_nums': [row_num],
                    'header': header}
            ...
            if cell['subcell']:
                ...
                # cell_text = extract_text_inside_bbox(table_spans,
                # cell['bbox'])
    ...
    for supercell in supercells:
        ...
        for subcell in subcells:
            ...
            if (subcell_rect.intersect(supercell_rect).get_area()
                ...
                # as header cells for a supercell to be classified as a
                # header cell;
                # otherwise, this could lead to a non-rectangular header region
                header = header and 'header' in subcell and subcell['header']
        if len(cell_rows) > 0 and len(cell_columns) > 0:
            cell = {'bbox': list(cell_rect), 'column_nums': list(cell_columns),
    ...
    span_nums_by_cell, _, _ = slot_into_containers(cells, table_spans,
    ...
    # Adjust the row, column, and cell bounding boxes to
    # reflect the extracted text
    num_rows = len(rows)
    rows = sort_objects_top_to_bottom(rows)
...
def remove_supercell_overlap(supercell1, supercell2):
    ...
    # While the supercells have overlapping grid cells,
    # continue shrinking the less-confident
    # supercell one row or one column at a time
    while len(common_rows) > 0 and len(common_columns) > 0:
        # Try to shrink the supercell as little as possible to
        # remove the overlap;
        # if the supercell has fewer rows than columns, remove an overlapping
        # column,
