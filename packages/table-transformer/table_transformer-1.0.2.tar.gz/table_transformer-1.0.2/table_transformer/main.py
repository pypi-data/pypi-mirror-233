...
def get_detection_class_thresholds(table: float = 0.5,
                                   table_rotated: float = 0.5,
                                   no_object: int = 10):
    """
    Get detection class thresholds in dictionary format
    """
    return {
        'table': table,
...
def get_args():
    ...
    parser.add_argument('--mode',
                        help="The processing to apply to the input image \
                            and tokens",
    ...
    parser.add_argument('--structure_model_path', help="The path to the \
                        structure model")
    ...
    parser.add_argument('--crop_padding', type=int, default=10,
                        help="The amount of padding to add around \
                            a detected table when cropping.")
...
def objects_to_crops(img, tokens, objects, class_thresholds, padding=10):
    ...
    for obj in objects:
        ...
        table_tokens = [token for token in tokens
                        if iob(token['bbox'], bbox) >= 0.5]
        ...
        # If table is predicted to be rotated,
        # rotate cropped image and tokens/words:
...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    for table in tables:
        table_objects = [obj for obj in objects if iob(obj['bbox'],
                                                       table['bbox']) >= 0.5]
        table_tokens = [token for token in tokens if iob(token['bbox'],
                                                         table['bbox']) >= 0.5]
        structure = {}
...
def visualize_detected_tables(img, det_tables, out_path):
    """
    Visualize all detected table and save the figure in the desired dir.
    """
...
def visualize_cells(img, cells, show_plot=True):
    ...
    new_img = Image.fromarray(np.array(buf))
...
class TableExtractionPipeline(object):
    ...
    def __init__(self, det_device=None, str_device=None,
                 ...
                 detection_class_thresholds=None,
                 structure_class_thresholds=None,
                 onnx_str_model_path="Lib\site-packages\/table_transformer\model\TATR-v1.1-All-msft.onnx",
                 onnx_str_model=None):
        ...
        self.det_class_idx2name = {v: k
                                   for k, v in self.det_class_name2idx.items()}
        ...
        if det_config_path is not None:
        ...
        if str_config_path is not None:
            ...
            if str_model_path is not None:
                self.str_model.load_state_dict(
                    torch.load(str_model_path,
        ...
        if onnx_str_model_path is not None:
            self.onnx_str_model = onnxruntime.InferenceSession(onnx_str_model_path)
            print("Structure model initialized.")


    def __call__(self, page_image, page_tokens=[],
                out_objects=False, out_cells=False,
                out_html=False, out_csv=False,
                onnx_inference=True):
        return self.recognize(page_image, tokens=page_tokens,
    ...
    def detect(self, img, tokens=None, out_objects=True, out_crops=False,
        ...
        if out_crops:
            tables_crops = objects_to_crops(img, tokens, objects,
    ...
    def recognize(self, img, tokens=None, out_objects=False, out_cells=False,
                  out_html=False, out_csv=False,
                  reader=None, text_threshold=0.7,
                  onnx_inference=True) -> Dict:
        """
        ...
        """
        out_formats = {}
        if self.onnx_str_model is None:
        ...
        if onnx_inference:
            np_image = np.expand_dims(
                ...
                axis=0)
            ort_inputs = {self.onnx_str_model.get_inputs()[0].name: np_image}
            ort_output = self.onnx_str_model.run(["pred_logits", "pred_boxes"],
                                                 ort_inputs)
            outputs = {
        ...
        tables_structure = objects_to_structures(objects, tokens,
    ...
    def extract(self, img, tokens=None, out_objects=True, out_crops=False,
                out_cells=False,
                out_html=False, out_csv=False, crop_padding=10):

        detect_out = self.detect(img, tokens=tokens, out_objects=False,
                                 out_crops=True,
        ...
        for table in cropped_tables:
            ...
            extracted_table = self.recognize(img, tokens=tokens,
...
def recognize_text(image: np.ndarray,
    ...
    if not isinstance(reader, easyocr.easyocr.Reader):
        raise TypeError("reader must be an EasyOCR reader object")

    for cell in stqdm(table, desc="Processing cells"):
        ...
        for idx, elem in enumerate(val):
            if key == 'crops':
                for idx, cropped_table in enumerate(val):
                    out_img_file = img_file.replace(
                        ".jpg",
                        "_table_{}.jpg".format(idx))
                    cropped_table['image'].save(os.path.join(args.out_dir,
                                                             out_img_file))
                    out_words_file = out_img_file.replace(".jpg",
                                                          "_words.json")
                    with open(os.path.join(args.out_dir,
                                           out_words_file), 'w') as f:
                        json.dump(cropped_table['tokens'], f)
            elif key == 'cells':
                out_file = img_file.replace(
                    ".jpg", "_{}_objects.json".format(idx)
                    )
                with open(os.path.join(args.out_dir, out_file), 'w') as f:
                    json.dump(elem, f)
                if args.verbose:
...
def main():
    ...
    for count, img_file in enumerate(img_files):
        ...
        if args.words_dir is not None:
            tokens_path = os.path.join(args.words_dir,
        ...
        if args.mode == 'recognize':
            extracted_table = pipe.recognize(img, tokens,
                                             out_objects=args.objects,
                                             out_cells=args.csv,
                                             out_html=args.html,
                                             out_csv=args.csv)
        ...
        if args.mode == 'detect':
            detected_tables = pipe.detect(img, tokens,
                                          out_objects=args.objects,
                                          out_crops=args.crops)
        ...
        if args.mode == 'extract':
            extracted_tables = pipe.extract(img, tokens,
                                            out_objects=args.objects,
                                            out_cells=args.csv,
                                            out_html=args.html,
                                            out_csv=args.csv,
