...
import src.table_datasets as TD
from src.table_datasets import PDFTablesDataset
from src.eval import eval_coco


def get_args():
    """
    Get arguments for training and testing
    """
    ...
    parser.add_argument('--model_save_dir', help="The output directory for , \
                        saving, model params and checkpoints")
...
def get_transform(data_type, image_set):
    """
    Get image processing arguments
    """
...
def train(args, model, criterion, postprocessors, device):
    ...
    if args.model_load_path:
        ...
        if not args.load_weights_only and 'optimizer_state_dict' in checkpoint:
            ...
            print("*** ERROR: Optimizer state of saved checkpoint not found. "
                  "To resume training with new initialized values add the, \
                    --load_weights_only flag.")
            raise Exception("ERROR: Optimizer state of saved checkpoint, \
                            not found. Must add --load_weights_only flag, \
                            to resume training without.")
        if not args.load_weights_only and 'epoch' in checkpoint:
            args.start_epoch = checkpoint['epoch'] + 1
        elif args.load_weights_only:
            print("*** WARNING: Resuming training and, \
                  ignoring previously saved epoch."
                  "To resume from previously saved epoch, \
                    remove the --load_weights_only flag.")
        else:
            print(f"*** WARNING: Epoch of saved model not found. \
                  Starting at epoch {args.start_epoch}.")

    # Use user-specified save directory, if specified
    if args.model_save_dir:
        output_directory = args.model_save_dir
    # If resuming from a checkpoint with optimizer state, save into same dir.
    ...
    if not resume_checkpoint and os.path.exists(model_save_path):
        print("*** WARNING: Output model path exists but is not being, \
              used to, \resume training; training will overwrite it.")

    if args.start_epoch >= args.epochs:
        print(f"*** WARNING: Starting epoch ({args.start_epoch}) is greater \
              or equal to, \
              the number of training epochs ({args.epochs}).")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch+1) % args.checkpoint_freq == 0:
            model_save_path_epoch = os.path.join(output_directory,
                                                 'model_'
                                                 + str(epoch+1)
                                                 + '.pth')
...
def main():
    ...
    if args.mode == 'eval' and args.debug:
        print(f"Running evaluation/inference in DEBUG mode, processing \
              will take longer. \
              Saving output to: {args.debug_save_dir}.")
