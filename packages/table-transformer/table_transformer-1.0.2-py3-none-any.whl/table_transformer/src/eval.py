...
import src.grits as grits
from src.grits import grits_con, grits_top, grits_loc
...
def objects_to_cells(bboxes, labels, scores, page_tokens,
                     structure_class_names,
                     structure_class_thresholds,
                     structure_class_map):
    ...
    table_class_objects = [obj for obj in table_objects
    ...
    tokens_in_table = [token for token in page_tokens
                       if postprocess.iob(token['bbox'], table_bbox) >= 0.5]
...
def cells_to_adjacency_pair_list(cells, key='cell_text'):
    ...
    # - For every row the cell occupies, what is the first cell
    # to the right with text that
    #   also occupies that row
    # - For every column the cell occupies, what is the first
    # cell below with text that
    ...
    for cell1_num, cell1 in enumerate(cells):
        ...
        for column_num in cell1['column_nums']:
            # Start from the next row and stop when we encounter a non-blank
            # cell
        ...
        for row_num in cell1['row_nums']:
            # Start from the next column and stop when we encounter a
            # non-blank cell
            # This cell is considered adjacent
                    ...
                    adj_bbox = [cell1['bbox'][2],
                                ...
                                (max(cell1['bbox'][1],
                                     cell2['bbox'][1])+min(
                                         cell1['bbox'][3],
                                         cell2['bbox'][3]))/2+3]
                    adjacent_cell_props[cell2_num] = ('H',
                                                      current_column
                                                      - max_column - 1,
                                                      adj_bbox)
                    break
        ...
        for adjacent_cell_num, props in adjacent_cell_props.items():
            ...
            adjacency_list.append((cell1['cell_text'],
                                   cell2['cell_text'], props[0], props[1]))
...
def cells_to_adjacency_pair_list_with_blanks(cells, key='cell_text'):
    ...
    for cell1_num, cell1 in enumerate(cells):
        ...
        for adjacent_cell_num, props in adjacent_cell_props.items():
            cell2 = cells[adjacent_cell_num]
            adjacency_list.append((cell1['cell_text'], cell2['cell_text'],
                                   props[0], props[1]))
            adjacency_bboxes.append(props[2])
...
def dar_con(true_adjacencies, pred_adjacencies):
    ...
    num_true_positives = (sum(true_c.values())
                          - sum((true_c - pred_c).values()))
...
def compute_metrics(mode, true_bboxes, true_labels, true_scores, true_cells,
    ...
    true_text_grid = np.array(grits.cells_to_grid(true_cells, key='cell_text'),
                              dtype=object)
    ...
    pred_text_grid = np.array(grits.cells_to_grid(pred_cells, key='cell_text'),
                              dtype=object)
    ...
    if mode == 'grits-all':
        # Compute grids/matrices for comparison
        true_cell_dilatedbbox_grid = np.array(
            grits.output_to_dilatedbbox_grid(true_bboxes, true_labels,
                                             true_scores))
        pred_cell_dilatedbbox_grid = np.array(
            grits.output_to_dilatedbbox_grid(pred_bboxes,
        ...
        (metrics['grits_rawloc'],
         ...
         metrics['grits_rawloc_upper_bound']) = grits_loc(
             true_cell_dilatedbbox_grid,
             pred_cell_dilatedbbox_grid)

        # Compute original DAR (directed adjacency relations) metric
        (metrics['dar_recall_con_original'],
         metrics['dar_precision_con_original'],
         metrics['dar_con_original']) = dar_con_original(true_cells,
                                                         pred_cells)
...
def compute_statistics(structures, cells):
    ...
    statistics['num_spanning_cells'] = len([cell for cell in cells
    ...
    row_heights = [float(row['bbox'][3]-row['bbox'][1])
                   for row in structures['rows']]
    if len(row_heights) >= 2:
        statistics['row_height_coefficient_of_variation'] = stat.stdev(
            row_heights) / stat.mean(row_heights)
    else:
        statistics['row_height_coefficient_of_variation'] = 0
    column_widths = [float(column['bbox'][2]-column['bbox'][0])
                     for column in structures['columns']]
    if len(column_widths) >= 2:
        statistics['column_width_coefficient_of_variation'] = stat.stdev(
            column_widths) / stat.mean(column_widths)
    else:
...
def compute_metrics_summary(sample_metrics, mode):
    ...
    simple_samples = [entry for entry in sample_metrics
                      if entry['num_spanning_cells'] == 0]
    metrics_summary['simple'] = {'num_tables': len(simple_samples)}
    if len(simple_samples) > 0:
        for metric_name in metric_names:
            metrics_summary['simple'][metric_name] = np.mean(
                [elem[metric_name] for elem in simple_samples])

    complex_samples = [entry for entry in sample_metrics
                       if entry['num_spanning_cells'] > 0]
    metrics_summary['complex'] = {'num_tables': len(complex_samples)}
    if len(complex_samples) > 0:
        for metric_name in metric_names:
            metrics_summary['complex'][metric_name] = np.mean(
                [elem[metric_name] for elem in complex_samples])

    metrics_summary['all'] = {'num_tables': len(sample_metrics)}
    if len(sample_metrics) > 0:
        for metric_name in metric_names:
            metrics_summary['all'][metric_name] = np.mean(
                [elem[metric_name] for elem in sample_metrics])
...
def print_metrics_summary(metrics_summary, all=False):
    ...
    for table_type in ['simple', 'complex', 'all']:
        ...
        print("Results on {} tables ({} total):".format(table_type,
                                                        metrics['num_tables']))
        ...
        if all:
            ...
            print_metrics_line("DAR_Con (original)", metrics,
                               'dar_con_original')
...
def eval_tsr_sample(target, pred_logits, pred_bboxes, mode):
    ...
    true_bboxes = [elem.tolist() for elem in rescale_bboxes(true_bboxes,
                                                            true_img_size)]
    ...
    true_table_structures, true_cells, _ = objects_to_cells(
        true_bboxes,
        true_labels, true_scores,
    ...
    metrics = compute_metrics(mode, true_bboxes, true_labels, true_scores,
                              true_cells,
...
def visualize(args, target, pred_logits, pred_bboxes):
    ...
    bboxes_out_filepath = os.path.join(args.debug_save_dir,
                                       bboxes_out_filename)
    ...
    for bbox, label, score in zip(pred_bboxes, pred_labels, pred_scores):
        if ((args.data_type == 'structure' and not label > 5)
            or (args.data_type == 'detection' and not label > 1)
                and score > 0.5):
            color, alpha, linewidth, hatch = get_bbox_decorations(
                args.data_type,
                label)
            # Fill
            rect = patches.Rectangle(bbox[:2], bbox[2]-bbox[0],
                                     bbox[3]-bbox[1],
    ...
    if args.data_type == 'structure':
        img_words_filepath = os.path.join(args.table_words_dir,
                                          img_filename.replace(
                                              ".jpg", "_words.json"))
        cells_out_filename = img_filename.replace(".jpg", "_cells.jpg")
        cells_out_filepath = os.path.join(args.debug_save_dir,
                                          cells_out_filename)

        with open(img_words_filepath, 'r') as f:
            tokens = json.load(f)

        _, pred_cells, _ = objects_to_cells(pred_bboxes, pred_labels,
                                            pred_scores,
        ...
        for cell in pred_cells:
            ...
            rect = patches.Rectangle(bbox[:2], bbox[2]-bbox[0],
                                     ...
                                     edgecolor='none', facecolor="magenta",
                                     alpha=alpha)
            ax.add_patch(rect)
            rect = patches.Rectangle(bbox[:2], bbox[2]-bbox[0],
                                     bbox[3]-bbox[1],
...
@torch.no_grad()
def evaluate(args, model, criterion, postprocessors, data_loader, base_ds,
             device):
    ...
    for samples, targets in metric_logger.log_every(data_loader,
                                                    print_every,
                                                    header):
        batch_num += 1
        ...
        if args.debug:
            for target, pred_logits, pred_boxes in zip(targets,
                                                       outputs['pred_logits'],
                                                       outputs['pred_boxes']):
        ...
        if args.data_type == "structure":
            pred_logits_collection += list(
                outputs['pred_logits'].detach().cpu())
            pred_bboxes_collection += list(
            ...
            if batch_num % args.eval_step == 0 or batch_num == num_batches:
                arguments = zip(targets_collection, pred_logits_collection,
                                pred_bboxes_collection,
                                repeat(args.mode))
                with multiprocessing.Pool(args.eval_pool_size) as pool:
                    metrics = pool.starmap_async(
                        eval_tsr_sample, arguments).get()
                tsr_metrics += metrics
                pred_logits_collection = []
                pred_bboxes_collection = []
                targets_collection = []
    ...
    print("Total time taken for {} samples: {}".format(len(base_ds),
                                                       datetime.now()
                                                       - st_time))
...
def eval_coco(args, model, criterion, postprocessors, data_loader_test,
              dataset_test, device):
    """
    Use this function to do COCO evaluation. Default implementation runs it on
    the test set.
    """
    pubmed_stats, coco_evaluator = evaluate(args, model, criterion,
                                            postprocessors,
                                            data_loader_test, dataset_test,
                                            device)
    print("COCO metrics summary: AP50: {:.3f}, AP75: {:.3f}, \
          AP: {:.3f}, AR: {:.3f}".format(
