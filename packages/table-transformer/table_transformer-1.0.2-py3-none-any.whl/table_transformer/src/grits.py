...
def align_1d(sequence1, sequence2, reward_lookup, return_alignment=False):
    ...
    for seq1_idx in range(1, sequence1_length+1):
        for seq2_idx in range(1, sequence2_length+1):
            reward = reward_lookup[sequence1[seq1_idx-1]
                                   + sequence2[seq2_idx-1]]
...
def align_2d_outer(true_shape, pred_shape, reward_lookup):
    ...
    for row_idx in range(1, true_shape[0] + 1):
        for col_idx in range(1, pred_shape[0] + 1):
            reward = align_1d([(row_idx-1, tcol)
                               for tcol in range(true_shape[1])],
                              [(col_idx-1, prow)
                               for prow in range(pred_shape[1])],
                              reward_lookup)
...
def factored_2dmss(true_cell_grid, pred_cell_grid, reward_function):
    ...
    This is a polynomial-time heuristic to computing the
    2D-MSS of two matrices,
    which is NP hard.

    A substructure of a matrix is a subset of its rows and its columns.

    The most similar substructures of two matrices, A and B,
    ...
    for trow, tcol, prow, pcol in itertools.product(
        ...
        reward = reward_function(true_cell_grid[trow, tcol],
                                 pred_cell_grid[prow, pcol])
    ...
    upper_bound_score, _, _ = compute_fscore(pos_match_score_upper_bound,
                                             num_pos, num_true)

    positive_match_score = 0
    for true_row_num, pred_row_num in zip(true_row_nums, pred_row_nums):
        for true_column_num, pred_column_num in zip(true_column_nums,
                                                    pred_column_nums):
            positive_match_score += pre_computed_rewards[(true_row_num,
                                                          true_column_num,
                                                          pred_row_num,
                                                          pred_column_num)]
...
def lcs_similarity(string1, string2):
    ...
    lcs = ''.join([string1[block.a:(block.a + block.size)]
...
def get_spanning_cell_rows_and_columns(spanning_cells, rows, columns):
    ...
    for spanning_cell in spanning_cells:
        ...
        for row_num, row in enumerate(rows):
            bbox1 = [
                spanning_cell['bbox'][0], row['bbox'][1],
...
def output_to_dilatedbbox_grid(bboxes, labels, scores):
    """
    Compute the matrix of grid cell features for GriTS_Loc but
    using the raw predicted
    ...
    *dilated*, which means they are larger than the actual ground truth boxes.
    ...
    """
    rows = [{'bbox': bbox}
            for bbox, label in zip(bboxes, labels) if label == 2]
    columns = [{'bbox': bbox}
               for bbox, label in zip(bboxes, labels) if label == 1]
    spanning_cells = [{'bbox': bbox, 'score': 1}
    ...
    for matches, spanning_cell in zip(matches_by_spanning_cell,
                                      spanning_cells):
    ...
    while len(stack) > 0:
        ...
        if current.tag == 'td' or current.tag == 'th':
            ...
            try:
                ...
                current_column = min(
                    set(range(max_occupied_column+2)
                        ).difference(occupied_columns_by_row[current_row]))
            except:
                current_column = 0
        ...
        for child in children[::-1]:
            stack.append((child,
                          in_header or current.tag == 'th'
                          or current.tag == 'thead'))
...
def grits_from_html(true_html, pred_html):
    ...
    true_text_grid = np.array(cells_to_grid(true_cells, key='cell_text'),
                              dtype=object)
    pred_text_grid = np.array(cells_to_grid(pred_cells, key='cell_text'),
                              dtype=object)
