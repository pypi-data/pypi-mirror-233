...
class Tester(unittest.TestCase):
    ...
    def test_hungarian(self):
        ...
        indices_single = matcher({'pred_logits': logits,
                                  'pred_boxes': boxes}, targets)
    ...
    def test_model_script_detection(self):
        ...
        x = nested_tensor_from_tensor_list([torch.rand(3, 200, 200),
...
class ONNXExporterTester(unittest.TestCase):
        ...
        for test_inputs in inputs_list:
            with torch.no_grad():
                if (isinstance(test_inputs, torch.Tensor)
                        or isinstance(test_inputs, list)):
    ...
    def ort_validate(self, onnx_io,
        ...
        for i in range(0, len(outputs)):
            try:
                torch.testing.assert_allclose(outputs[i],
                                              ort_outs[i],
