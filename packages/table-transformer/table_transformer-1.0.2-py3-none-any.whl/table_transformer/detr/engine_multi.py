...
def train_one_epoch(model_list, criterion_list,
    ...
    while not max_batches_per_epoch is None and batch_count <= max_batches_per_epoch:
        for model, criterion, data_loader in zip(model_list,
                                                 criterion_list, data_loader_list):
            alternating_count = 0
            for samples, targets in metric_logger.log_every(data_loader,
                                                            print_freq,
                                                            header):
                alternating_count += 1
                if (alternating_count > print_freq
                        or (not max_batches_per_epoch is None
...
def evaluate(model, criterion, postprocessors,
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1,
                                                               fmt='{value:.2f}'))
    header = 'Test:'
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if panoptic_evaluator is not None:
            res_pano = postprocessors["panoptic"](outputs,
                                                  target_sizes,
                                                  orig_target_sizes)
