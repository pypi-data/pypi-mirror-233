...
from arthurai.common.constants import API_PREFIX, API_PREFIX_V4, AccuracyMetric, Enrichment, IMAGE_FILE_EXTENSION_MAP, \
    ...
    Stage, TextDelimiter, ValueType, ENRICHMENTS_SPINNER_MESSAGE, ENRICHMENTS_UPDATE_MESSAGE, EnrichmentStatus, \
    ENRICHMENT_READY_OR_FAILED_STATES
from arthurai.common.exceptions import ArthurUnexpectedError, ArthurUserError, ExpectedParameterNotFoundError, \
    MethodNotApplicableError, MissingParameterError, UnexpectedTypeError, UserTypeError, UserValueError, arthur_excepted
...
from arthurai.core.dataset_validation_utils import validate_attr_names, validate_series_data_type, valid_rec_obj
...
from arthurai.core.util import update_column_in_list_of_dicts, is_list_like, is_str_like
...
class ArthurModel(ArthurBaseJsonDataclass):
    ...
    def save(self, skip_validation: bool = False) -> str:
        ...
        # remove ranked list attributes from reference data until we have parquet ranked list support implemented:
        # https://arthurai.atlassian.net/browse/OBS-481
        if self.output_type == OutputType.RankedList and self.reference_dataframe is not None and self.attributes is not None:
            ranked_list_attrs = [attr.name for attr in self.attributes if attr.value_type == ValueType.RankedList]
            self.reference_dataframe = self.reference_dataframe.drop(columns=ranked_list_attrs, errors='ignore')
            for attr in ranked_list_attrs:  # fill with nones so data passes validation
                self.reference_dataframe[attr] = None
    ...
    def await_model_ready(self):
        ...
        self.status = ModelStatusWaiter(self, self._client).\
            wait_for_valid_status([ModelStatus.Ready, ModelStatus.CreationFailed],
    ...
    def build(self,
        ...
        self.attributes = []
        ground_truth_columns = [ground_truth_column] if ground_truth_column else list(pred_to_ground_truth_map.values())
        ...
        if self.output_type == OutputType.Multiclass:
            ...
            if len(pred_columns) != 1 or len(ground_truth_columns) != 1:
                raise UserValueError(f"Ranked List models require 1 predicted attribute and 1 ground truth attribute. "
                ...
                self.add_ranked_list_output_attributes(predicted_attr_name=pred_columns[0],
    ...
    def _validate_ranked_list_model(self, data: DataFrame) -> None:
        ...
        if len(pred_attrs) != 1:
            raise UserValueError(f"Ranked List models must have exactly one predicted attribute. Your model has "
                                 f"{len(pred_attrs)} predicted attributes.")
        else:
            pred_attr = pred_attrs[0]
        if len(gt_attrs) != 1:
            raise UserValueError(f"Ranked List models must have exactly one ground truth attribute. Your model has "
        ...
        if pred_attr.value_type != ValueType.RankedList:
            raise UserValueError(f"The Ranked List model predicted attribute must be of value type "
            ...
            raise UserValueError(f"The Ranked List model ground truth attribute must be of value type "
            ...
            raise UserValueError(f"The Ranked List model predicted attribute {pred_attr.name} cannot be categorical.")
        elif gt_attr.categorical:
            raise UserValueError(f"The Ranked List model ground truth attribute {gt_attr.name} cannot be categorical.")

        # validate recommendation data size limits
        if data is not None:
            if pred_attr.name not in data.columns:
                raise UserValueError(f"The provided dataset does not have a column for the predicted attribute: "
                                     f"{pred_attr.name}")
            elif gt_attr.name not in data.columns:
                raise UserValueError(f"The provided dataset does not have a column for the ground truth attribute: "
                                     f"{gt_attr.name}")

            # enforce 100 recommendations per inference limit
            pred_gt_df = data[[gt_attr.name, pred_attr.name]]
            rec_count_per_inf_df = pred_gt_df.applymap(lambda x: len(x) > 100 if is_list_like(x) else None)
            if rec_count_per_inf_df.any(axis=None):
                raise UserValueError(f"Each inference in a Ranked List model can have a maximum of 100 recommendations "
            ...
            unique_gt_values = pred_gt_df[gt_attr.name].explode().apply(lambda x: x if is_str_like(x) else None)
            unique_pred_values = pred_gt_df[pred_attr.name].explode().apply(
                lambda x: dict(x).get("item_id") if valid_rec_obj(x) else None)
            unique_recs = set(unique_gt_values.dropna()) | set(unique_pred_values.dropna())
            if len(unique_recs) > 1000:
                raise UserValueError(f"A Ranked List model can have a maximum of 1000 total unique recommended items "
    ...
    @arthur_excepted("failed to add output attributes")
    def add_ranked_list_output_attributes(self, predicted_attr_name: str,
                                          gt_attr_name: str) -> Dict[str, ArthurAttribute]:
        ...
        if predicted_attr_name == gt_attr_name:
            ...
            raise MethodNotApplicableError("This function can only be called for models with Ranked List output.")
    ...
    def set_reference_data(self, directory_path: Optional[str] = None, data: Optional[Union[DataFrame, Series]] = None,
        ...
        if self.output_type == OutputType.RankedList and self.attributes is not None:
            ranked_list_attrs = [attr.name for attr in self.attributes if attr.value_type == ValueType.RankedList]
            if data is not None:
                data = data.drop(columns=ranked_list_attrs, errors='ignore')
                for attr in ranked_list_attrs:  # fill with nones so data passes validation
                    data[attr] = None
            else:
                parquet_files = core_util.retrieve_parquet_files(directory_path) if directory_path is not None else files
                ...
                for file in parquet_files:
                    if isinstance(file, str) or isinstance(file, PathLike):
                        with open(file, 'rb') as open_file:
                            df = pandas.read_parquet(open_file)
                    else:
                        df = pandas.read_parquet(file)
                    df.drop(columns=ranked_list_attrs, inplace=True, errors='ignore')
                    for attr in ranked_list_attrs:  # fill with nones so data passes validation
                        df[attr] = None
                    fname = self.id if self.id is not None else "ref_data"
        ...
        if not self.model_is_saved():
            if data is not None:
                ...
                if temp_directory and directory_path is not None:  # remove during OBS-481
