...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[PathDetermination]:
    if property_count != 2:
        return None

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x559542ab
    path_link_type = enums.PathLinkType.from_stream(data)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x4150490c
    path_determination_method_type = PathDeterminationMethodType.from_stream(data, property_size)

    return PathDetermination(path_link_type, path_determination_method_type)
...
_decode_path_determination_method_type = PathDeterminationMethodType.from_stream
