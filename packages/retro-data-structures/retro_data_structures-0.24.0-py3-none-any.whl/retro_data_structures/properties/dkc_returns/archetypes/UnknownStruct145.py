...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[UnknownStruct145]:
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x6d4029d0
    move_left = struct.unpack(">Q", data.read(8))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x601838ab
    move_right = struct.unpack(">Q", data.read(8))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xecc78b13
    run_grab = struct.unpack(">Q", data.read(8))[0]
    ...
    return UnknownStruct145(unknown_struct29, title, how_to, back, back_core, move_left, move_right, run_grab, jump, pause, crouch, ground_pound, blow, mount, dismount, roll, blow_input, strg, mount_input, dismount_input, roll_input, text_background)
