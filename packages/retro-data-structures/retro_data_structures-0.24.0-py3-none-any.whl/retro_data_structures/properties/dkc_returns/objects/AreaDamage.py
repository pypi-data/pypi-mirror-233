...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[AreaDamage]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x8e07e9d3
    pulse_time = struct.unpack('>f', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xc052bc02
    grace_time = struct.unpack('>f', data.read(4))[0]

    return AreaDamage(editor_properties, damage, pulse_time, grace_time)
