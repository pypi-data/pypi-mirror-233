...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[VolumeGroup]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x6fe8333d
    group_name = data.read(property_size)[:-1].decode("utf-8")
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x70fdfcc4
    inverted = struct.unpack('>?', data.read(1))[0]

    return VolumeGroup(editor_properties, group_name, volume, inverted)
