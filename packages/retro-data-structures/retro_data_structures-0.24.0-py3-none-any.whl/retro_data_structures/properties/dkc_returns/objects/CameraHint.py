...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[CameraHint]:
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xe8eaaf0e
    tracking_behavior = CameraTracking.from_stream(data, property_size)
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x4f2ca324
    camera_position = CameraPosition.from_stream(data, property_size)
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xe389796e
    framing_behavior = CameraFraming.from_stream(data, property_size)
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xd3696c11
    clip_behavior = CameraClip.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x95d0d437
    control_frame_interpolation = InterpolationMethod.from_stream(data, property_size)

    return CameraHint(editor_properties, priority, timer, flags_camera_hint, constraints, tracking_behavior, position_behavior, camera_position, navigation_behavior, motion_behavior, orientation_behavior, rotation_behavior, field_of_view_behavior, framing_behavior, interpolation_behavior, clip_behavior, control_frame_interpolation)
...
_decode_tracking_behavior = CameraTracking.from_stream

_decode_position_behavior = CameraPosition.from_stream

_decode_camera_position = CameraPosition.from_stream
...
_decode_framing_behavior = CameraFraming.from_stream

_decode_interpolation_behavior = CameraInterpolation.from_stream

_decode_clip_behavior = CameraClip.from_stream
