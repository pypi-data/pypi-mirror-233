...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[Checkpoint]:
    if property_count != 3:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb9a97fee
    checkpoint_data = CheckpointData.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x416bd9eb
    layer_list = UnknownStruct24.from_stream(data, property_size)

    return Checkpoint(editor_properties, checkpoint_data, layer_list)


_decode_editor_properties = EditorProperties.from_stream

_decode_checkpoint_data = CheckpointData.from_stream
