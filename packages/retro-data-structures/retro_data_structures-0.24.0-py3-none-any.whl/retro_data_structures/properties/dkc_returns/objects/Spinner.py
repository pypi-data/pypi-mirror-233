...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[Spinner]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x11e0ab90
    unknown = struct.unpack('>?', data.read(1))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xba84f23e
    unknown_struct267 = UnknownStruct267.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xc243df95
    unknown_struct268 = UnknownStruct268.from_stream(data, property_size)

    return Spinner(editor_properties, unknown, unknown_struct267, unknown_struct268)
...
_decode_unknown_struct267 = UnknownStruct267.from_stream

_decode_unknown_struct268 = UnknownStruct268.from_stream
