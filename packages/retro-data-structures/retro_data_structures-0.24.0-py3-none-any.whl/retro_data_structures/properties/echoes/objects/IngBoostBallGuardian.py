...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[IngBoostBallGuardian]:
    if property_count != 5:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x22e574aa
    unknown_struct28 = UnknownStruct28.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x78656d1e
    unknown_struct29 = UnknownStruct29.from_stream(data, property_size)

    return IngBoostBallGuardian(editor_properties, patterned, actor_information, unknown_struct28, unknown_struct29)
