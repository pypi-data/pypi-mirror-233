...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[SpecialFunction]:
    ...
    sound1 = struct.unpack('>l', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x227a6411
    sound2 = struct.unpack('>l', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xe926b7b4
    sound3 = struct.unpack('>l', data.read(4))[0]

    return SpecialFunction(editor_properties, function, string_parm, value_parm, value_parm2, value_parm3, value_parm4, int_parm1, int_parm2, inventory_item_parm, sound1, sound2, sound3)
