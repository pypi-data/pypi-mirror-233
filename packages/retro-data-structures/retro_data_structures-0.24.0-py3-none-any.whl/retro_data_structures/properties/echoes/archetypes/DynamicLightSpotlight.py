...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[DynamicLightSpotlight]:
    if property_count != 3:
        return None

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xfeeca0d6
    spotlight_angle = Spline.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x79aa798b
    spotlight_angle_duration = struct.unpack('>f', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x2fd08300
    spotlight_angle_loops = struct.unpack('>?', data.read(1))[0]

    return DynamicLightSpotlight(spotlight_angle, spotlight_angle_duration, spotlight_angle_loops)


_decode_spotlight_angle = Spline.from_stream
