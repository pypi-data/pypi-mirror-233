...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[PathMeshCtrl]:
    if property_count != 3:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xf53dcdd6
    type = struct.unpack('>l', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7a6e0de9
    initial_count = struct.unpack('>l', data.read(4))[0]

    return PathMeshCtrl(editor_properties, type, initial_count)
