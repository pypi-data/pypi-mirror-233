...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[ActorParameters]:
    ...
    actor_material_type = enums.ActorMaterialType.from_stream(data)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x583895fa
    actor_collision_response = enums.ActorCollisionResponse.from_stream(data)
    ...
    return ActorParameters(lighting, scannable, x_ray_model, x_ray_skin, use_global_render_time, fade_in_time, fade_out_time, visor, force_render_unsorted, takes_projected_shadow, unknown_0xf07981e8, unknown_0x4d55f7d4, actor_material_type, actor_collision_response, max_volume, is_hostile)
