...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[FederationData]:
    if property_count != 3:
        return None

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x5c747710
    can_blink = struct.unpack('>?', data.read(1))[0]
    ...
    return FederationData(can_blink, unknown_struct29, unknown)
