...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[PhazonBall]:
    if property_count != 2:
        return None

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x47ffd5d6
    phazon_ball_damage_delay = struct.unpack('>f', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7eff0eb1
    phazon_ball_damage = TDamageInfo.from_stream(data, property_size, default_override={'weapon_type': 5, 'damage_amount': 50.0, 'radius_damage_amount': 50.0, 'damage_radius': 5.0})

    return PhazonBall(phazon_ball_damage_delay, phazon_ball_damage)
