...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[ScrewAttack]:
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x315df4d7
    unknown_0x315df4d7 = struct.unpack('>f', data.read(4))[0]
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xc5b758f5
    unknown_0xc5b758f5 = struct.unpack('>f', data.read(4))[0]
