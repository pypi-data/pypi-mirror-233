...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[PirateDrone]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x16e8d4e8
    pirate_drone_data = PirateDroneData.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb3774750
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'creature_size': 1, 'detection_range': 50.0, 'freeze_duration': 10.0})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7e397fed
    actor_information = ActorParameters.from_stream(data, property_size)

    return PirateDrone(editor_properties, pirate_drone_data, patterned, actor_information)


_decode_editor_properties = EditorProperties.from_stream

_decode_pirate_drone_data = PirateDroneData.from_stream
