...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[LightVolume]:
    if property_count != 3:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x3edbe372
    unknown = Color.from_stream(data)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7e4359d1
    num_subdivisions = struct.unpack('>l', data.read(4))[0]

    return LightVolume(editor_properties, unknown, num_subdivisions)
