...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[KorbaMaw]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x33d8ede3
    korba_maw_properties = KorbaMawData.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb3774750
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'collision_radius': 0.5, 'collision_height': 0.5, 'step_up_height': 1.0, 'creature_size': 1, 'leash_radius': 100.0})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7e397fed
    actor_information = ActorParameters.from_stream(data, property_size)

    return KorbaMaw(editor_properties, korba_maw_properties, patterned, actor_information)


_decode_editor_properties = EditorProperties.from_stream

_decode_korba_maw_properties = KorbaMawData.from_stream
