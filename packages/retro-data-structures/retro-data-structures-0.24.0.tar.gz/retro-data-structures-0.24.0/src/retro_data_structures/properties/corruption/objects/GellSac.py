...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[GellSac]:
    if property_count != 5:
        return None
    ...
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'detection_range': 10.0})
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x2766cd96
    unknown_struct43 = UnknownStruct43.from_stream(data, property_size)

    return GellSac(editor_properties, patterned, actor_information, wall_crawler_data, unknown_struct43)
