...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[SnagVineHelper]:
    if property_count != 3:
        return None
    ...
    hud_static_intensity = struct.unpack('>f', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x9f3e7f56
    hud_static_duration = struct.unpack('>f', data.read(4))[0]

    return SnagVineHelper(editor_properties, hud_static_intensity, hud_static_duration)
