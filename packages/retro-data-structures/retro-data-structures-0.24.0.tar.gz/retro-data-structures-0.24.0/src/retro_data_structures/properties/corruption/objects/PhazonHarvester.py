...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[PhazonHarvester]:
    if property_count != 4:
        return None
    ...
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'detection_range': 15.0})
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xd2dfd524
    phazon_harvester_data = PhazonHarvesterData.from_stream(data, property_size)

    return PhazonHarvester(editor_properties, patterned, actor_information, phazon_harvester_data)
...
_decode_phazon_harvester_data = PhazonHarvesterData.from_stream
