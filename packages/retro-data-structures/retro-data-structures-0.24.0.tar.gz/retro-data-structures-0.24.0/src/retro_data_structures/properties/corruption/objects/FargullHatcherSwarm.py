...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[FargullHatcherSwarm]:
    if property_count != 7:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x146f75d9
    fargull_hatcher_swarm_data = FargullHatcherSwarmData.from_stream(data, property_size)

    return FargullHatcherSwarm(editor_properties, actor_information, character_animation_information, active, swarm_basics_data, flyer_swarm_data, fargull_hatcher_swarm_data)
...
_decode_flyer_swarm_data = FlyerSwarmData.from_stream

_decode_fargull_hatcher_swarm_data = FargullHatcherSwarmData.from_stream
