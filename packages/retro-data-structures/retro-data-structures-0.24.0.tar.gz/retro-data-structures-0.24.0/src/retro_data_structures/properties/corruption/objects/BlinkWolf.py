...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[BlinkWolf]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x90688474
    blink_wolf_data = BlinkWolfData.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb3774750
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'creature_size': 1, 'detection_range': 50.0, 'min_attack_range': 10.0, 'max_attack_range': 20.0})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7e397fed
    actor_information = ActorParameters.from_stream(data, property_size)

    return BlinkWolf(editor_properties, blink_wolf_data, patterned, actor_information)


_decode_editor_properties = EditorProperties.from_stream

_decode_blink_wolf_data = BlinkWolfData.from_stream
