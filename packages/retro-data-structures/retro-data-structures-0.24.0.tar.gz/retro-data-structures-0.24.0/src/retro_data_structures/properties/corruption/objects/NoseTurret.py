...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[NoseTurret]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x3e5dd208
    ptc_nose_turret_data = PTCNoseTurretData.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb3774750
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'step_up_height': 1.0, 'detection_range': 50.0, 'min_attack_range': 5.0, 'max_attack_range': 25.0, 'leash_radius': 100.0})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7e397fed
    actor_information = ActorParameters.from_stream(data, property_size)

    return NoseTurret(editor_properties, ptc_nose_turret_data, patterned, actor_information)


_decode_editor_properties = EditorProperties.from_stream

_decode_ptc_nose_turret_data = PTCNoseTurretData.from_stream
