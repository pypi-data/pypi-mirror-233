...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[EyePod]:
    if property_count != 5:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x9214a518
    eye_pod_properties = EyePodData.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb3774750
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'step_up_height': 1.0, 'leash_radius': 100.0})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7e397fed
    actor_information = ActorParameters.from_stream(data, property_size)

    return EyePod(editor_properties, wall_crawler_properties, eye_pod_properties, patterned, actor_information)
...
_decode_eye_pod_properties = EyePodData.from_stream
