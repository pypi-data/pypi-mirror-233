...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[ScriptLayerController]:
    if property_count != 3:
        return None
    ...
    layer = LayerID.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x12a7d8b2
    is_dynamic = struct.unpack('>?', data.read(1))[0]

    return ScriptLayerController(editor_properties, layer, is_dynamic)


_decode_editor_properties = EditorProperties.from_stream

_decode_layer = LayerID.from_stream
