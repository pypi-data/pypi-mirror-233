...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[MetroidPhazeoid]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xe58a4113
    metroid_phazeoid_data = MetroidPhazeoidData.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb3774750
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'detection_range': 50.0})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x7e397fed
    actor_information = ActorParameters.from_stream(data, property_size)

    return MetroidPhazeoid(editor_properties, metroid_phazeoid_data, patterned, actor_information)


_decode_editor_properties = EditorProperties.from_stream

_decode_metroid_phazeoid_data = MetroidPhazeoidData.from_stream
