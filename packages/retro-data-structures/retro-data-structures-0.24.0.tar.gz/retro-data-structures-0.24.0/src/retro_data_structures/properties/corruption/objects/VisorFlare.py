...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[VisorFlare]:
    ...
    flare1 = FlareDef.from_stream(data, property_size)

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x05a84ee6
    flare2 = FlareDef.from_stream(data, property_size, default_override={'position': 0.25})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x12d35aa5
    flare3 = FlareDef.from_stream(data, property_size, default_override={'position': 0.5})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x76b2376c
    flare4 = FlareDef.from_stream(data, property_size, default_override={'position': 0.75})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x61c9232f
    flare5 = FlareDef.from_stream(data, property_size, default_override={'position': 1.0})
...
_decode_flare1 = FlareDef.from_stream
