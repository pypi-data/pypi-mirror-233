...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[DigitalGuardian]:
    if property_count != 4:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xc55918cc
    digital_guardian_data = DigitalGuardianData.from_stream(data, property_size)

    return DigitalGuardian(editor_properties, patterned, actor_information, digital_guardian_data)
...
_decode_digital_guardian_data = DigitalGuardianData.from_stream
