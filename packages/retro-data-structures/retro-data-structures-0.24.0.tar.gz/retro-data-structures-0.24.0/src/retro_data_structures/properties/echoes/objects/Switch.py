...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[Switch]:
    if property_count != 3:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x054a1e34
    is_auto_close = struct.unpack('>?', data.read(1))[0]

    return Switch(editor_properties, is_open, is_auto_close)
