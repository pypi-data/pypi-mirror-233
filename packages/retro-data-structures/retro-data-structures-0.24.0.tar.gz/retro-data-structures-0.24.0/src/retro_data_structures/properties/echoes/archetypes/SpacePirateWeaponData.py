...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[SpacePirateWeaponData]:
    if property_count != 16:
        return None

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x647670ac
    unknown_0x647670ac = struct.unpack('>l', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xa79bb82e
    grenade_launcher = struct.unpack(">L", data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xa95a025b
    unknown_0xa95a025b = struct.unpack('>l', data.read(4))[0]
    ...
    grenade_damage = DamageInfo.from_stream(data, property_size, default_override={'di_weapon_type': 11, 'di_damage': 40.0, 'di_radius': 8.0, 'di_knock_back_power': 10.0})
    ...
    return SpacePirateWeaponData(unknown_0x647670ac, grenade_launcher, unknown_0xa95a025b, unknown_0x25f822c4, unknown_0x765e3a20, grenade_damage, grenade_explosion, grenade_effect, grenade_trail, grenade_mass, unknown_0xed086ce0, unknown_0x00fc6646, unknown_0xa7c8e63f, unknown_0x454f16b1, sound_grenade_bounce, sound_grenade_explode)
