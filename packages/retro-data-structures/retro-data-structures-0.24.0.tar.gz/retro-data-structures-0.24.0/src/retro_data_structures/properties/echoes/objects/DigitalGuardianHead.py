...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[DigitalGuardianHead]:
    if property_count != 4:
        return None
    ...
    patterned = PatternedAITypedef.from_stream(data, property_size, default_override={'turn_speed': 360.0, 'min_attack_range': 0.0, 'creature_size': 2})
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x0f4d73b7
    digital_guardian_head_data = DigitalGuardianHeadData.from_stream(data, property_size)

    return DigitalGuardianHead(editor_properties, patterned, actor_information, digital_guardian_head_data)
...
_decode_digital_guardian_head_data = DigitalGuardianHeadData.from_stream
