...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[PlayerStateChange]:
    if property_count != 6:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x4a3491bd
    item_to_change = struct.unpack('>l', data.read(4))[0]
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x94ba5737
    command = struct.unpack('>l', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x363d98a4
    command_action = struct.unpack('>l', data.read(4))[0]

    return PlayerStateChange(editor_properties, item_to_change, capacity_increase, amount, command, command_action)
