...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[ControllerAction]:
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x4c6eefae
    controller_action_struct = ControllerActionStruct.from_stream(data, property_size, default_override={'command': 1})

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xead7b7bb
    one_shot = struct.unpack('>?', data.read(1))[0]

    return ControllerAction(editor_properties, controller_action_struct, one_shot)
