...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[Platform]:
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x981c2921
    unknown_0x981c2921 = struct.unpack('>f', data.read(4))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb5b16553
    unknown_0xb5b16553 = struct.unpack('>?', data.read(1))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x58ee3422
    unknown_0x58ee3422 = struct.unpack('>?', data.read(1))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xbe04e10b
    unknown_0xbe04e10b = struct.unpack('>?', data.read(1))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x1fa92ac4
    can_take_damage = struct.unpack('>?', data.read(1))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x47e9cc80
    unknown_0x47e9cc80 = struct.unpack('>?', data.read(1))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xa1172744
    unknown_struct245 = UnknownStruct245.from_stream(data, property_size)

    return Platform(editor_properties, collision_box, collision_offset, model, animation, shadow_data, actor_information, collision_model, health, vulnerability, unknown_0xf203bc81, motion_properties, unknown_0x24fdeea1, random_animation_offset, animation_time_scale, unknown_0x6b5e87a7, look_at_velocity, render_push, unknown_0x981c2921, unknown_0xb5b16553, unknown_0x58ee3422, unknown_0xbe04e10b, can_take_damage, unknown_0x47e9cc80, unknown_struct245)
...
_decode_unknown_struct245 = UnknownStruct245.from_stream
