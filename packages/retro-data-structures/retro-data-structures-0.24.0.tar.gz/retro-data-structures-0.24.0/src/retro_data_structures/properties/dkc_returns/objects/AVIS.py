...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[AVIS]:
    if property_count != 3:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0xb2556c66
    visualization_file = struct.unpack(">Q", data.read(8))[0]

    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x42097686
    texture_file = struct.unpack(">Q", data.read(8))[0]

    return AVIS(editor_properties, visualization_file, texture_file)
