...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[CameraManager]:
    if property_count != 2:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x146e64de
    camera_manager_data = CameraManagerData.from_stream(data, property_size)

    return CameraManager(editor_properties, camera_manager_data)


_decode_editor_properties = EditorProperties.from_stream

_decode_camera_manager_data = CameraManagerData.from_stream
