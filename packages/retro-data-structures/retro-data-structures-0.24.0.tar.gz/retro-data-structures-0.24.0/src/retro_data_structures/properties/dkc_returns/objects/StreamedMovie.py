...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[StreamedMovie]:
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x781ba115
    is_level_reveal = struct.unpack('>?', data.read(1))[0]

    return StreamedMovie(editor_properties, movie_file, loop, video_filter_enabled, unknown, volume, volume_type, cache_length, fade_out_time, is_level_reveal)
