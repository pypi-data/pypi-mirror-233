...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[ActorMultiKeyFrame]:
    if property_count != 3:
        return None
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x48b5b2a2
    actor_multi_keyframe_data = ActorMultiKeyframeData.from_stream(data, property_size)

    return ActorMultiKeyFrame(editor_properties, mirrored, actor_multi_keyframe_data)
...
_decode_actor_multi_keyframe_data = ActorMultiKeyframeData.from_stream
