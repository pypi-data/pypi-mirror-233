...
def _fast_decode(data: typing.BinaryIO, property_count: int) -> typing.Optional[CameraFilterKeyframe]:
    ...
    property_id, property_size = struct.unpack(">LH", data.read(6))
    assert property_id == 0x89f7f8c5
    prevent_pause_when_incremented = struct.unpack('>?', data.read(1))[0]

    return CameraFilterKeyframe(editor_properties, filter_type, filter_shape, filter_stage, which_filter_group, color, interpolate_in_time, interpolate_out_time, texture, model, prevent_pause_when_incremented)
