# Copyright 2020 The CLODE/TensorStore Authors
#
...
# Derived from Tensorstore's setup.py
...
SYSTEM_PYTHON_LIBS_ENVVAR = 'CLODE_SYSTEM_PYTHON_LIBS'
    ...
    def run(self):
        if not self.dry_run:
            ...
            if not prebuilt_path:
                ...
                bazelisk = os.getenv('CLODE_BAZELISK', 'bazelisk.py')
                # Controlled via `setup.py build_ext --debug` flag.
                default_compilation_mode = 'dbg' if self.debug else 'opt'
                compilation_mode = os.getenv('CLODE_BAZEL_COMPILATION_MODE',
                                             default_compilation_mode)
                startup_options = shlex.split(
                    os.getenv('CLODE_BAZEL_STARTUP_OPTIONS', ''))
                build_options = shlex.split(
                    os.getenv('CLODE_BAZEL_BUILD_OPTIONS', ''))
