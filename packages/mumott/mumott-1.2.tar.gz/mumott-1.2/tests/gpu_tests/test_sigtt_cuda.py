...
@pytest.mark.parametrize('new_coeff, valid, myhash', [(new_coeffs[0], True, '410110'),
...
def test_get_residuals():
    ...
    # nb: hash should now change because projector keeps track of numba arguments.
    assert old_hash != hash(meth)
    val = np.array([-12.88, -14.4, -21.55])
    assert np.allclose(residuals[0, 0, 2], val)
    grad_slice = np.array([[-47.631947], [-47.898132], [-48.970726], [-50.19862]])
