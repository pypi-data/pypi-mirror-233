...
from mumott.methods.projectors.saxs_projector_bilinear import SAXSProjectorBilinear
...
@pytest.mark.parametrize('proj', [f for f in projs])
def test_adj_noncontig(proj, gm):
    gm.append(gm[0])
    proj = np.concatenate((proj, proj, proj), axis=0)
    pr = SAXSProjectorBilinear(gm)
    with pytest.raises(ValueError, match='contiguous and row-major'):
        pr.adjoint(proj[::2])
...
def test_adj_subset(proj, adj, gm):
    ...
    assert np.allclose(adj, a[:, 0].ravel())
    gm.append(gm[0])
    proj_conc = np.concatenate((proj, proj), axis=0)
    pr = SAXSProjectorBilinear(gm)
    a = 0.5 * pr.adjoint(proj_conc, indices=np.array((0, 1)))
    assert np.allclose(adj, a[:, 0].ravel())


def test_str(gm_no_change):
    pr = SAXSProjectorBilinear(gm_no_change)
    string = str(pr)
