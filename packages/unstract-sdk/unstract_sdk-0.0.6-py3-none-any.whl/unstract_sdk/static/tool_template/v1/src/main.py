...
def run(params: dict, settings: dict, project_guid: str, utils: UnstractToolUtils):
    MOUNTED_FSSPEC_DIR_INPUT = '/tmp/unstract/fs_input/'
    MOUNTED_FSSPEC_DIR_OUTPUT = '/tmp/unstract/fs_output/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params', type=str, help='Parameters to use in RUN command', required=False)
    parser.add_argument('--settings', type=str, help='Settings to be used', required=False)
    parser.add_argument('--project-guid', type=str, help='Project GUID', required=False)
    parser.add_argument('--log-level', type=str, help='Log level', required=False, default='ERROR')
    ...
    if command == 'SPEC' or command == 'PROPERTIES' or command == 'ICON':
        ...
        if args.params is None:
            unstract_tools_util.stream_log('--params are required for RUN command', 'ERROR')
            exit(1)
        try:
        ...
        if args.settings is None:
            unstract_tools_util.stream_log('--settings are required for RUN command', 'ERROR')
            exit(1)
        try:
            print(args.settings)
        ...
        if args.project_guid is None:
            unstract_tools_util.stream_log('--project-guid is required for RUN command', 'ERROR')
            exit(1)
