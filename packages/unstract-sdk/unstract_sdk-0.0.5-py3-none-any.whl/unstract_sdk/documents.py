...
class UnstractToolDocs:
    ...
    def index_file(
        ...
        try:
            ...
            self.utils.stream_log(
                f"Error while uploading file: {file_path} / {e}", level="ERROR"
            )
            result["error"] = str(e)
            return result
    ...
    def get(self, project_id: str, unique_file_id: str) -> Optional[dict]:
        ...
        if response.status_code == 200:
            ...
            self.utils.stream_log(
                f"Data not found for key: {unique_file_id}", level="WARN"
            )
            return None
        else:
            self.utils.stream_log(
                f"Error while retrieving data for key: {unique_file_id} / {response.reason}",
                level="ERROR",
            )
            return None
    ...
    def delete(self, project_id: str, unique_file_id: str):
        ...
        if response.status_code == 200:
            ...
            self.utils.stream_log(
                f"Error while deleting data for key: {unique_file_id} / {response.reason}",
                level="ERROR",
            )
            return False
