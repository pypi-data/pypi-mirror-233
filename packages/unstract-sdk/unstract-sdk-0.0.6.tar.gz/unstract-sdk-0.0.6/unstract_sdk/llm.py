...
class UnstractToolLLM:
    ...
    def __init__(self, utils: UnstractToolUtils, llm_id: str):
        """
    ...
    def get_llm(self) -> Optional[LLM]:
        ...
        if self.llm_id == "Azure OpenAI":
            ...
            self.utils.stream_log(f"LLM not found for id: {self.llm_id}", level="ERROR")
            return None
    ...
    def get_callback_manager(self) -> Optional[CallbackManager]:
        ...
        Returns:
            Optional[CallbackManager]: The Callback Manager object for the tool. (llama_index.callbacks.CallbackManager)
        """
        if self.llm_id == "Azure OpenAI":
            ...
            self.utils.stream_log(f"LLM/Callback Manager not found for id: {self.llm_id}", level="ERROR")
            return None
    ...
    def get_max_tokens(self, reserved_for_output=0):
        """Returns the maximum number of tokens that can be used for the LLM.

        Args:
            reserved_for_output (int): The number of tokens reserved for the output. The default is 0.
