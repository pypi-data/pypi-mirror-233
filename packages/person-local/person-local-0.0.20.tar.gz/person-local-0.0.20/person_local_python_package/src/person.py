...
class PersonsLocal(GenericCRUD):
    def __init__(self, schema_name: str = "person", connection: Connector = None, id_column_name: str = None) -> None:
        super().__init__(schema_name, connection, id_column_name)
    ...
    def delete_by_person_id(self, person_id: int ) -> None:
        ...
        GenericCRUD(self.schema_name, id_column_name="person_id").delete_by_id(table_name="person_table", id_column_value=person_id)
        logger.end(f"Person deleted person_id= {person_id}", object={'person_id': person_id})

    def insert(self, person: PersonDto) -> int:
    ...
    def _insert_person_ml(self, person_id: int, lang_code: LangCode, first_name: str,last_name: str) -> int:
    ...
    def update_birthday_date(person_id: int, birthday_date: datetime.date) -> None:
        ...
        GenericCRUD(schema_name="person",id_column_name="person_id").update(table_name="person_table", json_data=person_json, id_column_value=person_id)
        logger.end()
    ...
    def update_first_name_by_profile_id( profile_id: int, first_name: str) -> None:
        ...
        GenericCRUD(schema_name="person", id_column_name="person_id",).update(table_name="person_table", json_data=person_json, id_column_value=profile_id)
        logger.end()

    @staticmethod
    def update_person_ml_first_name_by_person_id( person_id: int, lang_code: LangCode, first_name: str) -> None:
        logger.start(f"Update first name in ml table by person id person_id={person_id}",object={"person_id":person_id, lang_code: LangCode, "first_name":first_name})
        person_json = {
            "person_id": person_id,
            "lang_code": lang_code,
            "first_name": first_name
            }
        GenericCRUD(schema_name="person", id_column_name="person_id").update(table_name="person_ml_table", json_data=person_json, id_column_value=person_id)
        logger.end() 
    ...
    @staticmethod
    def update_person_ml_last_name_by_person_id(person_id: int, lang_code: LangCode, last_name: str) -> None:
