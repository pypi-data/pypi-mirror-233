...
def send_data(url, payload, offline_storage_path):
    ...
    try:
        ...
        new_payload = {
            "command": payload["data"][0]["command"],
...
def remote_log(url, command):
    ...
    # Use multiprocessing to send data in a separate process
    process = multiprocessing.Process(target=send_data, args=(url, payload, offline_storage_path))
    process.start()
    # The function will return immediately, while send_data runs in its own process.
