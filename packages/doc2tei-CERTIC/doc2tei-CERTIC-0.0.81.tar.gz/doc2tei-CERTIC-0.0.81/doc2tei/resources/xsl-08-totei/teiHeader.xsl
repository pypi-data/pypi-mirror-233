...
<!--              <xsl:copy-of select="//text:h[@text:outline-level='0']/@rendition"/>-->
              <xsl:apply-templates select="//text:h[@text:outline-level='0']//text()[not(ancestor::text:note)]" mode="teiHeader"/>
          </title>
            ...
            <!-- bloc auteur -->
            <xsl:for-each-group select="//text:p[starts-with(@text:style-name,'TEI_author') or starts-with(@text:style-name,'TEI_editor')][not(ancestor::*:back)]" group-starting-with="text:p[starts-with(@text:style-name,'TEI_author:') or starts-with(@text:style-name,'TEI_editor:')]">
              ...
              <ab type="editorial_workflow">
                  <xsl:if test="//text:p[@text:style-name='TEI_date_reception']">
                    <date type="received">
                        ...
                        <xsl:value-of select="//text:p[@text:style-name='TEI_date_reception']"/>
                    </date>
                  </xsl:if>
...
<!-- blocs <ab> Métopes [todo] -->
            <ab type="book">
                <bibl>
                    ...
                    <availability><licence></licence></availability>
                    <dim type="pagination" unit="page">
                        <xsl:if test="//text:p[@text:style-name='TEI_pagination']">
                        <xsl:variable name="pagination">
                            <xsl:analyze-string select="//text:p[@text:style-name='TEI_pagination']" regex="(\d+)(.)(\d+)">
            ...
            <ab type="digital_download" subtype="PDF">
                <bibl>
                    ...
                    <idno type="DOI" subtype="crossref|datacite|lsid"></idno>
                    <!-- Si contraire à ce qui est défini supra -->
                    <publisher></publisher>
                    <availability><licence><p></p></licence></availability>
                </bibl>
            ...
            <ab type="digital_download" subtype="EPUB">
                <bibl>
                    ...
                    <idno type="DOI"></idno>
                    <availability><licence></licence></availability>
                    <dim type="weight" unit="Ko|Mo" extent=""></dim>
                </bibl>
                <!-- XG : ajout des valeurs par défaut -->
                    <desc type="accessMode">textual</desc>
                    <desc type="accessibilityFeature">none</desc>
                    <desc type="accessibilityHazard">none</desc>
                    <desc type="accessModeSufficient">textual</desc>
                    <desc type="accessibilitySummary"></desc>
                <!-- Fin ajout XG -->
            </ab>
            <ab type="digital_download" subtype="HTML">
                <bibl>
                    <distributor><!--OpenEdition|Cairn.info|OJS|alt--></distributor>
                    <date type="publishing"></date>
    ...
    <!-- génération conditionnelle du tagsDecl : ajouter test table -->
          <xsl:if test="//text:list or //*[starts-with(@rendition,'#')]">
            <tagsDecl>
            <xsl:if test="//*[@rendition='#rtl']">
                <rendition scheme="css" xml:id="rtl">direction:right-to-left;</rendition>
            </xsl:if>
            <!-- traitement exhaustif cas par cas car syntaxe des instructions CSS parfois différentes de la valeur de l'@rendition -->
            <xsl:if test="//*[@rendition='#start']">
...
<xsl:template match="text:span[starts-with(@text:style-name,'TEI_author-inline:')]|text:span[starts-with(@text:style-name,'TEI_editor-inline:')]" mode="teiHeader">
    ...
    <xsl:element name="{$context}">
        ...
        <xsl:if test="following::text:p[@text:style-name='TEI_authority_mail']">
            <email></email>
        </xsl:if>
    </xsl:element>
</xsl:template>
<!--
<xsl:template match="text:p[starts-with(@text:style-name,'TEI_author:')]|text:p[starts-with(@text:style-name,'TEI_editor')]">
    <xsl:choose>
        <xsl:when test="ancestor::*:div[@type='review']">
            <docAuthor>
                <name><xsl:apply-templates/></name>
                <xsl:if test="following-sibling::text:*[1][@text:style-name='TEI_authority_affiliation']">
                    <affiliation><xsl:apply-templates select="following-sibling::text:*[1][@text:style-name='TEI_authority_affiliation']" mode="preserve"/></affiliation>
                </xsl:if>
            </docAuthor>
        </xsl:when>
        <xsl:otherwise/>
...
<xsl:template match="//text:span[@text:style-name='TEI_neutral-inline']" mode="teiHeader"><xsl:comment>mode teiHeader = <xsl:value-of select="."/></xsl:comment></xsl:template>
<xsl:template match="//text:span[@text:style-name='TEI_neutral-inline']"><xsl:comment>*txt*</xsl:comment><xsl:apply-templates/></xsl:template>
