...
<xsl:template match="table:table-cell">
    <cell>
        ...
        <xsl:choose>
            <xsl:when test="child::text:p[@text:style-name='TEI_cell' and @rendition='#rtl']">
                <p><xsl:copy-of select="@*"/><xsl:apply-templates select="child::text:p[@text:style-name='TEI_cell']/node()"/></p>
            </xsl:when>
            <xsl:when test="child::text:p[@text:style-name='TEI_cell' and not(@rendition='#rtl')]">
                <xsl:apply-templates select="child::text:p[@text:style-name='TEI_cell']/node()"/>
            </xsl:when>
            <xsl:when test="child::text:p[@text:style-name='Standard' and @rendition='#rtl']">
                <xsl:apply-templates/>
            </xsl:when>
...
<xsl:template match="draw:frame[child::draw:object]">
    <xsl:choose>
        ...
        <xsl:when test="parent::text:p[@text:style-name='TEI_formula']">
            <figure><formula notation="mathml"><xsl:apply-templates/></formula></figure>
        </xsl:when>
        <xsl:otherwise>
            <figure rend="inline">
