        ...
        <!-- enrichissements typo traitement de texte (l'application de styles de caractères ne génère pas de raccourcis de styles)-->
            ...
            <xsl:when test="matches(@text:style-name,'[T]\d{1,2}')">
                <xsl:choose>
                    ...
                    <xsl:when test="//style:style[@style:name=$currentStyle]/style:text-properties/(@fo:font-style|@fo:font-variant|@fo:font-weight|@style:text-position|@style:text-underline-style|@style:text-line-through-style and not(@fo:font-style='normal'))">
                    <xsl:element name="text:span">
...
<!--        <plop><xsl:value-of select="./following-sibling::*[1]/local-name()"/>@<xsl:value-of select="./following-sibling::*[1]/@text:style-name"/></plop>-->
...
<!--                 <xsl:comment>*0* coll. doing nada</xsl:comment> -->
            ...
            <xsl:when test="not(preceding::text:p[@text:style-name='Title'])">
<!--                 <xsl:comment>*1*</xsl:comment> -->
                <xsl:element name="text:h">
                    ...
                    <xsl:apply-templates/><!-- ici reconstituer titre ? juste dans le header ou aussi en titlepage ?  -->
                </xsl:element>
            </xsl:when>
...
<!--                 <xsl:comment>*2*</xsl:comment> -->
                <xsl:comment>recension double</xsl:comment>
                <xsl:element name="text:p">
                    <xsl:copy-of select="@*"/>
...
<!-- ajout d'un @outline-level sur le titre de section biblio (Métopes) /!\ -->
    ...
    <xsl:when test="$currentElementName='text:p' and parent::table:table-cell and //style:style[@style:name=$currentStyle]/style:paragraph-properties/@fo:text-align">
        <xsl:element name="{$currentElementName}">
            ...
            <xsl:attribute name="align"><xsl:value-of select="//style:style[@style:name=$currentStyle]/style:paragraph-properties/@fo:text-align"/></xsl:attribute>
