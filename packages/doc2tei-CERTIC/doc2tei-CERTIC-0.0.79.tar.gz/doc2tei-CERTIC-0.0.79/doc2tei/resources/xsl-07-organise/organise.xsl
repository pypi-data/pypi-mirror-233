...
<xsl:variable name="titlePageNodes" as="node()*">
    <xsl:sequence select="//text:p[starts-with(@text:style-name,'TEI_author')][not(ancestor::div[@type='review'])] | 
...
<!--  tous les éléments qui précèdent le premier paragraphe 'Standard' (et ne sont pas des éléments de titlePageNodes)  -->
    <xsl:sequence select="$firstPara/preceding::text:*[not(ancestor::text:note) and not(ancestor::*:div) and not(ancestor::*:figure) and not(self::text:note) and not(self::text:span) and not(self::text:a)] except ($titlePageNodes)"/>
</xsl:variable>
