...
def named_collision_objects(names):
    objs = []
    for name in names:
        if type(name) == str:
            obj = NamedCollisionObject(name)
        elif len(name) == 2:
            obj = NamedCollisionObject(name[0], name[1])
        else:
            raise ValueError(
                f"Could not parse NamedCollisionObject from value {name}."
            )
        objs.append(obj)
    return objs
