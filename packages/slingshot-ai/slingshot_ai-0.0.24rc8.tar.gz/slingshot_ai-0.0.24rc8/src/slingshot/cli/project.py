...
@app.command("details", requires_project=True, hidden=True)
async def project_details(sdk: SlingshotSDK) -> None:
    ...
    console.print(f"(via {client_settings.slingshot_config_path.parent}")
...
async def _set_project_id(sdk_: SlingshotSDK, project_id: str) -> None:
    ...
    if old_project_id != project_id:
        # We're changing the remote project and need to re-sync our local state with the remote. Reset the "last pushed"
        # manifest to be the current local (if we have one), as this gives more sane conflicts/diffs.
        # This was previously always set to None, which made it appear as if both the local and the remote had just
        # added all components even if there was no real change.
        project_config.last_pushed_manifest = (
            current_project_config.model_dump(exclude_local_only=True) if current_project_config else None
        )
