...
class SeriesBase:
    ...
    @staticmethod
    def _get_window_function_type(key: Optional[Union[str, WindowFunctionKind]]) -> Union[WindowFunctionKind, None]:
        ...
        raise ValueError(f"Window function '{key}' is not supported.")
    ...
    def _feature_name_filter(self: TSeries, *features: Tuple[Any], equals: bool) -> TSeries:
        if not features:
            raise ValueError("One or more Chalk Features must be supplied.")
        copy = self._copy_with()
    ...
    def _resolver_name_filter(self: TSeries, *resolvers: Union[ResolverProtocol, str], equals: bool) -> TSeries:
        if not resolvers:
            raise ValueError("One or more Chalk Resolvers must be supplied.")
        copy = self._copy_with()
        comparator = ComparatorKind.EQ if equals else ComparatorKind.NEQ
    ...
    def _string_filter(self: TSeries, *strings: str, kind: FilterKind, equals: bool = True) -> TSeries:
        if not strings:
            raise ValueError("One or more arguments must be supplied for this filter")
        copy = self._copy_with()
        comparator = ComparatorKind.EQ if equals else ComparatorKind.NEQ
    ...
    def _with_resolver_type_filter(
        ...
        if not resolver_types:
            raise ValueError("One or more resolver types from 'online', 'offline', or 'stream' must be supplied")
...
def _validate_feature(feature_string: str):
    ...
    try:
        ...
        raise ValueError(f"No feature found with name '{feature_string}'. Feature must be in the format 'user.name'")
