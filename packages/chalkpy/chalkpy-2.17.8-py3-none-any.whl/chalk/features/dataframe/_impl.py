...
from chalk.features.underscore_features import parse_underscore_df_item
    ...
    def __getitem__(cls, item: Any) -> Type[DataFrame]:
        ...
        for a in item:
            if isinstance(a, Underscore):
                fqn_columns = {ensure_tuple(column.root_fqn.split(".")): column for column in cls.columns}
                a = parse_underscore_df_item(a, fqn_columns)
...
class DataFrame(metaclass=DataFrameMeta):
    ...
    def __getitem__(self, item: Any) -> DataFrame:
        ...
        has_bool_or_filter_value = any(isinstance(x, (bool, Filter, Underscore)) for x in ensure_tuple(item))
        ...
        # Set columns based on existence of pydantic model
        if self._pydantic_model:
            fqn_columns = {
                ensure_tuple(["_pydantic_model"] + key.split(".")): key
                for key in self._pydantic_model.__fields__.keys()
            }
        else:
            fqn_columns = {ensure_tuple(column.root_fqn.split(".")): column for column in self.columns}

        item = tuple(
            parse_underscore_df_item(x, fqn_columns) if isinstance(x, Underscore) else x for x in ensure_tuple(item)
        )
    ...
    def group_by(
        ...
        return DataFrame(
            self._underlying.lazy().groupby(groupby).agg(cols).collect(),
    ...
    def histogram_list(
        ...
        #     raise ValueError(f"Cannot pass both `nbins` (got {nbins}) and `bin_width` (got {bin_width})")
        #
        ...
        max_value = underlying.select(col_str).max().collect().item()
        min_value = underlying.select(col_str).min().collect().item()
    ...
    @overload
    @classmethod
    def from_list(cls, *data: Features) -> Self:
        """Deprecated. Use DataFrame(...) instead."""
        ...

    @classmethod
    def from_list(cls, *data: Union[Features, Sequence[Features]]) -> Self:
        """Deprecated. Use DataFrame(...) instead."""
    ...
    def __eq__(self, other: Union[DataFrame, pl.DataFrame, pl.LazyFrame, pd.DataFrame, Any]):
        return self._perform_op("eq", other, convert_dtypes=False)

    def __ne__(self, other: Union[DataFrame, pl.DataFrame, pl.LazyFrame, pd.DataFrame, Any]):
    ...
    def __le__(self, other: Union[DataFrame, pl.DataFrame, pl.LazyFrame, pd.DataFrame, Any]) -> Self:
        return self._perform_op("le", other, convert_dtypes=False)

    def __add__(self, other: Union[DataFrame, pl.DataFrame, pl.LazyFrame, pd.DataFrame, Any]) -> Self:
        return self._perform_op(operator.add, other, convert_dtypes=True)

    def __sub__(self, other: Union[DataFrame, pl.DataFrame, pl.LazyFrame, pd.DataFrame, Any]) -> Self:
    ...
    def slice(self, offset: int = 0, length: Optional[int] = None) -> Self:
        """Slice the `DataFrame`.
