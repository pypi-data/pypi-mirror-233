...
def pyarrow_to_polars(pa_type: pa.DataType, name: str | None = None) -> pl.PolarsDataType:
    ...
    raise TypeError(f"Unsupported PyArrow type {pa_type}" + (f" for field {name}" if name is not None else ""))
