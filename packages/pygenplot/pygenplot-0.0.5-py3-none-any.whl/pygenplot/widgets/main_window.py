...
from pygenplot.dialogs.about_dialog import AboutDialog
from pygenplot.kernel.logger import log, LOGGER
from pygenplot.icons import ICONS, load_icons
from pygenplot.handlers.logger_popup import LoggerPopup
from pygenplot.handlers.logger_widget import LoggerWidget
from pygenplot.models.data_tree_model import DATA_ITEMS, DataTreeModelError
from pygenplot.models.plot_1d_model import Plot1DModelError
from pygenplot.widgets.data_widget import DataWidget
from pygenplot.widgets.actions_widget import ActionsWidget
from pygenplot.widgets.plot_1d_widget import Plot1DWidget
from pygenplot.widgets.plot_nd_widget import PlotNDWidget
from pygenplot.widgets.preview_widget import PreviewWidget
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build(self):
        ...
        self._actions_dock_widget.setAllowedAreas(QtCore.Qt.DockWidgetArea.LeftDockWidgetArea)
        self._actions_dock_widget.setFloating(False)
        self._actions_dock_widget.setFeatures(QtWidgets.QDockWidget.DockWidgetFeature.DockWidgetFloatable | QtWidgets.QDockWidget.DockWidgetFeature.DockWidgetMovable)
        ...
        self._logger_dock_widget.setWidget(logger)
        logger.setFormatter(logging.Formatter("%(asctime)s - %(levelname)s - %(message)s"))
        LOGGER["main"].addHandler(logger)
        LOGGER["main"].setLevel(logging.INFO)

        logger_popup = LoggerPopup()
        logger_popup.setFormatter(logging.Formatter("%(levelname)s - %(message)s"))
        LOGGER["popup"].addHandler(logger_popup)
        LOGGER["popup"].setLevel(logging.ERROR)
        
        self.addDockWidget(QtCore.Qt.DockWidgetArea.LeftDockWidgetArea,self._data_dock_widget)
        self.addDockWidget(QtCore.Qt.DockWidgetArea.LeftDockWidgetArea,self._actions_dock_widget)
        ...
        self._actions_widget.plot_in_current_figure.connect(self.on_plot_in_current_figure)
    ...
    def _build_menu(self):
        ...
        open_file_action = file_menu.addAction(ICONS["load"],"Open")
        open_file_action.triggered.connect(self.on_open_file)
        close_app_action = file_menu.addAction(ICONS["exit"],"Exit")
        ...
        about_action.triggered.connect(self.on_open_about)
    ...
    def on_open_about(self):
        """Callback called when the About menu item is clicked.
        """
    ...
    def on_open_file(self):
        ...
        filename, _ = QtWidgets.QFileDialog.getOpenFileName(self,"Open Data File(s)","",filter_mask)
        if not filename:
            return
        self.add_data(filename)
    ...
    def on_plot_in_new_figure(self,plot_type):
        ...
        if not axis_dataset_info:
            for dinfo in axis_dataset_info:
                if not dinfo["plottable"]:
                    log(f"One or more dependent axis of the dataset is not plottable: {dinfo['status']}",["main","popup"],"error")
                    return

        if not selected_dataset_info["plottable"]:
            log(f"The dataset is not plottable: {selected_dataset_info['status']}",["main","popup"],"error")
            return
        ...
        if plot_type == "Line":
            try:
                ...
                icon = ICONS["plot_1d"]
                self._plots_tab_widget.addTab(plot_widget,icon,selected_dataset_info["variable"])

        elif plot_type == "Image":
            try:
                plot_widget = PlotNDWidget(plot_type,axis_dataset_info,selected_dataset_info,self)
            except Exception as e:
                log(str(e),["main","popup"],"error")
                return
            else:
