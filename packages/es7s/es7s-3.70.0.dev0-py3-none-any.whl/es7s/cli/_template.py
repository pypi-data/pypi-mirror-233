...
from es7s.shared import get_logger, get_stdout, Regex


class TemplateCommand:
    REGEX_SECTION_START = '\x1b\x1e'
    REGEX_SECTION_END = '\x1b\x7f'
    REGEX_SUBSTITUTE_SEP = Regex('\t*\x1f\t*')
    ...
    def _print(self, postprocessed: str):
        # if get_stdout().io.isatty():
        #     tmp_file = open(tempfile.mkstemp()[1], 'w')
        #     reinit_io(get_stdout().io_params, tmp_file, get_stderr().io)
        #
