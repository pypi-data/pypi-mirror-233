...
from .alert_conditions import is_significant, should_publish
...
class SEventValidator(SEventCacheEntry):
    ...
    def _get_file_dataproduct_ev(
        ...
        if filename_path.exists():
            ...
            try:
                ...
                if exc.response.status != 404:  # type: ignore[attr-defined]
                    raise exc
    ...
    def _save_data(self, outdir: Path) -> None:
        ...
        data_dict = {
            ...
            'labels': self.labels_dictMDC,
            'files': self.files_dictMDC,
            ...
            for data_upload_event in data_upload_events:
                ...
                try:
                    if data_event['group'].lower() == 'cbc':
                        ...
                        localization = (
                            data_event['pipeline'].lower() + '.multiorder.fits,0'
                        )
                    else:
                        localization = 'dummy'
                    ...
                    credible90 = 0.0
                try:
                    extra_attributes = data_event['extra_attributes']
                    if data_event['group'].lower() == 'cbc':
                        snr_event = extra_attributes['CoincInspiral']['snr']
                    elif data_event['pipeline'].lower() == 'cwb':
                        snr_event = extra_attributes['MultiBurst']['snr']
                    elif data_event['pipeline'].lower() == 'olib':
                        snr_event = extra_attributes['LalInferenceBurst'][
                            'omicron_snr_network'
                        ]
                    else:
                        snr_event = 0.0
                    ...
                    snr_event = 0.0
                dump_log_lines.append(
                    '  {:8.1f}s ({:5.1f}s)  {:10} FAR={:8.3g} SNR={:5.2f}'
                    ' sky90%= {:8.3f} deg2 LOGBCI={:5.2f} {}'.format(
                        data_upload_event[0],
                        data_event['reporting_latency'],
                        id_event,
                        data_event['far'],
                        snr_event,
