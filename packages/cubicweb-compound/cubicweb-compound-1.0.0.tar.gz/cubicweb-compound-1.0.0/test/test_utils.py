...
class UtilityFunctionsTC(CubicWebTC):
    ...
    def test_optional_relations(self):
        ...
        expected = {
            "Anecdote": {("narrated_by", "subject")},
            "BiographyComment": {("commented_by", "object")},
            "Event": {("relates", "object")},
        }
    ...
    def test_graph_rql_expr(self):
        ...
        for etype, expected in (
            ...
            self.assertEqual(
                utils.graph_rql_expr(self.schema, graph, etype, "Agent"), expected
            )
            with self.assertRaisesRegex(
                ValueError,
                "^could not find a RQL path from BiographyComment to Agent$",
            ):
