...
class CompositeGraphTC(CubicWebTC):
    ...
    def test_parent_relations_comment(self):
        ...
        self.assertEqual(rels, [(("comments", "subject"), ["Comment", "Anecdote"])])
    ...
    def test_parent_related(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            anecdote2 = cnx.create_entity(
                "Anecdote", reverse_event=bio, narrated_by=agent, relates=event
            )
...
class ContainerAPITC(CubicWebTC):
    ...
    def test_parent_relations_direct(self):
        with self.admin_access.repo_cnx() as cnx:
            account = one(cnx, "OnlineAccount")
            icontained = account.cw_adapt_to("IContained")
            self.assertEqual(icontained.parent_relations, {("account", "object")})
    ...
    def test_parent_relations_indirect(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(
                icontained.parent_relations,
                {("event", "object"), ("relates", "object")},
            )
            self.assertEqual(icontained.parent_relation(), ("event", "object"))


def clone_agent(cnx, name="bob", skiprtypes=()):
    bob = cnx.find("Agent", name=name).one()
    clone = cnx.create_entity("Agent", name="bobby")
...
class CloneTC(WebCWTC):
    def setup_database(self):
        with self.admin_access.repo_cnx() as cnx:
            self.create_user(cnx, "georges")
            cnx.commit()

    def test_copy_entity(self):
        with self.admin_access.repo_cnx() as cnx:
            cnx.create_entity("Agent", name="bob", skip="skipped")
            cnx.commit()
        with self.new_access("georges").repo_cnx() as cnx:
            bob = cnx.find("Agent").one()
            bob2 = copy_entity(bob)
            alice = copy_entity(bob, name="alice", knows=bob)
            cnx.commit()
            self.assertEqual(bob2.name, "bob")
            self.assertIsNone(bob2.skip)
            self.assertEqual(alice.name, "alice")
            self.assertEqual([x.eid for x in alice.knows], [bob.eid])
            self.assertEqual(alice.created_by[0].login, "georges")
            self.assertEqual(bob2.created_by[0].login, "georges")
    ...
    def test_clone_adapter_registered(self):
        self.assertEqual(IClonableAdapter.clone_relations, {"clone_of": "object"})

    def test_clone_simple(self):
        with self.admin_access.repo_cnx() as cnx:
            bob = cnx.create_entity("Agent", name="bob")
            alice_eid = cnx.create_entity("Agent", name="alice", knows=bob).eid
            cnx.create_entity("OnlineAccount", reverse_account=bob)
            cnx.commit()
        with self.new_access("georges").repo_cnx() as cnx:
            bob = cnx.find("Agent", name="bob").one()
            clone = cnx.create_entity("Agent", name="bobby")
            ...
            cnx.commit()
            self.assertEqual(clone.name, "bobby")
    ...
    def test_clone_skiprtypes(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            clone = clone_agent(cnx, "bob", skiprtypes=("member",))
    ...
    def test_clone_skiprtypes_sublevel(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.create_entity("Agent", name="bob", biography=bio)
            cnx.commit()

            clone = clone_agent(cnx, "bob", skiprtypes=("see_also",))
            self.assertEqual(clone.biography[0].see_also, ())

    def test_clone_full(self):
        with self.admin_access.repo_cnx() as cnx:
            agent = cnx.create_entity("Agent", name="bob")
            cnx.create_entity("OnlineAccount", reverse_account=agent)
            bio = cnx.create_entity("Biography", reverse_biography=agent)
    ...
    def test_clone_action(self):
        with self.admin_access.web_request() as req:
            entity = req.create_entity("Agent", name="bob")
            req.cnx.commit()
            action = self.vreg["actions"].select("copy", req, rset=entity.as_rset())
    ...
    def test_clone_hook(self):
        ...
        with self.new_access("georges").repo_cnx() as cnx:
            bob = cnx.find("Agent", name="bob").one()
            clone = cnx.create_entity("Agent", name="bobby", clone_of=bob)
            cnx.commit()
            clone.cw_clear_all_caches()
            # Ensure all relation (to new entities) are present on the clone.
            rset = cnx.execute(
    ...
    def test_clone_controller(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.commit()
        with self.new_access("georges").web_request("compound.clone") as req:
            req.form["eid"] = original_eid
            self.app_handle_request(req)
            rset = req.execute(f"Any X WHERE X clone_of O, O eid {original_eid}")
            self.assertTrue(rset)
