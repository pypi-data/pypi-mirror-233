...
def graph_relations(schema, parent_structure):
    ...
    * mandatory relations (cardinality of the child in '1*').
    ...
    optionals = concat_sets(optional_relations(schema, parent_structure).values())
    mandatories = {
        (rdef.relation_type, neg_role(role))
    ...
    for etype, relations in graph_structure.items():
        for (rtype, role), targets in relations.items():
            for target in targets:
                rdef = schema[rtype].role_relation_definition(etype, target, role)
    ...
    * "U has_update_permission P, X related_to P" for `A` entity type.
    ...
    for child, relations in structure.items():
        ...
        for rtype, role in relations:
            ...
            exprs.append(f"{relexpr}, U has_update_permission A")
        if exprs:
            for action in ("update", "delete"):
                schema[child].set_action_permissions(
                    action, tuple(ERQLExpression(e) for e in exprs)
                )
...
def graph_set_write_rdefs_permissions(schema, graph, etype):
    ...
    for rdef, parent_role in mandatory_rdefs(schema, structure):
        ...
        expr = f"U has_update_permission {var}"
        for action in ("add", "delete"):
            rdef.set_action_permissions(action, (RRQLExpression(expr),))


def _rel_expr(rtype, role):
    return {"subject": "X {rtype} A", "object": "A {rtype} X"}[role].format(rtype=rtype)
    ...
    while q:
        ...
        for (rtype, role), targets in structure[etype].items():
            for target in targets:
                ...
                if rdef.role_cardinality(role) in "1+":
                    heapq.heappush(
                        q, (cost + 1, target, path + ((rtype, target, role),))
                    )
    if not path:
        raise ValueError(f"could not find a RQL path from {from_etype} to {to_etype}")
    return path
