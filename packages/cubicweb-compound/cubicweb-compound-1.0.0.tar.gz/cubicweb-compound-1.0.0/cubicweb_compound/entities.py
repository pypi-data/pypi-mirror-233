...
def copy_entity(original, cw_skip_copy_for=None, **attributes):
    ...
    original.complete()
    for rschema in original.e_schema.subject_relations.values():
        if (
            not rschema.final
            or rschema.meta
...
class IContainer(EntityAdapter):
    ...
    def build_class(cls, etype):
        ...
        return type(etype + "IContainer", (cls,), {"__select__": selector})
...
class IContained(EntityAdapter):
    ...
    def build_class(cls, etype, parent_relations):
        ...
        for parent_relation in parent_relations:
            assert (
                isinstance(parent_relation, tuple)
                and len(parent_relation) == 2
                and parent_relation[-1] in ("subject", "object")
            ), parent_relation
        ...
        if etype in cls._classes:
            ...
            assert contained_cls.__bases__ == (cls,), (
                ...
                "already exists".format(
                    etype=etype, cls=cls, bases=contained_cls.__bases__
                )
            )
        ...
        # else generate one
            ...
            selector = is_instance(etype)
            contained_cls = type(
                str(etype) + "IContained",
                (cls,),
                {"__select__": selector, "parent_relations": parent_relations},
            )
    ...
    def parent_relation(self):
        ...
        for parent_relation in self.parent_relations:
            ...
            assert len(parents) <= 1, "more than one parent on {}: {}".format(
                self.entity, parents
            )
            if parents:
    ...
    def __registered__(cls, *args):
        ...
        if registered:
            if cls.role != registered:
                raise ValueError(
                    f"{cls.__name__} already registered for {cls.rtype} but role differ"
                )
        else:
            cls.clone_relations[cls.rtype] = cls.role
        return super().__registered__(*args)

    @property
    def graph(self):
        """The composite graph associated with this adapter."""
        return CompositeGraph(
            self._cw.vreg.schema, skiprtypes=self.skiprtypes, skipetypes=self.skipetypes
        )
