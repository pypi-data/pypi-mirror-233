...
def structure_def(schema, etype, skiprtypes=(), skipetypes=()):
    ...
    return {
        child: set(relinfo) for child, relinfo in graph.parent_structure(etype).items()
    }
...
class CompositeGraph:
        ...
        for rschema, teschemas, role in eschema.relation_definitions():
            ...
            for target in teschemas:
                if target in self.skipetypes:
                    continue
                rdef = rschema.role_relation_definition(eschema, target, role)
                if (
                    rschema.type,
                    target_role,
                ) not in follow_relations and rdef.composite != target_role:
                    continue
    ...
    def child_related(self, entity, follow_relations=()):
        ...
        """
        return self._graph_related(entity, True, follow_relations=follow_relations)

    def _graph_related(self, entity, topdown, follow_relations=(), _visited=None):
        ...
        for (rtype, role), targettypes in self._graph_relations(
            ...
            for target in rset.entities():
                ...
                yield from self._graph_related(
                    target,
                    topdown,
