...
class MLflow(Base):
    ...
    def log_tag(self, experiment_id: str, key: str, value: str or int or float):
        self._client.set_tag(run_id=experiment_id, key=key, value=value)
    ...
    def stage_model(self, experiment_id: str) -> str:
        artifact_path: str = "model"

        model_uri = f"runs:/{experiment_id}/{artifact_path}"
        model_details = mlflow.register_model(model_uri=model_uri, name=self._model_name)

        self._client.transition_model_version_stage(
            name=self._model_name,
            version=model_details.version,  # Specify the version you want to stage
            stage="Staging"
        )
