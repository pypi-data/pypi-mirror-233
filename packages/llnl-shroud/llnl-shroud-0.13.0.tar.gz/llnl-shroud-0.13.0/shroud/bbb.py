def foo():
    ...
    fmt.hnameproto = wformat(
        "void {hnamefunc}({C_array_type} *outdesc, std::string *in, size_t nlen)", fmt)
    if literalinclude:
        fmt.lstart = "{}helper {}\n".format(cstart, name)
...
{hnameproto}
{{+
size_t nvect = std::min(outdesc->size, nlen);
...
{hnameproto}
{{+
std::vector<std::string> *cxxvec =\t static_cast< std::string * >\t(vec->addr);
...
-end interface""",
    ...
    fmt.hnameproto = wformat(
        "size_t {hnamefunc}(std::string *in, size_t nlen)", fmt)
    if literalinclude:
        fmt.lstart = "{}helper {}\n".format(cstart, name)
        fmt.lend = "\n{}helper {}".format(cend, name)
    CHelpers[name] = dict(
