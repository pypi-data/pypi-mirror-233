...
class ResultComparator():
    ...
    def get_rapport(self) :
        ...
        rapport = pd.DataFrame(columns=["key", "good", "wrong", "under_threshold", "not_found", "number", "accuracy", "raw_accuracy", "metadatas"])
        for key in keys :
            wrong_values = []
            ...
            accuracy = good / (number-under_threshold-not_found)
            raw_accuracy = good / number
            rapport = pd.concat([rapport, pd.DataFrame([[key, good, wrong, under_threshold, not_found, number, accuracy, raw_accuracy, wrong_values]],
