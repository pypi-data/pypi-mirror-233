...
def get_data(query_text, api_inputs: ApiInputs, query_language: QUERY_LANGUAGE = "kql", response_type: RESPONSE_TYPE = 'dataframe'):
    """Retrieve data.
    ...
    try:
        response = requests_retry_session2(status_forcelist=[408, 429, 500, 502, 503, 504]).post(url, json=payload, headers=headers)
...
def connect_to_sql(api_inputs: ApiInputs):
    ...
    sql_server_name, sql_server_user_name, sql_server_password = extract_sql_credentials(cs)
