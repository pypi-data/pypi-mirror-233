...
class Quiz(models.Model):
    ...
    questions_random = models.BooleanField(
        default=False, verbose_name=_("Random questions")
    )
...
class WeightedAnswersPlugin(BasePlugin, CMSPlugin):
    quiz = models.ForeignKey(WeightedAnswersQuiz, on_delete=models.SET_NULL, null=True)
...
class InterpretationPlugin(BasePlugin, CMSPlugin):
    quiz = models.ForeignKey(InterpretationQuiz, on_delete=models.SET_NULL, null=True)
...
class MoreInfoQuiz(models.Model):
    name = models.CharField(
        max_length=255, help_text=_("Will be displayed below the resuls.")
    )
