...
from dagworks.api.clients import UnauthorizedException
...
def hash_dag(dag: graph.FunctionGraph, dag_name: str) -> str:
    """Hashes a DAG.
...
def convert_function_graph_to_node_templates(fn_graph: graph.FunctionGraph) -> List[dict]:
    ...
    for node_ in fn_graph.nodes.values():
        node_templates.append(
            dict(
                ...
                else _get_fully_qualified_function_path(node_.originating_functions[0]),
    ...
    for node_ in fn_graph.nodes.values():
        ...
        for fn in originating_functions:
            ...
            if fn_name not in seen:
                seen.add(fn_name)
                path = os.path.relpath(inspect.getsourcefile(fn), repo_base_path)
                out.append(
                    dict(
                        name=fn_name,
                        type="p_function",
                        ...
                        end=inspect.getsourcelines(fn)[1] - 1 + len(inspect.getsourcelines(fn)[0]),
                        url=_derive_url(vcs_info, path, inspect.getsourcelines(fn)[1]),
                    )
                )
    return out
...
class DAGWorksGraphExecutor(driver.GraphExecutor):
    ...
    def execute(
        ...
        code_version = self.client.register_code_version_if_not_exists(
            project_id=self.project_id,
            code_hash=self.vcs_info.commit_hash,  # TODO -- ensure the right one is wired through
            vcs_info=self.vcs_info,
            slurp_code=functools.partial(_slurp_code, fg, self.repo_base),
        )

        # TODO -- get rid of this/figure out if we need it?
        dag_hash = hash_dag(fg, dag_name=self.dag_name)
        project_version = self.client.register_dag_template_if_not_exists(
            config=fg.config,
            code_version=code_version,
            dag_hash=dag_hash,
            nodes=convert_function_graph_to_node_templates(fg),
        ...
        run_id_generated = uuid.uuid4()
        tracking_state = TrackingState(str(run_id_generated))
        with monkey_patch_adapter(fg.adapter, tracking_state):
            tracking_state.clock_start()
            run_id = self.client.create_and_start_dag_run(
                dag_template_id=project_version,
                tags=self.run_tags,
            ...
            try:
                ...
                logger.warning(
                    ...
                    f"{self.dagworks_ui_url}/dashboard/project/{self.project_id}/runs/{run_id}\n"
                )
