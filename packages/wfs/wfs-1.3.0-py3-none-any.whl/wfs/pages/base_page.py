    ...
    try:
        ...
        if item == 'text':
            ...
            if lref == 'Y':
                ...
                aresults = gttext if testitem == '***' else testitem
                is_valid = getcheckorder(order_list=gtt, expected_order=aresults)
                if 'N' in is_valid:
                    if ptname:
                        gtt = set(gtt)
                    return 'Error : Items missing or not in order  -->' + str(is_valid)
                else:
                    if gtt == aresults:
                        print('Test data list is Equal....')
                        Assertion().assert_equals_results(case=case, testcase=testcase, gtext=gtt,
                                                          aresults=aresults,
                                                          exresults=exresults, stepline=stepline, gtitems=testitem)
                    else:
                        print('List is not Equal, but given test data are in list..')
