...
def get_results(xlcreate, features, userstory, testcase, teststeps, testdata, actResults, expResults, aresults, results, fontx,
    ...
    def execute_steps(testcase, ustory, teststeps, testdata, action, adescribe, aresults, exresults, case, depend):
        ...
        for steps_line in range(0, len(linex)):
            ...
            def getExe(get_all):
                ...
                gtitems = reportpath, features, ustory, teststeps, testdata, depend, tdatax[steps_line]  # , arets[steps_line]
...
def get_line_dict(ptname, dname):
    ...
    print(result[dxt][nmxt])
    return result[dxt][nmxt]
...
def validate_order(order_list, expected_order):
    expected_order = expected_order  # ['Active', 'Boarding', 'Unboarding', 'Inactive']
    current_index = 0

    for item in order_list:
        if item == expected_order[current_index]:
            current_index = (current_index + 1) % len(expected_order)

            if current_index == 0:
                break
    '''order_list = ['Inactive', 'Inactive', 'Inactive', 'Inactive', 'Inactive', 'Boarding', 'Inactive', 'Inactive',
                  'Unboarding', 'Boarding', 'Active', 'Boarding', 'Unboarding', 'Inactive', 'Active', 'Unboarding',
                  'Active', 'Active']
    is_valid = validate_order(order_list)
