...
from src.brightpearlapi.utils.exceptions import TokenExpiredException, RateLimitException
...
class Connection(object):
    def __init__(
        ...
        self.resource_base_path = protocol + "://{domain}.brightpearl.com/2.0.0/{account_id}/{resource}"
        self.domain = domain
        ...
        self._session.headers = {
            "Accept": "application/json",
            "brightpearl-account-token": access_token,
    ...
    def rate_limiting(self, headers):
        ...
        if 'brightpearl-requests-remaining' in headers:
            # If no rate limit management
            if not self.rate_limit_management:
                return
    ...
    def process_response(self, response, stream):
        """
            Method to process the responses from the brightpearl API.
