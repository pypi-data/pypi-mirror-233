...
class Operator:
    ...
    def draw_auto_exit_with_pic(self, course, picture, similar_numbers=None):
        ...
        threading.Thread(target=self.draw_auto_entrance_pic,
                         args=(picture, self.terminal.w / 6.4,
                               self.terminal.h / 2, crop_tuple),
                         kwargs={"resize": (100, 100)}).start()
        self.currentPage.orupAct(call_method='unknown_number',
                                 course=course)
    ...
    def draw_auto_entrance_pic(self, picture, x, y,
                               ...
                               img_tag=None, corners=None, resize: tuple = None):
        #print("TT18", locals())
        tags = ['orupentry']
        if img_tag:
            tags.append(img_tag)
        ...
        if resize:
            im_r = im_r.resize(resize)
        img_byte_arr = io.BytesIO()
        if im_r.mode in ("RGBA", "P"):
            im_r = im_r.convert("RGB")
        im_r.save(img_byte_arr, format='JPEG')
