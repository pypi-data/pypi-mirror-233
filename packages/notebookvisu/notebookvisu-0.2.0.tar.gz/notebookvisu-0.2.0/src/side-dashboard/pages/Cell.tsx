...
const Cell = (props: ICellPageProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${BACKEND_API_URL}/dashboard/${props.notebookId}/cell/${content.cellId}?timeWindow=${timeWindow}`
    )
  ...
  return (
    <>
      <div className="course-title-container">
        <div className="course-title-text">Cell ({content.cellId})</div>
      </div>
      {isFetching ? (
        <Loader />
      ) : (
        <>
          ...
          <Form className="cell-filter-container">
            ...
            <div className="cell-dropdowns-container">
              <SortDropDown setOrderBy={setOrderBy} />
              <TimeDropDown />
            </div>
          </Form>
          {apiData.length === 0 ? (
            <NoData text={'No data in selected time window'} />
          ) : (
            <>
              {/* Cell Executions */}
                      ...
                      <Col md={12}>
                        <Card className="cell-card">
                          <Card.Body style={{ gap: '10px' }}>
                            <Row>
                              <Col md={2} className="cell-user-title">
                                <Card.Text>User {index}</Card.Text>
                              </Col>
                              <Col md={10}>
                                <ReactMarkdown
                                  className="cell-content-container react-markdown-input"
                                  // {[rehypeRaw] as any}
                                >
                                  {value.cell_input}
                                </ReactMarkdown>
