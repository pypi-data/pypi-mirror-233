...
class NapariSTPT:
        ...
        if False:
            ...
            if self.cb_C2.isChecked():
                ...
                if self.axio:
                    ...
                    volume_2_temp = self.AlignAXIO(volume_2_temp, str(self.comboBoxPath.currentText()), self.slice_names, resolution, 1)
            ...
            if self.cb_C3.isChecked():
                ...
                if self.axio:
                    ...
                    volume_3_temp = self.AlignAXIO(volume_3_temp, str(self.comboBoxPath.currentText()), self.slice_names, resolution, 2)
    ...
    #                 volume_3[optical_slice::self.optical_slices, :, :] = volume_3_temp[start_slice_number:end_slice_number+1, start_y:start_y+size_y, start_z:start_z+size_z]
            ...
            if self.cb_C4.isChecked():
                ...
                if self.axio:
                    ...
                    volume_4_temp = self.AlignAXIO(volume_4_temp, str(self.comboBoxPath.currentText()), self.slice_names, resolution, 3)
    ...
    #                 volume_4[optical_slice::self.optical_slices, :, :] = volume_4_temp[start_slice_number:end_slice_number+1, start_y:start_y+size_y, start_z:start_z+size_z]
            ...
            for chn in range(50):
                if chn in self.selected_channels:
                    ...
                    if any(i.name == 'C'+str(chn) for i in self.viewer.layers):
                        self.viewer.layers.remove('C'+str(chn))

                    self.viewer.add_image([self.aligned_1], name='C'+str(chn), scale=(float(self.slice_spacing)/float(self.optical_slices) / output_resolution, 1, 1), 
    ...
    def CropToRegion(self):
        ...
        for chn in range(50):
            if chn in self.selected_channels:
                layer_name='C'+str(chn)
        
                for layer in self.viewer.layers:
                    if layer.name == layer_name and isinstance(layer, napari.layers.Image):
                        desired_layer = layer
                        ...
                        self.viewer.add_image([data], name=layer_name, scale=(float(self.slice_spacing)/float(self.optical_slices) / output_resolution, 1, 1), 
                                          blending=layer_blending, colormap=layer_colormap, contrast_limits=self.default_contrast_limits)
                        break
