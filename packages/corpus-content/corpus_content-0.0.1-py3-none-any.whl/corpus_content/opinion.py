...
class Opinion(NamedTuple):
    ...
    def segments(self):
        ...
        if self.lines:
            for y0, line in enumerate(self.lines, start=1):
                for y1, text in enumerate(text_to_chunks(line)):
                    order = int(f"{y0}{y1}")
                    row = {
                        "id": f"{self.id}-{str(order)}",
                        "order": order,
                        "text": text,
                        "char_count": len(text),
                    }
    ...
    def as_row(self) -> dict:
        # convert structure into a prospective opinion row
        data = self._asdict()
        data.pop("file_statutes")
        data.pop("file_citations")
        data["statutes"] = self.statutes
