...
# encoding: utf-8
# -*- coding: utf-8 -*-
"""GeoIP2Dat v1.1.2 - DAT file update for GeoIP2Fast (Country and ASN support)"""
"""
GeoIP2Dat - Version: v1.1.2 - 03/Oct/2023
    ...
    with elapsed_timer() as elapsed_total:
        ...
        if args.asn_dir != "":
            ...
            for FILE in file_list:
                ...
                with elapsed_timer() as elapsed:
                    try:
                        with io.open(os.path.join(asn_dir,FILE.replace("XX",args.language)), mode="rt",encoding="utf-8") as f:
                            next(f)  # skip the first line (the CSV's header)
                            while True:
                                try:
                                    counter += 1
            ...
            if args.debug == True:
                ...
                with elapsed_timer() as elapsed_debug:
                    ...
                    logDebug(f"- ASN IP coverage {'%.2f%%'%(percentage)}")
        else:
            asnNamesDict = {}
        ##────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
        ...
        for FILE in file_list:
            ...
            with elapsed_timer() as elapsed:
                try:
                    with io.open(os.path.join(country_dir,FILE.replace("XX",args.language)), mode="rt",encoding="utf-8") as f:
                        next(f)  # skip the first line (the CSV's header)
                        while True:
                            try:
                                counter += 1
        ...
        if args.debug == True:
            with elapsed_timer() as elapsed_debug:
                ...
                for key,val in geoip.items():
                    ipCounter += numIPs[int(val['cidr'].split("/")[1])]
                percentage = (ipCounter * 100) / numIPs[0]
                logDebug(f"- Country IP coverage {'%.2f%%'%(percentage)}")
        ##────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
        if args.asn_dir != "":
