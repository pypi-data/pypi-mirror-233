...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pyroherd.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pyroherd.Client.sign_in` or :meth:`~pyroherd.Client.sign_up`).

        next_type (:obj:`~pyroherd.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pyroherd.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pyroherd.Client.resend_code`.
    """
