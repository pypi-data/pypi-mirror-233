...
def df_stats_binary(preds, actuals, th=.5):
    ...
    df['balanced_accuracy'] = (df['sensitivity']+df['specificity'])/2
    return df
...
def plot_confusion(cm, cm_p, figsize=(8, 2), cbar=False):
    fig = plt.figure(figsize=figsize)
    ax1, ax2 = fig.add_subplot(121), fig.add_subplot(122)
    sns.set(font_scale=0.7)
    sns.heatmap(cm, cmap=mpl.cm.Blues, annot=True, fmt='d', ax=ax1, cbar=cbar)
    sns.heatmap(cm_p, cmap=mpl.cm.Blues, annot=True, fmt='.2f', ax=ax2, cbar=cbar)
    for t in ax1.texts:
        t.set_text('{:,d}'.format(int(t.get_text())).replace(',',' '))
    plt.show()
    plt.pause(.001)
